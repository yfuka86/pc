...
void solve() {
  ...
  //   ans += comb(h - i, i) * comb(w - j, j) * comb(h - i + w - j, w - j);
  // }

  vmi hs(h), ws(w); // 2の区間の数の選び方
  rep(i, h / 2 + 1) hs[i] = mint(1) / fac(i) / fac(h - i * 2);
  rep(i, w / 2 + 1) ws[i] = mint(1) / fac(i) / fac(w - i * 2);
  vmi all = convolution(hs, ws);
  // debug(all);

  mint ans = 0;
  rep(i, (h + w) / 2 + 1) { // 2の区間の数
    if (all[i] == 0) continue;
    // debug(fac(h + w - i), all[i]);
    ans += fac(h + w - i) * all[i];
  }
  OUT(ans.val());
}
