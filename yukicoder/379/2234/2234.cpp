...
void solve() {
  ...
  if (is_palindrome(s)) OUTRET(s);

  ll n = s.size();
  rep(mid, n / 2, n) {
    string ans1 = "";
    rep(i, mid) ans1.pb(s[i]);

    ans1.pb(s[mid]);
    string ans2 = ans1; ans2.pb(s[mid]);
    rep_r(i, mid) {
      ans1.pb(s[i]); ans2.pb(s[i]);
    }

    if (ans1.find(s) != string::npos) OUTRET(ans1);
    if (ans2.find(s) != string::npos) OUTRET(ans2);
  }
}
