...
void solve() {
  ...
  {
    ll cur = *min_element(all(d[0]));
    rep(i, 1, n) {
      auto it = lower_bound(all(d[i]), cur);
      if (it == d[i].end()) { OUTRET(-1); }
      else cur = *it;
    }
  }
  ...
  ll ans = binary_search([&](ll mid) {
    set<ll> s, t;
    rep(j, m) s.insert(d[0][j]);

    rep(i, 1, n) {
      rep(j, m) {
        auto lt = s.lower_bound(d[i][j] - mid);
        auto rt = s.upper_bound(d[i][j]);
        if (lt != rt) t.insert(d[i][j]);
      }
      swap(s, t);
      t.clear();
    }
