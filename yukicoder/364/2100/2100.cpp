...
void solve() {
  ...
  rep(i, m) {
    LL(x, y); --x; --y;
    if (x > y) swap(x, y);
    G.add_directed_edge(x, y, max(0, h[y] - h[x]));
    G2.add_directed_edge(y, x, max(0, h[x] - h[y]));
  }

  {
    vv(ll, dp, n, 2, -LINF); dp[0][0] = 0;
    rep(i, n) {
      fore(to, G[i]) {
        if (h[to] > h[i]) {
          chmax(dp[to][1], dp[i][0] + to.cost);
        } else {
          chmax(dp[to][0], max(dp[i][0], dp[i][1]) + to.cost);
        }
      }
    }
    if (max(dp[n - 1][0], dp[n - 1][1]) < 0) OUT(-1);
    else OUT(max(dp[n - 1][0], dp[n - 1][1]));
  }
  {
    vv(ll, dp, n, 2, -LINF); dp[n - 1][0] = 0;
    rep_r(i, n) {
      fore(to, G2[i]) {
        if (h[to] > h[i]) {
          chmax(dp[to][1], dp[i][0] + to.cost);
    ...
    // debug(dp);
    if (max(dp[0][0], dp[0][1]) < 0) OUT(-1);
    else OUT(max(dp[0][0], dp[0][1]));
  }
}
