...
void solve() {
  ...
  rep(i, n) {
    STR(s);
    auto sh = rh.build(s);
    if (S.find(rh.query(sh, 0, s.size())) != S.end()) OUT("Yes"); else OUT("No");

    S.insert(rh.query(sh, 0, s.size()));
    rep(j, s.size() - 1) {
      uint64_t h = rh.query(sh, 0, j);
      // debug(i, j, h);
      string _s = s.substr(j, 2); reverse(all(_s));

      auto mh = rh.build(_s); h = rh.combine(h, rh.query(mh, 0, 2), 2);
      if (j + 2 < s.size()) h = rh.combine(h, rh.query(sh, j + 2, s.size()), s.size() - j - 2);
      S.insert(h);
    }
  }
}
