...
stack<pair<char, ll>> merge(vector<pair<char, int>> rl) {
  stack<pair<char, ll>> st;
  for (auto [c, cnt]: rl) {
    if (st.size() && st.top().fi == c) {
      auto [_, cnt2] = st.top(); st.pop();
      st.push({c, cnt + cnt2});
    } else if (st.size() > 1) {
      auto [_, cnt2] = st.top();
      if (cnt2 & 1) {
        st.pop();
        auto [_, cnt3] = st.top(); st.pop();
        st.push({c, cnt + cnt2 + cnt3});
      } else st.push({c, cnt});
    } else {
      st.push({c, cnt});
    }
...
void solve() {
  ...
  if (sst.size() != tst.size()) { cout << -1 << "\n"; return; }

  while (sst.size()) {
    auto [c, cnt] = sst.top(); sst.pop();
    auto [c2, cnt2] = tst.top(); tst.pop();
    if (c != c2 || (cnt & 1) != (cnt2 & 1)) { cout << -1 << "\n"; return; }
  }
