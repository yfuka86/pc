...
void solve() {
  ...
  // debug(p);
  ll con = min(min(p[0][0], p[1][1]), p[2][2]);
  ll nco = min(min(p[0][1], p[1][2]), p[2][0]);
  ll onc = min(min(p[0][2], p[1][0]), p[2][1]);

  ll cur = n, ans = 0;

  while (con-- && cur) { cur--; ans++; }
  cur--;
  while (nco-- && cur) { cur--; ans++; }
  while (onc-- && cur) { cur--; ans++; }
  OUT(ans);
}
