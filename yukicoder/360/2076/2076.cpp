...
void solve() {
  ...
  mpq<ll> rem;
  ll ma = max(a, b), macnt = 0, cnt = 0;
  fore(n, t) {
    macnt += n / ma;
    cnt += (n % ma) / min(a, b);
    rem.push((n % ma) % min(a, b));
  }

  while (macnt > cnt + (a == max(a, b))) {
    if (!rem.empty()) {
      ll r = rem.top(); rem.pop();
      macnt--;
      cnt += (ma + r) / min(a, b);
      rem.push((a + r) % b);
    } else {
  ...
  ll acnt = max(a, b) == a ? macnt : cnt, bcnt = max(a, b) == b ? macnt : cnt;
  OUT(min(acnt, bcnt) * 2 + (acnt > bcnt));
}
