...
void solve() {
  ...
  rep(x, 1, 8) rep(y, 1, 8) {
    rep(k, 1, x + y) {
      ll cnt = x + y - k;
      function<ll(ll, ll, ll)> dfs = [&](ll x, ll y, ll t) {
        if (t == cnt) {
          if (p == -1 && (y & 1)) return 1ll;
          if (p == 1 && !(y & 1)) return 1ll;
          return 0ll;
        }
        if (!(t & 1)) { // alice
          ll ret = 0;
          if (x > 0) chmax(ret, dfs(x - 1, y, t + 1));
          if (y > 0) chmax(ret, dfs(x, y - 1, t + 1));
          return ret;
        } else {
          ll ret = 1;
          if (x > 0) chmin(ret, dfs(x - 1, y, t + 1));
          if (y > 0) chmin(ret, dfs(x, y - 1, t + 1));
          return ret;
        }
      };
      debug(x, y, cnt, dfs(x, y, 0));
    }
    OUT("\n");
  }
}
