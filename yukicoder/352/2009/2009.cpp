...
void solve() {
  ...
  auto tomerge = [](LP p1, LP p2) {
    auto [a1, b1] = p1;
    auto [a2, b2] = p2;
    return a1 > b1 && (__uint128_t)a1 * (__uint128_t)b2 > (__uint128_t)a2 * (__uint128_t)b1;
  };
  ...
  rep(i, n) {
    ll ta = a[i], tb = b[i];
    while (st.size() && tomerge(st.top(), mp(ta, tb))) {
      auto [a, b] = st.top();
      ta += a; tb += b;
      st.pop();
    }
  ...
  ld ans = 0;
  while (st.size()) {
    ld a = st.top().fi, b = st.top().se; st.pop();
    ld x = sqrt(a / b);
		chmax(x, (ld)1);
		ans += a / x + x * b;
  }
