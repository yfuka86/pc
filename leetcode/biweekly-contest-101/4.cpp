...
public:
  int findShortestCycle(int n, vector<vector<int>>& edges) {
    Graph<ll> G(n);
    fore(es, edges) G.add_edge(es[0], es[1]);

    ll ans = LINF;
    rep(start, n) {
      vl from(n, -1);
      vb vis(n); vis[start] = true;
      vl cost(n, LINF); cost[start] = 0;
      queue<ll> que; que.push(start);

      while (!que.empty()) {
        auto v = que.front(); que.pop();
        ll nc = cost[v] + 1;
        fore(to, G[v]) {
          if (!vis[to]) { vis[to] = true; from[to] = v; cost[to] = nc; que.push(to); }
          else if (from[v] != to && from[to] != v) { chmin(ans, cost[v] + cost[to] + 1); }
        }
      }
    }
