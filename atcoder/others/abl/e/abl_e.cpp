...
struct S { ll sz; mint sum; }; using F = ll;
S op(S l, S r) {
  S ret = {l.sz + r.sz, l.sum * mint(10).pow(r.sz) + r.sum};
  return ret;
}
S e() { return {0, 0}; }
S mapping(F f, S x) {
  if (f != -1) return { x.sz, dp[f][x.sz] };
  else return x;
}
