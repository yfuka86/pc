...
vmi dfact(int n) { vmi ret(n + 1); ret[0] = 1; ret[1] = 1; for (int i = 0; i + 2 <= n; ++i) ret[i + 2] = ret[i] * (i + 2); return ret; }
...
void solve() {
  ...
  rep2(i, 1, 100001) {
    if (freq[i] > 1) {
      vmi t = { 1 };
      rep2(j, 1, freq[i] / 2 + 1) {
        t.pb(comb(freq[i], j * 2) * df[j * 2 - 1]);
      }
      all.pb(t);
    }
  }
  // rep(i, invp + 1) { cout << i << " " << conv[i].val() << "\n"; }

  vmi conv = all_convolution(all);
  // debug(conv.size());
  ...
  rep(i, invp + 1) {
    mint t = 0;
    if (i == n) t = conv[i];
    else t = df[(n - i) * 2 - 1] * conv[i];
    if (i & 1) ans -= t; else ans += t;
    // cout << i << " " << ans.val() << "\n";
  }
  cout << ans.val() << "\n";
}
