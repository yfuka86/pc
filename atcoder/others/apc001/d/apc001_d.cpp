...
void solve() {
  ...
  map<ll, multiset<ll>> comp;
  rep(i, n) comp[uf.root(i)].insert(a[i]);
  ...
  ll ans = 0;
  while (que.size() > 1) {
    auto [sz1, r1] = que.top(); que.pop();
    auto [sz2, r2] = que.top(); que.pop();

    if (sz1 == 0 || sz2 == 0) OUTRET("Impossible");

    auto it1 = comp[r1].begin();
    auto it2 = comp[r2].begin();
    ans += *it1 + *it2;
    comp[r1].erase(it1);
    comp[r2].erase(it2);

    if (sz1 < sz2) swap(comp[r1], comp[r2]);
    fore(e, comp[r2]) comp[r1].insert(e);
    comp.erase(r2);
    que.push({sz1 + sz2 - 2, r1});
  }
  OUT(ans);
}
