...
struct imos3d {
  ...
  imos3d(tu s) {
    sz = s; auto [x, y, z] = sz; data.assign(x + 1, vvl(y + 1, vl(z + 1, 0)));
  }
  void add(int x, int y, int z, ll a) {
    data[x][y][z] += a;
  }
...
void solve() {
  ...
  vvl co(3, vl(n * 2));
  rep(i, n) {
    rep(j, 6) cin >> co[j % 3][i * 2 + j / 3];
  }

  vvl cco = co;
  rep(i, 3) comp(cco[i]);

  vector<map<ll, ll>> cinv(3);
  rep(i, 3) {
    rep(j, n * 2) {
      cinv[i][cco[i][j]] = co[i][j];
    }
  }

  imos3d imos({n * 2, n * 2, n * 2});
  rep(i, n) {
    imos.imos_add({cco[0][i * 2], cco[1][i * 2], cco[2][i * 2]}, {cco[0][i * 2 + 1], cco[1][i * 2 + 1], cco[2][i * 2 + 1]}, 1);
  }
  imos.build();
  ll ans = 0;
  rep(i, n * 2) rep(j, n * 2) rep(k, n * 2) {
    if (imos.data[i][j][k] >= K) {
      ans += (cinv[0][i + 1] - cinv[0][i]) * (cinv[1][j + 1] - cinv[1][j]) * (cinv[2][k + 1] - cinv[2][k]);
    }
  }
  cout << ans << "\n";
}
