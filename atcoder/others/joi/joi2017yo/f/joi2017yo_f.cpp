...
void solve() {
  ...
  v3l cost(n, vvl(3, vl(x, LINF)));
  cost[0][0][0] = 0;

  que.push({0, 0, 0, 0});
  while (!que.empty()) {
    auto [c, v, temp, time] = que.top(); que.pop();
    if (c > cost[v][temp][time]) continue;

    for (auto to: G[v]) {
      ll nc = c + to.cost;
      if (abs(T[to] - temp) == 2) {
        if (time + to.cost < x) continue;
        if (chmin(cost[to][T[to]][0], nc)) que.push({nc, to, T[to], 0});
      } else {
        ll ntemp = temp, ntime = time;
        if (T[to] == 1) {
          ntime += to.cost;
          if (ntime >= x) ntemp = 1;
          if (ntemp == 1) ntime = 0;
        ...
        if (chmin(cost[to][ntemp][ntime], nc)) que.push({nc, to, ntemp, ntime});
      }
    }
  }
  // rep(i, n) {
  ...
  rep(i, 3) rep(j, x) chmin(ans, cost[n-1][i][j]);
  cout << ans << "\n";
}
