...
void solve() {
  ...
  auto prob = [&](ld r1, ld r2) {
    return 1.0 / (1 + pow(10, (r2 - r1) / 400.0));
  };

  vector<vector<map<ll, ld>>> dp(K + 1, vector<map<ll, ld>>(1 << K));
  rep(i, 1 << K) dp[0][i][i] = 1;

  rep(i, K) {
    for (ll j = 0; j < (1 << K - i); j += 2) {
      for (auto [n1, p1]: dp[i][j]) for (auto [n2, p2]: dp[i][j + 1]) {
        ld pp = prob(r[n1], r[n2]);
        dp[i + 1][j / 2][n1] += p1 * p2 * pp;
        dp[i + 1][j / 2][n2] += p1 * p2 * (1 - pp);
      }
    }
  }
