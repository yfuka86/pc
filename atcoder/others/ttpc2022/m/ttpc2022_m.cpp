...
void solve() {
  ...
  // dp[いくつの素因数を使って][{積, 和}]
  vector<map<LP, mint>> dp(cnt + 1);
  dp[0][{1, 0}] = 1;
  rep(i, cnt) {
    fore(k, v, dp[i]) {
      auto [p, sum] = k;
      fore(d, div) {
        if (d == 1 || d > m / p) continue;
        dp[i + 1][{p * d, sum + d}] += v;
      }
    }
  }
  ...
  // dp[n][m] mはxが必要な分の次数
  vvmi dp2(n + 1, vmi(cnt + 1, 0));
  dp2[0][0] = 1;
  rep(i, n) rep(j, cnt + 1) {
    dp2[i + 1][j] += dp2[i][j] * (a[i] + 1);
    if (j + 1 <= cnt) dp2[i + 1][j + 1] += dp2[i][j];
  }

  function<mint(ll, ll)> C = [&](ll a, ll b) {
  ...
  debug(dp2);
  mint ans = dp2[n][0] * tot;
  rep(i, 1, cnt + 1) {
    fore(pp, v, dp[i]) {
      auto [p, sum] = pp;
      if (p != m) continue;
      ans += dp2[n][i] * C(sum - i, i) * v;
    }
  }
  OUT(ans);
}
