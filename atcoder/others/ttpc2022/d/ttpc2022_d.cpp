...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    ...
    fore(to, G[v]) {
      ...
      dfs(to, v);
      ll n0 = 0, n1 = 0;
      if (t0d == -LINF) n0 = dp[to][0]; else n0 = max(t0d + dp[to][0], t1d + dp[to][1]);
      if (t1d == -LINF) n1 = dp[to][1]; else n1 = max(t1d + dp[to][0], t0d + dp[to][1]);
      chmax(t0d, n0); chmax(t1d, n1);
    }

    if (t0d != -LINF) dp[v][0] += t0d;
    if (t1d != -LINF) dp[v][1] += t1d;
  };
  dfs(0, -1);
  // debug(dp);
