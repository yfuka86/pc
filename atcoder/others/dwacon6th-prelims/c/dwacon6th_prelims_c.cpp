...
void solve() {
  ...
  // 総積の総和は全ての選び方の中でさらに各日にその子供を選んで印をつける方法（各日にi個の印をつけて、最終的にn個の印をつける）
  vvmi dp(k + 1, vmi(n + 1, 0));
  dp[0][0] = 1;

  init_f();
  rep(i, k) rep(j, n + 1) rep(d, a[i] + 1) {
    // その日選ぶa[i]人の子供のうち、d人に印をつける通り数
    // まだ一度も印がついていない子供 n-j 人からd人選び、残りの n-d 人から a[i]-d 人を選ぶ方法
    if (j + d <= n) dp[i + 1][j + d] += dp[i][j] * comb(n - j, d) * comb(n - d, a[i] - d);
  }

  // k日目にn人に印がついている時の総積の総和
  cout << dp[k][n] << "\n";
}
