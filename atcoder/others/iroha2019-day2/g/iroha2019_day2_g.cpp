...
void solve() {
  ...
  while (!que.empty()) {
    auto [c, v, fl] = que.top(); que.pop();
    if (cost[v][fl] < c) continue;

    if (x[v]) {
      ll nfl = min(fl + x[v], k);
      if (chmin(cost[v][nfl], c + y[v])) que.push({c + y[v], v, nfl});
    }

    for (auto to: G[v]) {
      ll nc = c + to.cost;
      if (chmin(cost[to][fl], nc)) que.push({nc, to, fl});
    }
  }

  if (cost[n - 1][k] == LINF) cout << -1 << "\n";
  else cout << cost[n - 1][k] << "\n";
}
