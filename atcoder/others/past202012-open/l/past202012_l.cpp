...
void solve() {
  ...
  string a = T.substr(0, 1), b = T.substr(1, 1), c = T.substr(2, 1);
  string ab = T.substr(0, 2), bc = T.substr(1, 2);

  ll n = S.size();

  vvl dp(n + 1, vl(n + 1, -1));

  function<ll(ll, ll)> calc = [&](ll l, ll r) {
    if (dp[l][r] != -1) return dp[l][r];
    if (r - l < 3) return dp[l][r] = 0;

    ll ma = 0;
    rep2(i, l + 1, r) {
      chmax(ma, calc(l, i) + calc(i, r));
    }
    if ((r - l) % 3 == 0) {
      ll length = (r - l) / 3;
      if (S.substr(l, 1) == a && S.substr(r - 2, 2) == bc) {
        if (calc(l + 1, r - 2) == length - 1) return dp[l][r] = length;
      }
      if (S.substr(l, 2) == ab && S.substr(r - 1, 1) == c) {
        if (calc(l + 2, r - 1) == length - 1) return dp[l][r] = length;
      }
      if (S.substr(l, 1) == a && S.substr(r - 1, 1) == c && length >= 3) {
        for (ll nl = l + 4; nl <= r - 4; nl += 3) {
          if (calc(l + 1, nl) + calc(nl + 1, r - 1) == length - 1 && S[nl] == T[1]) return dp[l][r] = length;
        }
      }
    }
    return dp[l][r] = ma;
