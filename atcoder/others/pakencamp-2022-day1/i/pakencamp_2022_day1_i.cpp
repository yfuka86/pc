...
vl solve(ll n, vlp ps, vlp qs) {
  ll p = ps.size(), q = qs.size();
  UnionFind uf(n);
  vl mi(n); iota(all(mi), 0);
  rep(i, p) {
    auto [a, b] = ps[i];
    uf.unite(a, b);
  }
  rep(i, n) chmin(mi[uf.root(i)], i);

  vvl ref(n);
  rep(i, q) {
    auto [c, d] = qs[i];
    ll rc = uf.root(c), rd = uf.root(d);
    if (rc == rd) return {};
    ref[rc].pb(rd);
    ref[rd].pb(rc);
  }
  ...
  vl ord(n); iota(all(ord), 0);
  sort(all(ord), [&](ll a, ll b) {
    return mi[uf.root(a)] < mi[uf.root(b)];
  });

  fore(i, ord) {
    ll r = uf.root(i);
    vl t;
    fore(from, ref[r]) if (mi[from] < mi[r]) t.pb(dp[from]);
    dp[r] = mex(t);
  }
...
vl naive(ll n, vlp ps, vlp qs) {
  vl ans(n, LINF); ll p = ps.size();
  enum_check(n, 0, n - p + 1, [&](vl v) {
    bool valid = true;
    fore(a, b, ps) {
      if (v[a] != v[b]) valid = false;
    }
    fore(c, d, qs) {
      if (v[c] == v[d]) valid = false;
    }
    if (valid) chmin(ans, v);
    return true;
  });
  if (ans[0] == LINF) return {};
  else return ans;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    auto so = solve(n, ps, qs); auto na = naive(n, ps, qs);
    debug(so);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, ps, qs); debug(so); debug(na);
