...
void solve() {
  ...
  multiset<LP> ps;
  rep(i, n) ps.insert({a[i], i});

  ll off = 0;
  rep(i, q) {
    LL(t, x, y); --x;
    if (t == 1) {
      auto it = ps.find({a[x], x}); ps.erase(it);
      a[x] += y; ps.insert({a[x], x});

      off += y;
      vl t;
      while(ps.size() && ps.begin()->fi <= off) {
        t.pb(ps.begin()->se);
        ps.erase(ps.begin());
      }
      sort(all(t));
      cout << t.size() << " "; OUTARRAY(t, 1);
    } else {
      auto it = ps.find({a[x], x});
      ps.erase(it);
      a[x] -= y;
      if (a[x] <= off) OUT(1, x + 1); else { OUT(0); ps.insert({a[x], x}); }
    }
  }
}
