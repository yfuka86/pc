...
void solve() {
  ...
  queue<LP> magma; vvl magmacost(H, vl(W, INF));
  rep(i, H) {
    string s; cin >> s;
    rep(j, W) if (s[j] == '#') grid[i][j] = 1; else if (s[j] == '@') { magma.push({i, j}); magmacost[i][j] = 0; }
  }

  while(!magma.empty()) {
    auto [i, j] = magma.front(); magma.pop();
    rep(d, 4) {
      ll di = i + dx[d], dj = j + dy[d];
      if (0 <= di && di < H && 0 <= dj && dj < W && !grid[di][dj]) {
        if (chmin(magmacost[di][dj], magmacost[i][j] + 1)) magma.push({di, dj});
      }
    }
  }

  queue<LP> player; vvl playercost(H, vl(W, LINF));
  player.push({0, 0}); playercost[0][0] = 0;

  while(!player.empty()) {
    auto [i, j] = player.front(); player.pop();
    rep(d, 4) {
      ll di = i + dx[d], dj = j + dy[d];
      if (0 <= di && di < H && 0 <= dj && dj < W && !grid[di][dj]) {
        if (playercost[i][j] + 1 < magmacost[di][dj] * K) {
          if (chmin(playercost[di][dj], playercost[i][j] + 1)) player.push({di, dj});
        }
      }
    }
  }

  if (playercost[H - 1][W - 1] != LINF) cout << "Yes" << "\n"; else cout << "No" << "\n";
}
