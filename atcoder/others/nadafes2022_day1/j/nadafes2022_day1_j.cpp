...
void solve() {
  ...
  vvl lcm(1000001, vl(20, 0));

  rep(i, N) {
    pf[i] = ps.factorize(A[i]);
    for (auto [p, c]: pf[i]) lcm[p][c]++;
  }
  ...
  rep2(i, 2, 1000001) {
    if (ps.is_prime[i]) {
      ll maxidx = 0;
      rep(j, 20) if (lcm[i][j]) chmax(maxidx, j);
      ans *= mod_pow(i, maxidx);
    }
  }

  rep(_, Q) {
    ll K; cin >> K;
    vl q(K); rep(i, K) { cin >> q[i]; q[i]--; }
    mint anstemp = ans;

    rep(i, K) for (auto [p, c]: pf[q[i]]) {
      lcm[p][c]--;
      if (lcm[p][c] == 0) {
        ll maxidx = 0;
        rep(j, 20) if (lcm[p][j]) chmax(maxidx, j);
        if (maxidx < c) anstemp /= mod_pow(p, c - maxidx);
      }
    }
    rep(i, K) for (auto [p, c]: pf[q[i]]) {
      lcm[p][c]++;
