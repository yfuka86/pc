...
void solve() {
  ...
  rep(i, n) rep(j, i) {
    if (a[i][j] == 1) {
      if (uf.same(i, j)) { cout << 0 << "\n"; return; }
      uf.unite(i, j);
    }
  }
  vl roots, rinv(n, -1), ridx(n);
  rep(i, n) roots.pb(uf.root(i));
  uniq(roots);
  rep(i, roots.size()) rinv[roots[i]] = i;
  rep(i, n) ridx[i] = rinv[uf.root(i)];
  // debug(roots, rinv, ridx);

  vvmi m(roots.size(), vmi(roots.size()));
  rep(i, n) rep(j, i) {
    ll r1 = uf.root(i), r2 = uf.root(j);
    if (a[i][j] == -1 && r1 != r2) {
      m[ridx[r1]][ridx[r1]] += 1;
      m[ridx[r2]][ridx[r2]] += 1;
      m[ridx[r1]][ridx[r2]] -= 1;
  ...
  Matrix<mint> mat(m);
  rep(i, roots.size()) mat.A[i].pop_back();
  mat.A.pop_back();
  // debug(m);
  mint d = mat.determinant();
  cout << d << "\n";
}
