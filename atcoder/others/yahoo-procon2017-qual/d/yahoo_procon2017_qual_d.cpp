...
struct S { ll rem, vord, allord; }; using F = ll;
S op(S l, S r) {
  ll valid = l.vord;
  if (l.rem - l.vord > 0) {
    valid += min(r.vord + (l.rem - l.vord), r.allord);
  } else {
    valid += r.vord;
  }
  assert(valid <= l.rem + r.rem);
  return { l.rem + r.rem, valid, l.allord + r.allord };
}
S e() { return {0, 0, 0}; }
...
void solve() {
  ...
  comp(datec);
  map<ll, ll> datemap;
  rep(i, q) datemap[date[i]] = datec[i];

  vector<S> s;
  ll cur = 0;
  for (auto[rd, d]: datemap) {
    s.pb({(rd - cur) * k, 0});
    cur = rd;
  }
  ...
  rep(i, q) {
    auto [t,d,a] = query[i];
    if (t == 1) {
      S x = seg.get(datemap[d]);
      seg.set(datemap[d], {x.rem, min(x.rem, x.allord + a), x.allord + a});
    } else {
      // debug(k * d, seg.prod(0, datemap[d] + 1).order);
      cout << seg.prod(0, datemap[d] + 1).vord << "\n";
    }
  }
}
