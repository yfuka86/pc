...
ll calc(ll n, vl &a) {
  ...
  rep(i, n) {
    ...
    chmin(dp[i + 1][1], *min_element(dp[i].begin(), dp[i].begin() + 2) + d);
    chmin(dp[i + 1][2], *min_element(dp[i].begin(), dp[i].begin() + 3) + !(a[i] & 1));
    chmin(dp[i + 1][3], *min_element(dp[i].begin(), dp[i].begin() + 4) + d);
    chmin(dp[i + 1][4], *min_element(all(dp[i])) + a[i]);
  }
  // debug(dp);
