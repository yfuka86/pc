...
ll naive(ll n, vl A, vl B) {
  ...
  rep(i, n) {
    rep2(j, i + 1, n + 1) {
      chmin(ans, *max_element(A.begin() + i, A.begin() + j) * *min_element(B.begin() + i, B.begin() + j));
    }
  }
  return ans;
}
...
ll solve(ll N, vl A, vl B) {
  ...
  ll ans = LINF;
  rep(i, N) {
    if (B[i] >= 0) chmin(ans, A[i] * B[i]);
    if (B[i] <= 0) {
      if (amax < 0) chmin(ans, A[i] * B[i]);
      else chmin(ans, amax * B[i]);
    }
  }
  return ans;
}
...
void compare() {
  ...
  while (true) {
    ...
    ll N = 10;
    vl A = RandGen().vlint(10, -10, 10), B = RandGen().vlint(10, -10, 10);
    ll n = naive(N, A, B);
    ll s = solve(N, A, B);
    if (n != s) {
      cout << c << "times tried" << "\n";
