...
void solve() {
  LL(n);
  STR(s, t);
  vl oddsi, evensi, oddti, eventi;
  rep(i, n) {
    if (s[i] == '1') {
      if (!(i & 1)) oddsi.pb(i); else evensi.pb(i);
    }
    if (t[i] == '1') {
      if (!(i & 1)) oddti.pb(i); else eventi.pb(i);
    }
  }

  if (oddsi.size() != oddti.size() || evensi.size() != eventi.size()) OUTRET(-1);

  ll al = 0;
  vlp odd, even;
  rep(i, oddsi.size()) {
    odd.pb({oddsi[i], oddti[i]});
    al += abs(oddti[i] - oddsi[i]) / 2;
  }
  rep(i, evensi.size()) {
    even.pb({evensi[i], eventi[i]});
    al += abs(eventi[i] - evensi[i]) / 2;
  }
  ...
  rep(i, odd.size()) {
    ...
    debug(l, r);
    if (l < r) {
      ll l2 = upper_bound(all(evensi), l) - evensi.begin();
      ll r2 = upper_bound(all(eventi), r) - eventi.begin();
      ans += r2 - l2;
    } else {
      ll r2 = upper_bound(all(evensi), l) - evensi.begin();
      ll l2 = upper_bound(all(eventi), r) - eventi.begin();
      ans += r2 - l2;
    }
    // debug(ans);
  }
  ...
  // rep(i, even.size()) {
  //   auto [l, r] = even[i];
  //   ll i2 = upper_bound(all(oddti), r) - oddti.begin();
  //   ll i1 = upper_bound(all(oddsi), l) - oddsi.begin();
  //   ans += abs(i1 - i2);
  // }
  OUT(al - ans);
}
