...
vlp cos(ll r, ll c, ll rs, ll cs) {
  vlp ret;
  rep(dx, -1, 2)  rep(dy, -1, 2) {
    ll tx = rs, ty = cs;
    if (dx == 0 && dy == 0) continue;
    while (incl(tx, 0, r) && incl(ty, 0, c)) {
      if (tx != rs || ty != cs) ret.pb({tx, ty});
      tx += dx; ty += dy;
    }
  }
  return ret;
...
void solve() {
  LL(r,c,rs,cs,rt,ct); --rs, --cs, --rt, --ct;

  vlp st = cos(r, c, rs, cs);
  vlp go = cos(r, c, rt, ct);

  vl xs(100010), ys(100010), xys(200020), yxs(200020);
  ll off = 100000;
  set<LP> goals;
  fore(i, j, go) {
    xs[i]++;
  ...
  fore(i, j, st) {
    ...
    ans += xys[i+j];
    ans += yxs[i-j+off];
    if (goals.find({i, j}) != goals.end()) ans -= 4;
  }
  OUT(ans);
}
