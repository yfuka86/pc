...
void solve() {
  ...
  vl s(N); rep(i, N) s[i] = S[i] - 'a';
  vl mins = s; sort(all(mins));
  vl st = mins;

  function<pair<ll, vl>(vl)> check = [&](vl st) {
    ll c = 0, n = st.size(); vl vec(n, -1);
    rep(i, n) {
      auto itr = find(all(st), s[N - n + i]);
      if (itr != st.end()) { st.erase(itr); vec[i] = s[N - n + i]; }
    }
    sort(all(st)); reverse(all(st));
    rep(i, n) {
      if (vec[i] == -1) { c++; vec[i] = st.back(); st.pop_back(); }
    }
    return mp(c, vec);
  };
  ...
  rep(i, N) {
    rep(j, st.size()) {
      ...
      auto [cnt2, rem] = check(st);
      if (cnt + cnt2 <= K) {
        vl t = vec; t.insert(t.end(), all(rem)); ans = t;
        break;
      } else {
        st.insert(st.begin() + j, f);
        vec.pop_back();
        if (s[i] != f) cnt--;
      }
    }
