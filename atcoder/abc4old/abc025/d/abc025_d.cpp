...
void solve() {
  ...
  rep(i, 5) rep(j, 5) {
    cin >> grid[i][j];
    if (grid[i][j]) inv[grid[i][j] - 1] = i * 5 + j;
  }

  auto check = [&](ll S, ll idx) {
    if (S & 1 << idx) return false;

    ll i = idx / 5, j = idx % 5;
    if (i > 0 && i < 4) {
      if ((S >> idx - 5 & 1) ^ (S >> idx + 5 & 1)) return false;
    }
    if (j > 0 && j < 4) {
      if ((S >> idx - 1 & 1) ^ (S >> idx + 1 & 1)) return false;
    }
    return true;
  };
  ...
  vmi dp(1 << 25, 0); dp[0] = 1;
  rep(S, 1 << 25) {
    ll i = __builtin_popcount(S);
    if (dp[S] == 0) continue;
    if (inv[i] != -1) {
      if (check(S, inv[i])) dp[S | 1 << inv[i]] += dp[S];
    } else {
      rep(j, 25) {
        if (check(S, j)) dp[S | 1 << j] += dp[S];
      }
    }
  }
  cout << dp[(1 << 25) - 1] << "\n";
