...
void solve() {
  ll N, W; cin >> N >> W;
  vl v(N), w(N); rep(i, N) cin >> v[i] >> w[i];
  ...
  if (N <= 30) {
    ...
    rep(S, 1 << N / 2) {
      ll vsum = 0, wsum = 0;
      rep(i, N / 2) {
        if (S & 1 << i) { vsum += v[i]; wsum += w[i]; }
      }
      if (wsum <= W) m1[wsum] = vsum;
    }
    rep(S, 1 << N - N / 2) {
      ll vsum = 0, wsum = 0;
      rep(i, N - N / 2) {
        if (S & 1 << i) { vsum += v[i + N / 2]; wsum += w[i + N / 2]; }
      }
      if (wsum <= W) m2[wsum] = vsum;
    }

    ll ma = 0;
    for (auto [w, v]: m1) { chmax(ma, v); if (v < ma) m1[w] = ma; }
    ma = 0;
    for (auto [w, v]: m2) { chmax(ma, v); if (v < ma) m2[w] = ma; }

    ll ans = 0;
    for (auto [w, v]: m1) {
      auto itr = m2.upper_bound(W - w);
      itr--;
      chmax(ans, v + itr->second);
    }
    cout << ans << "\n";
  } else {
    if (*max_element(all(w)) <= 1000) {
      ...
      rep(i, N) {
        rep_r(j, 200001) {
          if (j + w[i] < 200001) chmax(dp[j + w[i]], dp[j] + v[i]);
        }
      }
      ...
      rep(i, N) {
        rep_r(j, 200001) {
          if (j + v[i] < 200001) chmin(dp[j + v[i]], dp[j] + w[i]);
        }
      }
      ll ans = 0; rep(i, 200001) if (dp[i] <= W) chmax(ans, i);
      cout << ans << "\n";
    }
  }
}
