...
ll rem(string S, ll divide_by) {
  ll pow10 = 1;
  ll res = 0;
  rep_r(i, S.size()) {
    res += pow10 * (S[i] - '0');
    res %= divide_by;
    pow10 *= 10; pow10 %= divide_by;
  }
...
void solve() {
  ...
  vl search = {b[a]};
  map<ll, ll> dict; dict[b[a]] = 0;
  ll cycle = -1;
  while(1) {
    ll next = b[search.back()];
    if (dict.count(next)) { cycle = dict[next]; break; }
    search.pb(next); dict[next] = search.size() - 1;
  }

  ll cyclesz = search.size() - cycle;

  if (k.size() < 19) {
    ll n = stoll(k);
    if (n <= search.size()) cout << search[n - 1] + 1 << "\n";
    else cout << search[cycle + (n - cycle - 1) % cyclesz] + 1 << "\n";
  } else {
    ll n = rem(k, cyclesz);
    while (n <= cycle) n += cyclesz;
    cout << search[cycle + (n - cycle - 1) % cyclesz] + 1 << "\n";
  }
}
