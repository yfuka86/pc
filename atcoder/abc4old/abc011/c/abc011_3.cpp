...
void solve() {
  ll N; cin >> N;
  vl ng(3); rep(i, 3) cin >> ng[i];
  if (find(all(ng), N) != ng.end()) { cout << "NO" << "\n"; return; }

  vvl dp(N + 1, vl(100 + 1, 0));
  dp[N][0] = 1;
  rep_r(i, N) {
    if (find(all(ng), i) != ng.end()) continue;
    rep(j, 100) {
      if (i + 1 <= N && dp[i + 1][j]) dp[i][j + 1] = 1;
      if (i + 2 <= N && dp[i + 2][j]) dp[i][j + 1] = 1;
      if (i + 3 <= N && dp[i + 3][j]) dp[i][j + 1] = 1;
    }
  }
  if (any_of(all(dp[0]), [](ll a) { return a ? true : false; })) cout << "YES" << "\n"; else cout << "NO" << "\n";
}
