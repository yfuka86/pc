...
void solve() {
  ...
  rep(i, n) {
    auto it = que.begin(), rt = que.end();

    if (s[i] == 'L') que.push_front(i + 1);
    if (s[i] == 'R') que.push_back(i + 1);
    if (s[i] == 'A') if (que.size() == 0) OUT("ERROR"); else { OUT(que.front()); que.pop_front(); }
    if (s[i] == 'B') if (que.size() <= 1) OUT("ERROR"); else { it = next(it, 1); OUT(*it); que.erase(it); }
    if (s[i] == 'C') if (que.size() <= 2) OUT("ERROR"); else { it = next(it, 2); OUT(*it); que.erase(it); }
    if (s[i] == 'D') if (que.size() == 0) OUT("ERROR"); else { OUT(que.back()); que.pop_back(); }
    if (s[i] == 'E') if (que.size() <= 1) OUT("ERROR"); else { rt = prev(rt, 2); OUT(*rt); que.erase(rt); }
    if (s[i] == 'F') if (que.size() <= 2) OUT("ERROR"); else { rt = prev(rt, 3); OUT(*rt); que.erase(rt); }
  }
}
