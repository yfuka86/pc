...
void solve() {
  ...
  rep(i, q) {
    ...
    ll sl = ceil(l, sq), sr = r / sq;


    if (sl > sr) {
      if (same[sr] != -1) {
        rep(i, sr * sq, sl * sq) {
          a[i] = incl(i, l, r) ? x : same[sr];
        }
        subt(same[sr], r - l);
        add(x, r - l);
        same[sr] = -1;
      ...
      rep(bi, sl, sr) {
        if (same[bi] != -1)  {
          subt(same[bi], sr == ceil(n, sq) ? n % sq : sq);
          same[bi] = x;
          add(same[bi], sr == ceil(n, sq) ? n % sq : sq);
        } else {
          rep(i, bi * sq, min((bi + 1) * sq, n)) {
            subt(a[i], 1);
            a[i] = x;
          }
          add(x, sq);
      ...
      if (sr < ceil(n, sq) && same[sr] != -1) {
        rep(i, sr * sq, min((sr + 1) * sq, n)) {
          a[i] = i < r ? x : same[sr];
        }
        subt(same[sr], r - sr * sq);
        add(x, r - sr * sq);
        same[sr] = -1;
      } else {
        rep(i, sr * sq, r) {
