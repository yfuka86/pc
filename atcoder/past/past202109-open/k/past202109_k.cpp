...
void solve() {
  ...
  ll bd = sum_of(b) + sum_of(d);
  ll ans = bd;
  const ll inf = 3e9;

  rep(climit, 1, 101) {
    mcf_graph<int, ll> mcf(p + q + 2);
    ll S = p + q, T = S + 1;

    rep(i, p) mcf.add_edge(S, i, 1, 0);
    rep(i, q) mcf.add_edge(p + i, T, 1, 0);

    rep(i, p) rep(j, q) {
      if (s[i][j] != '1') continue;
      ll diff = a[i] - b[i] + c[j] - d[j];
      if (diff <= 0) continue;
      mcf.add_edge(i, p + j, 1, inf - diff);
    }
    auto [cap, cost] = mcf.flow(S, T, climit);
    if (cap < climit) break;
    // debug(cap, (cost - (ll)INF * cap));
    chmax(ans, bd - (cost - inf * cap));
  }
