...
void solve() {
  ...
  bool bipartite = true;
  rep(i, n) {
    if (uf.same(i, i + n)) bipartite = false;
  }
  ...
  if (bipartite) {
    ...
    function<void(ll)> dfs = [&](ll v) {
      fore(to, G[v]) {
        ...
        vis[to.idx] = true;
        if (dpv[to] == -1) {
          dpv[to] = to.cost - dpv[v];
          dp[to] = -dp[v];
          if (dp[to] == 1) chmin(posmi, dpv[to]); else chmin(negmi, dpv[to]);
          dfs(to);
        } else {
          if (dpv[to] != to.cost - dpv[v]) valid = false;
        }
      }
    };
    dfs(0);
    if (!valid || posmi + negmi < 0) OUTRET(-1);
    if (posmi < 0) {
      ll adj = abs(posmi);
      rep(i, n) {
        if (dp[i] > 0) dpv[i] += adj; else dpv[i] -= adj;
      }
    }
    if (negmi < 0) {
      ll adj = abs(negmi);
    ...
    function<void(ll)> dfs = [&](ll v) {
      fore(to, G[v]) {
        ...
        if (dp[to] != -1) {
          if (dp[to] != to.cost - dp[v]) {
            ll diff = to.cost - (dp[to] + dp[v]);
            if (diff & 1) valid = false;
            else {
              retry = to; retryval = dp[to] + diff / 2;
            }
          }
        } else {
          dp[to] = to.cost - dp[v];
    ...
    // debug(retry, retryval);
    {
      ...
      function<void(ll)> dfs = [&](ll v) {
        fore(to, G[v]) {
          ...
          if (dp[to] == -1) {
            dp[to] = to.cost - dp[v];
            if (dp[to] < 0) valid = false;
            dfs(to);
          } else {
            if (dp[to] != to.cost - dp[v]) valid = false;
          }
        }
      };
      dfs(retry);
