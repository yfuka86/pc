...
void solve() {
  LL(n);
  vlp s(n), t(n); IN(s); IN(t);

  sort(all(s)); sort(all(t));
  if (s == t) OUTRET("Yes");

  ll xmi = LINF, ymi = LINF, xma = -LINF, yma = -LINF;
  rep(i, n) {
    chmin(xmi, s[i].fi); chmax(xma, s[i].fi);
    chmin(ymi, s[i].se); chmax(yma, s[i].se);
  }

  ll txmi = LINF, tymi = LINF, txma = -LINF, tyma = -LINF;
  rep(i, n) {
    chmin(txmi, t[i].fi); chmax(txma, t[i].fi);
    chmin(tymi, t[i].se); chmax(tyma, t[i].se);
  }

  if (xmi == txmi && xma == txma && tyma - tymi == yma - ymi) {
    // yの変換
    set<LP> S;
    rep(i, n) {
      auto [x, y] = s[i];
      S.insert({x, tymi + (yma - y)});
    }
    rep(i, n) {
      S.erase(t[i]);
    }
    if (S.size() == 0) OUTRET("Yes");
  }

  if (ymi == tymi && yma == tyma && txma - txmi == xma - xmi) {
    // xの変換
    set<LP> S;
    rep(i, n) {
      auto [x, y] = s[i];
      S.insert({txmi + (xma - x), y});
    }
    rep(i, n) {
      S.erase(t[i]);
    }
