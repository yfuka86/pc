...
void solve() {
  ...
  vl t;
  rep(i, n) { t.pb(y[i]); t.pb(y[i] + d[i] + 1); }
  rep(i, q) { t.pb(b[i]); }
  vl ct = t; comp(ct);
  unordered_map<ll, ll> cmp;
  rep(i, t.size()) cmp[t[i]] = ct[i];

  vector<tuple<ll, ll, ll, ll, ll, ll>> ev;

  rep(i, n) {
    ev.pb({x[i], 0ll, cmp[y[i]], cmp[y[i] + d[i] + 1], d[i], c[i]});
  }
  rep(i, q) {
    ev.pb({a[i], 1ll, cmp[b[i]], -1ll, -1ll, i});
  }
  sort(all(ev));

  vl ans(q);
  RangeAdd<ll> bt(200000);
  mpq<LT4> que;
  for(auto &[x, t, y, y2, d, cori] :ev) {
    while (!que.empty() && get<0>(que.top()) < x) {
      auto [_, y, y2, c] = que.top(); que.pop();
      bt.range_add(y, y2, -c);
    }
    if (t == 0) {
      bt.range_add(y, y2, cori);
      que.push({x + d, y, y2, cori});
    } else {
      ans[cori] = bt.get(y);
    }
  }
