...
void solve() {
  ...
  vvlp c(11);
  rep(i, n) rep(j, m) {
    if (g[i][j] == 'S') c[0].pb({i, j});
    if (g[i][j] == 'G') c[10].pb({i, j});
    rep(k, 1, 10) {
      if (g[i][j] - '0' == k) c[k].pb({i, j});
    }
  }
  vl dp(1); dp[0] = 0;
  rep(d, 1, 11) {
    vl dpt(c[d].size(), LINF);
    rep(i, dp.size()) {
      auto [fx, fy] = c[d - 1][i];
      rep(j, c[d].size()) {
        auto [tx, ty] = c[d][j];
        chmin(dpt[j], dp[i] + abs(tx - fx) + abs(ty - fy));
      }
    }
    dp = dpt;
  }
  if (dp[0] == LINF) OUT(-1); else OUT(dp[0]);
}
