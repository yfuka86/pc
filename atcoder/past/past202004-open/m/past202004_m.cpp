...
void solve() {
  ...
  rep(i, 100000) {
    ...
    rep(j, v.size()) {
      dp[i].pb(cur);
      if (j < v.size() - 1) {
        cur += ceil(v[j + 1] - v[j], l);
      } else {
        cur += ceil(v[0] + d - v[j], l);
      }
    }
    dpc[i] = cur;
  }
  ...
  rep(i, 100000) {
    if (dp[i].size()) dp2[i] = dp[i];
    rep(j, dp[i].size()) {
      dp2[i].pb(dp[i][j] + dpc[i]);
    }
  }
  rep(i, 100000) if (dp[i].size()) { debug(i, g[i], dp[i], dpc[i]); }

  rep(i, n) {
    LL(k, f, t); --k; --f;
    if (g[k].size() == 0) { OUT(0); continue; }
    auto it = lower_bound(all(g[k]), f); if (it == g[k].end()) it = g[k].begin();

    ll ans = 0;
    ll cur = ceil((*it + d - f) % d, l);
    if (cur >= t) { OUT(0); continue; }

    // debug(cur);
    ll cycle = (t - cur) / dpc[k];
    ll rem = (t - cur) % dpc[k];
    ans += cycle * dp[k].size();

    ll idx = it - g[k].begin();
    ans += upper_bound(all(dp2[k]), dp[k][idx] + rem - 1) - dp2[k].begin() - idx;
