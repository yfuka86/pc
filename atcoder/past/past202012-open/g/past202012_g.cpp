...
void solve() {
  ...
  vlp path;
  function<void(LP)> dfs = [&](LP v) {
    if (ans.size()) return;

    path.pb(v);
    if (path.size() == cnt) ans = path;
    auto [x, y] = v;
    rep(d, 4) {
      ll di = x + dx[d], dj = y + dy[d];
      if (incl(di, 0, h) && incl(dj, 0, w) && g[di][dj] == '#') {
        if (none_of(all(path), [&](LP p) {
          return p == mp(di, dj);
        })) dfs(mp(di, dj));
      }
    }
  ...
  rep(i, h) rep(j, w) if (g[i][j] == '#') dfs({i, j});

  OUT(cnt);
  rep(i, cnt) {
    OUT(ans[i].fi + 1, ans[i].se + 1);
  }
}
