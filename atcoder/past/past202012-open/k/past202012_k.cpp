...
const string drul = "DRUL"; vl dx = {1, 0, -1, 0, 0}; vl dy = {0, 1, 0, -1, 0};
...
void solve() {
  ...
  dp[0] = 0;
  rep(S, 1, 1 << 16) {
    rep(i, 16) { ll x = i / 4, y = i % 4;// 投げる場所
      ll no = 0;
      ld sum = 0;
      rep(d, 5) {
        ll di = x + dx[d], dj = y + dy[d];
        if (incl(di, 0, 4) && incl(dj, 0, 4) && (S & 1 << (di * 4 + dj))) {
          ll t = S; change_bit(t, di * 4 + dj, 0);
          sum += dp[t];
        } else no++;
      }
      if (no < 5) {
        chmin(dp[S], sum / (5 - no) + (ld)5 / (5 - no));
      }
    }
  }
