...
void solve() {
  ...
  rep(len, 3, n + 1) {
    rep(l, n - len + 1) {
      ...
      if (s.substr(l, 3) == t) chmax(dp[l][r], dp[l + 3][r] + 1);
      if (s.substr(r - 3, 3) == t) chmax(dp[l][r], dp[l][r - 3] + 1);

      if (s[l] == t[0] && s[r - 1] == t[2]) {
        rep(mid, l + 1, r - 1) {
          ll rng1 = mid - l - 1, rng2 = r - mid - 2;
          if (rng1 % 3 || rng2 % 3) continue;
          if (s[mid] == t[1] && dp[l + 1][mid] == rng1 / 3 && dp[mid + 1][r - 1] == rng2 / 3) {
            chmax(dp[l][r], len / 3);
          }
        }
      }
