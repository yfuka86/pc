...
void solve() {
  ...
  ll wallcnt = 0;
  rep(i, n) rep(j, m) if (s[i][j] == '#') wallcnt++;
  ...
  rep(i, n) rep(j, m) {
    ...
    UnionFind uf(n * m);
    rep(i, n) rep(j, m) {
      if (i < n - 1 && s[i][j] == '.' && s[i + 1][j] == '.') uf.unite(i * m + j, (i + 1) * m + j);
      if (j < m - 1 && s[i][j] == '.' && s[i][j + 1] == '.') uf.unite(i * m + j, i * m + j + 1);
    }
    ll ma = 0;
    rep(i, n * m) chmax(ma, uf.size(i));

    if (ma == n * m - wallcnt + 1) ans++;
