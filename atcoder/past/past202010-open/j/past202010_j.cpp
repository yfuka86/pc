...
void solve() {
  ...
  ll AB = n, BA = n+1, AC = n+2, CA = n+3, BC = n+4, CB = n+5;
  rep(i, n) {
    if (s[i] == 'A') {
      G.add_directed_edge(i, AB, x[0]);
      G.add_directed_edge(BA, i, 0);
      G.add_directed_edge(i, AC, x[1]);
      G.add_directed_edge(CA, i, 0);
    }
    if (s[i] == 'B') {
      G.add_directed_edge(i, BA, x[0]);
      G.add_directed_edge(AB, i, 0);
      G.add_directed_edge(i, BC, x[2]);
      G.add_directed_edge(CB, i, 0);
    }
    if (s[i] == 'C') {
      G.add_directed_edge(i, CA, x[1]);
      G.add_directed_edge(AC, i, 0);
      G.add_directed_edge(i, CB, x[2]);
      G.add_directed_edge(BC, i, 0);
    }
  }

  OUT(dijkstra(G, 0)[n - 1]);
}
