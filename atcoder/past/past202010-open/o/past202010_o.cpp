...
vector<ll> dijkstra(Graph<ll> &G, ll start) {
  ...
  while(!que.empty()) {
    ...
    for(auto &to: G[v]) { ll nc = cost[v] + to.cost; if (chmax(cost[to], nc)) que.push({nc, to}); }
  }
  return cost;
}
...
void solve() {
  ...
  Graph<ll> G(n + 1);
  rep(i, n) {
    G.add_directed_edge(i + 1, i, -a[i]);
    G.add_directed_edge(i, i + 1, 0);
  }
  rep(i, m) {
    G.add_directed_edge(l[i] - 1, r[i], as[r[i]] - as[l[i] - 1] - c[i]);
  }
