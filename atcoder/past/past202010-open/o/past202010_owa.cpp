...
using S2 = ll;
S2 op2(S2 l, S2 r) { return max(l, r); }
S2 e2() { return 0; }
...
void solve() {
  ...
  vl as = csum(ra); reverse(all(as));
  // debug(as);
  vector<S> v; rep(i, n + 1) v.pb({i, as[i]});

  segtree<S, op, e> seg(v);
  segtree<S2, op2, e2> segnorm(n + 1);

  VEC3(ll, l, r, c, m);
  vi ord = argsort(r);
  rep(i, m) {
    ll from = l[ord[i]] - 1, to = r[ord[i]];

    // その区間内の最適
    S x = seg.prod(from, to); ll idx = x.idx;
    ll score = (x.x - as[idx]) + asum[to] - asum[idx] - c[ord[i]];
    // from以前から飛んでくるケース
    ll off = segnorm.prod(0, from);
    chmax(score, off + asum[to] - asum[from] - c[ord[i]]);

    if (score < 0) continue;
    S now = seg.get(to);
    seg.set(to, { to, as[to] + score });
    segnorm.set(to, score);
  }

  OUT(segnorm.all_prod());
}
