...
void solve() {
  ...
  vl inv(k);
  rep(i, k) inv[i] = inv_num(seq[i]);

  vv(ll, invs, k, k);
  rep(i, k) rep(j, k) {
    vl t; t.insert(t.end(), seq[i].begin(), seq[i].end()); t.insert(t.end(), seq[j].begin(), seq[j].end());
    // debug(t, inv_num(t), inv[i], inv[j]);
    invs[i][j] = inv_num(t) - inv[i] - inv[j];
  }
  // debug(invs);
