...
void solve() {
  ...
  rep(sh, h) rep(sw, w) {
    vv(ll, c, h, w, LINF); c[sh][sw] = 0;
    mpq<LT> que; que.push({c[sh][sw], sh, sw});
    while(!que.empty()) {
      auto [co, x, y] = que.top(); que.pop();
      if (co > c[x][y]) continue;
      rep(d, 4) {
        ll di = x + dx[d], dj = y + dy[d];
        if (incl(di, 0, h) && incl(dj, 0, w)) {
          if (chmin(c[di][dj], c[x][y] + a[di][dj])) {
            que.push({c[di][dj], di, dj});
          }
        }
      }
    }
    chmin(ans, a[sh][sw] + c[h - 1][0] + c[h - 1][w - 1] + c[0][w - 1]);
  }
  OUT(ans);
}
