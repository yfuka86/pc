...
void solve() {
  ...
  ll ans = 0;
  lazy_segtree<S, op, e, F, mapping, composition, id> sego(co), sege(ce);
  LL(q);
  rep(i, q) {
    LL(t);
    if (t == 1) {
      ...
      if (x & 1) {
        if (sege.get(x / 2) >= a) {
          sege.apply(x / 2, x / 2 + 1, -a);
          ans+=a;
        }
      } else {
        if (sego.get(x / 2) >= a) {
          sego.apply(x / 2, x / 2 + 1, -a);
          ans+=a;
        }
      }
    } else if (t == 2) {
      LL(a);
      if (sego.all_prod() >= a) {
        sego.apply(0, co.size(), -a);
        ans += co.size() * a;
      }
    } else {
      LL(a);
      if (min(sego.all_prod(), sege.all_prod()) >= a) {
        sego.apply(0, co.size(), -a);
        sege.apply(0, ce.size(), -a);
        ans += n * a;
      }
    }
  }
