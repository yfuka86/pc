...
void solve() {
  ...
  v3(ld, dp, n, sum + 1, 120, LINF);
  v3(ll, dpr, n, sum + 1, 120, -1);
  dp[0][0][0] = 0;

  rep(i, n - 1) rep(j, sum + 1) rep(k, 120) rep(nk, 120) {
    if (j + nk > sum) continue;
    ll diff = abs(nk - k);
    if (chmin(dp[i + 1][j + nk][nk], dp[i][j][k] + sqrt(1 + diff * diff))) {
      dpr[i + 1][j + nk][nk] = k;
    }
  }

  if (0) { // 興味で復元しただけ
    ll cur = n - 1, curs = sum, curk = 0;
    vl restore; restore.pb(curk);
    while (cur > 0) {
      ll lastk = dpr[cur][curs][curk];
      restore.pb(lastk);
      curs = curs - curk;
      curk = lastk;
      cur--;
