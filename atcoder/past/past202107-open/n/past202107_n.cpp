...
// sum
struct S {ll w, b;}; using F = ll;
S op(S l, S r) { return {l.w + r.w, l.b + r.b}; }
S e() { return {0, 0}; }
S mapping(F f, S x) { if (f) return {x.b, x.w}; else return x; }
...
void solve() {
  ...
  auto [fx, rfx] = compmap(x);
  auto [fy, rfy] = compmap(y);
  // debug(fx, rfx);

  vl qi(q); iota(all(qi), 0);
  sort(all(qi), [&](ll i1, ll i2) { return a[i1] < a[i2];});

  vector<S> vs;
  for(auto it = fy.begin(); it != fy.end(); ++it) {
    if (next(it) == fy.end()) break;
    ll sz = next(it)->fi - it->fi;
    vs.pb({sz, 0});
  }
  lazy_segtree<S, op, e, F, mapping, composition, id> seg(vs);

  mpq<LP> que;// xの終わりとqidx;
  ll cur = 0;
  ll ans = 0;
  ...
  for(auto it = fx.begin(); it != fx.end(); ++it) {
    ...
    while (que.size() && que.top().fi <= it->fi) {
      ll qi = que.top().se; que.pop();
      seg.apply(fy[b[qi]], fy[d[qi]], 1);
    }

    while (cur < q && a[qi[cur]] <= it->fi) {
      seg.apply(fy[b[qi[cur]]], fy[d[qi[cur]]], 1);
      que.push({c[qi[cur]], qi[cur]});
      cur++;
    }

    ans += seg.all_prod().b * (rt->fi - it->fi);
  }
  OUT(ans);
}
