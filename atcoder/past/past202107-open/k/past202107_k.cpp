...
void solve() {
  ...
  mpq<LT> que; vlp cost(n, {LINF, 0});
  cost[0] = {0, -a[0]}; que.push({0, -a[0], 0});
  while(!que.empty()) {
    auto [c, s, v] = que.top(); que.pop();
    if (cost[v].fi < c) continue;
    for(auto &to: G[v]) {
      ll nc = cost[v].fi + to.cost;
      ll ns = cost[v].se - a[to];
      if (chmin(cost[to], mp(nc, ns))) que.push({nc, ns, to});
    }
  }
