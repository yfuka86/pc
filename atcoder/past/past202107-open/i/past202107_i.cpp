...
void solve() {
  ...
  ld e = sqrt(dx * dx + dy * dy) / 2;

  ld midx = (ld)(x1 + x2) / 2, midy = (ld)(y1 + y2) / 2;

  ld tx = x1 - midx, ty = y1 - midy;
  ld cos = -(tx / e), sin = -(ty / e);

  rep(i, n) {
    ld x = a[i] - midx, y = b[i] - midy;
    OUT(x * cos + y * sin, -x * sin + y * cos);
  }
}
