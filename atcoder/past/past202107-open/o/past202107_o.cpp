...
void solve() {
  LL(n);
  VEC2(ll, _a, _b, n);
  if (_a[0] < _b[0]) OUTRET(-1);

  vl a, b;
  a.pb(0); b.pb(0); // 初日のコンピュータと同じ価格のものを買わない場合の処理のために最初を0で埋める
  ll ma = 0, cur = 0;
  rep(i, n) {
    cur += _a[i];
    if (chmax(ma, _b[i])) {
  ...
  lazy_segtree<S, op, e, F, mapping, composition, id> seg(n + 5);
  seg.set(0, 0);

  rep(i, n) {
    ll sofar = seg.get(i) + b[i];
    ll r = upper_bound(all(b), as[i + 2] - sofar) - b.begin();
    if (i + 1 < r) seg.apply(i + 1, r, sofar);
  }

  if (seg.get(n - 1) == LINF) OUT(-1);
  else {
    OUT(as.back() - seg.get(n - 1) - b[n - 1]);
  }
}
