...
void solve() {
  ...
  vv(ll, dp, h * w, h * w, LINF);
  queue<LP> que;
  dp[st][a] = 0; que.push({st, a});

  while (!que.empty()) {
    auto [su, lo] = que.front(); que.pop();
    ll co = dp[su][lo];
    ll sux = su / w, suy = su % w;
    ll lox = lo / w, loy = lo % w;
    rep(d, 4) {
      ll di = sux + dx[d], dj = suy + dy[d];
      if (di == lox && dj == loy) {
        ll di2 = di + dx[d], dj2 = dj + dy[d];
        if (incl(di2, 0, h) && incl(dj2, 0, w) && s[di2][dj2] != '#') {
          if (chmin(dp[di * w + dj][di2 * w + dj2], co + 1)) que.push({di * w + dj, di2 * w + dj2});
        }
      } else {
        if (incl(di, 0, h) && incl(dj, 0, w) && s[di][dj] != '#') {
          if (chmin(dp[di * w + dj][lo], co + 1)) que.push({di * w + dj, lo});
        }
      }
    }
  }
