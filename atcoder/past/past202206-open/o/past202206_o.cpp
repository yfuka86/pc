...
void solve() {
  ...
  rep(i, q) {
    CHR(c); IN(ps[i]);
    if (c == '+') query[i] = 0; else query[i] = 1;
  }

  if (q <= 10) { // TODO コーナーケース結局拾えてない
    Points now;
    mint ans = 0;
    rep(i, q) {
      mint sum = 0;
      fore(p, now) {
        sum += abs((ll)cross(ps[i], p));
      }
      if (query[i]) {
        auto it = find(all(now), ps[i]); now.erase(it);
        ans -= sum;
      } else {
        now.pb(ps[i]);
        ans += sum;
  ...
  vector<pair<Point, ll>> v(q);
  rep(i, q) v[i] = {ps[i], i};

  sort(begin(v), end(v), [](pair<Point, ll> _a, pair<Point, ll> _b) {
    Point &a = _a.fi, &b = _b.fi;
    if (a.pos() != b.pos()) return a.pos() < b.pos();
    return cross(a, b) > 0;
  });

  vl inv(q);
  rep(i, q) inv[v[i].se] = i;

  {
    vector<pair<Point, ll>> t = v; t.insert(t.end(), v.begin(), v.end()); // 2周分用意
    v = t;
  }
  ...
  ll l = 0;
  rep(r, q * 2) {
    while(cross(v[l].fi, v[r].fi) < 0 || l + q <= r) {
      to[l] = r;
      l++;
      if (l >= q) break;
    }
  ...
  // 前処理パート終わり
  segtree<S, op, e> segx(q * 2), segy(q * 2);
  ...
  rep(_i, q) {
    ...
    mint p1 = idx < to[idx] ? mint(pp.x) * segy.prod(idx, to[idx]) - mint(pp.y) * segx.prod(idx, to[idx]) : 0;
    mint p2 = to[idx] < idx + q ? mint(pp.y) * segx.prod(to[idx], idx + q) - mint(pp.x) * segy.prod(to[idx], idx + q) : 0;
    // debug(idx, to[idx], pp, p1, p2);

    if (query[_i]) { // 引き算
      ans -= p1 + p2;

      ll nx = segx.get(idx) - pp.x;
      ll ny = segy.get(idx) - pp.y;
      segx.set(idx, nx); segx.set(idx + q, nx);
      segy.set(idx, ny); segy.set(idx + q, ny);
    } else {
      ans += p1 + p2;
