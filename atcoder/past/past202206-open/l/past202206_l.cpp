...
void solve() {
  ...
  v3(ll, dp, n + 1, m + 1, k + 1);

  rep(i, n) rep(j, m) rep(kk, k + 1) {
    if (s[i] == t[j]) {
      chmax(dp[i + 1][j + 1][kk], dp[i][j][kk] + 1);
    } else {
      if (kk < k) chmax(dp[i + 1][j + 1][kk + 1], dp[i][j][kk] + 1);
      chmax(dp[i + 1][j + 1][kk], max(dp[i + 1][j][kk], dp[i][j + 1][kk]));
    }
  }
