...
void solve() {
  ...
  set<LP> starts, goals;
  rep(i, 1, h) starts.insert({i, 0});
  rep(i, 1, w - 1) starts.insert({h - 1, i});

  rep(i, 1, w) goals.insert({0, i});
  rep(i, 1, h - 1) goals.insert({i, w - 1});
  ...
  fore(sx, sy, starts) {
    ...
    vv(LP, from, h, w, {-1, -1});
    dp[sx][sy] = c[sx][sy];
    mpq<LT> que; que.push({c[sx][sy], sx, sy});
    while (!que.empty()) {
      auto [co, x, y] = que.top(); que.pop(); if (dp[x][y] < co) continue;
      rep(dx, -1, 2) rep(dy, -1, 2) {
        if (dx == 0 && dy == 0) continue;
        ll di = x + dx, dj = y + dy;
        if (di == 0 && dj == 0) continue;
        if (di == h - 1 && dj == w - 1) continue;
        if (incl(di, 0, h) && incl(dj, 0, w)) {
          if (chmin(dp[di][dj], co + c[di][dj])) {
            from[di][dj] = {x, y};
            que.push({co + c[di][dj], di, dj});
          }
        }
      }
    }
    fore(gx, gy, goals) {
      if (chmin(ans, dp[gx][gy])) {
        restore.clear();
        ll curx = gx, cury = gy;
        restore.pb({curx, cury});
        while (from[curx][cury].fi != -1) {
          auto [nx, ny] = from[curx][cury];
          curx = nx, cury = ny;
          restore.pb({curx, cury});
        }
      }
