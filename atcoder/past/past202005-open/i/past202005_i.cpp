...
void solve() {
  LL(n);
  vl row(n), column(n); iota(all(row), 0); iota(all(column), 0);
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      LL(a, b); --a; --b;
      if (!trans) swap(row[a], row[b]); else swap(column[a], column[b]);
    } else if (t == 2) {
      LL(a, b); --a; --b;
      if (!trans) swap(column[a], column[b]); else swap(row[a], row[b]);
    } else if (t == 3) {
      trans = !trans;
    } else {
      LL(a, b); --a; --b;
      if (trans) swap(a, b);
      OUT(row[a] * n + column[b]);
    }
  }
