...
void solve() {
  ...
  vl ob(l + 20); rep(i, n) ob[x[i]] = 1;
  vl dp(l + 20, LINF); dp[0] = 0;

  rep(i, l) {
    chmin(dp[i + 1], dp[i] + t[0] + (ob[i + 1] ? t[2] : 0));
    chmin(dp[i + 2], dp[i] + t[0] + t[1] + (ob[i + 2] ? t[2] : 0));
    chmin(dp[i + 4], dp[i] + t[0] + t[1] * 3 + (ob[i + 4] ? t[2] : 0));

    if (i + 4 > l) chmin(dp[l], dp[i] + t[0] / 2 + t[1] * (l - i) - t[1] / 2);
  }
