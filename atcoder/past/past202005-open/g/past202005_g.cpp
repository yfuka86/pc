...
void solve() {
  ...
  while (que.size()) {
    ...
    rep(dx, -1, 2) rep(dy, -1, 2) {
      ...
      ll di = nx + dx, dj = ny + dy;
      if (incl(di, 0, 1000) && incl(dj, 0, 1000) && !ob[di][dj]) {
        if (chmin(cost[di][dj], nc)) que.push({di, dj});
      }
    }
  }

  if (cost[x + off][y + off] == LINF) OUT(-1); else OUT(cost[x + off][y + off]);
}
