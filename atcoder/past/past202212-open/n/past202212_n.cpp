...
void solve() {
  ...
  auto add = [&](int i) {
    ...
    if (it != S.begin()) {
      sum -= *prev(it) * a[i] * 2;
      sum += a[i] * a[i] + *prev(it) * *prev(it);
    }
    if (next(it) != S.end()) {
      sum -= *next(it) * a[i] * 2;
      sum += a[i] * a[i] + *next(it) * *next(it);
    }
    if (it != S.begin() && next(it) != S.end()) {
      sum += *next(it) * *prev(it) * 2;
      sum -= *prev(it) * *prev(it) + *next(it) * *next(it);
    }
  };
  auto erase = [&](int i) {
    auto it = S.find(a[i]);
  ...
  auto rem = [&](int idx, int lid, int rid) {
    ans[idx] = sum;
  };
