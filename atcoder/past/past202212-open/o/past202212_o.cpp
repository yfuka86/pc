...
ll d;
using S = array<ll, 9>; using F = ll;
S op(S l, S r) {
  S nx;
  rep(i, 9) nx[i] = l[i] ^ r[i];
  return nx;
}
S e() { return S(); }
S mapping(F f, S x) {
...
void solve() {
  ...
  vl p10(d, 1); rep(i, d - 1) p10[i + 1] = p10[i] * 10;
  vector<S> v(n * 2, S());
  rep(i, n) {
    ll t = a[i];
    S tt;
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      ...
      LL(l, r, y); --l;
      seg.apply(l + off, r + off, y);
      ll l2 = max(l + off - n, 0), r2 = max(r + off - n, 0);
      seg.apply(l2, r2, y);
      ll l3 = min(l + off + n, n * 2), r3 = min(r + off + n, n * 2);
      seg.apply(l3, r3, y);
    } else {
      LL(l, r); --l;
      OUT(seg.prod(l + off, r + off)[0]);
    }
  }
}
