...
vmi calc(vmi f, vmi g, ll n) {
  rep(i, n) {
    rep(j, 1 << n) { if (j >> i & 1) continue;
      f[j] += f[j | 1 << i];
      g[j] += g[j | 1 << i];
    }
  }
...
void solve() {
  ...
  vv(mint, ans, n * 2 + 1, 1 << n);
  ll filter = (1 << n) - 1;

  rep(i, n + 1) rep(j, n + 1) {
    vmi at(1 << n), bt(1 << n);
    rep(k, 1 << n) {
      if (__builtin_popcount(k) == i) at[k ^ filter] = a[k];
      if (__builtin_popcount(k) == j) bt[k ^ filter] = b[k];
    }

    // debug(i, j, at, bt);

    // vmi ret = bitwise_and_convolution(at, bt);
    vmi ret = calc(at, bt, n);
    rep(k, 1 << n) {
      ans[i + j][k] += ret[k ^ filter];
    }
    // debug(ans);
  }
