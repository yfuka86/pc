...
void solve() {
  ...
  OfflineDynamicConnectivity odc(n, m + q);
  rep(i, m) odc.insert(i, e[i].fi, e[i].se);
  ...
  odc.build();
  odc.run([&](int k) {
    // debug(k, odc.comp, odc.cnt, odc.appear);
    if (qu[k].fi != -1) {
      ans.pb(odc.uf.find(qu[k].fi) == odc.uf.find(qu[k].se));
    }
  });
