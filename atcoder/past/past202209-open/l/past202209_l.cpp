...
void solve() {
  ...
  v3(ll, dp, n + 1, 3, 3, -LINF);
  dp[0][0] = {0, 0, 0};

  ll cur = 0;
  rep(i, 1, n + 2) {
    vv(ll, t, 3, 3);
    while (cur < m && q[ord[cur]] <= i) {
      t[l[ord[cur]]][r[ord[cur]]] += p[ord[cur]];
      cur++;
    }
    // debug(i, t);
    ...
    // 飾る
    rep(j, 3) rep(k, 3) {
      chmax(dp[i][(j + 1) % 3][k], dp[i - 1][j][(k + 1) % 3] + a[i - 1] + t[j][(k + 1) % 3]);
    }
    // 飾らない
    rep(j, 3) rep(k, 3) {
      chmax(dp[i][j][k], dp[i - 1][j][k] + t[j][k]);
    }
    // debug(dp[i]);
  }
