...
void solve() {
  ...
  if (d <= w) {
    OUTRET(M_PI * d * d / 4 / (h * w));
  }

  // 片方からはみ出す
  if (w < d && d <= h) {
    ld rad = acos(w / d);
    ld sec = (M_PI / 2 - rad) / (M_PI * 2) * d * d * M_PI;
    ld S = d * w * sin(rad) / 2 + sec;
    OUTRET(S / (h * w));
  }
  // 両方からはみ出す
  ld rad1 = acos(w / d), rad2 = acos(h / d);
  ld sec = (M_PI / 2 - rad1 - rad2) / (M_PI * 2) * d * d * M_PI;
  ld S = d * (w * sin(rad1) + h * sin(rad2)) / 2 + sec;
  OUTRET(S / (h * w));
}
