...
void solve() {
  ...
  rep(i, n) {
    rep_r(j, n + 1) {
      ...
      ll rem = as[n] - as[i + 1];
      ll need = k - (rem + dp[j]); // この回必要な最低数
      if (j < n) {
        // dp[j + 1]に入れる
        ll good = as[i] == 0 ? 1 : ceil(as[i + 1] * dp[j] + 1, as[i]);
        if (good - dp[j] <= a[i] && good <= k) chmin(dpt[j + 1], max(good, dp[j] + need));
      }
      chmin(dpt[j], dp[j] + max(0, need));
    }
    dp = dpt;
    dpt.assign(n + 1, LINF);
  }
