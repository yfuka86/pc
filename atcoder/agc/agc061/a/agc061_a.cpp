...
ll dfs(ll n, ll k, ll off) {
  ...
  if (c == f) {
    ll off2 = (k - 2) / 8 * 8;
    ll ret = off2 + p[9][(k - 2) % 8 + 1];
    // if (k != 3 && (k - 2) % 8 == 1) ret--;
    if (k != n - 1 && (k - 2) % 8 == 6) ret++;
    return off + ret;
  } else {
    ll diff = n - POW(2, f);
    if (k <= diff) {
      return off + dfs(n - POW(2, f), k, 0);
    } else if (k > POW(2, f)) {
      return off + dfs(n - POW(2, f), k - POW(2, f), POW(2, f));
    } else {
      return off + k;
    }
  }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = rg.l(2, 18);
    ll k = rg.l(1, n + 1);
    auto so = solve(n, k); auto na = naive(n, k);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k); debug(so); debug(na);
