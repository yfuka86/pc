...
void solve() {
  ...
  vl ls(n), rs(n);
  rep(i,n){
    ls[i] = lower_bound(all(b), a[i]) - b.begin();
    rs[i] = lower_bound(all(a), b[i]) - a.begin() - 1;
  }

  vmi dp(n+1,0);
  dp[0] = 1;
  rep(i,n){
    int l = lower_bound(all(rs), ls[i]) - rs.begin();
    dp[i + 1] = dp[i] - dp[l] / mint(2).pow(rs[i]-ls[i]+1);
  }

  OUT(dp[n] * mint(2).pow(n));
}
