...
struct S {vl op(3), a(3), l(3), r(3);};
S op(S l, S r) {

  rep(left, 3) rep(right, 3) {
    S x = {l.op + r.op, l.a | r.a, 0, 0};
    if (l.a && r.a) {
      x.op += __builtin_popcount(l.r | r.l);
      x.l = l.l;
      x.r = r.r;
    } else if (l.a) {
      x.l = l.l;
...
vl calc(string s, vlp q) {
  ...
  rep(i, q.size()) {
    auto [l, r] = q[i];
    S x = seg.prod(l, r);
    debug(l, r, x.op, x.a, x.l, x.r, x.op + __builtin_popcount(x.l) + __builtin_popcount(x.r));
    chmin(ans[i], x.op + __builtin_popcount(x.l) + __builtin_popcount(x.r));
  }
...
void solve() {
  ...
  auto a1 = calc(s1, query);
  auto a2 = calc(s2, query);
  auto a3 = calc(s3, query);

  rep(i, q) {
    OUT(mine(a1[i], a2[i], a3[i]));
  }
}
