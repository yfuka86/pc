...
struct Reachable {
  vvl a;
  Reachable(Graph<ll> g) {
    a.assign(g.size(), vl(g.size(), 0));
    rep(i, g.size()) a[i][i] = 1;
...
void solve() {
...

  {
    ...
    rep(k, 1, 51) {
      rep(from, 51) {
        if (from != from % k) G.add_directed_edge(from, from % k);
      }
    }
    Reachable r(G);
    rep(i, n) if (!r.ok(a[i], b[i])) OUTRET(-1);
  }
  ...
  while (cur) {
    rep(ma, cur) {
      ...
      Reachable r(G);
      bool valid = true;
      rep(i, n) if (!r.ok(a[i], b[i])) valid = false;
      if (valid) {
        if (ma > 0) ans |= 1ll << ma;
        cur = ma;
        break;
