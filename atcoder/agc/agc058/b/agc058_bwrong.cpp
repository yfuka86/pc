...
void solve() {
  ...
  // vl diff(n, 0); diff[0] = p[0];
  // rep(i, n - 1) {
  //   diff[i + 1] = p[i + 1] - p[i];
  // }
  // debug(diff);
  // reverse(all(diff));
  // rep(i, n) dp[i][diff[i]] = 1;
  ...
  set<ll> ids; ids.insert(-1);
  rep(i, n + 1) ids.insert(i);

  rep(i, n) {
    ll id = pinv[i];
    ids.erase(id);
    auto rt = ids.upper_bound(id);
    auto lt = prev(rt);
    ll r = *rt, l = *lt + 1;

    mint tmp = 1; // 何もしない

    vmi rv, lv; rv.pb(1); lv.pb(1);
    mint rsum = 1, lsum = 1;
    rep2_r(j, id + 1, r) {
      debug(j, r, dp[j][r]);
      rsum += dp[j][r];
    ...
    debug(lv, rv);
    rep2(j, l + 1, id) {
      dp[j][r] = rsum * lv[lv.size() - 1 - (j - l)];
    }
    rep2(j, id + 1, r) {
      dp[l][j] = lsum * rv[j - id - 1];
    }

    debug(i + 1, l, r, rsum, lsum);
    dp[l][r] = rsum * lsum;
  }
  debug(dp);
