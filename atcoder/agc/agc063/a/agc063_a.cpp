...
void solve() {
  ...
  set<ll> aid, bid;
  aid.insert(LINF); bid.insert(LINF);
  rep(i, n + 1) {
    if (s[i] == 'A') aid.insert(i);
    if (s[i] == 'B') bid.insert(i);
  }

  rep(i, 1, n + 1) {
    if (i & 1) { // aliceターン
      if (*bid.begin() != LINF) bid.erase(bid.begin());
    } else { // bobターン
      if (*aid.begin() != LINF) aid.erase(aid.begin());
    }

    if (*aid.begin() < *bid.begin()) OUT("Alice"); else OUT("Bob");
  }
}
