...
void solve() {
  ...
  string rx = "";
  rep(i, n) rx.pb(x[i] == '0' ? '1' : '0');

  map<ll, ll> divma;
  fore(d, div) {
    if (!(n / d & 1)) continue;
    string ss = x.substr(0, d), rss = rx.substr(0, d);
    bool ok = true;
    for (ll i = d; i < n; i += d) {
      string xss = x.substr(i, d);
      if (xss < (i / d & 1 ? rss : ss)) {
        ok = false;
        break;
      } else if (xss > (i / d & 1 ? rss : ss)) {
        break;
      }
    }
    divma[d] = ok;
  ...
  fore(d, div) {
    if (!(n / d & 1)) continue;
    cnt[d] += dp[d][0] + divma[d];
  }

  for(auto it = cnt.begin(); it != cnt.end(); it++) {
    auto rt = it; rt++;
    while (rt != cnt.end()) {
      if (rt->fi % it->fi == 0) rt->se -= it->se;
      rt++;
    }
  }
  // debug(cnt);
