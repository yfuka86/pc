...
void solve() {
  ...
  //   while(ok > ng + 1) {
  //     ll mid = (ok + ng) / 2;
  //     if (floor(d[i], mid + 1) > floor(d[i], mid + 2) + 1) ng = mid; else ok = mid;
  //   }
  //   togo[i] = ok;
  // }
  // rep(i, m) {
  //   ll ds = sqrt_ceil(d[i]);
  //   ll mi = LINF;
  //   rep2(j, max(0ll, ds - 2), ds + 2) {
  //     if (chmin(mi, j + floor(d[i], j + 1))) togo[i] = j;
  //   }
  // }

  priority_queue<LP, vector<LP>, greater<LP>> que;
  vl costs(G.size(), LINF);
  que.push(make_pair(togo[0], 0));
  while(!que.empty()) {
    ...
    for(auto &to: G[v]) {
      ...
      while(ok > ng + 1) {
        ll mid = (ok + ng) / 2;
        if (floor(d[to.idx], mid + 1) > floor(d[to.idx], mid + 2) + 1) ng = mid; else ok = mid;
      }
      ll nc;
      // if (c < togo[to.idx]) {
      //   nc = togo[to.idx] + to.cost + floor(d[to.idx], togo[to.idx] + 1);
      // } else {
      //   nc = c + to.cost + floor(d[to.idx], c + 1);
      // }
      nc = ok + to.cost + floor(d[to.idx], ok + 1);
      if (costs[to] > nc) que.push(make_pair(nc, to));
    }
  }

  if (costs[n - 1] == LINF) cout << -1 << "\n";
  else cout << costs[n - 1] << "\n";
}
