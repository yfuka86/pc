...
void solve() {
  ...
  vector<RotateVector<mint>> dp(n + 1);
  rep(i, n + 1) {
    vmi v(i, 0);
    dp[i] = {0, v};
  }
  // vector<vmi> dp(n + 1, vmi(n + 1, 0));
  dp[1][0] = 1;

  rep2(i, 1, n) {
    rep2_r(j, 1, n + 1) {
      if (j < n) dp[j + 1][a[i] % (j + 1)] +=  dp[j][0];
      if (j == 0) continue;
      dp[j].rotate(a[i]);
      // rotate(dp[j].begin(), dp[j].begin() + (j - a[i] % j), dp[j].begin() + j);
    }
  }
  mint ans = 0;
  rep2(i, 1, n + 1) {
