...
void solve() {
  ...
  vvl grundy(51, vl(1350, -1));
  grundy[0][0] = 0;
  grundy[0][1] = 0;

  function<ll(ll, ll)> calc = [&](ll w, ll b) {
    if (grundy[w][b] != -1) return grundy[w][b];
    if (w == 0 && b <= 1) return grundy[w][b] = 0;
    vl me(1350, 0);
    if (w >= 1) me[calc(w - 1, b + w)] = 1;
    if (b >= 2) {
      rep2(i, 1, floor(b, 2) + 1) {
        me[calc(w, b - i)] = 1;
      }
    }

    rep(i, 1350) {
      if (me[i] == 0) return grundy[w][b] = i;
    }
  };

  rep(i, 51) rep(j, 51) calc(i, j);

  ll game = 0;
  rep(i, N) game ^= grundy[W[i]][B[i]];
  if (game) cout << "First" << "\n"; else cout << "Second" << "\n";
}
