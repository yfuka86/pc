...
ld angle(tuple<ld,ld,ld> a, tuple<ld,ld,ld> b) {
  ld x, y, z, x2, y2, z2;
  tie(x,y,z) = a; tie(x2,y2,z2) = b;

  ld tx = sqrt(pow(x2 - x, 2) + pow(y2 - y, 2));
  ld ty = z2 - z;
  return atan2(tx, ty) * 180 / M_PI;
}

void solve() {
  ld T, L, X, Y, Q; cin >> T >> L >> X >> Y >> Q;
  tuple<ld,ld,ld> statue = {X, Y, 0};

  rep(i, Q) {
    ld e; cin >> e;

    ld theta = 2 * M_PI * (e / T);
    tuple<ld,ld,ld> wh = {0, -sin(theta) * L / 2, (1 - cos(theta)) * L / 2};
    cout << fixed << setprecision(12) << 90 - angle(statue, wh) << "\n";
  }
}
