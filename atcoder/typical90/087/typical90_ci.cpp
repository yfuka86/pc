...
void solve() {
  ...
  function<ll(ll)> calc = [&](ll x) {
    vvl cost(N, vl(N, LINF));
    rep(i, N) rep(j, N) cost[i][j] = cost[j][i] = (A[i][j] == -1 ? x : A[i][j]);
    rep(k, N) rep(i, N) rep(j, N) chmin(cost[i][j], cost[i][k] + cost[k][j]);

    ll c = 0;
    rep(i, N) rep2(j, i + 1, N) if (cost[i][j] <= P) c++;
    return c;
  };
  ...
  while (ng > ok + 1) {
    ll mid = (ok + ng) / 2;
    if (calc(mid) >= K) ok = mid; else ng = mid;
  }
  ll ok2 = 1e10, ng2 = 0;
  while (ok2 > ng2 + 1) {
    ll mid = (ok2 + ng2) / 2;
    if (calc(mid) > K) ng2 = mid; else ok2 = mid;
  }
  if (ok - ok2 > 1e9) cout << "Infinity" << "\n"; else cout << ok - ok2 + 1 << "\n";
}
