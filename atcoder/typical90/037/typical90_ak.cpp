...
void solve() {
  ll W, N; cin >> W >> N;
  vector<LT> a(N);
  rep(i, N) { ll l, r, v; cin >> l >> r >> v; a[i] = {l, r, v}; }

  segtree<S, op, e> dp(W + 1);
  dp.set(0, 0);

  rep(i, N) {
    ll l, r, v; tie(l, r, v) = a[i];
    rep_r(j, W + 1) {
      if (j - l + 1 < 1) continue;
      ll from = dp.prod(max<ll>(0, j - r), j - l + 1);
      if (from >= 0 && from + v > dp.get(j)) dp.set(j, from + v);
    }
  }
  cout << dp.get(W) << "\n";
}
