...
void solve() {
  ...
  vvvl dp(N + 1, vvl(2, vl(S + 1, 0)));
  dp[0][0][0] = 1;

  rep(i, N) {
    rep(j, S) {
      if (!dp[i][0][j] && !dp[i][1][j]) continue;
      if (j + A[i] <= S) dp[i + 1][0][j + A[i]] = 1;
      if (j + B[i] <= S) dp[i + 1][1][j + B[i]] = 1;
    }
  }
  if (!dp[N][0][S] && !dp[N][1][S]) { cout << "Impossible" << "\n"; return; }

  string ans = "";
  ll rem = S;
  rep2_r(i, 1, N + 1) {
    if (dp[i][0][rem]) { ans.pb('A'); rem -= A[i - 1]; }
    else if (dp[i][1][rem]) { ans.pb('B'); rem -= B[i - 1]; }
  }
  reverse(all(ans));
  cout << ans << "\n";
}
