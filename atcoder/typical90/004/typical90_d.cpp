...
void solve() {
  ll H, W; cin >> H >> W;
  vvl grid(H, vl(W, 0)); rep(i, H) rep(j, W) cin >> grid[i][j];

  vvl hsum(H, vl(W + 1, 0));
  rep(i, H) rep(j, W) hsum[i][j + 1] = hsum[i][j] + grid[i][j];

  vvl wsum(W, vl(H + 1, 0));
  rep(j, W) rep(i, H) wsum[j][i + 1] = wsum[j][i] + grid[i][j];

  vvl ans(H, vl(W, 0));
  rep(i, H) rep(j, W) {
    ans[i][j] = hsum[i].back() + wsum[j].back() - grid[i][j];
  }
  coutmatrix(ans);
}
