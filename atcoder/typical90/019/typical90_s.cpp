...
void solve() {
  ...
  vvl dp(2 * N + 1, vl(2 * N + 1, LINF));

  function<ll(ll, ll)> calc = [&](ll l, ll r){
    assert((r - l) % 2 == 0);
    if (dp[l][r] != LINF) return dp[l][r];
    if (r - l == 2) return dp[l][r] = abs(a[r - 1] - a[l]);

    ll ret = LINF;
    chmin(ret, calc(l + 1, r - 1) + abs(a[l] - a[r - 1]));
    for (ll nl = l + 2; nl < r; nl += 2) {
      chmin(ret, calc(l, nl) + calc(nl, r));
    }
    return dp[l][r] = ret;
  };
