...
void solve() {
  ll N, K; cin >> N >> K;
  vector<vl> ab(5001, vl(5001, 0));
  vector<vl> sum(5002, vl(5002, 0));
  ...
  rep(i, 5002) {
    rep(j, 5001) {
      if (i < 5001) sum[i][j + 1] = sum[i][j] + ab[i][j];
  ...
  rep(i, 5001) {
    rep(j, 5001) {
      if (i + K + 1 < 5002 && j + K + 1 < 5002) {
        chmax(ans, sum[i + K + 1][j + K + 1] + sum[i][j] - sum[i][j + K + 1] - sum[i + K + 1][j]);
      }
    }
  }
  cout << ans << "\n";
