...
void solve() {
  ll R, G, B, K, X, Y, Z; cin >> R >> G >> B >> K >> X >> Y >> Z;
  init_f();
  vmi rcomb(200001), gcomb(200001);
  rep2(i, K - Y, R + 1) rcomb[i] = comb(R, i);
  rep2(i, K - Z, G + 1) gcomb[i] = comb(G, i);
  vmi conv = convolution(rcomb, gcomb);

  mint ans = 0;
  rep2(i, K - X, B + 1) {
    if (K - i < conv.size()) ans += comb(B, i) * conv[K - i];
  }
  cout << ans.val() << "\n";
}
