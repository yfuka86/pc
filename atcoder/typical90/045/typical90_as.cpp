...
void solve() {
  ll N, K; cin >> N >> K;
  vlp P(N); rep(i, N) cin >> P[i].first >> P[i].second;

  vl dp(1 << N, 0);
  rep(S, 1 << N) {
    rep(i, N) rep(j, N) {
      if (S & 1 << i && S & 1 << j) chmax(dp[S], dist(P[i], P[j]));
    }
  }

  vvl dp2(K + 1, vl(1 << N, LINF));
  dp2[0][0] = 0;
  rep(S, 1 << N) {
    for (ll s = S; s > 0; s = S & (s - 1)) {
      rep(k, K) {
        chmin(dp2[k + 1][S], max(dp2[k][S ^ s], dp[s]));
      }
    }
  }
