...
void solve() {
  ...
  rep(i, 60) {
    rep(S, 1 << N) {
      bool valid = true;
      rep(j, Q) {
        ll x, y, z; tie(x, y, z) = a[j];
        ll xb = S >> x & 1, yb = S >> y & 1, zb = S >> z & 1, wb = w[j] >> i & 1;
        if (wb != (xb | yb | zb)) { valid = false; break; }
      }
      // cout << "valid:" << valid << "i:" << i << "S:" << S << endl;
      if (valid) dp[i] += 1;
    }
  }
  // coutarray(dp);
