...
int main()
{
  ...
  rep(S, 1 << H)  {
    ...
    rep(j, W) {
      ...
      rep(i, H) {
        if (S >> i & 1) {
          if (color == 0) color = p[i][j];
          if (color != p[i][j]) valid = false;
        }
      }
      if (valid) possible[color]++;
    }

    ll row_count = 0;
    for (auto [color, count]: possible) {
      chmax(row_count, count);
    }
    chmax(ans, __builtin_popcount(S) * row_count);
  }
