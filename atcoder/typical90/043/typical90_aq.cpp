...
void solve() {
  ...
  queue<LP> que;
  que.push(s);
  vvvl costs(H, vvl(W, vl(4, LINF)));
  rep(i, 4) costs[s.first][s.second][i] = 0;

  while (!que.empty()) {
    auto [x, y] = que.front(); que.pop();
    ll ncost = *min_element(all(costs[x][y])) + 1;
    rep(i, 4) {
      ll k = 1;
      while (true) {
        ll nx = x + dx[i] * k;
        ...
        if (grid[nx][ny]) break;
        if (chmin(costs[nx][ny][i], ncost)) que.push({nx, ny});
        else break;
        k++;
      }
    }
  ...
  cout << *min_element(all(costs[t.first][t.second])) - 1 << "\n";
}
