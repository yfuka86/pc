...
void solve() {
  ...
  rep(i, n) rep(j, n) sum[i + 1][j + 1] += sum[i + 1][j] + sum[i][j + 1] + grid[i][j] - sum[i][j];
  // rep(i, n) rep(j, n) sum[i + 1][j + 1] += sum[i][j + 1] + grid[i][j];
  // rep(i, n) rep(j, n) sum[i + 1][j + 1] += sum[i + 1][j];

  map<ll, ll> count;
  rep(i, n + 1) rep(j, n + 1) count[sum[i][j]]++;

  rep(i, N) cout << count[i + 1] << "\n";
}
