...
void solve() {
  ...
  rep(i, N) {
    ll t; cin >> t;
    rep(_, t) { ll a; cin >> a; T[i].set(--a); }
  }
  rep(i, M) { ll s; cin >> s; if (s == 1) S.set(i); }

  BS I;
  rep(i, M) {
    BS op; bool found = false;
    for (auto itr = T.begin(); itr != T.end(); itr++) {
      BS bs = *itr;
      if (bs[i]) { op = bs; T.erase(itr); found = true; break; }
    }
    if (found) {
      if (I[i] != S[i]) I ^= op;
      for (auto &bs: T) if (bs[i]) bs ^= op;
    } else {
      if (I[i] != S[i]) { cout << 0 << "\n"; return; }
    }
  }

  cout << mod_pow(2, T.size()) << "\n";
}
