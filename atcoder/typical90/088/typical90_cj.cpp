...
void solve() {
  ...
  vvl banlist(N, vl(N, 1));
  rep(i, Q) {
    ll x, y; cin >> x >> y; x--; y--;
    banlist[x][y] = banlist[y][x] = 0;
  }
  ...
  bool done = false;
  function<bool(ll)> dfs = [&](ll cur) {
    rep2(i, cur, N) {
      bool valid = true;
      for (auto s: selected) valid &= banlist[s][i];
      if (!valid) continue;

      selected.pb(i);

      ll sum = 0;
      for (auto s: selected) sum += A[s];
      if (msum[sum].size() > 0) {
        cout << msum[sum].size() << "\n";
        coutarray(msum[sum], 1);
        cout << selected.size() << "\n";
        coutarray(selected, 1);
        return false;
      } else msum[sum] = selected;
