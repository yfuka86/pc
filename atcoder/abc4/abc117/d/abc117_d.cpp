...
void solve() {
  ...
  vl cnt(50);
  rep(b, 50) rep(i, n) if (a[i] & 1ll << b) cnt[b]++;

  vv(ll, dp, 51, 2, -LINF); dp[0][0] = 0;
  // debug(cnt);
  rep(i, 50) rep(ma, 2) rep(b, 2) {
    chmax(dp[i + 1][!(k & 1ll << i) && !ma ? 1 : ma], dp[i][ma] + (n - cnt[i]) * (1ll << i));
    chmax(dp[i + 1][(k & 1ll << i) && ma ? 0 : ma], dp[i][ma] + cnt[i] * (1ll << i));
  }
  OUT(dp[50][0]);
}
