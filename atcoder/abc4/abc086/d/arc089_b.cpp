...
// 二次元累積和（一応）
vvi csum2d(vvi &a) {
  ll H = a.size(), W = a[0].size(); vvi sum(H + 1, vi(W + 1, 0));
...
void solve() {
  ...
  rep(i, n) {
    LL(x, y); CHR(c);
    if (c == 'W') wc.pb({x, y}); else bc.pb({x, y});
  }

  vv(int, ans, k * 2 + 1, k * 2 + 1);
  {
    vv(int, b, k * 4, k * 4);
    rep(i, bc.size()) {
      auto [x, y] = bc[i];
      ...
      ll dy = y % (k * 2);
      rep(di, 2) rep(dj, 2) {
        b[dx + di * k * 2][dy + dj * k * 2]++;
      }
    }
    vvi bs = csum2d(b);
    rep(i, k * 2 + 1) rep(j, k * 2 + 1) {
      ans[i][j] += bs[i][j] + bs[i + k][j + k] - bs[i + k][j] - bs[i][j + k];
      ans[i][j] += bs[i + k][j + k] + bs[i + k * 2][j + k * 2] - bs[i + k * 2][j + k] - bs[i + k][j + k * 2];
    }
  }

  {
    ...
    rep(i, wc.size()) {
      ...
      ll dy = y % (k * 2);
      rep(di, 2) rep(dj, 2) {
        w[dx + di * k * 2][dy + dj * k * 2]++;
      }
    }
    vvi ws = csum2d(w);
    rep(i, k * 2 + 1) rep(j, k * 2 + 1) {
      ans[i][j] += ws[i + k][j] + ws[i + k * 2][j + k] - ws[i + k * 2][j] - ws[i + k][j + k];
      ans[i][j] += ws[i][j + k] + ws[i + k][j + k * 2] - ws[i + k][j + k] - ws[i][j + k * 2];
    }
  }
