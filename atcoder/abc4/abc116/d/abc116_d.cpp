...
void solve() {
  ...
  ll sum = 0;
  map<ll, ll> used;
  multiset<LP> extra, rem;
  rep(i, n) {
    auto [d, t] = sushi[i];

    if (i < k) {
      sum += -d;
      if (used[t]) extra.insert({-d, t});
      used[t]++;
    } else {
      rem.insert(sushi[i]);
    }
  ...
  ll ans = 0;
  chmax(ans, sum + POW(used.size(), 2));

  while (extra.size() && rem.size()) {
    while (rem.size() && used.find(rem.begin()->se) != used.end()) {
      rem.erase(rem.begin());
    }
    if (rem.size() == 0) break;

    auto [d, t] = *extra.begin();
    auto [d2, t2] = *rem.begin();
