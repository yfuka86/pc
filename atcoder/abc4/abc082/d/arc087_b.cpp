...
void solve() {
  ...
  vs cmd = split(s, 'T');
  rep(i, cmd.size()) {
    if (i & 1) yv.pb(cmd[i].size());
    else xv.pb(cmd[i].size());
  }

  vl dpx(20001, 0), dpy(20001, 0); dpx[10000] = 1; dpy[10000] = 1;
  {
    rep(i, xv.size()) {
      if (xv[i]) {
        vl dptx(20001);
  ...
  if (dpx[x + 10000] && dpy[y + 10000]) OUT("Yes"); else OUT("No");
}
