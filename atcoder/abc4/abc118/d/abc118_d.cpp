...
void solve() {
  LL(n, m); VL(a, m);
  vl cost = {LINF, 2, 5, 5, 4, 5, 6, 3, 7, 6};
  vl num; rep(i, m) num.pb(a[i]); sort(rall(num));
  ...
  rep(i, m) {
    rep(j, n + 1) {
      if (dp[j][0] != '$') continue;
      if (j + cost[num[i]] > n) break;
      string t = dp[j]; t.pb('0' + num[i]);
      if (dp[j + cost[num[i]]].size() < t.size()) {
        dp[j + cost[num[i]]] = t;
      } else if (dp[j + cost[num[i]]].size() == t.size()) chmax(dp[j + cost[num[i]]], t);
    }
  }
  OUT(dp[n].substr(1));
}
