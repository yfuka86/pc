...
void solve() {
  ...
  rep(i, 4) {
    vl t;
    if (i & 1) t.pb(1); else t.pb(0);
    if (i & 1 << 1) t.pb(1); else t.pb(0);
    rep(i, 2, n) {
      if (t.back()) { // 羊
        if (s[i - 1] == 'o') t.pb(t[t.size() - 2]);
        else t.pb(!t[t.size() - 2]);
      } else {
        if (s[i - 1] == 'o') t.pb(!t[t.size() - 2]);
        else t.pb(t[t.size() - 2]);
      }
    ...
    bool valid = true;
    if (t.front()) { // 羊
      if (s[0] == 'o' && t[1] != t[n - 1]) valid = false;
      if (s[0] == 'x' && t[1] == t[n - 1]) valid = false;
    } else {
      if (s[0] == 'o' && t[1] == t[n - 1]) valid = false;
      if (s[0] == 'x' && t[1] != t[n - 1]) valid = false;
    }
    if (t.back()) { // 羊
      if (s[n - 1] == 'o' && t[0] != t[n - 2]) valid = false;
      if (s[n - 1] == 'x' && t[0] == t[n - 2]) valid = false;
    } else {
      if (s[n - 1] == 'o' && t[0] == t[n - 2]) valid = false;
      if (s[n - 1] == 'x' && t[0] != t[n - 2]) valid = false;
    }
    ...
    if (valid) {
      string ans = "";
      rep(i, n) if (t[i]) ans.pb('S'); else ans.pb('W');
      OUT(ans);
      return;
    }
  }
