...
void solve() {
  ...
  vlp p;
  rep(i, N) {
    string s = time[i].substr(0, 4), t = time[i].substr(5, 4);
    ll from = floor(stoi(s), 5) * 5, to = ceil(stoi(t), 5) * 5;
    if (to % 100 == 60) to += 40;
    p.pb({from, to});
  }
  sort(all(p));
  ...
  rep2(i, 1, N) {
    if (end < p[i].first) {
      ans.pb({start, end});
      start = p[i].first, end = p[i].second;
    } else {
      chmax(end, p[i].second);
    }
  }
  ans.pb({start, end});

  for (auto [s, t]: ans) {
    string ss = to_string(s), tt = to_string(t);
    while (ss.size() < 4) ss.insert(0, 1, '0');
    while (tt.size() < 4) tt.insert(0, 1, '0');
    cout << ss << "-" << tt << "\n";
  }
}
