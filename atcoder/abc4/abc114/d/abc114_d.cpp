...
void solve() {
  ...
  map<ll, ll> pf;
  rep2(i, 2, n + 1) {
    for (auto [p, cnt]: prime_factorize(i)) {
      pf[p] += cnt;
    }
  }
  // debug(pf);
  ll ans = 0;
  for (auto [p, cnt] : pf) if (cnt >= 74) ans++;
  for (auto [p, cnt] : pf) {
    if (cnt >= 24) {
      for (auto [p2, cnt2] : pf) if (p2 != p && cnt2 >= 2) ans++;
    }
    if (cnt >= 14) {
      for (auto [p2, cnt2] : pf) if (p2 != p && cnt2 >= 4) ans++;
    }
  }
  for (auto [p, cnt] : pf) {
    if (cnt >= 2) {
      ll t = 0;
      for (auto [p2, cnt2] : pf) if (p2 != p && cnt2 >= 4) t++;
      ans += t * (t - 1) / 2;
    }
  }
  cout << ans << "\n";
