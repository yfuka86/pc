...
void solve() {
  ...
  rep(S1, 1, 1 << n) {
    ...
    vl rem;
    rep(i, n) {
      if (S1 & 1 << i) sum += l[i]; else rem.pb(l[i]);
    }
    cost = (__builtin_popcount(S1) - 1) * 10 + abs(sum - a);
    ...
    rep(S2, 1, 1 << m) {
      ...
      vl rem2;
      rep(i, m) {
        if (S2 & 1 << i) sum += rem[i]; else rem2.pb(rem[i]);
      }
      cost2 += (__builtin_popcount(S2) - 1) * 10 + abs(sum - b);

      ll m2 = rem2.size();
      rep(S3, 1, 1 << m2) {
        ll sum = 0;
        rep(i, m2) {
          if (S3 & 1 << i) sum += rem2[i];
        }

        chmin(ans, cost2 + (__builtin_popcount(S3) - 1) * 10 + abs(sum - c));
      }
    }
  }
  OUT(ans);
