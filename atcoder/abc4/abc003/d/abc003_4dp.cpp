...
void solve() {
  ...
  vvvmi dp(X * Y + 1, vvmi(D + L + 1, vmi(4, 0)));
  dp[0][0][0] = 1;
    ...
    rep(j, D + L + 1) {
      rep(k, 4) {
        ...
        if (j < D + L) {
          ...
          if (i % Y == 0) add |= 1;
          if (i % Y == Y - 1) add |= 2;
          dp[i + 1][j + 1][k | add] += dp[i][j][k];
        }
      }
    }
    if (i == Y - 1) rep(k, 4) dp[i + 1][0][k] = 0;
    if (i == (X - 1) * Y - 1) rep(k, 4) dp[i + 1][D + L][k] = 0;
  }
  // coutmatrix(dp[X * Y]);

  init_f();
  cout << a * comb(D + L, D) * dp[X * Y][D + L][3] << "\n";
}
