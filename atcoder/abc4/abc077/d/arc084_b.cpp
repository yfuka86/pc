...
void solve() {
  ...
  vl div(1000200), rem(1000200);
  rep(i, 1000200) {
    div[i] = i / 10;
    rem[i] = i % 10;
  }
  vl dsum(100020);
  rep(i, 100020) { vl d = digits(i); dsum[i] = sum_of(d); }

  const ll ma = 100000;
  vl dptot(100020, LINF);
  map<ll, ll> dp, dpt;
  dp[0] = 0;
  // dp[桁目まで確定][余り] = 確定桁和の最小
  ll ans = LINF;
  rep(_, ma) {
    fore(j, v, dp) {
      if (_ > 0 && j > 0) chmin(ans, v + dsum[j]);
      rep(d, 10) {
        ll t = n * d + j, nv = v + rem[t];
        if (nv < ans) {
          if (!chmin(dptot[div[t]], nv)) continue;

          if (dpt.find(div[t]) == dpt.end()) {
            dpt[div[t]] = v + rem[t];
          } else chmin(dpt[div[t]], v + rem[t]);
        }
      }
    }
    dp = dpt;
