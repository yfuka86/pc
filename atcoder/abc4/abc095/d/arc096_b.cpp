...
void solve() {
  ...
  rep(i, 1, n + 1) vsum[i] -= x[i - 1];
  rep_r(i, 1, n + 1) rvsum[i] -= c - x[n - i];

  vl vsumback = vsum;
  rep(i, 1, n + 1) vsumback[i] -= x[i - 1];
  vl rvsumback = rvsum;
  rep_r(i, 1, n + 1) rvsumback[i] -= c - x[n - i];
  // debug(vsum, rvsum);
  // debug(vsumback, rvsumback);

  ll ans = max(*max_element(all(vsum)), *max_element(all(rvsum)));
  {
    ll ma = 0;
    rep_r(i, 1, n + 1) {
      chmax(ma, rvsum[n - i]);
      chmax(ans, vsumback[i] + ma);
    }
  }
  {
    ll ma = 0;
