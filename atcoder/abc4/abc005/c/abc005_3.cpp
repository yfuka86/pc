...
void solve() {
  ll T; cin >> T;
  vlp ab;
  ll N; cin >> N; rep(i, N) { ll t; cin >> t; ab.pb({ t, 0ll }); }
  ll M; cin >> M; rep(i, M) { ll t; cin >> t; ab.pb({ t, 1ll }); }
  sort(all(ab));

  queue<ll> que;
  for (auto [time, type]: ab) {
    if (type) {
      if (que.size() == 0) {cout << "no" << "\n"; return;}
      bool valid = false;
      while (!que.empty()) {
        ll a = que.front(); que.pop();
        if (a + T >= time) { valid = true; break; }
      }
      if (!valid) { cout << "no" << "\n"; return;}
    } else {
      que.push(time);
