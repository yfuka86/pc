...
void solve() {
  LL(n);
  n++;
  // 下からbitでmaxを管理する時は半開区間で扱わないといけない
  ...
  rep(i, ma) {
    ...
    rep(k, 2) {
      ...
      // 0, 0
      dp[i + 1][o1][0] += dp[i][k][1]; // 1
      dp[i + 1][o0][0] += dp[i][k][0]; // 0

      // 0, 1
      dp[i + 1][o0][1] += dp[i][k][1]; // 0
      dp[i + 1][o1][0] += dp[i][k][0]; // 1

      // 1, 1
      dp[i + 1][o1][1] += dp[i][k][1]; // 1
      dp[i + 1][o0][1] += dp[i][k][0]; // 0
    }
    // debug(i + 1, dp[i + 1]);
  }
