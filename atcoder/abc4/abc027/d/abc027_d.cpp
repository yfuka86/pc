...
ll solve(string S) {
  ...
  sort(all(ope));
  ll move = ope.size();
  return accumulate(ope.begin() + move / 2, ope.end(), 0LL) - accumulate(ope.begin(), ope.begin() + move / 2, 0LL);
}

ll naive(string S) {
  ll move = 0;
  rep(i, S.size()) if (S[i] == 'M') move++;
  ll ans = -LINF;
  rep(s, 1 << move) {
...
void compare() {
  ...
  while (true) { c++; if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll N = 10;
    string S = RandGen().str(N, {'+', '-'});
    rep(_, 8) S.pb('M'); random_shuffle(all(S));
    auto s = solve(S); auto n = naive(S);
    if (n != s) {
      cout << c << "times tried" << "\n";
      cout << S << "\n";
      cout << "solve: " << s << "\n";
