...
void solve() {
  ...
  rep(i, n) {
    if (s[i] == '?' || s[i] == 'B') {
      ll acnt = lbs(dict['A'], i);
      ll q1cnt = lbs(dict['?'], i);
      ll ccnt = dict['C'].end() - upper_bound(all(dict['C']), i);
      ll q2cnt = dict['?'].end() - upper_bound(all(dict['?']), i);

      ll f = free - (s[i] == '?');

      ans += mint(acnt) * ccnt * mint(3).pow(f);
      ans += mint(q1cnt) * ccnt * mint(3).pow(f - 1);
      ans += mint(acnt) * mint(q2cnt) * mint(3).pow(f - 1);
      ans += mint(q1cnt) * mint(q2cnt) * mint(3).pow(f - 2);
    }
