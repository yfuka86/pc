...
ll solve(ll a, ll b, ll x) {
  ll ans = 0;
  if (a == 0) { ans++; a++; }
  return ans + b / x - (a - 1) / x;
}

ll naive(ll a, ll b, ll x) {
  ll ans = 0;
  rep(i, a, b + 1) {
    if (i % x == 0) ans++;
  }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll a = rg.l(0, 1e3), b = rg.l(a, 1e4), x = rg.l(1, 1e3);
    auto so = solve(a, b, x); auto na = naive(a, b, x);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(a, b, x); debug(so); debug(na);
