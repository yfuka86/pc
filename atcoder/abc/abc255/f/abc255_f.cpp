...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll l, ll r) {
    if (l + 1 >= r) return;
    ll par = P[l];
    ll firstright = binary_search([&](ll mid) {
      return ipos[P[mid]] <= ipos[par];
    }, l, r);
    firstright++;

    ll left = l + 1 == firstright ? 0 : P[l + 1] + 1;
    ll right = firstright == r ? 0 : P[firstright] + 1;
    assert(par < n);
    ans[par] = {left, right};
    if (l + 1 <= firstright && firstright <= r) {
      dfs(l + 1, firstright);
      dfs(firstright, r);
    }
  };
