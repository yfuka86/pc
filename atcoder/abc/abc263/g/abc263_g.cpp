...
void solve() {
  ...
  PrimeSieve ps(20000000);

  mf_graph<ll> mf(n * 2 + 2);
  ll s = n * 2, t = s + 1;

  rep(i, n) {
    if (a[i] & 1) mf.add_edge(s, i, b[i]);
    else mf.add_edge(i + n, t, b[i]);
  }

  vlp onep;
  rep(i, n) {
    rep2(j, i+1, n) {
      if (ps.is_prime[a[i] + a[j]]) {
        if (a[i] == 1) { onep.pb({i, j}); continue; }
        if (a[j] == 1) { onep.pb({j, i}); continue; }

        if (a[i] & 1) mf.add_edge(i, j + n, INF);
        else mf.add_edge(j, i + n, INF);
      }
    }
  }
