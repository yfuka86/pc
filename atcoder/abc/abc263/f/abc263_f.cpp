...
void solve() {
  ...
  rep2_r(i, 1, 1 << n) {
    ll lma = 0, rma = 0;
    ll num = floor_pow2(dp[i << 1].size());
    for (auto [id, x]: dp[i << 1]) chmax(lma, x + c[id][num]);
    for (auto [id, x]: dp[i << 1 | 1]) chmax(rma, x + c[id][num]);
    for (auto [id, x]: dp[i << 1]) chmax(dp[i][id], rma + x);
    for (auto [id, x]: dp[i << 1 | 1]) chmax(dp[i][id], lma + x);
  }
