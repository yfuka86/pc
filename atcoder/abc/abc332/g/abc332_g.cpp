...
void solve() {
  LL(n, m); VL(a, n); VL(b, m);

  vl box(m), nxt(n, 0);
  vector<map<ll, ll>> inside(m);


  // 有効である必要がある
  function<void(ll, ll, ll)> put = [&](ll i, ll j, ll num) {
    box[j] += num;
    inside[j][i] += num;
  };

  // 種類/消化したい数
  function<ll(ll, ll)> rem = [&](ll i, ll num) {
    rep(j, nxt[i], m) {
      ll lim = (i + 1) * (j + 1) - inside[j][i];
      ll available = mine(num, lim, b[j] - box[j]);
      put(i, j, available);
      num -= available;
      if (b[j] - box[j] == 0 || (i + 1) * (j + 1) - inside[j][i] == 0) nxt[i]++;
      if (num == 0) break;
    }
    // 残った数がかえる
    return num;
  ...
  rep(i, n) {
    rep(j, m) {
      ...
      if (b[j] - box[j] >= to_put) {
        ...
        for(auto it = inside[j].begin(); it != inside[j].end();) {
          if (nxt[it->first] == m) { it = inside[j].erase(it); continue; }

          ll now = it->second;
          ll nxt = rem(i, now);
          ll room = mine(to_put, lim, now - nxt);
          put(i, j, room);
          ans += room;
          to_put -= room;

          if (nxt > 0) it = inside[j].erase(it);
          else it++;
