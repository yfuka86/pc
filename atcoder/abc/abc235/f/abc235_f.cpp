...
int main()
{
  ...
  vector dp(n + 1, vector(1 << 10, modint(0)));
  vector dpsum(n + 1, vmi(1 << 10, modint(0)));

  ll maxS = 0;
  modint maxsum = modint(0);

  rep(i, n) {
    // cout << "index" << i << endl;
    rep(j, 10) {
      rep(S, 1 << 10) {
        ll nextS = S | 1 << j; if (S == 0 && j == 0) nextS = 0;

        // if (dp[i][S] > 0 && dpsum[i][S] > 0 && ((nextS & required) == required)) {
        //   cout << i << " " << j << " set(";
        //   outS(S);
        //   cout << ") next(";
        //   outS(nextS);
        //   cout << ")" << dp[i][S] << ":" << dpsum[i][S] << endl;
        // }

        // 通常の遷移
        dp[i + 1][nextS] += dp[i][S];
        dpsum[i + 1][nextS] += dpsum[i][S] * modint(10) + dp[i][S] * modint(j);
      }
      if (vn[i] > j) {
        // ここまでmaxのものを加えていく集合を考える
        ll s = maxS | 1 << j; if (maxS == 0 && j == 0) s = 0;
        dp[i + 1][s] += 1;
        dpsum[i + 1][s] += maxsum * modint(10) + modint(j);
      }
    }
    // maxのものの遷移
    maxS |= 1 << vn[i];
    maxsum = maxsum * modint(10) + modint(vn[i]);
  }
  // 最後の桁までmaxだったものを足す
  dp[n][maxS] += 1;
  dpsum[n][maxS] += maxsum;
