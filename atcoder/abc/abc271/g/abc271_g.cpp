...
void solve() {
  ...
  rep(off, 24) {
    ...
    rep(i, 1, 25) {
      if (c[(off + i) % 24] == 'T') {
        dp[(off + i) % 24][off] = cur * x / 100;
        cur *= (100 - x);
        cur /= 100;
      } else {
        dp[(off + i) % 24][off] = cur * y / 100;
        cur *= (100 - y);
        cur /= 100;
      }
    }
    rep(i, 24) dp[(off + i) % 24][off] /= mint(1) - cur;
  }

  vmi first(24);
  rep(i, 24) first[i] = dp[i][23];
  vmi ans = m_map(m_pow(dp, n - 1), first);
  mint a = 0;
  // debug(ans);
  rep(i, 24) if (c[i] == 'A') a += ans[i];
  OUT(a);
}
