...
void solve() {
  ...
  rep(i, k) big.insert(0); rep(i, k, n) small.insert(0);
  vl a(n, 0);

  ll thres = 0; // 以上ならbig
  ll bsum = 0, ssum = 0;
  rep(i, q) {
    ll from = a[x[i] - 1]; a[x[i] - 1] = y[i];
    if (from >= thres) {
      big.erase(big.find(from));
      big.insert(y[i]);
      bsum += y[i] - from;
    } else {
      small.erase(small.find(from));
      small.insert(y[i]);
      ssum += y[i] - from;
    }

    if (*prev(small.end()) > *big.begin()) {
      ll v1 = *prev(small.end()), v2 = *big.begin();
