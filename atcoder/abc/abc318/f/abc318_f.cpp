...
void solve() {
  ...
  ll ans = 0;
  rep(i, n + 1) {
    ll left = i, right = n - left;
    LP defa = {i > 0 ? x[i - 1] + 1 : -LINF, i < n ? x[i] - 1 : LINF};
    vv(LP, dp, n+1, n+1, {LINF, -LINF});
    dp[0][0] = defa;

    rep(arm, n) {
      rep(lside, arm + 1) {
        ll rside = arm - lside;
        auto [rngl, rngr] = dp[lside][rside];
        // 左をとりたい
        if (lside + 1 <= left) {
          ll nr = min(rngr, x[left - lside - 1] + l[arm]); // ありうる右端

          if (nr >= rngl) {
            chmin(dp[lside + 1][rside].fi, rngl);
            chmax(dp[lside + 1][rside].se, nr);
          }
        }
        // 右をとりたい
        if (rside + 1 <= right) {
          ll nl = max(rngl, x[left + rside] - l[arm]); // ありうる左端

          if (rngr >= nl) {
            chmin(dp[lside][rside + 1].fi, nl);
            chmax(dp[lside][rside + 1].se, rngr);
          }
        }
      }
    }
    // debug(dp);
    auto [lans, rans] = dp[left][right];
    // debug(lans, rans);
    if (rans > lans) ans += rans - lans + 1;
  }

  rep(i, n) {
    mpq<ll> que;
    rep(j, n) {
      que.push(abs(x[j] - x[i]));
    }
    bool valid = true;
    rep(arm, n) {
      if(que.top() > l[arm]) valid = false;
      que.pop();
    }
    if (valid) ans++;
  }
