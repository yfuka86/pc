...
vvl build_btrie(vl &a, ll maxb = 30) {
  ...
  rep(i, n) {
    ...
    rep_r(b, maxb) {
      ll nx = (a[i] & 1 << b) ? 1 : 0;
      if (trie[cur][nx] == -1) { trie[cur][nx] = trie.size(); trie.pb({-1, -1}); }
      cur = trie[cur][nx];
    }
  }
  return trie;
...
void solve() {
  ...
  auto trie = build_btrie(a);

  ll ans = LINF;
  function<void(ll, ll, ll)> dfs = [&](ll v, ll cur, ll d) {
    if (trie[v][0] == -1 && trie[v][1] == -1) { chmin(ans, cur); return; }
    if (trie[v][0] != -1 && trie[v][1] != -1) {
      dfs(trie[v][0], cur | 1 << d - 1, d - 1);
      dfs(trie[v][1], cur | 1 << d - 1, d - 1);
      return;
    }
    if (trie[v][0] == -1) dfs(trie[v][1], cur, d - 1);
    if (trie[v][1] == -1) dfs(trie[v][0], cur, d - 1);
  };
