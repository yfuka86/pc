...
int calculateIntersectionVolume(int x1, int y1, int z1, int x2, int y2, int z2) {
    // それぞれの軸に沿った重なっている部分の長さを計算
    int overlapX = std::max(0, std::min(x1 + 7, x2 + 7) - std::max(x1, x2));
    int overlapY = std::max(0, std::min(y1 + 7, y2 + 7) - std::max(y1, y2));
    int overlapZ = std::max(0, std::min(z1 + 7, z2 + 7) - std::max(z1, z2));

    // 重なっている部分の体積を計算
    return overlapX * overlapY * overlapZ;
}

int calculateIntersectionVolumeForThreeCubes(int x1, int y1, int z1,
                                             int x2, int y2, int z2,
                                             int x3, int y3, int z3) {
    // 各軸に沿った3つの立方体の重なる部分の長さを計算
    int overlapX = std::max(0, std::min({x1 + 7, x2 + 7, x3 + 7}) - std::max({x1, x2, x3}));
    int overlapY = std::max(0, std::min({y1 + 7, y2 + 7, y3 + 7}) - std::max({y1, y2, y3}));
    int overlapZ = std::max(0, std::min({z1 + 7, z2 + 7, z3 + 7}) - std::max({z1, z2, z3}));
    return overlapX * overlapY * overlapZ;
}
...
void solve() {
  ...
  rep(di, 15) rep(dj, 15) rep(dk, 15) {
    rep(di2, 15) rep(dj2, 15) rep(dk2, 15) {
      ll v2 = calculateIntersectionVolume(7, 7, 7, di, dj, dk);
      ll v3 = calculateIntersectionVolume(di, dj, dk, di2, dj2, dk2);
      ll v4 = calculateIntersectionVolume(7, 7, 7, di2, dj2, dk2);
      ll v5 = calculateIntersectionVolumeForThreeCubes(7, 7, 7, di, dj, dk, di2, dj2, dk2);

      ll vv2 = v2 + v3 + v4 - v5 * 3;
      ll vv3 = v5;
      ll vv1 = 343 * 3 - vv2 * 2 - vv3 * 3;
      // if (di == 0 && dj == 6 && dk == 0 && di2 == 6 && dj2 == 0 && dk2 == 0) {
      //   debug(vv1, vv2, vv3);
      // }
      if (vv1 == v[0] && vv2 == v[1] && vv3 == v[2]) {
        OUT("Yes");
        OUTRET(7, 7, 7, di, dj, dk, di2, dj2, dk2);
      }
    }
  }
  OUT("No");
