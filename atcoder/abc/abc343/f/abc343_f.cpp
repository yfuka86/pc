...
S op(S l, S r) {
  vl a;
  a.pb(l.ma); a.pb(l.ma2);
  a.pb(r.ma); a.pb(r.ma2);
  a.pb(- LINF - 1);
  uniq(a);
  ll nma = a[a.size() - 1], nma2 = a[a.size() - 2];
  ll nsz = 0, nsz2 = 0;
  if (l.ma == nma) nsz += l.sz;
  if (l.ma == nma2) nsz2 += l.sz;
  if (l.ma2 == nma2) nsz2 += l.sz2;
  if (r.ma == nma) nsz += r.sz;
  if (r.ma == nma2) nsz2 += r.sz;
  if (r.ma2 == nma2) nsz2 += r.sz2;
  return {nma, nma2, nsz, nsz2};
}

S e() { return {-LINF, -LINF, 0, 0}; }
