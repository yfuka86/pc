...
struct S { uint64_t h; ll sz; };
S op(S l, S r) {
  return {rh.combine(l.h, r.h, r.sz), l.sz + r.sz};
}
S rop(S l, S r) {
  return {rh.combine(r.h, l.h, l.sz), l.sz + r.sz};
}
S e() { return {0, 0}; }
...
void solve() {
  ...
  segtree<S, op, e> seg(sv);
  segtree<S, rop, e> rseg(sv);
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      ...
      LL(l, r); l--;
      Yes(seg.prod(l, r).h == rseg.prod(l, r).h);
    }
  }
