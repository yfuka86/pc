...
void solve() {
  ...
  queue<LT> que; que.push({1, 1, 0});
  v3(ll, cost, n, m, 5, LINF);
  cost[1][1][0] = 0;

  while (!que.empty()) {
    auto [x, y, dir] = que.front(); que.pop();
    rep(d, 4) {
      if (dir != 0 && d != dir - 1) continue;
      ll di = x + dx[d], dj = y + dy[d];
      if (incl(di, 0, n) && incl(dj, 0, m) && s[di][dj] == '.' && chmin(cost[di][dj][d + 1], cost[x][y][dir] + 1)) {
        que.push({di, dj, d + 1});
        if (incl(di + dx[d], 0, n) && incl(dj + dy[d], 0, m) && s[di + dx[d]][dj + dy[d]] == '#' && chmin(cost[di][dj][0], cost[x][y][dir] + 1)) {
          que.push({di, dj, 0});
        }
      }
    }
  ...
  rep(i, n) rep(j, m) {
    ...
    rep(k, 5) if (cost[i][j][k] != LINF) reachable = true;
    if (reachable) ans++;
  }
  OUT(ans);
}
