...
void solve() {
  ...
  ll thres = 500;
  vector<set<ll>> adj(n), adjs(n);
  map<LP, ll> ss;
  rep(i, q) {
    LL(a, b, c);
    ll A = 1 + (a * (1 + lasta) % 998244353) % 2;
    ll B = 1 + (b * (1 + lasta) % 998244353) % n;
    ll C = 1 + (c * (1 + lasta) % 998244353) % n;
    --B; --C;
    ...
    if (A == 1) {
      ...
      if (adj[B].size() == thres) { // 超えた
        fore(ad, adj[B]) {
          fore(ad2, adjs[ad]) {
            ss[{min(B, ad2), max(B, ad2)}] = ad;
          }
          adjs[ad].insert(B);
        }
      }
      if (adj[C].size() == thres) { // 超えた
        fore(ad, adj[C]) {
          fore(ad2, adjs[ad]) {
            ss[{min(C, ad2), max(C, ad2)}] = ad;
          }
          adjs[ad].insert(C);
        }
      }

      if (adj[B].size() >= thres) { // Bが星である
        adjs[C].insert(B);
      }
      if (adj[C].size() >= thres) { // Cが星である
        adjs[B].insert(C);
      }
      if (adj[B].size() >= thres) {
        fore(ad, adjs[C]) {
          LP key = {min(B, ad), max(B, ad)};
          if (ad != B) ss[key] = C;
        }
      }
      if (adj[C].size() >= thres) { // Cが星である
        fore(ad, adjs[B]) {
          LP key = {min(C, ad), max(C, ad)};
          if (ad != C) ss[key] = B;
        }
      }
