...
void solve() {
  ...
  vlt bs;
  rep(i, n) {
    bs.pb({h[i], -min(w[i], d[i]), max(w[i], d[i])});
    bs.pb({w[i], -min(h[i], d[i]), max(h[i], d[i])});
    bs.pb({d[i], -min(w[i], h[i]), max(w[i], h[i])});
  }
  sort(all(bs));

  vl ws;
  fore(h,w,d, bs) ws.pb(-w);
  auto [mp, rmp] = compmap(ws);

  segtree<S, op, e> seg(mp.size());
  ...
  fore(h, w, d, bs) {
    if (h > hcur) {
      hcur = h;
      while (que.size()) {
        auto [i, v] = que.front(); que.pop();
        if (seg.get(i) > v) seg.set(i, v);
      }
    }
    ll mind = seg.prod(0, mp[-w]);
    if (mind < d) OUTRET("Yes");
    que.push({mp[-w], d});
  }
  OUT("No");
}
