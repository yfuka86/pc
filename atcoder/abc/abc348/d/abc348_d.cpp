...
vvl bfs(vs &a, ll sx, ll sy) {
  ...
  while(!que.empty()) {
    ...
    rep(d, 4) {
      ll di = i + dx[d];
      ll dj = j + dy[d];
      if (incl(di, 0, h) && incl(dj, 0, w) && a[di][dj] != '#') {
        if (chmin(cost[di][dj], cost[i][j] + 1)) que.push({di, dj});
      }
    }
  }
...
void solve() {
  ...
  ll sidx = -1;
  rep(i, n) if (r[i] == sx && c[i] == sy) sidx = i;
  if (sidx == -1) OUTRET("No");
  ...
  ll ans = dijkstra(G, sidx)[n - 1];
  if (ans == LINF) OUT("No"); else OUT("Yes");
}
