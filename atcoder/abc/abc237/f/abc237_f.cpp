...
int main()
{
  ...
  vector<vector<vector<vector<modint>>>> dp(N + 1, vector(M + 1, vector(M + 1, vector(M + 1, modint(0)))));
    ...
    rep(j1, M + 1) {
      rep(j2, M + 1) {
        rep(j3, M + 1) {
          rep2(k, 1, M + 1) {
            if (k <= j1) {
              dp[i + 1][k][j2][j3] += dp[i][j1][j2][j3];
            } else if (j2 == 0 || k <= j2) {
              dp[i + 1][j1][k][j3] += dp[i][j1][j2][j3];
            } else if (j3 == 0 || k <= j3) {
              dp[i + 1][j1][j2][k] += dp[i][j1][j2][j3];
            }
          }
        }
      }
