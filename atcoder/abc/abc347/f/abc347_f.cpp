...
void solve() {
  ...
  csum2d<ll> csum(a);
  vv(ll, sum, n - m + 1, n - m + 1);
  rep(i, n - m + 1) rep(j, n - m + 1) {
    sum[i][j] = csum.query(i, j, i + m, j + m);
  }
  debug(sum);
  ll k = n - m + 1;
  ...
  ll ans = 0;
  {
    ...
    SegmentTree2D seg(k, k, [](ll a, ll b) { return max(a, b); }, 0LL);
    rep(i, k) rep(j, k) seg.set(i, j, sum[i][j]);
    seg.build();
    // 縦分割
    rep(bordx, m, k) {
      ll upper = 0;
      rep(bordy, m, k) {
        chmax(upper, seg.query(0, bordx - m + 1, 0, bordy - m + 1) + seg.query(0, bordx - m + 1, bordy, k));
      }
      chmax(ans, upper + seg.query(bordx, k, 0, k));

      ll lower = 0;
      rep(bordy, m, k) {
        chmax(lower, seg.query(bordx, k, 0, bordy - m + 1) + seg.query(bordx, k, bordy, k));
      }
      chmax(ans, lower + seg.query(0, bordx - m + 1, 0, k));
    }

    rep(bordx, m, k) {
      ll v1 = seg.query(0, bordx - m + 1, 0, k);
      rep(bordx2, bordx + m, k) {
        chmax(ans, v1 + seg.query(bordx, bordx2 - m + 1, 0, k) + seg.query(bordx2, k, 0, k));
      }
    }
  }
