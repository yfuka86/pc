...
int main()
{
  ...
  int N, M;
  cin >> N >> M;
  vector<vector<bool>> toy(N, vector<bool>(N, false)), toy2(N, vector<bool>(N, false));
  ...
  rep(i, M) {
    int a, b; cin >> a >> b; a--; b--;
    toy[a][b] = true; toy[b][a] = true;
  }
  rep(i, M) {
    int a, b; cin >> a >> b; a--; b--;
    toy2[a][b] = true; toy2[b][a] = true;
  }
  ...
  do {
    ...
    rep(i, N) {
      rep(j, N) {
        if (!toy[i][j]) continue;
        if (!toy2[perm[i]][perm[j]]) { err = true; break; }
      }
      if (err) break;
    }
    if (!err) { cout << "Yes" << "\n"; return 0; }
