...
void solve() {
  ...
  ll ss = s.size();
  vv(ll, fr, 26, ss + 1);
  rep(i, ss) fr[s[i] - 'a'][i + 1]++;
  rep(i, 26) rep(j, ss) fr[i][j + 1] += fr[i][j];

  // 無理な場合
  rep(i, t.size()) {
    if (fr[t[i] - 'a'][ss] == 0) OUTRET(0);
  }
  ...
  ll ans = binary_search([&](ll mid) {
    ...
    rep(i, t.size()) {
      ll c = t[i] - 'a';
      ll nx = binary_search([&](ll mid2){
        ll a = fr[c][mid2 % ss] + fr[c][ss] * (mid2 / ss);
        ll b = fr[c][cur % ss] + fr[c][ss] * (cur / ss);
        return a - b >= mid;
      }, cur + ceil(mid, fr[c][ss]) * ss, cur);
      cur = nx;
      if (ceil(cur, ss) > n) return false;
    }

    return ceil(cur, ss) <= n;
  }, 0, n * ss / t.size() + 5);
  OUT(ans);
}
