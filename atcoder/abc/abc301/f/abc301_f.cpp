...
void solve() {
  ...
  vmi fh(n); // 同一の大文字が2つ以上含まれる文字列を前から数え上げる
  vl cnt(26); ll ma = 0, free = 0;
  rep(i, n) {
    if (s[i] == '?') {
      free++;
    } else if (isupper(s[i])) {
    ...
    if (ma >= 2) fh[i] = mint(52).pow(free);
    else {
      ll f1 = 0; rep(j, 26) if (cnt[j] == 1) f1++;
      ll big = 26 - f1;
      // debug(i, f1, big, free);
      // 2つ以上含まれないものを大文字を含む数（場所）ごとに数え上げて全体から引く
      rep(j, min(free, big) + 1) {
        mint t = mint(26).pow(free - j);
        // 出現する大文字の選び方 * 位置 * 並び順
        t *= comb(big, j) * comb(free, j) * fac(j);
        fh[i] += t;
      }
      fh[i] = mint(52).pow(free) - fh[i];
    }
  }
  // debug(fh);
  ...
  mint ans = 0;
  reverse(all(s));
  vv(mint, dp, n + 1, 3); dp[0][0] = 1;
  rep(i, n - 1) {
    if (s[i] == '?') {
      dp[i + 1][0] += dp[i][0] * 26; // 小文字
      dp[i + 1][1] += dp[i][0] * 26; // 大文字
      dp[i + 1][2] += dp[i][1] * 26; // 小文字
      dp[i + 1][1] += dp[i][1] * 26; // 大文字
    } else if (isupper(s[i])) {
      dp[i + 1][1] += dp[i][0];
      dp[i + 1][1] += dp[i][1];
    } else {
    ...
    ans += dp[i + 1][2] * fh[n - i - 2];
  }
  OUT(mint(52).pow(free) - ans);
}
