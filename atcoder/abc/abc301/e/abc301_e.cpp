...
void solve() {
  ...
  LP S, G;
  rep(i, h) rep(j, w) {
    if (a[i][j] == 'o') okashi.pb({i, j});
    if (a[i][j] == 'S') S = {i, j};
    if (a[i][j] == 'G') G = {i, j};
  }

  okashi.pb(S); okashi.pb(G);

  vvl dist;

  fore(sx, sy, okashi) {
    queue<LP> que; que.push({sx, sy});
    vv(ll, cost, h, w, LINF); cost[sx][sy] = 0;
    while (!que.empty()) {
      auto [x, y] = que.front(); que.pop();
      rep(d, 4) {
        ll di = dx[d], dj = dy[d];
        if (incl(x + di, 0, h) && incl(y + dj, 0, w) && a[x + di][y + dj] != '#') {
          if (chmin(cost[x + di][y + dj], cost[x][y] + 1)) que.push({x + di, y + dj});
        }
      }
    }
  ...
  vv(ll, dp, 1 << okashi.size(), okashi.size(), LINF);
  dp[1 << okashi.size() - 2][okashi.size() - 2] = 0;

  rep(S, 1 << okashi.size()) {
    rep(from, okashi.size()) {
      rep(to, okashi.size()) { if (S & 1 << to) continue;
        chmin(dp[S | 1 << to][to], dp[S][from] + dist[from][to]);
      }
    }
  }
  ...
  ll ans = -1;
  rep(S, 1 << okashi.size()) {
    if (dp[S].back() <= t) chmax(ans, __builtin_popcount(S) - 2);
  }
  OUT(ans);
}
