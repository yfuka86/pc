...
void solve() {
  ...
  function<vvl(vvl, ll, ll)> op = [&](vvl ori, ll ioff, ll joff) {
    vvl ret = ori;
    rep(i, h - 1) rep(j, w - 1) ret[h - 2 - i + ioff][w - 2 - j + joff] = ori[i + ioff][j + joff];
    return ret;
  };

  vv(ll, st, h, w);
  rep(i, h) rep(j, w) st[i][j] = i * w + j + 1;
  map<vvl, ll> dp; dp[st] = 0;
  queue<vvl> que; que.push(st);
  while(que.size() > 0) {
    vvl from = que.front(); que.pop();
    ll c = dp[from];
    rep(i, 2) rep(j, 2) {
      vvl nx = op(from, i, j);
      if (dp.find(nx) == dp.end()) {
        dp[nx] = dp[from] + 1;
        if (c + 1 < 10) que.push(nx);
      }
    }
  }

  map<vvl, ll> dp2; dp2[s] = 0;
  que.push(s);
  while(que.size() > 0) {
    vvl from = que.front(); que.pop();
    ll c = dp2[from];
