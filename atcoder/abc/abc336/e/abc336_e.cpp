...
ll solve(ll n) {
  vl ds = digits(n);
  ll ma = ds.size() * 9;
  while(ds.size() < 20) ds.pb(0);
  reverse(all(ds));

  // dp[桁][mod][和][ここまで最大のものが0]
  v4(ll, dp, 21, ma, ma + 1, 2);

  ll ans = 0;
  rep(sum, 1, ma + 1) { // 桁和ごとに走査
    rep(i, 21) rep(j, ma) rep(k, ma + 1) rep(l, 2) dp[i][j][k][l] = 0;
    dp[0][0][0][0] = 1;

    rep(i, 20) {
      rep(d, 10) {
        rep(j, sum) rep(k, sum + 1) {
          if (k + d > sum) continue;

          if (ds[i] >= d) dp[i + 1][(j * 10 + d) % sum][k + d][ds[i] > d] += dp[i][j][k][0];
          dp[i + 1][(j * 10 + d) % sum][k + d][1] += dp[i][j][k][1];
        }
      }
    }
    // debug(sum, dp[20][0][sum][0], dp[20][0][sum][1]);
    ans += dp[20][0][sum][0] + dp[20][0][sum][1];
  }
  return ans;
}
...
ll naive(ll n) {
  ll ans = 0;
  rep(i, 1, n + 1) {
    ans += i % sum_of(digits(i)) == 0;
    debug(i, i % sum_of(digits(i)) == 0);
  }
  return ans;
}
