...
ll solve(string s, ll k) {
  ...
  rep_r(l, n) rep(r, l + 2, n + 1) {
    rep(m, l + 1, r) chmax(dp[l][r], dp[l][m] + dp[m][r]);

    rep(m, l + 1, r) {
      if (s[l] != 'o' || s[m] != 'f') continue;
      if (dp[l + 1][m] != m - (l + 1)) continue;
      ll x = dp[m + 1][r];
      x = r - (m + 1) - x;
      if (x <= k) dp[l][r] = r - l;
    }
...
string removeSubstring(const std::string& str, std::size_t index, std::size_t length) {
    if (index >= str.size()) {
        return str;
    }
...
ll naive(string s, ll k) {
  ...
  function<ll(string)> dfs = [&](string s) {
    ...
    rep(i, s.size() - 1) {
      if (s[i] == 'o' && s[i + 1] == 'f') {
        rep(kk, k + 1) {
          if (i + 2 + kk > s.size()) break;
          chmin(cur, dfs(removeSubstring(s, i, 2 + kk)));
        }
      }
    }
    return cur;
