...
void solve() {
  ...
  vv(ll, cost, n, 2, LINF); cost[0][0] = 0;
  mpq<LT> que; que.push({0, 0, 0});

  while (!que.empty()) {
    auto [co, t, v] = que.top(); que.pop();
    if (cost[v][t] < co) continue;

    rep(j, n) {
      if (v == j) continue;

      if (t == 0) {
        if (chmin(cost[j][0], co + d[v][j] * a)) {
          que.push({cost[j][0], 0, j});
        }
      }
      if (chmin(cost[j][1], co + d[v][j] * b + c)) {
        que.push({cost[j][1], 1, j});
      }
    }
  }

  OUT(min(cost[n - 1][1], cost[n - 1][0]));
}
