  ...
  rep(_, 3) {
    ...
    rep(z, 101) {
      ...
      { // いずれかの端点が含まれる
        ...
        csum2d<ll> cs(a);
        fore(i, zs[z]) {
          if (i == 1) debug(from[i][0], from[i][1], to[i][0], to[i][1], cs.query(from[i][0], from[i][1], to[i][0], to[i][1]));
          ans[i] += cs.query(from[i][0], from[i][1], to[i][0], to[i][1]);
        }
      }
      rep(i, 101) rep(j, 101) a[i][j] = 0;
      { // 終点が含まれる
        fore(i, zs[z]) {
          a[100 - to[i][0]][100 - to[i][1]]++;
        }
        csum2d<ll> cs(a);
        fore(i, zs[z]) {
          if (i == 1) debug(100 - to[i][0], 100 - to[i][1], 100 - from[i][0], 100 - from[i][1], cs.query(100 - to[i][0], 100 - to[i][1], 100 - from[i][0], 100 - from[i][1]));
          ans[i] += cs.query(100 - to[i][0], 100 - to[i][1], 100 - from[i][0], 100 - from[i][1]) - 1;
        }
      }
    }
    ...
    rep(i, n) {
      rotate(from[i].begin(), from[i].begin() + 1, from[i].end());
      rotate(to[i].begin(), to[i].begin() + 1, to[i].end());
    }
  }
