...
void solve() {
  ...
  function<mint(ll, ll, ll)> calc = [&](ll l, ll r, ll d) {
    ...
    if (memo.count({l, r, d})) return memo[{l, r, d}];
    vv(mint, dp, n + 1, 11);
    dp[0][0] = 1;

    rep(i, l, r) rep(j, l + 1, r + 1) rep(from, 10) rep(to, from + 1, 11) {
      if (rng[d][i][j][to - 1]) {
        dp[r][to] += dp[l][from] * calc(i, j, d + 1);
      }
    }
    debug(l, r, d, dp);
    return memo[{l, r, d}] = sum_of(dp[r]);
  };
