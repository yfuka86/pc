...
void solve() {
  ...
  rep(i, k) {
    LL(x, y); --x; --y;
    if (uf.same(x, y)) invalid = true;
    ll rx = uf.root(x), ry = uf.root(y);
    if (rx > ry) swap(rx, ry);
    cand.insert({rx, ry});
  }
  LL(q);
  if (invalid) { rep(i, q) OUT("No"); return; }
  rep(i, q) {
    LL(x, y); --x; --y;
    ll rx = uf.root(x), ry = uf.root(y);
    if (rx > ry) swap(rx, ry);
    if (cand.find({rx, ry}) != cand.end()) OUT("No"); else OUT("Yes");
  }
}
