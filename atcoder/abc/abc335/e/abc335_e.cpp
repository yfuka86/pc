...
void solve() {
  ...
  rep(i, m) {
    auto [u, v] = es[i];
    if (a[u] < a[v]) {
      G.add_directed_edge(uf.root(u), uf.root(v));
    } else if (a[v] < a[u]) {
      G.add_directed_edge(uf.root(v), uf.root(u));
    }
  }
  // debug(G.g);
  ...
  vl cost(n, -LINF); cost[uf.root(0)] = 1;
  rep(i, n) {
    ll v = ord[i];
    fore(to, G[v]) {
      chmax(cost[to], cost[v] + 1);
    }
  }
  OUT(max(0, cost[uf.root(n-1)]));
}
