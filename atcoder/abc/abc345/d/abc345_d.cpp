...
void solve() {
  ...
  function<bool(ll, ll, ll)> put = [&](ll h2, ll w2, ll s){
    if (dp.find({h2, w2, s}) != dp.end()) return dp[{h2, w2, s}];
    if (h2 == 0 || w2 == 0) return dp[{h2, w2, s}] = true;
    bool valid = false;
    rep(i, n) if (s & 1 << i) {
      ll ns = s ^ (1 << i);
      {
        ll nh = h2 - a[i];
        ll nw = w2 - b[i];
        if (nh >= 0 && nw >= 0) for(ll nns = ns; nns > 0; nns = (nns - 1) & ns) {
          if (put(nh, w, nns) && put(h - nh, nw, ns ^ nns)) valid = true;
          if (put(nh, w - nw, nns) && put(h, nw, ns ^ nns)) valid = true;
        }
      }
      {
        ll nh = h2 - b[i];
