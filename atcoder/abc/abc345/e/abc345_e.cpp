...
void solve() {
  ...
  // dp[x][y]のとき、x-yごとにmaxをとる
  vv(LP, ma, 200005, vlp(2, {-LINF, -1}));
  ma[0][0] = {0, -1};

  rep(i, 1, n + 1) {
    rep(j, k + 1) {
      if (!incl(i - j - 1, 0, 200005)) continue;
      // 被り
      if (ma[i - j - 1][0].se == c[i - 1]) {
        if (ma[i - j - 1][1].fi != -LINF) chmax(dp[i][j], ma[i - j - 1][1].fi + v[i - 1]);
      } else {
        if (ma[i - j - 1][0].fi != -LINF) chmax(dp[i][j], ma[i - j - 1][0].fi + v[i - 1]);
      }
      {
        {
          bool inserted = false;
          rep(k, ma[i - j].size()) {
            if (ma[i - j][k].se == c[i - 1]) {
              inserted = true;
              chmax(ma[i - j][k].fi, dp[i][j]);
            }
          }
          if (!inserted) ma[i - j].pb({dp[i][j], c[i - 1]});
        }
        uniq(ma[i - j]);
        reverse(all(ma[i - j]));
        while (ma[i - j].size() > 2) ma[i - j].pop_back();
        while (ma[i - j].size() < 2) ma[i - j].pb({-LINF, -1});
      }
    }
  }
  // debug(dp);
  ...
  ll ans = -LINF;
  rep(i, n + 1) {
    if (incl(k - (n - i), 0, k + 1)) chmax(ans, dp[i][k - (n - i)]);
  }
