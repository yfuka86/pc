...
void solve() {
  LL(h, w);
  vs s(h); IN(s);
  if (s[0][0] != 's') OUTRET("No");

  string t = "snuke";
  v3(ll, cost, 5, h, w, LINF);
  cost[0][0][0] = 0;
  queue<LT> que; que.push({0, 0, 0});

  while (!que.empty()) {
    auto [x, i, j] = que.front(); que.pop();
    ll nx = (x + 1) % 5;
    rep(d, 4) {
      ll di = i + dx[d], dj = j + dy[d];
      if (incl(di, 0, h) && incl(dj, 0, w) && s[di][dj] == t[nx]) {
        if (chmin(cost[nx][di][dj], cost[x][i][j] + 1)) {
          que.push({nx, di, dj});
        }
      }
    }
  ...
  rep(i, 5) {
    if (cost[i][h - 1][w - 1] != LINF) OUTRET("Yes");
  }
  OUT("No");
}
