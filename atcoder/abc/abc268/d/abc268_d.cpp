...
void solve() {
  ...
  set<string> t; rep(i, m) { string ss; cin >> ss; t.insert(ss); }

  if (n == 1) {
    if (t.find(s[0]) != t.end() || s[0].size() < 3 || s[0].size() > 16) cout << -1 << "\n";
    else cout << s[0] << "\n";
    return;
  }

  ll sz = n - 1; rep(i, n) sz += s[i].size();
  ...
  do {
    rep(fr, 16 - sz + 1) {
      if (aans != "-1") break;
      function<void(ll, ll, string)> dfs = [&](ll i, ll free, string ss) {
        if (free < 0) return;
        ss += s[p[i]];
        if (i == n - 1) {
          if ( ss.size() <= 16) {
            if (t.find(ss) == t.end()) aans = ss;
          }
          return;
        }
        ss.pb('_');
