...
void solve() {
  ...
  if (N < abs(X) + abs(Y) + abs(Z) || (N - abs(X) - abs(Y) - abs(Z)) & 1) { cout << 0 << "\n"; return; }

  init_f();
  mint ans = 0;
  ll x = abs(X + Y), y = abs(X - Y);
  for (ll z = abs(Z); z <= N - abs(X) - abs(Y); z += 2) {
    ll tot = (N - z);
    // cout << tot << " " << x << " " << y << "\n";
    mint cx = comb(tot, x + (tot - x) / 2);
    mint cy = comb(tot, y + (tot - y) / 2);
    mint cz = comb(N, z) * comb(z, (z - abs(Z)) / 2);
    // cout << cx << " " << cy << " " << cz << "\n";
    ans += cx * cy * cz;
  }
  cout << ans << "\n";
