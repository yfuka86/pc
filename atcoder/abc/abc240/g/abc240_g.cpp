...
void solve() {
  ...
  ll free = N - abs(X) - abs(Y) - abs(Z);
  if (free < 0 || free % 2 != 0) { cout << 0 << "\n"; return; }

  init_f();
  if (free == 0) {
    cout << fact[N] * factinv[abs(X)] * factinv[abs(Y)] * factinv[abs(Z)] << "\n";
    return;
  }

  ll xp = 0, xn = 0, yp = 0, yn = 0, zp = 0, zn = 0;
  if (X < 0) xn += abs(X); else xp += abs(X);
  if (Y < 0) yn += abs(Y); else yp += abs(Y);
  if (Z < 0) zn += abs(Z); else zp += abs(Z);

  modint ans = modint(0);
  ll c = free / 2;
  rep(i, c + 1) {
    rep2(j, i, c + 1) {
      ans += fact[N] * factinv[xp + i] * factinv[xn + i] * factinv[yp + j - i] * factinv[yn + j - i] * factinv[zn + c - j] * factinv[zp + c - j];
    }
  }
  cout << ans << "\n";
}
