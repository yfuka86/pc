...
//------------------------------------------------------------------------------
// 二次元累積和（一応）
template<typename T> vector<vector<T>> cumsum2d(vector<vector<T>> &a) {
...
void solve() {
  ...
  rep(i, n) {
    rep(j, d + 1) {
      rep(k, d + 1) {
        rep2(dd, max(p[i] - d, q[i] - d), min(p[i] + d, q[i] + d) + 1) {
          if (j + abs(p[i] - dd) <= d && k + abs(q[i] - dd) <= d) dpt[j + abs(p[i] - dd)][k + abs(q[i] - dd)] += dp[j][k];
        }
      }
    }
    dp = dpt;
