...
void solve() {
  ...
  ll b = sqrtll(n).se / 4 + 1;
  // queryの動作はまだ未検証
  SquareRootDecomposition<LP, ll> seg(n, b);
  vector<pq<LP>> a2(n), a3(seg.size());

  // 対象ブロック b[b.id], 範囲 [l, r), 作用素 x
  auto update_part = [&](auto &b, ll l, ll r, LP x) {
    rep(i, l, r) a2[i].push(x);
  };
  // 対象ブロック b[b.id], 作用素 x, ( 範囲[b.l, b.r) )
  auto update_all = [&](auto &b, LP x) {
    a3[b.id].push(x);
  };
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      LL(l, r, x);
      --l;
      seg.update(update_part, update_all, l, r, mp(x, i));
    } else if (t == 2) {
      LL(j); --j;
      del[j] = 1;
    } else {
      ...
      while (a2[j].size()) {
        auto [x, i] = a2[j].top();
        if (del[i]) a2[j].pop();
        else { chmax(ans, a2[j].top().fi); break; }
      }
      while (a3[j / b].size()) {
        auto [x, i] = a3[j / b].top();
        if (del[i]) a3[j / b].pop();
        else { chmax(ans, a3[j / b].top().fi); break; }
      }
