...
void solve() {
  ...
  pq<LP> que; vl cost(G.size(), -LINF);
  cost[n - 1] = LINF; que.push({LINF, n - 1});
  ...
  while(!que.empty()) {
    ...
    for(auto &to: G[v]) {
      ll j = to.idx;
      ll l = info[j][0], d = info[j][1], k = info[j][2], c = info[j][3];
      ll la = l + (k - 1) * d + c;
      if (la <= cost[v]) {
        if (chmax(cost[to], la - c)) que.push({cost[to], to});
      } else if (l + c <= cost[v]) {
        ll nx = l + (cost[v] - c - l) / d * d;
        if (chmax(cost[to], nx)) que.push({cost[to], to});
      }
    }
  }
  // debug(cost);
  ...
  rep(i, n - 1) {
    if (cost[i] == -LINF) OUT("Unreachable");
    else OUT(cost[i]);
  }
}
