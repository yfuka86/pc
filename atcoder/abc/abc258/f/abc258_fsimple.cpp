...
void solve() {
  ...
  vector<vector<LT>> pp(2);

  rep(i, 2) {
    auto [x, y] = p[i];
    ll x1 = x / b * b, y1 = y / b * b, x2 = x1 + b, y2 = y1 + b;
    pp[i].pb({x1, y, abs(x1 - x) * k});
    pp[i].pb({x2, y, abs(x2 - x) * k});
    pp[i].pb({x, y1, abs(y1 - y) * k});
    pp[i].pb({x, y2, abs(y2 - y) * k});
  }


  function<vlp(LP)> inter = [&](LP a) {
    vlp ret;
    auto [x, y] = a;
    if (x % b && y % b) ret.pb(a);
    else if (x % b) {
  ...
  ll ans = dist(p[0], p[1]) * k;
  rep(i, 4) rep(j, 4) {
    auto [x1, y1, c1] = pp[0][i];
    auto [x2, y2, c2] = pp[1][j];
    ll tmp = c1 + c2;

    LP ss = {x1, y1}, tt = {x2, y2};
    if ((x1 == x2 && x1 % b == 0) || (y1 == y2 && y1 % b == 0)) chmin(ans, tmp + dist(ss, tt));
    else {
      ll tm = LINF;
      for (auto s: inter(ss)) for (auto t: inter(tt)) {
        chmin(tm, dist(ss, s) + dist(s, t) + dist(t, tt));
      }
      chmin(ans, tmp + tm);
    }
  }
