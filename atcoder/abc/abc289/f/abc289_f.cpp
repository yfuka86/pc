...
vlp reachable(ll s, ll llim, ll rlim) {
  vlp ret; ret.pb({s, s});
  rep(i, MA) {
    auto [l, r] = ret.back();
    LP fl = {llim * 2 - l, rlim * 2 - l}, fr = {llim * 2 - r, rlim * 2 - r};

    if (fl.fi > fl.se) swap(fl.fi, fl.se);
    if (fr.fi > fr.se) swap(fr.fi, fr.se);
    ret.pb({min(fl.fi, fr.fi), max(fl.se, fr.se)});
  }
  return ret;
}

vl restore(ll t, ll llim, ll rlim, vlp& rng, ll idx) {
  ll cur = t;

  vl ans;
  rep_r(i, idx) {
    ll l = llim * 2 - cur, r = rlim * 2 - cur;
    if (l > r) swap(l, r);
    ll l2 = max(rng[i].fi, l), r2 = min(rng[i].se, r);
    ans.pb((cur + l2) / 2);
    cur = l2;
  }
  reverse(all(ans));
...
void solve() {
  LL(sx, sy); LL(tx, ty);
  LL(a, b, c, d);
  if (sx == tx && sy == ty) OUTRET("Yes");
  if ((abs(sx - tx) & 1) || (abs(sy - ty) & 1)) OUTRET("No");

  vlp rngx = reachable(sx, a, b);
  vlp rngy = reachable(sy, c, d);

  rep(i, 1, MA + 1) {
    auto [lx, rx] = rngx[i];
    auto [ly, ry] = rngy[i];
    if (incl(tx, lx, rx + 1) && incl(ty, ly, ry + 1)) {
      OUT("Yes");
      vl xans = restore(tx, a, b, rngx, i);
      vl yans = restore(ty, c, d, rngy, i);
      rep(j, i) {
        OUT(xans[j], yans[j]);
      }
      return;
