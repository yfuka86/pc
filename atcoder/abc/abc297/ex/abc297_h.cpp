...
//   a = query[0] ? A() : A();
// }
template<typename A, typename R> void IANSWER(initializer_list<A> a, string str = "! ", R &r) { cout << str; vector<A> v(a); OUTARRAY(v); cout.flush(); cin >> r; }
template<typename A, typename R> void IANSWER(vector<A> a, string str = "! ", R &r) { cout << str; vector<A> v(a); OUTARRAY(v); cout.flush(); cin >> r; }
// 数値系
