...
void solve() {
  ...
  ll lastns = -1;
  rep(i, N) {
    if (A[i] < Y || A[i] > X) { lastns = i; lastx = -1; continue; }
    if (A[i] == X) lastx = i;

    if (lastx >= 0) {
      ll from;
      auto itr = upper_bound(all(yidx), i);
      if (itr == yidx.begin()) continue;
      else from = *(--itr);
      ans += max<ll>(min(from, lastx) - lastns, 0);
      // cout << i << ":" << from << " " << max<ll>(from - lastns, 0) << "\n";
    }
  }
  cout << ans << "\n";
}
