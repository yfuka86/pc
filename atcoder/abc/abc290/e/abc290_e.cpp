...
void solve() {
  ...
  auto midc = [&](ll mid) {
    ll rcnt = n - mid, lcnt = mid;
    ll mi = min(rcnt, lcnt);
    return mi * (mi + 1) / 2 + (mid - mi) * mi;
  };
  ...
  rep(i, n) {
    ll cnt = midc(i);
    ll rroom = n - i;
    ll thres = upper_bound(all(f[a[i]]), rroom) - f[a[i]].begin();
    ll del = (thres > 0 ? fsum[a[i]][thres - 1] + thres : 0) + (f[a[i]].size() - thres) * rroom;

    // debug(cnt, del);
    ans += cnt - del;

    f[a[i]].pb(i + 1);
    if (fsum[a[i]].size()) fsum[a[i]].pb(fsum[a[i]].back() + i); else fsum[a[i]].pb(i);
  }
  OUT(ans);
}
