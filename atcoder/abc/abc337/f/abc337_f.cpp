...
void solve() {
  ...
  rep(l, n) {
    while (cur < m && r < l + n) {
      if ((idx[c[r]].size() - off[c[r]]) % k == 0) {
        boxes[c[r]]++;
        ans += min(boxes[c[r]] * k, cnt[c[r]]) - min((boxes[c[r]] - 1) * k, cnt[c[r]]);
        cur++;
      }
      idx[c[r]].pb(r);
      r++;
    ...
    // 先頭潰す処理
    off[c[l]]++;
    if ((idx[c[l]].size() - off[c[l]]) % k == 0) {
      cur--;
      boxes[c[l]]--;
      ans += min(boxes[c[l]] * k, cnt[c[l]]) - min((boxes[c[l]] + 1) * k, cnt[c[l]]);
    }
  }
}
