...
void solve() {
  ...
  ll ans = 0;
  do {
    vvl xs(3), ys(3), diag(2);
    rep(i, 9) {
      ll x = p[i] / 3, y = p[i] % 3;
      xs[x].pb(c[x][y]);
      ys[y].pb(c[x][y]);
      if (x == y) diag[0].pb(c[x][y]);
      if (x + y == 2) diag[1].pb(c[x][y]);
    }
    bool valid = true;
    rep(i, 3) {
      if (xs[i][0] == xs[i][1]) valid = false;
      if (ys[i][0] == ys[i][1]) valid = false;
      if (i < 2 && diag[i][0] == diag[i][1]) valid = false;
    }
    if (valid) ans++;
