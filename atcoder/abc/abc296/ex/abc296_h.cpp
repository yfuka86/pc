...
ll solve(ll n, ll m, vs s) {
  while (s.front() == string(m, '.')) s.erase(s.begin());
  ...
  rep(S, 1 << m) {
    UnionFind uf(m);
    rep(i, m - 1) {
      if ((S >> i & 1) && (S >> (i + 1) & 1)) uf.unite(i, i+1);
    }
    set<ll> s;
    rep(i, m) if (S >> i & 1) s.insert(uf.root(i));
    con[S] = s.size();
  }
  // debug(con);

  v3(ll, trans, 1 << m, 1 << 3, 1 << m, -1); // 遷移した後の結合状態
  rep(from, 1 << m) rep(to, 1 << m) {
    vl s, s2;
    {
      UnionFind uf(m); ll S = from;
      rep(i, m - 1) {
        if ((S >> i & 1) && (S >> (i + 1) & 1)) uf.unite(i, i+1);
      }
      rep(i, m) if (S >> i & 1) s.pb(uf.root(i));
      uniq(s);
    }
    {
      UnionFind uf(m); ll S = to;
      rep(i, m - 1) {
        if ((S >> i & 1) && (S >> (i + 1) & 1)) uf.unite(i, i+1);
      }
      rep(i, m) if (S >> i & 1) s2.pb(uf.root(i));
      uniq(s2);
    }
    ...
    rep(co, 1 << con[from] - 1) {
      ...
      rep(i, m - 1) {
        if ((from >> i & 1) && (from >> (i + 1) & 1)) uf.unite(i, i+1);
        if ((to >> i & 1) && (to >> (i + 1) & 1)) uf.unite(i+m, i+m+1);
      }
      rep(i, m) {
        if ((from >> i & 1) && (to >> i & 1)) uf.unite(i, i+m);
      }
      vl ns;
      bool valid = true;
      rep(i, m) if (to >> i & 1) ns.pb(uf.root(i+m));
      rep(i, m) if (from >> i & 1) {
        if (find(all(ns), uf.root(i)) == ns.end()) valid = false;
      }

      ll nco = 0;
      rep(i, s2.size() - 1) if (uf.root(s2[i]+m) == uf.root(s2[i+1]+m)) nco |= 1 << i;
      if (valid) trans[from][co][to] = nco;
    }
  }
  if (0) rep(from, 1 << m) rep(co, 1 << 3) rep(to, 1 << m) {
    //if (trans[from][co][to] != -1) {
    if (from == 21 && to == 15) {
      OUTBIN(from, m);
      OUTBIN(to, m);
      OUTBIN(co, 3);
      debug(trans[from][co][to]);
      cout << "\n";
    }
  }
  ...
  v3(ll, dp, n, 1 << m, 1 << 3, LINF);

  rep(S, 1 << m) {
    bool valid = true;
    ll co = __builtin_popcount(S);
    rep(i, m) {
      if (s[0][i] == '#') {
    ...
    if (valid) dp[0][S][0] = co;
  }

  rep(i, 1, n) {
    rep(from, 1 << m) rep(co, 1 << 3) rep(to, 1 << m) {
      bool valid = true;
      ll nc = __builtin_popcount(to);
      rep(j, m) {
        if (s[i][j] == '#') {
          if (!(to >> j & 1)) valid = false;
          else nc--;
        }
      }
      ll t = trans[from][co][to];
      if (valid && t != -1) chmin(dp[i][to][t], dp[i - 1][from][co] + nc);
    }
  }
  ...
  rep(S, 1 << m) {
    ll co = 0;
    rep(i, con[S] - 1) co |= 1 << i;
    chmin(ans, dp[n - 1][S][co]);
  }
  // debug(dp[n - 1]);
  return ans;
}

ll naive(ll n, ll m, vs s) {
  ll ans = LINF;
  ...
  rep(S, 1 << n * m) {
    ...
    rep(i, n) rep(j, m) { ll id = i * m + j;
      if (s[i][j] == '#') {
        if (!(S >> id & 1)) valid = false; else co--;
      }
    }
    if (!valid) continue;
    ...
    rep(i, n) rep(j, m) {
      ll id = i * m + j;
      ll xid = (i + 1) * m + j, yid = i * m + j + 1;
      if (i < n - 1 && (S >> id & 1) && (S >> xid & 1)) {
        uf.unite(id, xid);
      }
      if (j < m - 1 && (S >> id & 1) && (S >> yid & 1)) {
        uf.unite(id, yid);
      }
    }
    ...
    set<ll> con;
    rep(i, n) rep(j, m) {
      if (s[i][j] == '#') con.insert(uf.root(i * m + j));
    }
    if (con.size() == 1) chmin(ans, co);
  }
  return ans;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 3, m = 5;
    vs s(n, string(m, '.'));
    rep(i, n) rep(j, m) if (rg.l(0, 2) == 1) s[i][j] = '#';

    ll cn = 0;
    rep(i, n) rep(j, m) if (s[i][j] == '#') cn++;
    if (cn == 0) continue;

    auto so = solve(n, m, s); auto na = naive(n, m, s);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, m); debug(so); debug(na);
      rep(i, n) debug(s[i]);
