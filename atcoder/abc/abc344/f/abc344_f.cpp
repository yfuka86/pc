...
void solve() {
  ...
  vl c;
  rep(i, n) rep(j, n) c.pb(p[i][j]);
  auto [mp, rmp] = compmap(c);


  ll m = mp.size();

  v3(LP, dp, n, n, m, {LINF, LINF});
  dp[0][0][mp[p[0][0]]] = {0, 0};

  rep(i, n) rep(j, n) rep(k, m) {
    auto [times, rem] = dp[i][j][k];
    if (times == LINF) continue;

    ll nrate = max(mp[p[i][j]], k);
    if (i < n - 1) {
      ll balance = -rem - d[i][j];
      ll need = 0;
      if (balance < 0) {
        need += ceil(-balance, rmp[nrate]);
        balance += need * rmp[nrate];
      }
      LP nx = {times + need, -balance};
      chmin(dp[i + 1][j][nrate], nx);
    }
    if (j < n - 1) {
      ll balance = -rem - r[i][j];
      ll need = 0;
      if (balance < 0) {
        need += ceil(-balance, rmp[nrate]);
        balance += need * rmp[nrate];
      ...
      LP nx = {times + need, -balance};
      chmin(dp[i][j + 1][nrate], nx);
    }
  }

  ll ans = LINF;
  rep(i, m) {
    chmin(ans, dp[n-1][n-1][i].fi);
  }
  OUT(ans + n * 2 - 2);
}
