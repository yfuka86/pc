...
void solve() {
  STR(t);
  LL(n);
  vl dp(t.size() + 1, LINF); dp[0] = 0;
  rep(i, n) {
    LL(a); vs s(a); IN(s);
    vl dpt = dp;
    rep(j, a) {
      rep(off, t.size() - s[j].size() + 1) {
        // 一致
        if (t.substr(off, s[j].size()) == s[j]) chmin(dpt[off + s[j].size()], dp[off] + 1);
      }
    }
    dp = dpt;
  }

  if (dp[t.size()] == LINF) OUT(-1); else OUT(dp[t.size()]);
}
