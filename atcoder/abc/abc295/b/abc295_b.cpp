...
void solve() {
  ...
  rep(i, r) rep(j, c) {
    if (incl(s[i][j] - '0', 1, 10)) {
      t[i][j] = '.';
      rep(_d, s[i][j] - '0') {
        ll d = _d + 1;
        rep(dx, d + 1) {
          ll dy = d - dx;
          ll di = i + dx, di2 = i - dx, dj = j + dy, dj2 = j - dy;
          if (incl(di, 0, r) && incl(dj, 0, c)) t[di][dj] = '.';
          if (incl(di, 0, r) && incl(dj2, 0, c)) t[di][dj2] = '.';
          if (incl(di2, 0, r) && incl(dj, 0, c)) t[di2][dj] = '.';
          if (incl(di2, 0, r) && incl(dj2, 0, c)) t[di2][dj2] = '.';
        }
      }
    }
  }
