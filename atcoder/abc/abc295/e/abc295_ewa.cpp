...
void solve() {
  ...
  ll cur = 0;
  rep(i, 1, m + 1) { // k個目がiである場合の数を求める
    ll exist = f[i];
    rep(j, cur, k) { // i - 1までで占められている箇所
      ll used = j - cur; //そこまでに使われた0の数
      ll need = max(k - j - exist, 0); // ここからk個目がiであるために必要な数
      ll rem = c0 - used;
      mint t = comb(c0, used) * mint(i - 1).pow(used) * comb(rem, need) * mint(m - i + 1).pow(rem - need);
      debug(i, j, t, used, need, rem);
      sum += t * i;
      cnt += t;
    }
    cur += f[i];
