...
ll win(vvl &a) {
  rep(i, 3) {
    if (sum_of(a[i]) == 3 && a[i][0] == a[i][1] && a[i][0] == a[i][2]) return 1;
    if (sum_of(a[i]) == 6 && a[i][0] == a[i][1] && a[i][0] == a[i][2]) return 2;
  }
  rep(j, 3) {
    ll sum = 0;
    rep(i, 3) sum += a[i][j];
    if (sum == 3 && a[0][j] == a[1][j] && a[0][j] == a[2][j]) return 1;
    if (sum == 6 && a[0][j] == a[1][j] && a[0][j] == a[2][j]) return 2;
  }
...
void solve() {
  ...
  function<ll(vvl, ll)> dfs = [&](vvl s, ll t) {
    bool zero = false;
    rep(i, 3) rep(j, 3) if (s[i][j] == 0) zero = true;
    if (!zero) return 0ll;
    ...
    if (t == 1) {
      ...
      rep(i, 3) rep(j, 3) {
        if (s[i][j] == 0) {
          vvl ns = s; ns[i][j] = 1;
          if (win(ns) == 1) cur = LINF;
          else chmax(cur, a[i][j] + dfs(ns, 2));
        }
      }
      chmin(cur, LINF);
      return dp[s] = cur;
      ...
      rep(i, 3) rep(j, 3) {
        if (s[i][j] == 0) {
          vvl ns = s; ns[i][j] = 2;
          if (win(ns) == 2) cur = -LINF;
          else chmin(cur, -a[i][j] + dfs(ns, 1));
        }
      }
      chmax(cur, -LINF);
      return dp[s] = cur;
  ...
  ll ans = dfs(t, 1);
  // OUT(ans);
  if (ans > 0) OUT("Takahashi"); else OUT("Aoki");
}
