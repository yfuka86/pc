...
void solve() {
  ...
  function<bool(ll)> dfs = [&](ll v) {
    ...
    if (G[v].size() == 0) return dp[v] = false;

    bool alltrue = true;
    fore(to, G[v]) {
      if (!dfs(to)) alltrue = false;
    }
    if (alltrue) return dp[v] = false;
    else return dp[v] = true;
  };

  if (dfs((1 << n) - 1)) OUT("Takahashi"); else OUT("Aoki");
}
