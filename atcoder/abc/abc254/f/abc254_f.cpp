...
void solve() {
  ...
  segtree<S, op, e> sega(diffa);
  segtree<S, op, e> segb(diffb);

  rep(i, Q) {
    ll h1, h2, w1, w2; cin >> h1 >> h2 >> w1 >> w2; h1--; w1--; h2--; w2--;
    ll v = a[h1] + b[w1];
    if (h1 != h2) v = gcd(v, sega.prod(h1, h2));
    if (w1 != w2) v = gcd(v, segb.prod(w1, w2));
    cout << v << "\n";
  }
