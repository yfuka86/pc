...
struct S {ll sz, masz, sum, masum;};
S op(S l, S r) {
  ld lav = (ld)l.masum / l.masz, av = (ld)(l.sum + r.masum) / (l.sz + r.masz);

  return {
    l.sz + r.sz,
    lav > av ? l.masz : l.sz + r.masz,
    l.sum + r.sum,
    lav > av ? l.masum : l.sum + r.masum
  };
}
S e() { return {0, 0, 0, 0}; }
...
void solve() {
  ...
  vector<S> v(n);
  rep(i, n) v[i] = {1, 1, a[i], a[i]};
  segtree<S, op, e> seg(v);

  rep(i, n) {
    S x = seg.prod(i, n);
    OUT((ld)x.masum / x.masz);
  }
}
