...
void solve() {
  ...
  // 発電所の根
  set<ll> roots;
  rep(i, m) roots.insert(uf.root(n + i));

  vl ans(q);
  ll sum = -m;
  for (auto v: roots) sum += uf.size(v);

  rep_r(qi, q) {
    ans[qi] = sum;
    auto [u, v] = edges[query[qi]];
    ll ur = uf.root(u), vr = uf.root(v);
    if (ur == vr) continue;

    if (roots.find(ur) == roots.end() && roots.find(vr) == roots.end()) {
      uf.unite(u, v);
    } else {
      if (roots.find(ur) == roots.end()) sum += uf.size(u);
      if (roots.find(vr) == roots.end()) sum += uf.size(v);
      uf.unite(u, v);
      ll nr = uf.root(u);
      roots.insert(nr);
    }
