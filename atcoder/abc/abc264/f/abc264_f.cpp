...
ll calc(vvl& g, ll start = 0) {
  rep(i, h) rep(j, w) rep(k, 2) rep(l, 2) dp[i][j][k][l] = LINF;
  dp[0][0][0][0] = start;
  ...
  rep(i, h) {
    rep(j, w) {
      ...
      if (i < h - 1) {
        if (g[i + 1][j]) {
          chmin(dp[i + 1][j][0][1], min(dp[i][j][0][1], dp[i][j][1][1]));
          chmin(dp[i + 1][j][1][0], min(dp[i][j][0][0], dp[i][j][1][0]) + r[i + 1]);
        } else {
          chmin(dp[i + 1][j][0][0], min(dp[i][j][0][0], dp[i][j][1][0]));
          chmin(dp[i + 1][j][1][1], min(dp[i][j][0][1], dp[i][j][1][1]) + r[i + 1]);
        }
      }
      ...
      if (j < w - 1) {
        if (g[i][j + 1]) {
          chmin(dp[i][j + 1][1][0], min(dp[i][j][1][0], dp[i][j][1][1]));
          chmin(dp[i][j + 1][0][1], min(dp[i][j][0][0], dp[i][j][0][1]) + c[j + 1]);
        } else {
          chmin(dp[i][j + 1][0][0], min(dp[i][j][0][0], dp[i][j][0][1]));
          chmin(dp[i][j + 1][1][1], min(dp[i][j][1][0], dp[i][j][1][1]) + c[j + 1]);
        }
      }
    }
  }
...
void solve() {
  ...
  vvl g(h, vl(w, 0)), rg(h, vl(w, 1));
  rep(i, h) {
    string s; cin >> s;
    rep(j, w) {
      if (s[j] == '1') { g[i][j] = 1; rg[i][j] = 0; }
    }
  }
  if (g[0][0] == 1) swap(g, rg);

  ll ans = LINF;
  chmin(ans, calc(g, 0));
  rep(i, h) g[i][0] = !g[i][0];
  rep(i, w) g[0][i] = !g[0][i];
  chmin(ans, calc(g, r[0] + c[0]));

  rep(i, h) rg[i][0] = !rg[i][0];
  chmin(ans, calc(rg, c[0]));
  rep(i, h) rg[i][0] = !rg[i][0];
  rep(i, w) rg[0][i] = !rg[0][i];
  chmin(ans, calc(rg, r[0]));
