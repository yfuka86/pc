...
void solve() {
  ...
  function<ld(ll, ll, ll, ll)> dist = [&](ll x, ll y, ll x2, ll y2) {
    ll dx = (x2 - x), dy = (y2 - y);
    return sqrt(dx * dx + dy * dy);
  };

  vd diff(n - 1), gain(n - 1);
  rep(i, n - 1) {
    diff[i] = dist(x[i + 1], y[i + 1], x[i], y[i]);
    gain[i] = dist(sx, sy, x[i], y[i]) + dist(sx, sy, x[i + 1], y[i + 1]) - diff[i];
  }
  ...
  rep(i, 1, n) {
    ld tmp = seg.prod(max(0, i + 1 - k), i + 1);
    seg.set(i + 1, tmp + gain[i - 1]);
  }

  ld ans = seg.prod(n + 1 - k, n + 1);
  ans += sum_of(diff) + dist(sx, sy, x[0], y[0]) + dist(sx, sy, x[n - 1], y[n - 1]);
