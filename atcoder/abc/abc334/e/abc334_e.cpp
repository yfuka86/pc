...
void solve() {
  ...
  rep(i, h) rep(j, w) {
    if (i < h - 1) {
      if (s[i][j] == '#' && s[i][j] == s[i + 1][j]) uf.unite(i * w + j, (i + 1) * w + j);
    }
    if (j < w - 1) {
      if (s[i][j] == '#' && s[i][j] == s[i][j + 1]) uf.unite(i * w + j, i * w + j + 1);
    }
  }

  set<ll> roots;
  rep(i, h) rep(j, w) if (s[i][j] == '#') roots.insert(uf.root(i * w + j));
  ...
  rep(i, h) rep(j, w) {
    if (s[i][j] == '.') {
      times++;
      set<ll> tmp;
      if (i < h - 1 && s[i + 1][j] == '#') tmp.insert(uf.root((i+1)*w+j));
      if (i > 0 && s[i - 1][j] == '#') tmp.insert(uf.root((i-1)*w+j));
      if (j < w - 1 && s[i][j + 1] == '#') tmp.insert(uf.root(i*w+j+1));
      if (j > 0 && s[i][j - 1] == '#') tmp.insert(uf.root(i*w+j-1));
  ...
  // debug(roots.size(), times, sum);

  OUT(mint(roots.size() * times + sum) / times);
  // debug(roots.size());
}
