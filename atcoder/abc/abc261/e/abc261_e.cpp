  ...
  rep(i, n) {
    ...
    rep(j, 30) {
      if (t == 1 && !(a & 1 << j)) {
        ...
        if (op[j][1] == 1) op[j][1] = 0; else op[j][1] = 1;
      }
    }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp q(n);
    rep(i, n) {
      q[i] = { rg.l(1, 4), rg.l(0, 8)};
    }

    auto so = solve(n, c, q); auto na = naive(n, c, q);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, c, q); debug(so); debug(na);
