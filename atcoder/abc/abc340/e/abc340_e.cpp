...
void solve() {
  ...
  RangeAdd<ll> bt(n);
  rep(i, n) bt.range_add(i, i + 1, a[i]);

  rep(i, m) {
    ll s = bt.get(b[i]);
    bt.range_add(b[i], b[i] + 1, -s);
    ll cycle = s / n, rem = s % n;
    bt.range_add(0, n, cycle);
    bt.range_add(b[i] + 1, min(b[i] + 1 + rem, n), 1);
    bt.range_add(0, max(0, b[i] + 1 + rem - n), 1);
  }
