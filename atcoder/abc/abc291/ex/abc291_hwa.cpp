...
void solve() {
  ...
  function<ll(ll, ll, ll, ll)> find = [&](ll v, ll p, ll l, ll r = -1) {
    ...
    fore(to, G[v]) { if (to == p || vis[to]) continue;
      ll sztmp;
      if (hld.depth[to] < hld.depth[v]) sztmp = sz(l, to) + 1; else sztmp = sz(to, r);
      if (chmax(maxsz, sztmp)) maxv = to;
    }
    debug(maxsz, maxv, sz(l, r));
    if (maxsz * 2 <= sz(l, r) || maxv == -1) return v; else return find(maxv, v, l, r);
  };

  function<void(ll, ll, ll, ll)> dec = [&](ll v, ll p, ll l, ll r) {
    ll nx = find(v, -1, l, r);
    par[nx] = p != -1 ? p + 1 : p;
    vis[nx] = 1;
