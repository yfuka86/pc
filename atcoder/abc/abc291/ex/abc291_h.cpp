...
void solve() {
  ...
  vl ans(n, -1);
  vl t(n);
  function<ll(ll, ll)> calcT = [&](ll v, ll p) { // 部分木のサイズ
    t[v] = 1;
    fore(u, G[v]) if (!used[u]) {
      if (u == p) continue;
      t[v] += calcT(u, v);
    }
    return t[v];
  };
  function<LP(ll, ll, ll)> getCent = [&](ll T, ll v, ll p) {
    ll mx = T - t[v];
    LP res(-1, -1);
    fore(u, G[v]) if (!used[u]) {
      if (u == p) continue;
      res = min(res, getCent(T, u, v));
      mx = max(mx, t[u]);
    }
    return min(res, LP(mx, v));
  };
  function<ll(ll)> dec = [&](ll v) {
    int T = calcT(v, -1);
    int c = getCent(T, v, -1).second;
    used[c] = true;
    fore(u, G[v]) if (!used[u]) {
      ans[dec(u)] = c + 1;
    }
