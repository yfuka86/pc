...
void solve() {
  ...
  int sz = [&](ll r, ll v) {
    ll lc = hld.lca(r, v);
    if (lc == v) return n - hld.size[v] + 1; else hld.size[v];
  };

  vl dec(n, -1);
  vb vis(n);

  function<ll(ll, ll, ll, ll)> find = [&](ll v, ll p, ll r, ll allsz) {
    ll maxsz = 0, maxv = -1;
    fore(to, G[v]) { if (to == p) continue;


      if (chmax(maxsz, sz(r, to))) maxv = to;
    }
    if (maxsz <= allsz / 2) {
      return v;
    } else {
      return find(maxv, v, r, allsz);
    }
  };

  function<void(ll, ll)> dec = [&](ll v, ll r, ll sz) {
    vis[v] = 1;
    // rは元の根付き木の親（行き止まり）
    ll nx = find(v, r, r, sz);
    fore(to, G[nx]) {
