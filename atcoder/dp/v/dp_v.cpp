...
void solve() {
  ...
  function<void(ll, ll, ll)> dfs2 = [&](ll v, ll dpp, ll p) {
    ...
    rep(i, n) { ll to = G[v][i];
      if (to == p) continue;
      ans[v] = ans[v] * (dp[to] + 1) % M;
      t[i] = dp[to] + 1;
    }

    vl cum1(n + 1, 1), cum2(n + 1, 1);
    rep(i, n) cum1[i + 1] = cum1[i] * t[i] % M;
    rep(i, n) cum2[i + 1] = cum2[i] * t[n - 1 - i] % M;

    rep(i, n) { ll to = G[v][i];
      if (to == p) continue;
      ll prod = (dpp + 1) * cum1[i] % M * cum2[n - 1 - i] % M;
      dfs2(to, prod, v);
    }
  };
  dfs2(0, 0, -1);

  rep(i, N) cout << ans[i] % M << "\n";
}
