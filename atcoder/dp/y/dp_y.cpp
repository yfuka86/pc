...
void solve() {
  ll H, W, N; cin >> H >> W >> N;
  vlp ob(N); rep(i, N) { ll r, c; cin >> r >> c; ob[i] = {r, c}; }
  sort(all(ob));

  init_f();
  // その前の障害物を通らずにその障害物までくる方法
  vmi dp(N, 0);

  rep(i, N) {
    auto [h, w] = ob[i];
    mint sum = comb(h - 1 + w - 1, h - 1);
    rep(j, i) {
      auto [hh, ww] = ob[j];
      if (ww > w) continue;
      sum -= dp[j] * comb(h - hh + w - ww, h - hh);
    }
    dp[i] = sum;
  }

  mint ans = comb(H - 1 + W - 1, H - 1);
  rep(i, N) {
    auto [h, w] = ob[i];
    ans -= dp[i] * comb(H - h + W - w, H - h);
  }
  cout << ans << "\n";
}
