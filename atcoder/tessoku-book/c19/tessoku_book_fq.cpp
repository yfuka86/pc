...
void solve() {
  LL(n, l, k);
  VEC2(ll, a, c, n);
  segtree<S, op, e> seg(700001);
  rep(i, n) seg.set(a[i], min(seg.get(a[i]), c[i]));
  seg.set(l, 0);
  ...
  ll rem = k - a[0];
  rep(i, n) {
    if (i > 0) rem -= (a[i] - a[i - 1]);
    // 同じ座標にあるケースは最小のもの以外飛ばす
    if (rev[a[i]] != i) continue;

    ll mi = seg.prod(a[i] + 1, min(a[i] + k + 1, 700001));
    if (c[i] < mi) {
      ll gas = max(0, min(l - a[i], k) - rem);
      // debug(rem, i, gas);
      ans += gas * c[i];
      rem += gas;
    } else {
      ll nxt = seg.max_right(a[i], [&](S x) {
        return x >= c[i];
      });
      ll gas = max(0, (nxt == l ? l : a[rev[nxt]]) - a[i] - rem);
      // debug(rem, i, gas);
      ans += gas * c[i];
      rem += gas;
    }
