...
void solve() {
  ...
  vl st; rep(i, N) if (deg[i] == 0) st.pb(i);
  ll ans = 0;

  function<bool(ll)> dfs = [&](ll depth) -> bool {
    if (depth == N) { ans++; return true; }
    ...
    rep_r(i, st.size()) {
      ...
      for(ll next: G[v]) {
        deg[next]--;
        if (deg[next] == 0) st.pb(next);
      }

      if (!dfs(depth + 1)) return false;

      for(ll next: G[v]) {
        if (deg[next] == 0) st.pop_back();
        deg[next]++;
      }

      st.insert(st.begin() + i, v);
    }
