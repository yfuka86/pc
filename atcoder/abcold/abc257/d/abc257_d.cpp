...
void solve() {
  ...
  rep(i, n) rep2(j, i + 1, n) {
    auto [x1, y1, p1] = jump[i];
    auto [x2, y2, p2] = jump[j];
    ll dist = abs(x2 - x1) + abs(y2 - y1);
    edges.pb({ceil(dist, p1), i, j});
    edges.pb({ceil(dist, p2), j, i});
  }
  sort(all(edges));

  ll ans = binary_search([&](ll mid) {
    Graph<ll> G(n);
    for (auto &[s, a, b]: edges) {
      if (s > mid) break;
    ...
    rep(i, n) {
      ...
      used[i] = 1;
      dfs(i);
      if (accumulate(all(used), 0ll) == n) return true;
    }
    return false;
  }, LINF, 0);
