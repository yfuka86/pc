...
void solve() {
  ...
  const ll table = 5000000;
  vmi dp(table, 0);
  unordered_map<ll, mint> mp;

  function<mint(ll)> dfs = [&](ll x) {
    cnt++;
    if (x <= 0) return mint(0);
    if (x < table) {
      if (dp[x] != 0) return dp[x];
    } else if (mp.find(x) != mp.end()) return mp[x];
    ...
    exp /= (n - 1);
    // debug(x, exp);
    if (x < table) return dp[x] = exp;
    else return mp[x] = exp;
  };
