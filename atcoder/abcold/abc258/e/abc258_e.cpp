...
pair<vl, ll> solve(ll n, ll x, vl w) {
  w.insert(w.end(), w.begin(), w.end());
  vl ws = csum(w);
  vl search, cnt;
  ll from = -1;
  {
    vb found(n); found[0] = 1;
    ll cur = 0;
    while(1) {
      ll tmpc = 0;
      ll tmpx = x;
      ...
      if (found[cur] == 1) {
        from = find(all(search), cur) - search.begin();
        break;
      } else found[cur] = 1;
    }
  }
...
pair<vl, ll> naive(ll n, ll x, vl w) {
  vl search, cnt;
  ll from = -1;
  vb found(n);
  found[0] = 1;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    auto s = solve(N, x, a); auto n = naive(N, x, a);
...
void solve() {
  ...
  rep(i, q) {
    ...
    if (k < cnt.size()) {
      ...
      k -= from;
      ll cyclesz = cnt.size() - from;
      cout << cnt[from + k % cyclesz] << "\n";
    }
  }
}
