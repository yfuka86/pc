...
void solve() {
  ...
  vector<vlp> pp(2), ppp(2);

  rep(i, 2) {
    auto [x, y] = p[i];
    ll x1 = x / b * b, y1 = y / b * b, x2 = x1 + b, y2 = y1 + b;
    pp[i].pb({x1, y});
    pp[i].pb({x2, y});
    pp[i].pb({x, y1});
    pp[i].pb({x, y2});
  ...
  rep(i, 4) {
    G.add_directed_edge(s, i, dist(p[0], pp[0][i]) * k);
    rep(j, 4) {
      G.add_directed_edge(i, j + 4, dist(pp[0][i], ppp[0][j]));
    }
  }
  rep(i, 4) {
    G.add_directed_edge(i + 8, t, dist(p[1], pp[1][i]) * k);
    rep(j, 4) {
      G.add_directed_edge(j + 12, i + 8, dist(pp[1][i], ppp[1][j]));
    }
  }
  rep(i, 4) rep(j, 4) {
    if ((pp[0][i].fi == pp[1][j].fi && pp[0][i].fi % b == 0) ||
		 (pp[0][i].se == pp[1][j].se && pp[0][i].se % b == 0)) G.add_directed_edge(i, j + 8, dist(pp[0][i], pp[1][j]));
  }
  rep(i, 4) rep(j, 4) {
    G.add_directed_edge(i + 4, j + 12, dist(ppp[0][i], ppp[1][j]));
  }
  G.add_directed_edge(s, t, dist(p[0], p[1]) * k);
