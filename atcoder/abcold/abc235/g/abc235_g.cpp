...
namespace COM {
  ...
  void incr() { fact.push_back(fact.back() * cur); inv.push_back(-inv[mod % cur] * ((double)mod / cur)); factinv.push_back(factinv.back() * inv[cur]); cur++; }
...
void solve() {
  ...
  rep(i, 3) dp[i][0] = 1;
  rep(j, 3) rep(i, n) {
    dp[j][i + 1] = dp[j][i] * 2 - comb(i, a[j]);
  }
  // debug(dp);
  mint ans = 0;
  rep(i, n + 1) {
    mint t = comb(n, i); // 何もない庭の選び方
    rep(j, 3) t *= dp[j][n - i]; // 残りの部屋にそれぞれを配る方法
    if (i & 1) ans -= t; else ans += t; // 包除
  }
  cout << ans << "\n";
}
