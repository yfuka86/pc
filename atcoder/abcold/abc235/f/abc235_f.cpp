...
void solve() {
  ...
  vvmi dp(1 << 10, vmi(2, 0)), dpn(1 << 10, vmi(2, 0)), dpt(1 << 10, vmi(2, 0)), dpnt(1 << 10, vmi(2, 0));

  {
    ll ma = n[0] - '0';
    rep2(d, 1, ma + 1) { dp[1 << d][d == ma] = d; dpn[1 << d][d == ma] += 1;}
  }

  rep2(i, 1, n.size()) {
    ll ma = n[i] - '0';
    rep2(d, 1, 10) { dpt[1 << d][0] += d; dpnt[1 << d][0] += 1; }
    rep(S, 1 << 10) {
      rep(d, 10) {
        if (dp[S][0] != mint(0)) {
          dpnt[S | 1 << d][0] += dpn[S][0];
          dpt[S | 1 << d][0] += dp[S][0] * 10 + dpn[S][0] * d;
        }
        if (dp[S][1] != mint(0) && d <= ma) {
          dpnt[S | 1 << d][d == ma] += dpn[S][1];
          dpt[S | 1 << d][d == ma] += dp[S][1] * 10 + dpn[S][1] * d;
        }
      }
    }
    dp = dpt;
    dpn = dpnt;
    dpt.assign(1 << 10, vmi(2, 0));
    dpnt.assign(1 << 10, vmi(2, 0));
  }

  mint ans = 0;
  rep(S, 1 << 10) {
    bool valid = true;
    rep(i, m) if (!(S & 1 << c[i])) { valid = false; break; }
    if (!valid) continue;
    ans += dp[S][0] + dp[S][1];
  }
  cout << ans << "\n";
