...
void solve() {
  ...
  // string s = "RGRGRGRGRGB";
  // sort(all(s));
  // map<ll, ll> mp;
  // do {
  //   ll cnt = 0;
  //   rep(i, s.size() - 1) {
  //     if (s[i] == 'R' && s[i+1] == 'G') cnt++;
  //   }
  //   mp[cnt]++;
  ...
  mint ans = 0;
  rep2_r(i, k, min(r, g) + 1) {
    ll rr = r - i, gg = g - i;
    ll len = rr + gg + b + i;
    mint c = comb(len, i) * comb(len - i, rr) * comb(len - i - rr, gg);

    if ((i - k) & 1) ans -= c * comb(i, k); else ans += c * comb(i, k);
  }
  cout << ans << "\n";
}
