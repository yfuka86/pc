...
void solve() {
  ...
  mcf_graph<ll, ll> mcf(n + 3);
  ll f = n - 1, e = n, s = n + 1, t = s + 1;
  ll ini = 1000000000;

  rep(i, n - 1) {
    if (i & 1) {
      mcf.add_edge(i, t, 1, ini - a[i]);
    } else {
      mcf.add_edge(s, i, 1, ini - a[i]);
      if (i < n - 2) mcf.add_edge(i, i + 1, 1, 0);
      if (i > 1) mcf.add_edge(i, i - 1, 1, 0);
    }
  }
  mcf.add_edge(0, f, 1, 0);
  mcf.add_edge(f, t, 1, ini);

  auto [cap, cost] = mcf.flow(s, t, ru);
  cout << ru * ini * 2 - cost << "\n";
