...
void solve() {
  ...
  vector<map<ll, ll>> dep(n);
  rep(i, m) { a[i]--; b[i]--; }
  rep(i, m) dep[a[i]][s[i]] = i;
  // debug(dep);

  vv(ll, dp, m, 20);
  rep(i, m) {
    auto it = dep[b[i]].lower_bound(t[i]);
    if (it == dep[b[i]].end()) dp[i][0] = i;
    else dp[i][0] = it->se;
  }
  ...
  rep(i, q) {
    auto it = dep[--y[i]].lower_bound(x[i]);
    if (it == dep[y[i]].end()) OUT(y[i] + 1);
    else {
      ll cur = it->se;

      // z[i] <= t[cur]となるような最初のcurを探す
      while (z[i] > t[cur]) {
        if (z[i] <= s[dp[cur][0]]) break;
        if (t[dp[cur][19]] < z[i]) { cur = dp[cur][19]; break; }
        ll nx = -1;
        rep(j, 20) {
          if (s[dp[cur][j]] < z[i]) {
            nx = dp[cur][j];
      ...
      if (z[i] <= s[cur]) {
        OUT(a[cur] + 1);
      } else if (incl(z[i], s[cur] + 1, t[cur] + 1)) {
        OUT(a[cur] + 1, b[cur] + 1);
      } else {
        OUT(b[cur] + 1);
      }
