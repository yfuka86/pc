...
void solve() {
  ...
  vl cnta(N), cntb(N), hasha(N), hashb(N);
  set<ll> sa, sb;

  auto h = [&](ll a, ll b) {
    return (a + (b + P_B) * (b + 9185) % P_M * (b + P_M) % P_M) % P_M;
  };

  rep(i, N) {
    if (i) hasha[i] = hasha[i - 1], hashb[i] = hashb[i - 1];

    if (sa.find(a[i]) == sa.end()) {
      sa.insert(a[i]);
      hasha[i] = h(hasha[i], a[i]);
    }
    cnta[i] = sa.size();

    if (sb.find(b[i]) == sb.end()) {
      sb.insert(b[i]);
      hashb[i] = h(hashb[i], b[i]);
    }
    cntb[i] = sb.size();
  }
  // coutarray(hasha);
