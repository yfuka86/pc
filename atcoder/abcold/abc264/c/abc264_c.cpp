...
void solve() {
  ...
  rep(s1, 1 << h1) {
    if (__builtin_popcount(s1) != h2) continue;
    rep(s2, 1 << w1) {
      if (__builtin_popcount(s2) != w2) continue;

      vl i1, j1;
      rep(i, h1) if (s1 & 1 << i) i1.pb(i);
      rep(i, w1) if (s2 & 1 << i) j1.pb(i);

      bool valid = true;
      rep(i, h2) rep(j, w2) {
        if (!valid) break;
        if (m2[i][j] != m1[i1[i]][j1[j]]) { valid = false; break; }
      }
      if (valid) { cout << "Yes" << "\n"; return; }
    }
  }
