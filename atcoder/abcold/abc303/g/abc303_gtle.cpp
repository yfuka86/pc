...
void solve() {
  ...
  function<ll(ll, ll, ll)> dfs = [&](ll l, ll r, ll p) {
    if (dp[l][r][p] != LINF && dp[l][r][p] != -LINF) return dp[l][r][p];
    ll nxp = !p;

    ll ma = -LINF;
    chmax(ma, -dfs(l + 1, r, nxp) + x[l]);
    chmax(ma, -dfs(l, r - 1, nxp) + x[r - 1]);
    {
      ll rem = r - l - min(b, r - l);
      rep(i, r - l - rem + 1) {
        chmax(ma, -dfs(l + i, l + i + rem, nxp) + cx[r] - cx[l] - (cx[l + i + rem] - cx[l + i]) - a);
      }
    }
    {
      ll rem = r - l - min(d, r - l);
      rep(i, r - l - rem + 1) {
        chmax(ma, -dfs(l + i, l + i + rem, nxp) + cx[r] - cx[l] - (cx[l + i + rem] - cx[l + i]) - c);
      }
    }
    return dp[l][r][p] = ma;
  };
