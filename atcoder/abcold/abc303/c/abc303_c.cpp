...
const string drul = "DRUL"; const vl dx = {0, 1, 0, -1}, dy = {-1, 0, 1, 0};
...
void solve() {
  ...
  set<LP> items;
  rep(i, m) {
    LL(x, y); items.insert({x, y});
  }

  ll curx = 0, cury = 0, hp = h;
  rep(i, n) {
    rep(d, 4) {
      if (drul[d] == s[i]) {
        curx += dx[d]; cury += dy[d]; hp--;
        if (hp < 0) OUTRET("No");
        auto it = items.find({curx, cury});
        if (it != items.end() && hp < k) {
          hp = k;
          items.erase(it);
        }
      }
