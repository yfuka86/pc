...
void solve() {
  ...
  for (auto it = mp.begin(); it != prev(mp.end()); it++) {
    ...
    s.pb({t.size(), (a2 - a1) * c1});
    t.pb({t.size() + 1, a1, c1});
  }
  s.pb({t.size(), LINF});
  t.pb({-1, mp.rbegin()->fi, LINF});

  segtree<S, op, e> seg(s);

  // rep(i, s.size()) {
  //   debug(s[i].id, s[i].cost);
  //   auto [to, a, c] = t[i];
  //   debug(to, a, c);
  // }
  ...
  ll ans = 0, initsz = mp.size();
  while(1) {
    S p = seg.all_prod();
    if (p.cost >= x) break;
    auto [to, a1, c1] = t[p.id];
    auto [to2, a2, c2] = t[to];
    // debug(p.id, p.cost);
    // debug(to, a1, c1);
    // debug(to2, a2, c2);
    ...
    seg.set(to, {to, LINF});
    if (to2 != -1) {
      seg.set(p.id, {p.id, (get<1>(t[to2]) - a2) * (c1 + c2)});
    } else {
      seg.set(p.id, {p.id, LINF});
    }
  }
