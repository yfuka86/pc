...
void solve() {
  ...
  SegmentTree2D seg(h, w, [](ll a, ll b) { return a + b; }, 0LL);
  rep(i, h) rep(j, w) seg.set(i, j, a[i][j]);
  seg.build();

  chmin(h2, h1); chmin(w2, w1);

  SegmentTree2D segm(h - h2 + 1, w - w2 + 1, [](ll a, ll b) { return max(a, b); }, 0LL);
  rep(i, h - h2 + 1) rep(j, w - w2 + 1) {
    segm.set(i, j, seg.query(i, j, i + h2, j + w2));
  }
  segm.build();

  ll ans = 0;
  rep(i, h - h1 + 1) rep(j, w - w1 + 1) {
    ll sum = seg.query(i, j, i + h1, j + w1);
    ll ma = segm.query(i, j, i + h1 - h2 + 1, j + w1 - w2 + 1);
    chmax(ans, sum - ma);
  }
  OUT(ans);
}
