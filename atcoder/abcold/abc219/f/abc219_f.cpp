...
void solve() {
  ...
  rep(i, s.size()) {
    rep(d, 4) {
      if (drul[d] != s[i]) continue;
      p.fi += dx[d]; p.se += dy[d];
    }
    S.insert(p);
  }

  if (p.fi == 0 && p.se == 0) { coutret(S.size()); }
  ll a = p.fi, b = p.se;
  {
    set<LP> ts;
    for (auto [x, y]: S) {
      if (a == 0) ts.insert({b < 0 ? -y : y, a < 0 ? -x : x});
      else ts.insert({a < 0 ? -x : x, b < 0 ? -y : y});
    }
    S = ts;
    a = abs(a);
    b = abs(b);
  ...
  for (auto [x, y]: S) {
    x += 200000; y += 200000;
    LP p = {x - x / a * a, y - x / a * b};

    if (mp[p].size() == 0) {
      mp[p].pb(0);
      norm[p] = {x, y};
    } else {
      ll dx = x - norm[p].fi, dy = y - norm[p].se;
      if (a == 0) mp[p].pb(dy / b); else mp[p].pb(dx / a);
    }
  }
  debug(mp);
  debug(norm);
