...
void solve() {
  ...
  ll sq = sqrtll(m).se;
  vb star(n);
  rep(i, n) if (G[i].size() >= sq) star[i] = true;
  vvl starG(n);
  rep(i, n) fore(to, G[i]) if (star[to]) starG[i].pb(to);

  vlp color(n); rep(i, n) color[i] = {-1, i + 1};
  vlp scolor(n, {-1, -1});

  rep(i, q) {
    LL(x); --x;
    fore(s, starG[x]) if (scolor[s].fi != -1) chmax(color[x], scolor[s]);

    if (star[x]) {
      scolor[x] = {i, color[x].se};
    } else {
      fore(v, G[x]) {
        color[v] = {i, color[x].se};
      }
    }
    // debug(color);
  }

  rep(i, n) fore(s, starG[i]) chmax(color[i], scolor[s]);
  vl ans(n);
  rep(i, n) ans[i] = color[i].se;
  OUTARRAY(ans);
}
