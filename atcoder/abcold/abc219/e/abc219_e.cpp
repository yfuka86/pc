...
void solve() {
  ...
  rep(S, 1 << n * n) {
    bool valid = true;
    rep(i, n * n) {
      if (grid[i / n][i % n] && !(S & 1 << i)) valid = false;
    }
    ...
    rep(i, n * n) {
      ...
      rep(d, 4) {
        ...
        ll id = xx * n + yy;
        if (0 <= xx && xx < n && 0 <= yy && yy < n && !(S & 1 << id)) uf.unite(i, id);
        if (xx < 0 || n <= xx || yy < 0 || n <= yy) uf.unite(i, n * n);
      }
    }
    rep(i, n * n) {
      if (!(S & 1 << i) && !uf.same(n * n, i)) valid = false;
    }
    ...
    if (uf.size(c) == __builtin_popcount(S)) {
      // coutbin(S, n * n);
      ans++;
    }
  }
