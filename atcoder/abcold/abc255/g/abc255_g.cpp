...
void solve() {
  ...
  auto grundy = [&](ll x){
    if (gru.find(x) != gru.end()) return gru[x];
    else return x - (lower_bound(all(gruv), x) - gruv.begin());
  };
  ...
  for (auto [from, tov]: p) {
    ...
    ll gr = from - gruv.size();
    for (auto [k, v]: mp) {
      if (gruinv[k].size() < v.size()) { chmin(gr, k); break; }
    }

    if (gr != from - gruv.size()) {
      gruinv[gr].pb(from);
      gru[from] = gr;
      gruv.pb(from);
    }
