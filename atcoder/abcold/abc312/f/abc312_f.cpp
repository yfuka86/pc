...
void solve() {
  ...
  rep(i, n) {
    if (t[i] == 0) { now.push(x[i]); sum += x[i]; }
    if (t[i] == 1) need.push(x[i]);
    if (t[i] == 2) cut.push(x[i]);
  }
  // debug(now.size(), need.size(), cut.size());

  ll ans = 0, cur = m, availableneed = 0;
  ...
  while (1) {
    ...
    if (cut.size()) {
      availableneed += cut.top(); cut.pop();
      cur--;
      while (need.size() && availableneed > 0) {
        sum += need.top();
        now.push(need.top());
        need.pop();
        availableneed--;
