...
void solve() {
  ...
  rep(S, 1 << N) {
    ll i = __builtin_popcount(S), cnt = 0;
    rep(j, N) {
      if (S & 1 << j) { cnt++; continue; }
      // i - cntで、このindexまでで使われていないものの数＝決まってない位置の中でBjの左にあるものの数＝swapの必要数となる
      chmin(dp[S | 1 << j], dp[S] + Y * (i - cnt) + X * abs(A[i] - B[j]));
    }
  }
  cout << dp[(1 << N) - 1] << "\n";
}
