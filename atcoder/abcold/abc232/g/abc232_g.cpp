...
void solve() {
  ...
  for(auto it = mp.begin(); it != mp.end(); ++it) {
    auto nx = it; nx++; if (nx == mp.end()) nx = mp.begin();
    ll diff = nx->fi - it->fi; if (diff < 0) diff += m;
    G.add_directed_edge(n + it->se, n + nx->se, diff);
    // debug(n + it->se, n + nx->se, diff);
  }
  rep(i, n) {
    // debug(i, mp[-a[i] + m]);
    // debug(mp[b[i]], i);
    G.add_directed_edge(i, n + mp[-a[i] + m], 0);
    G.add_directed_edge(n + mp[b[i]], i, 0);
  }
