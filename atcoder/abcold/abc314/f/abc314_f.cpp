...
void solve() {
  ...
  vmi dp(n); dp[uf.root(0)] = 0;

  rep_r(i, n - 1) {
    ll r = uf.root(p[i]);
    uf.undo();
    ll r1 = uf.root(p[i]), r2 = uf.root(q[i]);
    ll sz1 = uf.size(p[i]), sz2 = uf.size(q[i]);

    // debug(r, r1, r2, sz1, sz2);

    mint nr1 = dp[r] + (mint(sz1) / (sz1 + sz2));
    mint nr2 = dp[r] + (mint(sz2) / (sz1 + sz2));
    dp[r1] = nr1, dp[r2] = nr2;
    // debug(dp);
  }
  OUTARRAY(dp, mint(0));
