...
void solve() {
  ...
  auto div = [&](ll x, ll y) {
    auto it = prev(lower_bound(all(rng), mp(x, -LINF)));
    auto [from, to] = *it;
    rng.erase(it);
    if (x - from >= l) rng.pb({from, x});
    if (to - x - y >= l) rng.pb({x + y, to});
  };

  ll x, y;
  if (dp[n]) {
    OUT("First"); cout.flush();
  } else {
    OUT("Second"); cout.flush();
    cin >> x >> y; --x;
    div(x, y);
  }
  ...
  while (1) {
    ...
    fore(from, to, rng) {
      ...
      rep(i, l, r + 1) {
        if (sz < i || opx != -1) break;
        if (g ^ dp[sz] ^ dpsum[sz - i] == 0) {
          opy = i;
          rep(j, from, to) {
            if (j + opy >= to) break;
            if (dpsum[sz - i] == dp[j] ^ dp[sz - j - opy]) {
              opx = j;
              break;
            }
          }
