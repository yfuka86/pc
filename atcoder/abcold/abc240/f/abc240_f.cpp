...
ll solve(ll n, vl x, vl y) {
}

ll naive(ll n, vl x, vl y) {
  ll ans = 0, cur = 0;
  rep(i, n) {
  }
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vl x(n), y(n);
    rep(i, n) {
      x[i] = rg.l(-4, 5); y[i] = rg.l(0, 5);
    }
    auto so = solve(n, x, y); auto na = naive(n, x, y);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, x, y); debug(so); debug(na);
...
void solve() {
  ...
  { // 極大の計算
    ...
    rep(i, n) {
      if (x[i] < 0) {
        if (cur > 0 && abs(x[i] * y[i]) >= cur) {
          cand.push(id + cur / abs(x[i]));
        }
      }
      cur += x[i] * y[i];
      id += y[i];
  ...
  ll ans = -LINF;
  ll sum = 0, cur = 0, id = 0;
  function<ll(ll, ll, ll)> nsum = [&](ll a, ll d, ll n) {
    return (a + (a + d * (n - 1))) * n / 2;
  };
  rep(i, n) {
    while (cand.size() && cand.front() <= id + y[i]) {
      ll d = cand.front() - id; cand.pop();
      chmax(ans, sum + nsum(cur + x[i], x[i], d));
    }
    sum += nsum(cur + x[i], x[i], y[i]);
    chmax(ans, sum);
    cur += x[i] * y[i];
    id += y[i];
  }
