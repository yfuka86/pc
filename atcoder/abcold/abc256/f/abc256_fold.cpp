...
void solve() {
  ...
  vmi b = csum(a); b.erase(b.begin());
  vmi c = csum(b); c.erase(c.begin());
  vmi d = csum(c); d.erase(d.begin());
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      ll x, v; cin >> x >> v; x--;
      S s = seg.get(x);
      seg.set(x, { s.a + mint(v) - a[x], s.sum + mint(v) - a[x], s.sz});
      a[x] = v;
    } else {
      ll x; cin >> x;
      cout << d[x] + seg.prod(0, x).sum << "\n";
    }
  }
}
