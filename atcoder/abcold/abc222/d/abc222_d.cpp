...
void solve() {
  ...
  rep(i, N) cin >> a[i];
  rep(i, N) cin >> b[i];
  vector<vmi> dp(N + 1, vmi(3001, 0));
  vector<vmi> dpsum(N + 1, vmi(3002, 0));
  dp[0][0] = 1;

  rep(j, 3001) {
    dpsum[0][j + 1] = dpsum[0][j] + dp[0][j];
  }

  rep(i, N) {
    rep2(j, a[i], b[i] + 1) {
      dp[i + 1][j] += dpsum[i][j + 1] - dpsum[i][0];
    }
    rep(j, 3001) {
      dpsum[i + 1][j + 1] = dpsum[i + 1][j] + dp[i + 1][j];
    }
  }
