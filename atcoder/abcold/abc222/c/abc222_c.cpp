...
void solve() {
  ...
  rep(i, 2 * N) que.push(mp(0, -i));
  rep(j, M) {
    vector<LP> match;
    while(!que.empty()) {
      match.pb(que.top()); que.pop();
    }

    rep(i, N) {
      auto [wc1, p1] = match[i * 2];
      auto [wc2, p2] = match[i * 2 + 1];
      bool win1 = false, win2 = false, draw = false;
      char h1 = A[-p1][j], h2 = A[-p2][j];
      ...
      if (win1) {
        que.push(mp(wc1 + 1, p1));
        que.push(mp(wc2, p2));
      }
      if (win2) {
        que.push(mp(wc1, p1));
        que.push(mp(wc2 + 1, p2));
      }
      if (draw) {
        que.push(mp(wc1, p1));
        que.push(mp(wc2, p2));
