...
void solve() {
  ...
  rep(i, n) {
    rep(j, m) {
      if (i + j + 1 < n && s[i][j] == '1') {
        G.add_directed_edge(i, i + j + 1);
        rG.add_directed_edge(i + j + 1, i);
      }
    }
  }
  ...
  rep(k, 1, n - 1) {
    ...
    rep(i, 1, m) {
      ll from = k - i;
      // k-i => k-i + j (k-i + j > k) がある
      if (from < 0) continue;
      if (sc[from] < LINF) {
        rep(j, m) { // 飛び越える
          if (s[from][j] == '1' & from + j + 1 > k) {
            // debug(k - i, k - i + j + 1);
            chmin(mi, sc[from] + tc[from + j + 1] + 1);
          }
        }
      }
    }
    if (mi == LINF) cout << -1; else cout << mi;
    cout << " ";
  }
}
