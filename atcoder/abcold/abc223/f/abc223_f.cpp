...
void solve() {
  ...
  sum[0] = 300000;
  rep(i, N) {
    if (s[i] == '(') sum[i + 1] = sum[i] + 1; else sum[i + 1] = sum[i] - 1;
  }
  ...
  rep(i, Q) {
    ...
    if (t == 1) {
      if (s[l] == s[r]) continue;
      if (s[l] == ')') seg.apply(l + 1, r + 1, 2); else seg.apply(l + 1, r + 1, -2);
      swap(s[l], s[r]);
    } else {
      ll lv = seg.get(l), rv = seg.get(r + 1);
      // rep(i, N + 1) cout << seg.get(i) << " ";
      // cout << endl;
      if (lv == rv && seg.prod(l, r + 2) >= lv) cout << "Yes" << "\n"; else cout << "No" << "\n";
    }
  }
