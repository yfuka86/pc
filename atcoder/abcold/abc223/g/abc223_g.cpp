...
void solve() {
  ...
  ll ans = 0;
  function<void(LP, ll, ll)> dfs2 = [&](LP pdp, ll v, ll p) {
    set<ll> add;
    ll sum = pdp.se;
    if (p != -1 && pdp.fi == pdp.se) add.insert(p);
    for (auto &to :g[v]) {
      if (to == p) continue;
      sum += dp[to][1];
      if (dp[to][1] == dp[to][0] && add.size() < 2) add.insert(to);
    }
    debug(pdp, v, add);
    if (sum == ma) ans++;
    for (auto &to :g[v]) {
      if (to == p) continue;
      LP npdp = {sum - dp[to][1], sum - dp[to][1] + (add.size() != 0)};
      if (add.size() == 1 && *add.begin() == to) npdp.se--;
      dfs2(npdp, to, v);
    }
  };
  dfs2(mp(0, 0), 0, -1);
