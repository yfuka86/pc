...
void solve() {
  ...
  rep(i, n) {
    ...
    while (!que.empty()) {
      ...
      if (inf[uf.root(v)]) break;
      fore(to, G[v]) {
        if (!vis[to]) {
          vis[to] = 1;
          cost[to] = cost[v] + to.cost;
          que.push(to);
        } else {
          if (chmax(cost[to], cost[v] + to.cost)) {
            inf[uf.root(to)] = 1;
          }
        }
      }
  ...
  rep(i, q) {
    LL(x, y); --x; --y;
    if (uf.root(x) != uf.root(y)) { OUT("nan"); continue; }
    if (inf[uf.root(x)]) { OUT("inf"); continue; }
    OUT(cost[y] - cost[x]);
  }
}
