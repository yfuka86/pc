...
ll calc(ll n, vlt e) {
  ...
  if (uf.size(0) == n) return cost; else return LINF;
}
...
void solve() {
  ...
  {
    vlt te = e;
    rep(i, n) te.pb({x[i], i, n});
    chmin(ans, calc(n + 1, te));
  }
  {
    vlt te = e;
    rep(i, n) te.pb({y[i], i, n});
    chmin(ans, calc(n + 1, te));
  }
  {
    vlt te = e;
    rep(i, n) te.pb({x[i], i, n});
    rep(i, n) te.pb({y[i], i, n + 1});
    chmin(ans, calc(n + 2, te));
  }
