...
constexpr ld EPS = 1e-12;  // 問題によって変える！
...
void solve() {
  ...
  rep(i, n) cin >> ps[i];
  sort(all(ps));
  Points lh = LowerHull(ps);
  Points uh = UpperHull(ps);

  ConvexHullTrick<ld, true> ucht;
  ConvexHullTrick<ld, false> lcht;
  // debug(lh);
  // debug(uh);

  ld xmi = LINF, xma = -LINF;
  rep(i, n) { chmin(xmi, ps[i].x); chmax(xma, ps[i].x); }

  rep(i, lh.size() - 1) {
    Point d = lh[i + 1] - lh[i];
    if (d.x == 0) continue;
    ld a = (ld)d.y / d.x;
    ld b = (ld)lh[i].y + (ld)-lh[i].x * a;
    lcht.add(a, b);
  }
  rep(i, uh.size() - 1) {
    Point d = uh[i + 1] - uh[i];
  ...
  rep(i, q) {
    LD(a, b);
    bool on = false;
    if (a == xmi || a == xma) on = true;
    else if (a < xmi || xma < a) { OUT("OUT"); continue; }

    ld upper = ucht.query(a);
    ld lower = lcht.query(a);
    // debug(upper, lower);
    if (lower == b || upper == b) OUT("ON");
    else if (lower < b && b < upper) {
      if (on) OUT("ON"); else OUT("IN");
    } else OUT("OUT");
  }
}
