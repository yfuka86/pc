...
public:
  ...
  int add(T x, ll add) {
    ...
    for(int b = MAXB - 1; b >= 0; --b) {
      ...
      // 編集したパート
      dp[cur] += add;
      if (add > 0 && dp[cur] == 2) s[b].insert(cur);
      if (add < 0 && dp[cur] < 2) s[b].erase(cur);
    }
    return cur;
  }
      ...
      if (dp[0] == 2) {
        ...
        rep(i, 30) {
          if (s[i].size()) {
            ...
            fore(v, s[i]) {
              ll t = i - 1;
              ll p1 = bt.nxt[v][0], p2 = bt.nxt[v][1];
              ll anscand = 1 << t;
              ...
              while (t > 0) {
                ...
                rep(j, 2) {
                  nx1 = bt.nxt[p1][j];
                  if (nx1 != -1 && dp[nx1] == 1) { b1 = j; break;}
                }
                rep(j, 2) {
                  nx2 = bt.nxt[p2][j];
                  if (nx2 != -1 && dp[nx2] == 1) { b2 = j; break;}
                }
                p1 = nx1; p2 = nx2;
                anscand += (b1 ^ b2) << (t - 1);

                // debug(t, nx1, nx2, b1, b2, anscand);
                t--;
              }
              chmin(ans, anscand);
            }
