...
void solve() {
  ...
  for (auto&[_, v]: dictr) sort(all(v));
  for (auto&[_, v]: dictc) sort(all(v));
  ...
  vlp chr, chc;
  while (!que.empty()) {
    auto [a, i, _] = que.top(); que.pop();
    if (chmin(cur, a)) {
      for (auto [r, co]: chr) chmax(mar[r], co);
      for (auto [c, co]: chc) chmax(mac[c], co);
      chr.clear();
      chc.clear();
    }
    auto [__, r, c] = koma[i];
