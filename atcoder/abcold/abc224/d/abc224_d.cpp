...
void solve() {
  ...
  vl p(9, -1);
  rep(i, 8) { ll pos; cin >> pos; p[--pos] = i; }
  rep(i, 9) if (p[i] == -1) p[i] = 8;

  // coutarray(p);

  queue<pair<ll, vl>> que;
  map<ll, ll> visited;
  que.push(mp(0LL, p));

  while(!que.empty()) {
    auto [count, p] = que.front(); que.pop();
    if (key(p) == 876543210LL) { cout << count << "\n"; return; }
    ...
    rep(i, 9) {
      if (p[i] == 8) {
        for (ll next: G[i]) {
          swap(p[i], p[next]);
          ll k = key(p);
          if (!visited[k]) { visited[k]++; que.push(mp(count + 1, p)); }
          swap(p[i], p[next]);
        }
      }
    }
