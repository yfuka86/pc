...
void solve() {
  ...
  // センサーの使用個数でdp
  vv(ll, dp, k[0]+1, k[1]+1, 1);
  vv(ll, dp2, k[0]+1, k[1]+1);

  rep(t, n) {
    rep(i, k[0] + 1) rep(j, k[1] + 1) {
      if (!dp[i][j]) continue;
      if (i < k[0]) chmax(dp2[i + 1][j], dp2[i][j] + l[0]);
      if (j < k[1]) chmax(dp2[i][j + 1], dp2[i][j] + l[1]);
    }
    rep(i, k[0] + 1) rep(j, k[1] + 1) if (dp2[i][j] < d[t]) dp[i][j] = 0;
    rep(i, k[0] + 1) rep(j, k[1] + 1) dp2[i][j] = 0;
  }

  ll ans = LINF;
  rep(i, k[0] + 1) rep(j, k[1] + 1) {
    if (dp[i][j]) chmin(ans, c[0] * i + c[1] * j);
  }
  if (ans == LINF) {
    OUT(-1);
  } else {
