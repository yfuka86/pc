...
bool is_b_in_middle(int a, int b, int c) {
    // bが真ん中の場合
    if ((a < b && b < c) || (c < b && b < a))
        return true;
    // bが真ん中でない場合
...
void solve() {
  VL(st, 6);
  function<void(vl&)> swp = [&](vl &v) {
    rep(i, 3) {
      swap(v[i * 2], v[i * 2 + 1]);
    }
  };

  if (st[2] == st[4] && st[3] == st[5]) OUTRET(0);

  if (st[2] == st[4] || st[3] == st[5]) { // 片軸のみ移動
    if (st[2] == st[4]) swp(st);

    // st[3] == st[5]の状態
    ll from = st[2] < st[4] ? st[2] - 1 : st[2] + 1;

    ll cur = abs(from - st[0]) + abs(st[3] - st[1]) + abs(st[4] - st[2]);
    if (st[1] == st[3] && is_b_in_middle(st[0], st[2], from)) cur += 2;
    OUTRET(cur);
  }

  ll fromx = st[2] < st[4] ? st[2] - 1 : st[2] + 1;
  ll fromy = st[3] < st[5] ? st[3] - 1 : st[3] + 1;

  ll cur = min(abs(fromx - st[0]) + abs(st[1] - st[3]), abs(fromy - st[1]) + abs(st[0] - st[2]));
  cur += abs(st[4] - st[2]) + abs(st[5] - st[3]) + 2;
  OUT(cur);
}
