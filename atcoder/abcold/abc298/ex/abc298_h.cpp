...
vl solve(ll n, Graph<ll> &G, vlp q) {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    fore(to, G[v]) { if (to == p) continue;
      dfs(to, v);
      dp[v] += dp[to] + hld.size[to];
    }
  };
  dfs(0, -1);

  vl dpr(n);
  function<void(ll, ll, ll)> dfsr = [&](ll v, ll p, ll par) {
    dpr[v] += par + (n - hld.size[v]);
    fore(to, G[v]) { if (to == p) continue;
      dpr[v] += dp[to] + hld.size[to];
    }

    fore(to, G[v]) { if (to == p) continue;
      dfsr(to, v, dpr[v] - dp[to] - hld.size[to]);
    }
  };
  dfsr(0, -1, 0);
  ...
  vl ret;
  fore(l, r, q) {
    if (l == r) { ret.pb(dpr[l]); continue; }

    ll d = hld.dist(l, r);
    ll lc = hld.lca(l, r);
    {
      ll d1 = hld.dist(l, lc), d2 = hld.dist(lc, r);
      if (d1 > d2) swap(l, r);
    }
    ll lp = hld.la(r, hld.depth[r] - (d + 1) / 2);
    ll rp = hld.la(r, hld.depth[r] - (d - 1) / 2);

    assert(hld.par[rp] == lp);

    // lpは祖先
    ll ans = dpr[r] + dpr[l];
    // debug(l, r, lc, lp, rp, ans);

    ans -= (dpr[lp] - (dp[rp] + hld.size[rp])) + (ll)hld.dist(lp, r) * (n - hld.size[rp]);
    // debug(ans);
    ans -= dp[rp] + (ll)hld.dist(rp, l) * hld.size[rp];
...
vl naive(ll n, Graph<ll> &G, vlp q) {
  HeavyLightDecomposition<Graph<ll>> hld(G);
  ...
  fore(u, v, q) {
    ll sum = 0;
    rep(i, n) {
      sum += min(hld.dist(u, i), hld.dist(v, i));
    }
    ans.pb(sum);
  }
  return ans;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp q;
    rep(l, n) rep(r, l, n) q.pb({l, r});

    auto so = solve(n, G, q); auto na = naive(n, G, q);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, e, q); debug(so); debug(na);
