...
void solve() {
  ...
  vl dp(n); rep(i, n) dp[i] = r[i];
  vl tp = topo_sort(G);
  if (tp.size() == 0) OUTRET("No");
  rep_r(i, n) {
    fore(from, rG[tp[i]]) {
      chmin(dp[from], dp[tp[i]] - 1);
    }
  }
  // debug(dp);

  vl deg(n), ret;
  mpq<LP> que, wait; // {終端, idx}と{始端, idx}
  rep(i, n) for (Edge e: G[i]) deg[e.to]++;
  rep(i, n) if (deg[i] == 0) wait.push({l[i], i});
  vl ans(n);

  rep(i, n) {
    while (wait.size() && wait.top().fi <= i + 1) {
      ll v = wait.top().se; wait.pop();
      que.push({dp[v], v});
    }
    if (que.empty()) OUTRET("No");
    ll v = que.top().se; que.pop();
    ans[i] = v;
    if (i >= dp[v]) OUTRET("No");
    for(ll next: G[v]) {
      deg[next]--;
      if (deg[next] == 0) wait.push({l[next], next});
    }
    G[v].clear();
  }
  vl aans(n);
