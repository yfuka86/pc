...
bool solve() {
  vl A(6), B(6); rep(i, 5) cin >> A[i + 1]; rep(i, 5) cin >> B[i + 1];

  auto tohave = [&](ll a, ll b) {
    assert(b >= a);
    ll t = min(A[a], B[b]);
    A[a] -= t; B[b] -= t;
    if (b - a) B[b - a] += t;
  ...
  if (B[5] * 2 + B[4] * 2 + B[3] + B[2] < A[2]) return false;

  ll asum = A[1] + A[2] * 2, bsum = 0;
  rep(i, 5) bsum += B[i + 1] * (i + 1);
  if (bsum >= asum) return true; else return false;
}
