...
void solve() {
  ...
  vv(ll, cost, n, n);
  rep(i, n) rep(j, n) cost[i][j] = abs(a[i] - b[j]);

  vv(ll, dp, 1 << n);
  rep(i, n) {
    dp[1 << i].assign(n, LINF);
    dp[1 << i][i] = cost[i][0];
  }

  rep(i, 1, n) {
    rep(S, 1, 1 << n) {
      ll bc = __builtin_popcount(S);
      if (bc == i + 1) dp[S].assign(n, LINF);
    }

    rep(S, 1, 1 << n) {
      ll done = __builtin_popcount(S);
      if (i != done) continue;
      ll mi = *min_element(all(dp[S]));
      rep(j, n) {
        if (S & 1 << j) continue;
        chmin(dp[S | 1 << j][j], mi + c + cost[j][done]);
      }
      rep(i, n - 1) {
        ll j = i + 1;
        if (S & 1 << j) continue;
        chmin(dp[S | 1 << j][j], dp[S][i] + cost[j][done]);
      }
      dp[S].clear();
      dp[S].shrink_to_fit();
    }
