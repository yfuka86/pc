...
void solve() {
  ...
  mf_graph<ll> mf(h * w + 3);
  ll p = h * w, s = p + 1, t = s + 1;
  mf.add_edge(s, p, LINF);

  rep(i, h) rep(j, w) {
    ll id = i * w + j;
    mf.add_edge(s, id, 0);
    mf.add_edge(id, t, a[i][j]);

    ll di = i - 1, dj = j - 1, dj2 = j + 1;

    if (incl(di, 0, h) && incl(dj, 0, w)) {
      mf.add_edge(di * w + dj, id, c);
    } else mf.add_edge(p, id, c);

    if (incl(di, 0, h) && incl(dj2, 0, w)) {
      mf.add_edge(di * w + dj2, id, c);
    } else mf.add_edge(p, id, c);
  }

  ll sum = 0; rep(i, h) sum += sum_of(a[i]);
