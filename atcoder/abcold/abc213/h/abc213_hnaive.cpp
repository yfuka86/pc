...
template<typename T> void coutarray(vector<T>& v, int offset = 0, string sep = " ") { rep(i, v.size()) { if (i > 0) cout << sep; if (offset) cout << (v[i] + mint(offset)).val(); else cout << v[i].val(); } cout << "\n"; }
