...
void solve() {
  ...
  rep(S, 1 << n) {
    ll cnt = 0;
    rep(i, m) if (S & 1 << edge[i].fi && S & 1 << edge[i].se) cnt++;
    dp[S] = mint(2).pow(cnt);
  }
  // debug(dp);
  ...
  rep(S, 1 << n) {
    mint sum = 0;
    for (int s = (S - 1) & S; s > 0; s = (s - 1) & S) {
      if (__builtin_clz(S) != __builtin_clz(s)) continue;
      sum += dp2[s] * dp[S ^ s];
    }
    dp2[S] = dp[S] - sum;
  }
  ...
  vmi ans(n, 0);
  rep(S, 1 << n) {
    rep(i, n) if (S & 1 && S & 1 << i) ans[i] += dp2[S] * dp[(1 << n) - 1 ^ S];
  }
  ans.erase(ans.begin());
  coutarray(ans, 0, "\n");
}
