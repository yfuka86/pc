...
int main()
{
  ...
  function<vl(vl)> dfs = [&](vl rem) -> vl{
    ...
    rep2(j, i + 1, rem.size()) {
      vl next;
      rep(k, rem.size()) {
        if (k != i && k != j) next.pb(rem[k]);
      }

      vl d = dfs(next);
      for(ll ret: d) {
        ans.pb(a[rem[i]][rem[j]] ^ ret);
      }
    }
    return ans;
  };

  vl v(2 * N); iota(all(v), 0);
  vl ansv = dfs(v);
  ll ans = 0; rep(i, ansv.size()) { chmax(ans, ansv[i]); }
  cout << ans << "\n";
}
