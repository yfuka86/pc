...
struct S { ll sum, ma; };
S op(S l, S r) {
  return { l.sum + r.sum, maxe(l.sum + r.sum, l.sum + r.ma, l.ma) };
}
S e() { return {0, -LINF}; }
...
void solve() {
  ...
  vector<S> v(n);
  rep(i, n) v[i] = {a[i] - b, a[i] - b};
  segtree<S, op, e> seg(v);

  rep(i, q) {
    LL(c, x); --c;
    seg.set(c, {x - b, x - b});
    ll r = seg.max_right(0, [&](S x) {
      return x.ma < 0;
    });
    if (r == n) OUT((ld)seg.all_prod().sum / n + b);
    else {
      ll ri = min(n, r + 1);
      OUT((ld)seg.prod(0, ri).sum / (ld)ri + b);
    }
  }
}
