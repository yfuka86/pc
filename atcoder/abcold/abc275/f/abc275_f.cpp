...
void solve() {
  ...
  v3(ll, dp, n + 1, m + 1, 2, LINF); dp[0][0][1] = 0;

  rep(i, n) {
    rep(j, m + 1) {
      chmin(dp[i + 1][j][0], min(dp[i][j][1] + 1, dp[i][j][0]));
      if (j + a[i] <= m) chmin(dp[i + 1][j + a[i]][1], min(dp[i][j][1], dp[i][j][0]));
    }
  }

  rep(i, 1, m + 1) {
    ll t = min(dp[n][i][0], dp[n][i][1]);
    if (t == LINF) OUT(-1); else OUT(t);
  }
}
