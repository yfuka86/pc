...
void solve() {
  ...
  for (auto [d, cnt]: f) {
    ...
    vmi od1 = convolution(eva, od);
    vmi ev2 = convolution(oda, od);
    vmi od2 = convolution(oda, ev);
    ...
    while(evt.back() == 0) evt.pop_back();
    while(odt.back() == 0) odt.pop_back();
    eva = evt; oda = odt;
  }

  if (oda.size() < m + 1) cout << 0 << "\n";
  else cout << oda[m].val() << "\n";
}
