...
void solve() {
  ...
  segtree<S, miop, mie> rmiseg(n); // 右の大きいもの
  segtree<S, miop, mie> lmiseg(n); // 左の大きいもの

  rep_r(i, n) {
    {
      ll sum = i + rev[i];
      ll res = rmiseg.prod(rev[i] + 1, n);
      chmin(ans[rev[i]], res - sum);
      rmiseg.set(rev[i], sum);
    }
    {
      ll sum = i + (n - rev[i]);
      ll res = lmiseg.prod(0, rev[i]);
      chmin(ans[rev[i]], res - sum);
      lmiseg.set(rev[i], sum);
    }
  }

  segtree<S, maop, mae> rmaseg(n); // 右の小さいもの
  segtree<S, maop, mae> lmaseg(n); // 左の小さいもの

  rep(i, n) {
    {
      ll sum = i - rev[i];
      ll res = rmaseg.prod(rev[i] + 1, n);
      chmin(ans[rev[i]], sum - res);
      rmaseg.set(rev[i], sum);
    }
    {
      ll sum = i - (n - rev[i]);
      ll res = lmaseg.prod(0, rev[i]);
      chmin(ans[rev[i]], sum - res);
      lmaseg.set(rev[i], sum);
    }
  }
  // debug(ans);
  OUTARRAY(ans);
