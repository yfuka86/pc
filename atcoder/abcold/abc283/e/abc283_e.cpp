...
void solve() {
  ...
  VV(ll, a, h, w);
  vv(ll, ra, h, w);
  rep(i, h) rep(j, w) ra[i][j] = 1 - a[i][j];

  vvl cand1(h), cand2(h);

  rep(j, w) {
    bool valid = false;
    if (j > 0 && a[0][j] == a[0][j - 1]) valid = true;
    if (j < w - 1 && a[0][j] == a[0][j + 1]) valid = true;
    if (!valid) { cand1[0].pb(j); cand2[0].pb(j); }
  }

  rep(i, 1, h) rep(j, w) {
    bool valid = false;
    if (j > 0 && a[i][j] == a[i][j - 1]) valid = true;
    if (j < w - 1 && a[i][j] == a[i][j + 1]) valid = true;

    if (!valid) {
      if (a[i][j] != a[i - 1][j]) cand1[i].pb(j);
      if (a[i][j] != ra[i - 1][j]) cand2[i].pb(j);
    }
  }
  ...
  rep(i, h - 1) {
    { // dp[i][0][k]
      ...
      fore(j, cand1[i]) {
        if (a[i][j] != a[i + 1][j]) valid1 = false;
        if (a[i][j] != ra[i + 1][j]) valid2 = false;
      }
      rep(k, 2) {
        if (valid1) chmin(dp[i + 1][0][k], dp[i][0][k] + (k ? 1 : 0));
        if (valid2) chmin(dp[i + 1][1][1 - k], dp[i][0][k] + (!k ? 1 : 0));
      }
    }
    { // dp[i][1][k]
      bool valid1 = true, valid2 = true;
      ...
      rep(k, 2) {
        if (valid1) chmin(dp[i + 1][0][k], dp[i][1][k] + (k ? 1 : 0));
        if (valid2) chmin(dp[i + 1][1][1 - k], dp[i][1][k] + (!k ? 1 : 0));
      }
    }
  }
  // debug(dp);
  ...
  ll ans = LINF;
  rep(i, 2) rep(j, 2) {
    if (i == 0 && cand1[h - 1].size()) continue;
    if (i == 1 && cand2[h - 1].size()) continue;
    chmin(ans, dp[h - 1][i][j]);
  }
