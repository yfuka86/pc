...
void solve() {
  ...
  // binom(n - 1, k)のkに対するそこまでの和
  // 各項のdiffの1, 2(mod 3)の部分に関する和が定まった時、初項が動くときの決め方の合計を先に計算しておく
  vmi sum(n); sum[0] = 1;
  rep(i, n - 1) sum[i + 1] = sum[i] + comb(n - 1, i + 1);
  // debug(sum);

  // 初項と1, 2の部分の和に対して、
  // 3の倍数の項（n - 1項の差の部分に対して）を定める方法を掛け合わせる
  rep(i, m + 1) {
    ll rem = i - (n - 1);
    if (rem < 0) continue;
    ll t = (m - i) / 3;
    ans += sum[min(rem, n - 1)] * comb(t + n - 1, t);
  }
