...
void solve() {
  ...
  v3mi dp(n + 1, vvmi(n + 1, vmi(n + 1, 0)));
  dp[0][0][0] = 1;

  function<LP(ll, ll, ll)> ff = [&](ll tot, ll j, ll k) {
    ll x = (tot - j - k) * a + c * j + e * k;
    ll y = (tot - j - k) * b + d * j + f * k;
    return mp(x, y);
  };
  ...
  rep(i, n) {
    rep(j, n) {
      rep(k, n) {
        if (i < j + k) continue;
        if (pro.find(ff(i + 1, j, k)) == pro.end()) dp[i + 1][j][k] += dp[i][j][k];
        if (pro.find(ff(i + 1, j + 1, k)) == pro.end()) dp[i + 1][j + 1][k] += dp[i][j][k];
        if (pro.find(ff(i + 1, j, k + 1)) == pro.end()) dp[i + 1][j][k + 1] += dp[i][j][k];
      }
    }
  }
