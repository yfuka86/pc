...
void solve() {
  ...
  vvmi dp(d + 1, vmi(d + 1, 0)), dpt(d + 1, vmi(d + 1, 0));
  dp[0][0] = 1;

  rep(i, n) {
    vvmi sum(d * 2 + 1, vmi(d + 2, 0));
    rep(i, d * 2 + 1) {
      rep(j, d + 1) {
        sum[i][j + 1] += sum[i][j] + (0 <= j + i - d && j + i - d <= d ? dp[j][j + i - d] : mint(0));
      }
    }

    vvmi sum2(d * 2 + 1, vmi(d + 2, 0));
    rep(i, d * 2 + 1) {
      rep(j, d + 1) {
        sum2[i][j + 1] += sum2[i][j] + (0 <= i - j && i - j <= d ? dp[j][i - j] : mint(0));
      }
    }
    ...
    rep(j, d + 1) {
      rep(k, d + 1) {
        ...
        if (j + k - diff >= 0) {
          // {0,diff}と{diff,0}の遷移は上に含まれているのでj+1-diffからjとしている
          // p[i]とq[i]が同じ場合は、上で{0, 0}の遷移が重複しているのでそれをここで取り除いている
          dpt[j][k] += sum2[j + k - diff][j] - sum2[j + k - diff][max(j + 1 - diff, 0ll)];
        }

      }
    }
    dp = dpt;
    dpt.assign(d + 1, vmi(d + 1, 0));
  }
