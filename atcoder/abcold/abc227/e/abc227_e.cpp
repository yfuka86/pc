...
void solve() {
  ...
  vlp kidx, eidx, yidx;
  rep(i, n) {
    if (s[i]=='K') kidx.pb({eidx.size(), yidx.size()});
    if (s[i]=='E') eidx.pb({kidx.size(), yidx.size()});
    if (s[i]=='Y') yidx.pb({kidx.size(), eidx.size()});
  }
  ll kc = kidx.size();
  ll ec = eidx.size();
  ll yc = yidx.size();

  v4(ll, dp, n + 1, kc + 1, ec + 1, 1000); dp[0][0][0][0] = 1;

  rep(i, n) {
    rep(know, kc + 1) rep(enow, ec + 1) rep(sw, 1000) {
      ll ynow = i - know - enow;
      if (dp[i][know][enow][sw] == 0) continue;

      if (know < kc) {
        ll diff = max(0, kidx[know].fi - enow) + max(0, kidx[know].se - ynow);
        dp[i + 1][know + 1][enow][sw + diff] += dp[i][know][enow][sw];
      }
      if (enow < ec) {
        ll diff = max(0, eidx[enow].fi - know) + max(0, eidx[enow].se - ynow);
        dp[i + 1][know][enow + 1][sw + diff] += dp[i][know][enow][sw];
      }
      if (ynow < yc) {
        ll diff = max(0, yidx[ynow].fi - know) + max(0, yidx[ynow].se - enow);
        dp[i + 1][know][enow][sw + diff] += dp[i][know][enow][sw];
      }
    }
  }
