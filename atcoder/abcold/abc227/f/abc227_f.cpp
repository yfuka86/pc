...
void solve() {
  ...
  rep(ik, H) {
    rep(jk, W) {
      ll pivot = a[ik][jk];
      // tuple そこまでのsum, 個数
      vector<vector<vlp>> dp(H + 1, vector<vlp>(W + 1, vlp(K + 1, mp(0, 0))));
      if (a[0][0] > pivot) a[0][0][0] = mp(a[0][0], 1);
      else if (a[0][0] == pivot) a[0][0][1] = mp(0, 0);
      else a[0][0][0] = mp(0, 0);

      rep(i, H) {
        rep(j, W) {
          rep(k, K) {
            auto [sum, num] = rep[i][j][k];
            if (a[i + 1][j] == pivot) chmin(rep[i + 1][j][k + 1], mp(sum, num));
            else if (a[i + 1][j] > pivot) rep[i + 1][j][k] = mp(sum + a[i + 1][j], num + 1);
            else rep[i + 1][j][k] = mp(sum, num);

            if (a[i][j + 1] == pivot) chmin(rep[i][j + 1][k + 1], mp(sum, num));
            else if (a[i][j + 1] > pivot) rep[i][j + 1][k] = mp(sum + a[i][j + 1], num + 1);
            else rep[i + 1][j][k] = mp(sum, num);
          }
        }
      }
