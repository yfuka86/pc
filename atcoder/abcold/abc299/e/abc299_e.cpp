...
void solve() {
  ...
  rep(i, k) {
    vl c = bfs(G, p[i] - 1);
    rep(j, n) {
      if (c[j] < d[i]) mustwhite[j] = 1;
      else if (c[j] == d[i]) needtobeblack[i].pb(j);
    }
  }

  rep(i, k) {
    bool valid = false;
    fore(v, needtobeblack[i]) {
      if (!mustwhite[v]) valid = true;
    }
    if (!valid) OUTRET("No");
  }

  rep(i, n) mustwhite[i] = !mustwhite[i];
  OUT("Yes");
  OUTARRAY(mustwhite, 0, "");
}
