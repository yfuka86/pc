...
vl solve(ll n, ll m, vl a) {
  ...
  segtree<S, op, e> segf(m);
  rep(i, m) {
    segf.set(i, *prev(idx[i].end()));
  }

  ll cur = 0;
  vl ans;
  while (ans.size() < m) {
    ll r = segf.all_prod(); // inclusive
    ll v = seg.prod(cur, r + 1);
    ans.pb(v);
    // debug(v); cout.flush();
    segf.set(v, LINF);
