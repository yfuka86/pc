...
void solve() {
  ...
  mpq<LT> que;
  vv(ll, cost, n, 2, LINF);
  cost[0][1] = 0; que.push({0, 0, 1});
  if (swi[0]) cost[0][0] = 0; que.push({0, 0, 0});
  while(!que.empty()) {
    auto [c, v, k] = que.top(); que.pop();
    if (cost[v][k] < c) continue;
    for(auto &to: G[v]) {
      if (a[to.idx] != k) continue;
      ll nc = cost[v][k] + to.cost;
      if (chmin(cost[to][k], nc)) que.push({nc, to, k});
      if (swi[to] && chmin(cost[to][1 - k], nc)) que.push({nc, to, 1 - k});
    }
  }

  ll ans = min(cost[n - 1][0], cost[n - 1][1]);
