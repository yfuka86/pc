...
void solve() {
  ...
  if (cnt.size() == m) OUTRET(0);

  auto it = cnt.begin();
  ll l = 0, cur = 0;
  while (1) {
    cur++;
    if (cnt.count(m - cur)) l = m - cur; else break;
  }

  bool vis = false;
  ll ans = LINF, sum = 0, allsum = sum_of(a);
  for(auto it = cnt.lower_bound(l); ;) {
    sum += it->se;
    chmin(ans, allsum - sum);

    ll from = it->fi;
    it++;
    if (it == cnt.end()) {
      if (vis) break;
      else { it = cnt.begin(); vis = true; }
    }
    if ((from + 1) % m != (it->fi) % m) sum = 0;
  }
