...
void solve() {
  ...
  if (ps.size() <= ma) OUTRET(t.size());

  // debug(ps);
  map<LP, ll> mp;
  function<ll(ll, ll)> dfs = [&](ll n, ll pi) {
    if (mp.count({n, pi})) return mp[{n, pi}];
    if (pi == ma - 1) { return mp[{n, pi}] = upper_bound(all(t), n) - t.begin(); }
