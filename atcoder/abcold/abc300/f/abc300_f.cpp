...
void solve() {
  ...
  rep(num, max(0, k / xcnt - 2), k / xcnt + 1) {
    if (num >= m) {
      chmax(ans, n * m);
    } else if (num == m - 1) {
      ll t = n * (m - 1);
      ll tk = k - xcnt * (m - 1);
      rep(i, tk + 1) {
        chmax(ans, t + front[min(xcnt, i)]);
        chmax(ans, t + back[min(xcnt, i)]);
      }
    } else {
      ll t = n * num;
      ll tk = k - num * xcnt;
      rep(i, tk + 1) {
        chmax(ans, t + front[min(xcnt, i)] + back[min(xcnt, tk - i)]);
      }
    }
  }
