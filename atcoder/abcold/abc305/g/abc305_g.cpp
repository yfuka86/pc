...
void solve() {
  ...
  vs _s(m); IN(_s);
  vv(ll, s, m);
  rep(i, m) fore(c, _s[i]) s[i].pb(c - 'a');
  ...
  if (n < 6) {
    ...
    rep(S, 1 << n) {
      bool valid = true;
      rep(i, m) {
        rep(off, n - s[i].size() + 1) {
          bool all = true;
          rep(j, s[i].size()) {
            if (s[i][j] != ((S >> j + off) & 1)) all = false;
          }
          if (all) valid = false;
        }
      }
    ...
    vmi ini(1 << 6); vb valids(1 << 6);
    rep(S, 1 << 6) {
      bool valid = true;
      rep(i, m) {
        rep(off, 6 - s[i].size() + 1) {
          bool all = true;
          rep(j, s[i].size()) {
            if (s[i][j] != ((S >> j + off) & 1)) all = false;
          }
      ...
      if (valid) { ini[S] = 1; valids[S] = 1; }
    }

    vv(mint, M, 1 << 6, 1 << 6);
    ll filter = (1 << 6) - 1;
    rep(from, 1 << 6) {
      ll bas = ((from << 1) & filter);
      if (valids[bas ^ 1]) M[bas ^ 1][from] = 1;
      if (valids[bas]) M[bas][from] = 1;
    }

    OUT(sum_of(m_map(m_pow(M, n - 6), ini)));
  }
}
