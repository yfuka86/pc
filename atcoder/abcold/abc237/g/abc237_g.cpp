...
// テンプレ
struct S{ ll sz0, sz1; }; using F = ll;
S op(S l, S r) {
  return {l.sz0 + r.sz0, l.sz1 + r.sz1};
}
S e() { return {0, 0}; }
S mapping(F f, S x) {
  if (f == -1) return x;
  if (f == 1) {
    return {0, x.sz0 + x.sz1};
  } else {
    return {x.sz0 + x.sz1, 0};
  }
}
...
void solve() {
  ...
  vector<S> v1(n), v2(n);
  rep(i, n) {
    if (p[i] > x) v1[i] = {0,1}; else v1[i] = {1,0};
    if (p[i] >= x) v2[i] = {0,1}; else v2[i] = {1,0};
  }

  lazy_segtree<S, op, e, F, mapping, composition, id> seg1(v1), seg2(v2);

  rep(i, q) {
    LL(c, l, r); --l;
    {
    S x = seg1.prod(l, r);
    // debug(l, r, x.sz0, x.sz1);
    if (c == 1) {
      seg1.apply(l, l + x.sz0, 0);
      seg1.apply(l + x.sz0, r, 1);
    } else {
      seg1.apply(l, l + x.sz1, 1);
      seg1.apply(l + x.sz1, r, 0);
    }
    }
    {
    S x = seg2.prod(l, r);
    if (c == 1) {
      seg2.apply(l, l + x.sz0, 0);
      seg2.apply(l + x.sz0, r, 1);
    } else {
      seg2.apply(l, l + x.sz1, 1);
      seg2.apply(l + x.sz1, r, 0);
    }
    }
  }
  rep(i, n) {
    // debug(seg1.get(i).sz0);
    // debug(seg2.get(i).sz0);

    if (seg1.get(i).sz0 != seg2.get(i).sz0) OUTRET(i + 1);
  }
}
