...
void solve() {
  ...
  auto co = [&](ll i) {
    if (i < n) {
      ...
      return mp(p[i - n], q[i - n]);
    } else {
      return mp(0ll, 0ll);
    }
  };

  function<ld(ll, ll)> dis = [&](ll i, ll j) {
    auto p1 = co(i);
    auto p2 = co(j);
    ld d1 = abs(p1.fi - p2.fi), d2 = abs(p1.se - p2.se);
    return sqrt(d1 * d1 + d2 * d2);
  };

  vv(ld, dist, n + m + 1, n + m + 1);
  ...
  rep(S, 1 << n + m + 1) {
    ll boost = 0;
    rep(i, m) if (S & 1 << n + i) boost++;

    rep(from, n + m + 1) {
      rep(to, n + m + 1) {
        if (S & 1 << to) continue;
        chmin(dp[S | 1 << to][to], dp[S][from] + dist[from][to] / POW(2, boost));
      }
    }
  }
