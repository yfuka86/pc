...
void solve() {
  LL(n, x, y);
  VL(a, n);
  vl dpx(20001), dpy(20001);
  vl dpxt(20001), dpyt(20001);
  dpx[10000] = 1;
  dpy[10000] = 1;
  ...
  rep(i, 1, n) {
    rep(j, 20001) {
      if (i & 1) { // y軸
        if (dpy[j] == 0) continue;
        if (j + a[i] <= 20000) dpyt[j + a[i]] = 1;
        if (j - a[i] >= 0) dpyt[j - a[i]] = 1;
      } else { // x軸
        if (dpx[j] == 0) continue;
        if (j + a[i] <= 20000) dpxt[j + a[i]] = 1;
        if (j - a[i] >= 0) dpxt[j - a[i]] = 1;
      }
    }
    if (i & 1) {
      dpy = dpyt;
  ...
  if (dpx[x + 10000] && dpy[y + 10000]) OUT("Yes");
  else OUT("No");
}
