...
void solve() {
  ...
  rep(i, n) {
    ...
    rep(j, n) {
      ...
      ll diff = x[i] - x[j];
      if (abs(relv) < 1e-6) {
        if (incl(diff, -a, 1)) sum += w[j];
        continue;
      }

      ld t1 = (ld)diff / relv, t2 = (ld)(diff + a) / relv;
      if (t1 > t2) swap(t1, t2);
      ev.pb({t1, -w[j]}); ev.pb({t2, w[j]});
    }
    sort(all(ev));
    // debug(ev);
...
signed main() {
  cin.tie(0)->sync_with_stdio(0); cout.tie(0); cout << fixed << setprecision(3);
  int t = 1; // cin >> t;
  while (t--) solve();
  // while (t--) compare();
}
