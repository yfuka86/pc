...
void solve() {
  ...
  queue<ll> xid, yid, xyid;
  vl invalid;
  rep(i, n) {
    if (a[i] == x) xid.push(i), xyid.push(i);
    if (a[i] == y) yid.push(i), xyid.push(i);
    if (a[i] < y || x < a[i]) invalid.pb(i);
  }
  invalid.pb(n);

  ll ans = 0;
  ll cur = 0, xcur = 0, ycur = 0, xycur = 0;
  for (auto to: invalid) {
    ll nn = to - cur;
    ans += nn * (nn + 1) / 2;
    while (!xid.empty() && xid.front() < to) {
      ll xx = xid.front();
      ans -= (xx - xcur) * (xx - xcur + 1) / 2;
      xcur = xx + 1;
    }
    ans -= (to - xcur) * (to - xcur + 1) / 2;

    while (!yid.empty() && yid.front() < to) {
      ll yy = yid.front();
      ans -= (yy - ycur) * (yy - ycur + 1) / 2;
      ycur = yy + 1;
    }
    ans -= (to - ycur) * (to - ycur + 1) / 2;

    while (!xyid.empty() && xyid.front() < to) {
      ll xy = xyid.front();
      ans += (xy - xycur) * (xy - xycur + 1) / 2;
      xycur = xy + 1;
    }
    ans += (to - xycur) * (to - xycur + 1) / 2;

    cur = xcur = ycur = xycur = to + 1;
  }
  cout << ans << "\n";
}
