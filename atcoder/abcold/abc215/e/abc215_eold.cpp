...
void solve() {
  ll N; cin >> N;
  string S; cin >> S;
  string kind = "ABCDEFGHIJ";

  vector<vvmi> dp(N + 1, vvmi(1 << 10, vmi(10, 0)));

  rep(i, N) {
    ll totype = find(all(kind), S[i]) - kind.begin();
    dp[i + 1][1 << totype][totype] += 1;
    rep(s, 1 << 10) {
      rep(type, 10) {
        dp[i + 1][s][type] += dp[i][s][type];
        if (totype == type) dp[i + 1][s][totype] += dp[i][s][type];
        else if (!(s & 1 << totype)) dp[i + 1][s | 1 << totype][totype] += dp[i][s][type];
      }
    }
  }
