...
void solve() {
  ...
  vl xtmp(N);
  vl ytmp(N);
  rep(i, N) { xtmp[i] = P[i].first; ytmp[i] = P[i].second; }
  segtree<S, maxop, e> maxseg(ytmp);
  segtree<S, minop, mine> minseg(ytmp);
  ...
  while (ng > ok + 1) {
    ...
    rep(i, N) {
      auto [x, y] = P[i];
      auto itr = lower_bound(all(xtmp), x + mid);
      if (itr == xtmp.end()) continue;
      ll l = itr - xtmp.begin();
      if (maxseg.prod(l, N) >= y + mid || minseg.prod(l, N) <= y - mid) { valid = true; break; }
    }
