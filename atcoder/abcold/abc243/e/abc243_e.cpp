...
template<typename T> void coutarray(vector<T>& v, int offset = 0) { rep(i, v.size()) { if (i > 0) cout << " "; if (offset) cout << v[i] + offset; else cout << v[i]; } cout << "\n"; }
...
void solve() {
  ...
  vvl cost(N, vl(N, LINF)), found(N, vl(N, 0));
  rep(i, N) cost[i][i] = 0;
  rep(i, N) for (auto &to: G[i]) { cost[i][to] = to.cost; cost[to][i] = to.cost; }

  rep(k, N) rep(i, N) rep(j, N) {
    if (cost[i][j] == cost[i][k] + cost[k][j] || chmin(cost[i][j], cost[i][k] + cost[k][j])) {
      if (k != i && k != j) found[i][j] = 1;
    }
  }
