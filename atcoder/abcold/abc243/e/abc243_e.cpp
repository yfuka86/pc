...
void solve() {
  ...
  rep(k, n) rep(i, n) rep(j, n) {
    ...
    if (costs[i][j] == costs[i][k] + costs[k][j]) chmax(edges[i][j], edges[i][k] + edges[k][j]);
    if (chmin(costs[i][j], costs[i][k] + costs[k][j])) {
      chmax(edges[i][j], edges[i][k] + edges[k][j]);
    }
  }
