...
void solve() {
  ...
  string s; cin >> s;
  map<ll, ll> leftmax, rightmin;
  rep(i, N) {
    auto [x, y] = p[i];
    if (s[i] == 'L') {
      if (!leftmax.count(y)) leftmax[y] = x; else chmax(leftmax[y], x);
    } else {
      if (!rightmin.count(y)) rightmin[y] = x; else chmin(rightmin[y], x);
    }
  }

  for (auto[y, x]: leftmax) {
    if(rightmin.count(y) && x > rightmin[y]) { cout << "Yes" << "\n"; return; }
  }
  cout << "No" << "\n";
}
