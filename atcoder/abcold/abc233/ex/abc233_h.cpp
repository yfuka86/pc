  ...
  while(1) {
    ...
    rep(i, q) {
      if (ans[i].fi - 1 > ans[i].se) {
        ...
        query.pb({max(a[i] - mid, 0), min(a[i] + mid + 1, 200001), max(b[i] - mid, 0), min(b[i] + mid + 1, 200001), i, mid});
      }
    }
    if (query.size() == 0) break;
    Mo mo(200001, query.size());
    fore(v, query) mo.insert(v[0], v[1]);
    BIT<ll> bt(200001);
    set<ll> btlist;

    auto add = [&](int i) {
      if (gusebt[i]) btlist.insert(i);
      else fore(v, g[i]) bt.add(v, 1);
    };
    auto erase = [&](int i) {
      if (gusebt[i]) btlist.erase(i);
      else fore(v, g[i]) bt.add(v, -1);
    };
    auto rem = [&](int idx) {
      auto v = query[idx];
      ll sum = bt.sum(v[2], v[3]);
      fore(id, btlist) sum += bts[id].sum(v[2], v[3]);

      if (sum >= k[v[4]]) ans[v[4]].fi = v[5];
      else ans[v[4]].se = v[5];
    };
