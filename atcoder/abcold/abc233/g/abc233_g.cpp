...
void solve() {
  ...
  vvl s(n, vl(n, 0));
  rep(i, n) rep(j, n) if (grid[i][j] == '#') s[i][j]++;
  vvl imos = cumsum2d(s);
  auto f = [&](ll x, ll y, ll x2, ll y2) {
    return imos[x2][y2] + imos[x][y] - imos[x][y2] - imos[x2][y];
  };

  v4l dp(n + 1, v3l(n + 1, vvl(n + 1, vl(n + 1, -1))));

  function<ll(ll, ll, ll, ll)> dfs = [&](ll x, ll y, ll x2, ll y2) {
    if (dp[x][y][x2][y2] != -1) return dp[x][y][x2][y2];
    if (x2 - x == 0 || y2 - y == 0 || f(x,y,x2,y2) == 0) return dp[x][y][x2][y2] = 0;

    ll co = max(x2 - x, y2 - y);

    rep2(i, x, x2) if (f(i, y, i + 1, y2) == 0) chmin(co, dfs(x,y,i,y2) + dfs(i+1,y,x2,y2));
    rep2(j, y, y2) if (f(x, j, x2, j + 1) == 0) chmin(co, dfs(x,y,x2,j) + dfs(x,j+1,x2,y2));
    return dp[x][y][x2][y2] = co;
  };
