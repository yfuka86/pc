...
void solve() {
  ...
  vl ans;
  function<void(ll, ll)> swape = [&](ll v, ll u) {
    ans.pb(edgenum[{v, u}]);
    swap(p[v], p[u]);
    revp[p[v]] = v;
    revp[p[u]] = u;
  ...
  rep(i, n) {
    ...
    {
      set<ll> s, t;
      fore(d, v, vertices) { s.insert(v); t.insert(p[v]);}
      if (s != t) OUTRET(-1);
    }
    ...
    fore(d, v, vertices) {
      ...
      if (from == v) continue;
      vl path, tt;
      function<void(ll, ll)> getpath = [&](ll v, ll p) {
        tt.pb(v);
        if (v == from) path = tt;
        fore(to, tree[v]) {
          if (to == p) continue;
