...
struct Trie {
public:
  vector<vector<int>> nxt; vector<bool> end; vector<int> cs, fail;
  Trie() { nxt.emplace_back(vector<int>(X, -1)); end.emplace_back(0); cs.emplace_back(-1); }
  size_t size() const { return nxt.size(); }

  int add_node(int v, int n, bool isend = false) {
    assert(fail.size() == 0);
    if (nxt[v][n] == -1) {
      nxt.emplace_back(vector<int>(X, -1));
      cs.emplace_back(n);
      end.emplace_back(isend);
      return nxt[v][n] = nxt.size() - 1;
    } else {
      if (isend) end[nxt[v][n]] = 1;
      return nxt[v][n];
    }
  }
  int add(vector<int> &s) { int n = s.size(), v = 0; for (int i = 0; i < n; ++i) v = add_node(v, s[i], i == n - 1); return v; }
  int add(string &s) { int n = s.size(), v = 0; for (int i = 0; i < n; ++i) v = add_node(v, s[i] - margin, i == n - 1); return v; }
...
ll n; cin >> n;
  ...
  ll gn = G.size();
  vl sz(gn);
  function<void(ll, ll)> dfssz = [&](ll v, ll p) {
    if (G.end[v]) sz[v] = 1;
    for (auto &to: G[v]) {
      if (to == p || to == -1) continue;
      dfssz(to, v);
      sz[v] += sz[to];
    }
  };
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    ...
    for (auto &to: G[v]) {
      if (to == p || to == -1) continue;
      dp[to] = dp[v] + (G.end[to] ? 1 : 0) + (sum - sz[to]) / 2;
      dfs(to, v);
    }
