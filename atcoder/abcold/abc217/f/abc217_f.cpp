...
void solve() {
  init_f();
  ll n, m; cin >> n >> m;
  vvb rng(n * 2 + 1, vb(n * 2 + 1));
  rep(i, m) {
    ll a, b; cin >> a >> b; a--;
    rng[a][b] = true;
  }

  vvmi dp(n * 2 + 1, vmi(n * 2 + 1, -1));

  function<mint(ll, ll)> dfs = [&](ll l, ll r) {
    if (dp[l][r] != -1) return dp[l][r];
    if (r == l) return dp[l][r] = 1;

    mint ret = 0;
    for (int i = l + 2; i <= r; i += 2) if (rng[l][i]) {
      // debug(l, i, r);
      ret += dfs(l + 1, i - 1) * dfs(i, r) * comb((r - l) / 2, (i - l) / 2);
    }
    return dp[l][r] = ret;
  };
