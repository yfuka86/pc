...
void solve() {
  ...
  rep(i, h) rep(j, w) {
    ...
    mf.add_edge(n + i * w + j, t, 1);
  }

  auto f = [&](ll x, ll y, ll x2, ll y2) {
    mf.add_edge(x * w + y, n + x2 * w + y2, 1);
  };

  rep(i, h) rep(j, w) {
    if (i < h - 1) {
      if (c[i][j] == '2' && c[i + 1][j] == '2') {
        f(i, j, i + 1, j);
        f(i + 1, j, i, j);
      }
      if (c[i][j] == '2' && c[i + 1][j] == '?') f(i, j, i + 1, j);
      if (c[i][j] == '?' && c[i + 1][j] == '2') f(i + 1, j, i, j);
    }
    if (j < w - 1) {
      if (c[i][j] == '2' && c[i][j + 1] == '2') {
        f(i, j, i, j + 1);
        f(i, j + 1, i, j);
      }
      if (c[i][j] == '2' && c[i][j + 1] == '?') f(i, j, i, j + 1);
      if (c[i][j] == '?' && c[i][j + 1] == '2') f(i, j + 1, i, j);
    }
  }

  ll fl = mf.flow(s, t);
  // debug(fl, cnt2);
  if (fl == cnt2) OUT("Yes"); else OUT("No");
}
