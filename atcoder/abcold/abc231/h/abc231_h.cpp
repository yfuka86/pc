...
void solve() {
  ...
  ll sv = h + w, tv = sv + 1, mv = tv + 1;
  mcf_graph<int, ll> G(mv + 1);

  rep(i, h) G.add_edge(sv, i, 1, 0);
  rep(i, w) G.add_edge(h + i, tv, 1, 0);
  rep(i, h) G.add_edge(mv, i, INF, 0);
  rep(i, w) G.add_edge(h + i, mv, INF, 0);

  rep(i, n) {
    ll a, b, c; cin >> a >> b >> c; a--; b--;
    G.add_edge(a, b + h, 1, c);
  }
  G.add_edge(sv, mv, w, 0);
  G.add_edge(mv, tv, h, 0);

  ll ans = G.flow(sv, tv).se;
  cout << ans << "\n";
}
