...
void solve() {
  ...
  bool bipartite = true;
  rep(i, n) if (uf.same(i, i + n)) { bipartite = false; break; }
  if (!bipartite) OUTRET(0);

  vl vis(n), c(n);
  function<void(ll)> color = [&](ll v) {
    vis[v] = true;
    fore(to, G[v]) {
      if (!vis[to]) {
        c[to] = !c[v];
  ...
  rep(i, n) {
    ll sz = ufr.size(i);
    ll t;
    if (!c[i]) t = sz - cnt0[ufr.root(i)]; else t = cnt0[ufr.root(i)];
    t += n - sz - deg[i];
    ans += t;
  }
  OUT(ans / 2);
