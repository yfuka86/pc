...
S op(S l, S r) { return {l.x + r.x, l.sum + r.sum}; }
S e() { return {0, 0}; }
...
void solve() {
  ...
  rep(i, ca.size()) ca[i] = -ca[i];
  auto [mp, revmp] = compmap(ca);
  segtree<S, op, e> seg(ca.size());

  auto add = [&](ll key, ll x) {
    ll idx = mp[-a[key]];
    auto s = seg.get(idx);
    seg.set(idx, {s.x + x, (s.x + x) * a[key]});
  };
  ...
  rep(i, q) {
    ...
    if (t == 1) {
      ...
      if (seg.all_prod().x < x) OUT(-1);
      else {
        ll r = seg.max_right<function<bool(S)>>(0, [&](S s) { return s.x <= x; });
        auto s1 = seg.prod(0, r);
        ll ans = s1.sum;

        if (r < ca.size()) {
          auto s2 = seg.get(r);
          ans += (x - s1.x) * (s2.sum / s2.x);
        }
