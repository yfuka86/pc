...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    fore(to, G[v]) {
      ...
      auto t00 = convolution(dp0[v], dp0[to]);
      auto t01 = convolution(dp0[v], dp1[to]);
      auto t10 = convolution(dp1[v], dp0[to]);
      auto t11 = convolution(dp1[v], dp1[to]);

      while (t00.size() <= t01.size() + 1) t00.pb(0);
      rep(i, t01.size()) { t00[i + 1] += t01[i]; }
      rep(i, t11.size()) { t10[i] += t11[i]; }
      dp0[v] = t00; dp1[v] = t10;
    }
  };
  dfs(0, -1);
  ...
  vmi ans(n);
  rep(i, 1, n + 1) {
    if (i < dp0[0].size()) ans[i - 1] += dp0[0][i];
    if (i <= dp1[0].size()) ans[i - 1] += dp1[0][i - 1];
  }
  OUTARRAY(ans, 0, "\n");
}
