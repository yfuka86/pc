...
void solve() {
  ...
  reverse(all(s2)); reverse(all(t2));

  RollingHash rh;

  vb a1(t.size() + 1), a2(t.size() + 1);
  ...
  auto sh1 = rh.build(s1), th1 = rh.build(t1);
  a1[0] = true;
  rep(i, 1, t.size() + 1) {
    a1[i] = rh.query(sh1, 0, i) == rh.query(th1, 0, i);
  }
  ...
  auto sh2 = rh.build(s2), th2 = rh.build(t2);
  a2[0] = true;
  rep(i, 1, t.size() + 1) {
    a2[i] = rh.query(sh2, 0, i) == rh.query(th2, 0, i);
  }

  rep(i, t.size() + 1) {
    if (a1[i] && a2[t.size() - i]) OUT("Yes"); else OUT("No");
  }
}
