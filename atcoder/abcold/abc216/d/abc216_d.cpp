...
void solve() {
  ...
  rep(i, m) {
    ll k; cin >> k;
    vlin(t, k, 0);
    rep(j, k) a[i].push({t[j], i});
  }

  ll cnt = 0;
  map<ll, vl> s;
  function<void(LP)> f = [&](LP p) {
    auto [c, id] = p;
    debug(s, c, id);
    ...
    if (s[c].size()) {
      ...
      ll id2 = s[c].back();
      s[c].pop_back();
      if (a[id].size()) { auto t = a[id].front(); a[id].pop(); f(t); }
      if (a[id2].size()){ auto t = a[id2].front(); a[id2].pop(); f(t); }
    } else {
      s[c].pb(id);
    }
  };
  ...
  rep(i, m) {
    if (a[i].size()) { auto t = a[i].front(); a[i].pop(); f(t); }
  }
  if (cnt == n) cout << "Yes" << "\n"; else cout << "No" << "\n";
}
