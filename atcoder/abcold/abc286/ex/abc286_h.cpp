...
void solve() {
  ...
  Points p(n + 2);
  rep(i, n + 2) cin >> p[i];
  Points pp; pp.pb(p[n]); pp.pb(p[n + 1]);

  sort(all(p));
  Points ch = ConvexHull(p);

  bool usech = find(all(ch), pp[0]) != ch.end() && find(all(ch), pp[1]) != ch.end();
  if (!usech) OUTRET(distance(pp[0], pp[1]));
  ...
  rep(_, 2) {
    ...
    ld dist = 0;
    rep(i, ch.size() * 2) {
      if (ch[i % ch.size()] == pp[0]) st = true;
      if (st && ch[i % ch.size()] == pp[1]) break;
      if (st) {
        dist += distance(ch[i % ch.size()], ch[(i + 1) % ch.size()]);
      }
    }
    chmin(ans, dist);
    swap(pp[0], pp[1]);
