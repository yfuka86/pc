...
void solve() {
  ...
  rep(k, n) rep(i, n) rep(j, n) {
    if (cost[i][k].fi == LINF || cost[k][j].fi == LINF) continue;
    auto [d1, s1] = cost[i][k];
    auto [d2, s2] = cost[k][j];
    ll stot = s1 + s2 - a[k];

    if (cost[i][j].fi > d1 + d2) {
      cost[i][j] = {d1 + d2, stot};
    } else if (cost[i][j].fi == d1 + d2) {
      cost[i][j] = {d1 + d2, max(cost[i][j].se, stot)};
    }
  }
