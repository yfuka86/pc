...
void solve() {
  ...
  function<ll(ll, ll, ll)> dfs2 = [&](ll v1, ll v2, ll b) {
    if (b < 0) return 0ll;
    ll mi = LINF;
    rep(p, 2) {
      if (bt.nxt[v1][p] != -1 && bt.nxt[v2][p] != -1) chmin(mi, (b ? dfs2(bt.nxt[v1][p], bt.nxt[v2][p], b - 1) : 0));
    }
    if (mi != LINF) return mi;
    rep(p, 2) {
      if (bt.nxt[v1][p] != -1 && bt.nxt[v2][1-p] != -1) chmin(mi, (1 << b) + (b ? dfs2(bt.nxt[v1][p], bt.nxt[v2][1-p], b - 1) : 0));
    }
    return mi;
  };

  function<ll(ll, ll)> dfs = [&](ll v, ll b) {
    if (b < 0) return 0ll;
    if (bt.nxt[v][1] != -1 && sz[bt.nxt[v][1]] & 1) {
      return (1 << b) + dfs2(bt.nxt[v][0], bt.nxt[v][1], b - 1);
    } else {
      ll ma = 0;
      rep(p, 2) if (bt.nxt[v][p] != -1) chmax(ma, dfs(bt.nxt[v][p], b - 1));
      return ma;
    }
  };
