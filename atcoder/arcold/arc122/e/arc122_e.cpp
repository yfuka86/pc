...
bool naive(ll n, vl a) {
  do {
    ...
    bool valid = true;
    rep(i, n) {
      if (lcm(lc, a[i]) <= lc) { valid = false; break; }
      lc = lcm(lc, a[i]);
    }
    if (valid) { debug(a); return true; }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 6;
    vl a = rg.vecl(n, 2, 10); sort(all(a));
...
void solve() {
  ...
  vector<map<ll, ll>> pf(n); rep(i, n) pf[i] = factor_count(a[i]);
  set<ll> fs; rep(i, n) fore(f, _, pf[i]) fs.insert(f);
  vl ps; fore(f, fs) ps.pb(f);
  ll m = ps.size();
  ...
  rep(i, n) {
    bool valid = false;
    rep(j, m) {
      if (ma[j] < pfs[i][j]) { valid = true; ma[j] = pfs[i][j]; }
    }
    if (!valid) Yes(false);
  }
