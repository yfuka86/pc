...
void solve() {
  ...
  vv(mint, dp, n * 2 + 1, free + 1); // dp[iまで][?に対して使った+の電荷数]
  vv(mint, dpsum, n * 2 + 1, free + 1);
  dp[0][0] = 1;
  ...
  rep(i, n * 2) {
    rep(j, free + 1) {
      ll pos = ps[i] + j;
      ll neg = i - pos;
      mint cost = abs(pos - neg);

      if (t[i] == '?') {
        if (j < free) {
          dp[i + 1][j + 1] += dp[i][j];
          dpsum[i + 1][j + 1] += dpsum[i][j] + dp[i][j] * cost;
        }
      }
      dp[i + 1][j] += dp[i][j];
      dpsum[i + 1][j] += dpsum[i][j] + dp[i][j] * cost;
    }
  }
