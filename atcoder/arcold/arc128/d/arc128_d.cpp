...
ll exp(vl &_p) {
  ...
  set<vlp> s; s.insert(p);
  queue<vlp> que; que.push(p);
  while (!que.empty()) {
    vlp t = que.front(); que.pop();
    rep(i, t.size()) {
      bool valid = true;
      if (i == 0 || i == t.size() - 1)  valid = false;
      if (i > 0 && t[i - 1].fi == t[i].fi) valid = false;
      if (i < t.size() - 1 && t[i + 1].fi == t[i].fi) valid = false;
      if (valid) {
        vlp tt = t;
        tt.erase(tt.begin() + i);
        if (s.find(tt) == s.end()) {
...
mint calc(vl &a) {
  ...
  rep(r, n) {
    s[a[r]]++;
    while (s.size() > 3 || (s.size() == 3 && s[a[l]] > 1)) {
      s[a[l]]--;
  ...
  incr_csum<mint> dp(n); dp.set(0, 1);
  rep(i, 1, n) {
    if (lasti[i] < i - 2) {
      dp.set(i, dp.sum(0, max(0, lasti[i] + 1)) + dp.sum(i - 2, i));
    } else {
      dp.set(i, dp.sum(0, i));
    }
  }
