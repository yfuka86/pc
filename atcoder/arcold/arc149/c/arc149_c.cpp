...
void solve() {
  ...
  vv(ll, ans, n, n, -1);
  vvl seed = {{1,3,5}, {8,6,9}, {4,2,7}};
  rep(i, 3) rep(j, 3) ans[i][j] = seed[i][j];


  rep(i, 4, n + 1) {
    set<ll> odd, even;
    rep(j, (i - 1) * (i - 1) + 1, i * i + 1) {
      if (j & 1) odd.insert(j); else even.insert(j);
    }

    if (i & 1) {
      rep(k, i - 1) {
        ans[i - 1][k] = *even.begin();
        even.erase(even.begin());
      }
      ll e1 = ans[i - 1][i - 2], e2 = ans[i - 2][i - 2];
      for(auto it = odd.begin(); it != odd.end(); ) {
        if (!ps.is_prime[*it + e1]) { ans[i - 1][i - 1] = *it; it = odd.erase(it); break; } else ++it;
      }
      for(auto it = odd.begin(); it != odd.end(); ) {
        if (!ps.is_prime[*it + e2]) { ans[i - 2][i - 1] = *it; it = odd.erase(it); break; } else ++it;
      }
      rep(k, i - 2) {
        ans[k][i - 1] = *odd.begin();
        odd.erase(odd.begin());
      ...
      ll o1 = ans[i - 2][i - 1], o2 = ans[i - 2][i - 2];
      for(auto it = even.begin(); it != even.end(); ) {
        if (!ps.is_prime[*it + o1]) { ans[i - 1][i - 1] = *it; it = even.erase(it); break; } else ++it;
      }
      for(auto it = even.begin(); it != even.end(); ) {
        if (!ps.is_prime[*it + o2]) { ans[i - 1][i - 2] = *it; it = even.erase(it); break; } else ++it;
      }
      rep(k, i - 2) {
        ans[i - 1][k] = *even.begin();
        even.erase(even.begin());
  ...
  // bool valid = true;
  // rep(i, n) rep(j, n) {
  //   if (i > 0 && ps.is_prime[ans[i - 1][j] + ans[i][j]]) {valid = false; break; }
  //   if (j > 0 && ps.is_prime[ans[i][j - 1] + ans[i][j]]) {valid = false; break; }
  // }
  // assert(valid);
