...
void solve() {
  ...
  // 係数
  vmi a1(1000010); a1[0] = 1;
  rep(i, 1, 1000001) {
    a1[i] = a1[i-1] * mint(fib[i * 2]) * fib[i * 2];
  }
  // rep(i, 100) debug(a1[i]);

  // 正方形から行を増やす
  vmi a2(2000010); a2[0] = 0; a2[1] = 1;
  rep(i, 2, 2000010) {
    a2[i] = a2[i - 1] * 3 - a2[i - 2];
  }
  // rep(i, 100) debug(a2[i]);

  LL(t);
  while(t--) {
    LL(h,w);
    OUT(a1[min(h, w)] * a2[min(h, w) + 1].pow(max(h, w) - min(h, w)));
  }


  // vv(mint, dp, 2000010, 2);
  // dp[0][0] = 1;
  // rep(i, 2000000) {
  //   dp[i + 1][1] = dp[i][0];
