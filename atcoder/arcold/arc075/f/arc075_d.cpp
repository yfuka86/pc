...
bool mirrored(vl &a, vl &b) {
  if (a.back() == 0) return false;
  vl mirrored = add(a, b);
  reverse(all(mirrored));
  // if (a == mirrored) debug(a);
  return a == mirrored;
}

bool checkmirrored(ll x, ll d) {
  ll sum = x + d;
...
ll solve(ll d) {
  ...
  vl ds = digits(d);
  ll trailing0 = 0; fore(d, ds) if (d) break; else trailing0++;

  ll n = ds.size();

  vl p10(19); rep(i, 19) p10[i] = POW(10, i);
  ...
  rep(dn, n, n * 2 + 1) { // 桁数
    ...
    function<void(ll, ll)> dfs = [&](ll p, ll carry) {
      if (p < min(trailing0, dn - n) - 1) { // 前後の0の自由度を取り除いておく
        if (p == 0) cur *= 9; else cur *= 10;
        ac += 1 * p10[p];
        ac += 1 * p10[dn - p - 1];
        dfs(p + 1, 0);
        if (p == 0) cur /= 9; else cur /= 10;
      ...
      rep(nx, 10) {
        ...
        if (p == pmir) { // 奇数桁数の場合
          if (sum[p] == nx) {
            if (checkmirrored(ac, d)) { ans+=cur; }
          }
          a[p] = -1; ac -= nx * p10[p];
          continue;
        }

        a[pmir] = sum[p]; ac += sum[p] * p10[pmir];
        ll shouldbenx = sum[p] + (pmir < ds.size() ? ds[pmir] : 0);

        // if (p == 1 && dn == 6) debug(a, carry, shouldbenx);
        if (p + 1 == pmir) {
          if (checkmirrored(ac, d)) { ans+=cur; }
        } else {
          if (carry) shouldbenx -= 10;
          if (shouldbenx == nx) {
            dfs(p + 1, 0);
          } else if (shouldbenx == nx - 1) {
            dfs(p + 1, 1);
          }
        }

        a[p] = -1; ac -= nx * p10[p];
        a[pmir] = -1; ac -= sum[p] * p10[pmir];
      }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll d = rg.l(1, 10000);
    auto so = solve(d); auto na = naive(d);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(d); debug(so); debug(na);
