...
void solve() {
  ...
  rep(i, n - 1) {
    d[i % 3].pb(d[i % 3].back() + s[i + 1] - s[i]);
  }
  // coutmatrix(d);

  vl ans;
  rep(i, 3) {
    ans.pb(max(0ll, 0ll - *min_element(all(d[i]))));
  }
  if (accumulate(all(ans), 0ll) > s[0]) cout << "No" << "\n";
  else {
    ans.back() += s[0] - accumulate(all(ans), 0ll);
    rep2(i, 1, n) {
      ans.pb(s[i] - ans[i] - ans[i + 1]);
    }
    cout << "Yes" << "\n";
    coutarray(ans);
  }
