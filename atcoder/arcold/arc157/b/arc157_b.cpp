...
ll solve(ll n, ll k, string s) {
  ...
  if (k == 0) {
    ll tmp = 0;
    rep(i, n - 1) {
      if (s[i] == 'Y' && s[i + 1] == 'Y') tmp++;
    }
    return tmp;
  } else if (k == n) {
    ll tmp = 0;
    rep(i, n - 1) {
      if (s[i] == 'X' && s[i + 1] == 'X') tmp++;
    }
    return tmp;
  }
  rep(i, n) if (s[i] == 'X') xcount++;
  if (xcount == n) return k - 1;
  if (xcount == 0) return n - k - 1;
...
ll naive(ll n, ll k, string s) {
  ...
  rep(S, 1 << n) {
    ...
    rep(i, n) {
      if (S & 1 << i) {
        if (t[i] == 'X') t[i] = 'Y'; else t[i] = 'X';
      }
    }
    ll sum = 0;
    rep(i, n - 1)  {
      if (t[i] == 'Y' && t[i + 1] == 'Y') sum++;
    }
    chmax(ret, sum);
  }
  return ret;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 5, k = rg.l(0, n + 1);
    string s = rg.str(n, "XY");
    auto so = solve(n, k, s); auto na = naive(n, k, s);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, s); debug(so); debug(na);
