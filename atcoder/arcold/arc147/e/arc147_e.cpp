...
template<typename T> using min_p_queue = priority_queue<T, vector<T>, greater<T>>;
...
void solve() {
  ...
  if (rem.size() == 0) coutret(n);

  ll cur = 0;
  priority_queue<ll> cand;
  while (!rem.empty()) {
    auto [c, t] = rem.top(); rem.pop();
    cur += t;
    while (!que.empty() && c >= que.top().fi) {
      cand.push(que.top().se); que.pop();
    }
    if (cur > 0) {
      if (cand.size() == 0 || cand.top() <= c) coutret(-1);
      rem.push({cand.top(), 1}); cand.pop();
      cur--;
    }
  }

  cout << que.size() + cand.size() << "\n";
}
