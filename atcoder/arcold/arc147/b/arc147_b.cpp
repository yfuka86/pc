...
void solve() {
  ...
  // vl p = RandGen().vecperm(n);
  // vl p(n); iota(rall(p), 0);
  // debug(p);

  {
    ll oddcnt = 0, odd1 = 0, odd2 = 0;
    rep(i, n) {
      if (p[i] - i & 1) {
        oddcnt++;
        if (i & 1) odd1++; else odd2++;
      }
    }
    // debug(oddcnt, odd1, odd2);
  }

  vlp ans;
  rep(i, n) {
    ll id;
    rep(j, n) if (p[j] == i) id = j;
    ...
    rep(j, n) if (p[j] == i) id = j;
    while (id > i) {
      if (id - i & 1 && (id - i == 1 || (id - 1 - p[id - 1] & 1))) {
        ans.pb({0, id - 1});
        swap(p[id - 1], p[id]);
        id -= 1;
      } else {
