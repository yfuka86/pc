...
void solve() {
  ...
  fore(v, a) {
    sort(all(v));
    if (v.size() & 1) odd.pb(v); else even.pb(v);
  }
  // debug(odd, even);
  if (odd.size() == 0) OUTRET(0);

  ll ans = LINF;
  rep(i, odd[0].size()) {
    vlp t;
    auto it = lower_bound(all(odd[1]), odd[0][i]);
    if (it != odd[1].begin()) chmin(ans, abs(*prev(it) - odd[0][i]));
    if (it != odd[1].end()) chmin(ans, abs(*it - odd[0][i]));
  }

  vvlp t(2);
  rep(j, 2) rep(i, odd[j].size()) {
    auto it = lower_bound(all(even[0]), odd[j][i]);
    if (it != even[0].begin()) t[j].pb({abs(*prev(it) - odd[j][i]), prev(it) - even[0].begin()});
    if (it != even[0].end()) t[j].pb({abs(*it - odd[j][i]), it - even[0].begin()});
  }
  sort(all(t[0]));
  sort(all(t[1]));

  rep(i, min(t[0].size(), 2)) rep(j, min(t[1].size(), 2)) {
    if (t[0][i].se == t[1][j].se) continue;
    chmin(ans, t[0][i].fi + t[1][j].fi);
  }
