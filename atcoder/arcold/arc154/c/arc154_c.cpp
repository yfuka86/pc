...
bool solve(ll n, vl a, vl b) {
  if (a == b) return true;
  auto rlb = RLE(b);
  if (rlb.size() == n && b[0] != b[n - 1]) return false;
  auto rla = RLE(a);
  ...
  rep(i, n) {
    ...
    ll cur = 0;
    fore(v, _, rlb) {
      while (cur < rla.size() && v != rla[cur].fi) {
        cur++;
      }
      if (cur >= rla.size() || v != rla[cur].fi) { valid = false; break; }
      cur++;
    }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 5;
    vl a = rg.vecl(n, 1, n + 1);
    vl b = rg.vecl(n, 1, n + 1);
