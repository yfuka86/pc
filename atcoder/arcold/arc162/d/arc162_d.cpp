...
std::vector<std::vector<int>> decodePrufer(const vl& prufer) {
    int n = prufer.size() + 2;
    ...
    for (int v : prufer) {
        int leaf = leaves.top();
        leaves.pop();
        tree[leaf].push_back(v);
        tree[v].push_back(leaf);
        if (--degree[v] == 1) {
            leaves.push(v);
        }
    }
    ...
    leaves.pop();
    int last = leaves.top();
    tree[leaf].push_back(last);
    tree[last].push_back(leaf);
...
void solve() {
  ...
  vl p;
  rep(i, n) {
    rep(_, d[i] - (i == 0)) p.pb(i);
  }

  ll cnt = 0, pro = 0;
  do {
    vvi g = decodePrufer(p);
