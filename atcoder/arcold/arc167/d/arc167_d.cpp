...
void solve() {
  ...
  set<ll> mergedidx;
  fore(i, cycs[0]) { seg.set(i, p[i]); mergedidx.insert(i); }

  rep(i, 1, cycs.size()) {
    ll mi = cycs[i][0];
    ll nx = seg.max_right(0, [&](ll ma) {
      return ma < mi;
    });
    if (nx > *prev(mergedidx.end())) { // 大きいのがない（交差してない)
      ll idx = *prev(mergedidx.end());
      ll tidx = pinv[cycs[i][0]];
      seg.set(idx, p[tidx]);
      swap(p[idx], p[tidx]);
    } else {
      ll idx = nx;
    ...
    fore(i, cycs[i]) { seg.set(i, p[i]); mergedidx.insert(i); }
