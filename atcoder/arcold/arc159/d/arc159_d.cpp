...
ll solve(ll n, vlp rng) {
  vl cx; cx.pb(0); cx.pb(LINF);
  rep(i, n) {
    cx.pb(rng[i].fi); cx.pb(rng[i].se);
  }

  auto [mp, rmp] = compmap(cx);
  // debug(mp, rmp);

  vector<S> v;
  v.pb({0, 0});
  rep(i, 1, 500000) v.pb({i, -LINF});
  segtree<S, op, e> seg1(v), seg2(v);
  // 普通のseg1とdp-iのseg2
  rep(i, n) {
    auto [l, r] = rng[i];
    S s1 = seg1.prod(0, mp[l]);
    S s2 = seg2.prod(mp[l], mp[r]);
    ll nx = maxe(s2.x + r, s1.x + r - l + 1, seg1.get(mp[r]).x);
    // debug(s1.x, s1.idx, s2.x, s2.idx, nx);
    seg1.set(mp[r], {mp[r], nx});
    seg2.set(mp[r], {mp[r], nx - r});
  }
  return seg1.all_prod().x;
}

ll naive(ll n, vlp p) {
  vl a;
  rep(i, n) {
    rep(j, p[i].fi, p[i].se + 1) {
      a.pb(j);
    }
  }
  return get_lis(a);
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp p;
    rep(i, n) {
      ll l = rg.l(1, 10), r = rg.l(l, 10);
      p.pb({l, r});
    }
    auto so = solve(n, p); auto na = naive(n, p);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, p); debug(so); debug(na);
