...
void solve() {
  ...
  set<ll> s0, s1, st0, st1;

  rep(i, n) {
    if (s[i][0] == '1') s1.insert(i); else s0.insert(i);
  }

  rep(i, 1, m) {
    fore(e, s0) {
      if (s[e][i] == '1') st1.insert(e); else st0.insert(e);
    }
    fore(e, s1) {
      if (s[e][i] == '0') st1.insert(e); else st0.insert(e);
    }
    s0 = st0; s1 = st1; st0.clear(); st1.clear();
  }
