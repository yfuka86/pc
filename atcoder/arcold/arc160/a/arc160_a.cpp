...
vl solve(ll n, vl a, ll k) {
  vl rev(n); rep(i, n) rev[a[i]] = i;

  BIT<ll> bt(n); rep(i, n) bt.add(i, 1);
  vl ans;

  ll rem = n * (n + 1) / 2;
  rep(i, n) {
    ll small = bt.sum(0, a[i]);
    ll big = n - i - 1 - small;
    rem -= n - i - 1;
    if (k <= small) {
      rep(j, n) {
        if (bt.sum(0, j) == k) {
          reverse(a.begin() + i, a.begin() + rev[j - 1] + 1);
          return a;
        }
      }
    } else if (small + rem < k) {
      // debug(a[i]);
      rep(j, a[i], n) {
        // debug(j, bt.sum(a[i] + 1, j + 1));
        if (bt.sum(a[i] + 1, j + 1) == k - small - rem) {
          reverse(a.begin() + i, a.begin() + rev[j] + 1);
          return a;
        }
      }
    }
    ...
    k -= small;
    bt.add(a[i], -1);
    // debug(small, big, k, rem);
  }
  return a;
}

vl naive(ll n, vl p, ll k) {
  vvl al;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 9;
    vl p = rg.vecperm(n);
    ll k = rg.l(1, n * (n + 1) / 2 + 1);

    auto so = solve(n, p, k); auto na = naive(n, p, k);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, p, k); debug(so); debug(na);
