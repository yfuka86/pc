...
void solve() {
  LL(n);
  // if (n < 10000) debug(quotient_range(n));

  auto qrng = quotient_range(n);

  vl sum;
  rep(i, qrng.size()) {
    auto [rng, j] = qrng[i];
    sum.pb((rng.se - rng.fi + 1) * j);
  }
  vl cs = csum(sum);
  // debug(sum);
  ...
  mint ans = 0;
  rep(idx, qrng.size()) {
    auto [xrng, i] = qrng[idx];
    ll r = binary_search([&](ll mid) { // 必要な最後の区間（inclusive）
      if (qrng.size() == mid) return false;
      return qrng[mid].fi.fi <= i;
    }, 0, qrng.size());
    ...
    if (r >= idx) {
      t += cs[r] - cs[idx];
      // debug(cs[r] - cs[idx]);
      // debug((i - qrng[r].fi.fi + 1) * qrng[r].se);
      t += (i - qrng[r].fi.fi + 1) * qrng[r].se;
    }
    ans += (xrng.se - xrng.fi + 1) * t;
    // debug(xrng, ans);
    if (0 && n < 10000) {
      ll t = 0;
      rep(y, 1, i + 1) { // y
