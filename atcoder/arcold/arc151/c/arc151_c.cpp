...
void solve() {
  function<ll(vl, ll)> dfs = [&](vl r, ll d) {
    ll cnt = 0;
    ll valid = false;
    ll res = 0;
    rep(i, r.size() - 1) {
      vb no(2);
      if (r[i] == -1) {
        if (i > 0 && r[i - 1] != -1) no[r[i - 1]] = 1;
        if (r[i + 1] != -1) no[r[i + 1]] = 1;
        rep(j, 2) { if (no[j]) continue;
          valid = true;
          vl t = r;
          t[i] = j;
          if (dfs(t, d + 1) == 0) res = 1;
        }
      }
    }
    // if (d < 6) debug(r, res);
  ...
  LL(n,m);
  if (m == 0) {
    if (n & 1) OUT("Takahashi"); else OUT("Aoki");
    return;
  }

  ll ans = 0;
  VEC2(ll, x, y, m); rep(i, m) x[i]--;
  ...
  if (ans) OUT("Takahashi"); else OUT("Aoki");
}
