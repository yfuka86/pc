...
void solve() {
  ...
  if (n == 1) {
    if (t == "1") OUT(ans * 2); else OUT(ans);
    return;
  }
  if (n == 2) {
    if (t == "11" || t == "10") OUT(ans);
    else if (t == "01") OUT(ans - 1);
    else OUT(0);
    return;
  }
  ...
  string ss = "110";
  rep(len, 1, 4) {
    if (ss.substr(3 - len, len) == t.substr(0, len)) {
      bool valid = true;
      rep(i, len, n) {
        if (t[i] != ss[(i - len) % 3]) valid = false;
      }
      ll length = ceil(n - len, 3);
      if (valid) OUTRET(ans - length);
    }
  }
  OUT(0);
}
