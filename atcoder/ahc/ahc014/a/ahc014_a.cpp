...
struct Cand {
  ll x1, y1, x2, y2, nx, ny;

  ll edgesize() const {
    return abs(x2 - x1) * 2 + abs(y2 - y1) * 2;
  }
  ll score() const {
    ll dx = (nx - center), dy = (ny - center);
    return dx * dx + dy * dy + 1;
  }
  bool valid() {
    bool ret = true;
    rep(i, x1 + 1, x2) if (g[i][y1] || g[i][y2]) { ret = false; break; }
    rep(j, y1 + 1, y2) if (g[x1][j] || g[x2][j]) { ret = false; break; }
    if (ex[x1].lower_bound(y1) != ex[x1].lower_bound(y2)) ret = false;
    if (ex[x2].lower_bound(y1) != ex[x2].lower_bound(y2)) ret = false;
    if (ey[y1].lower_bound(x1) != ey[y1].lower_bound(x2)) ret = false;
    if (ey[y2].lower_bound(x1) != ey[y2].lower_bound(x2)) ret = false;
    return ret;
  }
  void add() {
    assert(valid());
  ...
  bool operator<(const struct Cand& other) const {
    return this->score() * other.edgesize() < other.score() * this->edgesize();
  }
};
...
void solve() {
  cin >> n >> m;
  center = (n - 1) / 2;
  g.assign(n, vl(n)); ex.resize(n); ey.resize(n);
  ...
  // 行ごとに持って交互に割り当てためす？
  rep(i, n) {
    vl t; rep(j, n) if (g[i][j]) t.pb(j);
    rep(j, t.size() - 1) {
      ll a = t[j], b = t[j + 1];
      rep_r(k, i) {
        if (g[k][a] && g[k][b]) break;
        if (g[k][a]) { cand.pb({k, a, i, b, k, b}); break; }
        if (g[k][b]) { cand.pb({k, a, i, b, k, a}); break; }
      }
      rep(k, i + 1, n) {
        if (g[k][a] && g[k][b]) break;
        if (g[k][a]) { cand.pb({i, a, k, b, k, b}); break; }
        if (g[k][b]) { cand.pb({i, a, k, b, k, a}); break; }
      }
    }
  }
  pq<Cand> que; fore(c, cand) que.push(c);

  while(!que.empty()) {
    Cand c = que.top(); que.pop();
    if (c.valid()) c.add();
  }

  OUT(ans.size());
  rep(i, ans.size()) OUTARRAY(ans[i], 1);
