...
struct Cand {
  ...
  vl sortattr() {
    ...
    // debug(esc);
    // return {-a * (10000 * esc / ssc)};
    // if (que.size() > candcount) return {-a / (esz * esz)};
    // else return {-a / (esz * esz * esz * esz)};
    ll cs = (ld)500 / (cnt + 1) * (inv + 1);
    return {esz * cs, -sc};
  }
  bool valid() {
    if (t == 0) {
...
ll solve(bool toans = true) {
  ...
  rep(i, n) rep(j, n) {
    if (g[i][j]) {
      fore(c, enum_cand(i, j)) v.pb(c);
      auto [x, y] = tr(i, j);
      fore(c, enum_dcand(x, y)) v.pb(c);
    }
  }

  candcount = v.size();
  fore(c, v) cpush(c);
  fore(p, que) p.fi = p.se.sortattr();
    ...
    if (c.valid()) {
      ...
      for (auto it = que.begin(); it != que.end(); ) {
        ...
        if (!c.valid()) it = que.erase(it);
          ...
          if (cnt <= 1080000 / que.size()) it->fi = c.sortattr(); // スコア更新入れるとさすがにTLEする
          it++;
        }
      }
