...
struct Cand {
  ...
  ll dscore() {
    ...
    fore(c, enum_cand(p.fi, p.se)) {
      if (cands.find(c.attr()) == cands.end()) cnt++;
    }
  ...
  bool operator<(const struct Cand& other) const {
    // return this->score() * ds * other.edgesize() < other.score() * ds * this->edgesize();
    // return this->score() * other.edgesize() < other.score() * this->edgesize();
    // return ds * other.edgesize() < other.ds * this->edgesize();
    // return 1 * other.edgesize() < 1 * this->edgesize();
    return mp(-this->edgesize(), ds) < mp(-other.edgesize(), other.ds);
  }
};
...
ll inv_count() {
  ll ret = 0;
  fore(v, cands) {
    if (!Cand(v[0],v[1],v[2],v[3],v[4],v[5],v[6]).valid()) ret++;
  }
  return ret;
}
