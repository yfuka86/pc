...
ll H, W, C;
ll sol(vvl& A) {
  vvl cummin(H, vl(W, LINF));
  ll ans = LINF;

  rep(i, H) rep(j, W) {
    ll c = A[i][j] + C * (i + j);
    if (i > 0) chmin(ans, c + cummin[i - 1][j]);
    if (j > 0) chmin(ans, c + cummin[i][j - 1]);


    cummin[i][j] = A[i][j] - C * (i + j);
    if (i > 0) chmin(cummin[i][j], cummin[i - 1][j]);
    if (j > 0) chmin(cummin[i][j], cummin[i][j - 1]);
  }
  return ans;
}
