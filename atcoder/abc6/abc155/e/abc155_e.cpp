...
void solve() {
  ...
  vl dp(n.size(), -1), cdp(n.size(), -1);

  function<ll(ll, bool)> dfs = [&](ll i, bool carry) {
    if (i == 0) return (ll)carry;
    if (carry) {
      if (cdp[i] != -1) return cdp[i];
      else {
        ll num = (n[i] - '0') + 1;
        return cdp[i] = min(dfs(i - 1, true) + 10 - num, dfs(i - 1, false) + num);
      }
    } else {
      if (dp[i] != -1) return dp[i];
      else {
        ll num = n[i] - '0';
        return dp[i] = min(dfs(i - 1, true) + 10 - num, dfs(i - 1, false) + num);
      }
    }
  };

  cout << dfs(n.size() - 1, false) << "\n";
  // debug(dp);
}
