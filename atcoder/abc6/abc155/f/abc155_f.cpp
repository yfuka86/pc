...
void solve() {
  LL(n, m);
  VEC2(ll, a, b, n);
  vlp ev; ev.pb({0, 0}); ev.pb({LINF, 0});
  rep(i, n) ev.pb({a[i], b[i]});
  sort(all(ev));

  vl bomb(n + 1);
  rep(i, n + 1) bomb[i] = ev[i].se ^ ev[i + 1].se;
  // 差を計算
  ...
  rep(i, m) {
    LL(l, r);
    ll a = lower_bound(all(ev), mp(l, -1ll)) - ev.begin() - 1;
    ll b = upper_bound(all(ev), mp(r, 1ll)) - ev.begin() - 1;
    if (a == b) continue;
    G.add_edge(a, b);
    e[{b, a}] = e[{a, b}] = i;
  }
