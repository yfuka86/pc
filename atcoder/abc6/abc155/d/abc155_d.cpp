...
ll solve(ll n, ll k, vl a) {
  ...
  ll ncnt = p * ns;
  ll zcnt = zero * (n - zero) + zero * (zero - 1) / 2;
  ll pcnt = p * (p - 1) / 2 + ns * (ns - 1) / 2;

  if (k <= ncnt) { // 負
    rep(i, neg.size()) neg[i] = abs(neg[i]);
    sort(all(neg)); sort(all(pos));
    ll ans = binary_search([&](ll mid) {
      ll sum = 0;
      rep(i, neg.size()) {
        sum += pos.end() - upper_bound(all(pos), mid / neg[i]);
      }
      return sum < k;
    }, 1e18, 0);
    return -ans;
    ...
    ll ans = binary_search([&](ll mid) {
      ll sum = 0;
      rep(i, neg.size()) {
        sum += max(0, upper_bound(all(neg), mid / neg[i]) - neg.begin() - i - 1);
      }
      rep(i, pos.size()) {
        sum += max(0, upper_bound(all(pos), mid / pos[i]) - pos.begin() - i - 1);
      }
      // debug(mid, sum);
      return sum < kd;
    }, 0, 1e18);
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 10, k = rg.l(1, n * (n - 1) / 2);
    vl a = rg.vecl(n, -100, 100);
