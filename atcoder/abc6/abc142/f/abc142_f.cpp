...
void solve() {
  ...
  function<void(ll)> bfs = [&](ll v) {
    ...
    while (!que.empty()) {
      auto [v, from] = que.front(); que.pop();
      if (from == -1) used[v] = {v};
      else { used[v] = used[from]; used[v].pb(v); }

      for (auto &to: G[v]) {
        vl t; bool start = false;
        rep(i, used[v].size()) {
          if (start || used[v][i] == to) {
            t.pb(used[v][i]); start = true;
          }
        }
        if (t.size() > 0 && chmin<ll>(len, t.size())) ans = t;

        if (used[to].size() == 0) que.push({to, v});
      }
    }
  };

  rep(i, n) {
    if (used[i].size() == 0) bfs(i);
  }
