...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    mint t = 1; ll cur = sz[v] - 1;
    fore(to, G[v]) {
      if (to == p) continue;
      dfs(to, v);
      t *= comb(cur, sz[to]) * dp[to];
      cur -= sz[to];
    }
    dp[v] = t;
  };
  ...
  vmi dp2(n);
  function<void(ll, ll, mint)> dfs2 = [&](ll v, ll p, mint d) {
    ll cur = 0; dp2[v] = 1;
    fore(to, G[v]) {
      if (to == p) continue;
      dp2[v] *= dp[to] * comb(cur + sz[to], sz[to]);
      cur += sz[to];
    }
    dp2[v] *= d * comb(n - 1, n - sz[v]);

    fore(to, G[v]) {
      if (to == p) continue;
      dfs2(to, v, dp2[v] / dp[to] / comb(n - 1, sz[to]));
    }
  };
  dfs2(0, -1, 1);
  OUTARRAY(dp2, mint(0), "\n");
