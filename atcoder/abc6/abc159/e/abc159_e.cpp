...
void solve() {
  ...
  rep(S, 1 << h - 1) {
    bool svalid = true;
    ll cur = 0; vl mp(h);
    rep(i, h) { mp[i] = cur; if (S & 1 << i) cur++; }

    ll p = __builtin_popcount(S) + 1;
    vl sum(p);
    ll cut = 0;
    rep(j, w) {
      vl tmp(p);
      ...
      bool valid = true;
      rep(i, p) if (tmp[i] > k) svalid = false;
      rep(i, p) if (sum[i] + tmp[i] > k) valid = false;
    ...
    if (svalid) chmin(ans, __builtin_popcount(S) + cut);
  }
  OUT(ans);
}
