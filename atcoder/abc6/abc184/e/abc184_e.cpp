...
void solve() {
  ...
  rep(i, h) rep(j, w) {
    if (grid[i][j] == 'S') s = {i, j};
    if (grid[i][j] == 'G') g = {i, j};
    if (grid[i][j] != '.' && grid[i][j] != '#') mp[grid[i][j]].pb({i, j});
  }
  ...
  while (!que.empty()) {
    ...
    if (mp[grid[x][y]].size()) {
      fore(tox, toy, mp[grid[x][y]]) {
        if (chmin(cost[tox][toy], cost[x][y] + 1)) que.push({tox, toy});
      }
      mp.erase(grid[x][y]);
    }
    ...
    rep(d, 4) {
      ...
      if (incl(di, 0, h) && incl(dj, 0, w) && grid[di][dj] != '#') {
        if (chmin(cost[di][dj], cost[x][y] + 1)) que.push({di, dj});
      }
    }
  }
  if (cost[g.fi][g.se] == LINF) OUT(-1); else OUT(cost[g.fi][g.se]);
}
