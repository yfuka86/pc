...
void solve() {
  ...
  v3(ld, dp, 105, 105, 105, LINF);
  rep(i, 105) rep(j, 105) rep(k, 105) {
    if (i >= 100 || j >= 100 || k >= 100) dp[i][j][k] = 0;
  }

  rep_r(i, 100) rep_r(j, 100) rep_r(k, 100) {
    ld sum = 0;
    sum += (ld)i / (i + j + k) * dp[i + 1][j][k];
    sum += (ld)j / (i + j + k) * dp[i][j + 1][k];
    sum += (ld)k / (i + j + k) * dp[i][j][k + 1];
    dp[i][j][k] = sum + 1;
  }
