...
void solve() {
  ...
  vvvl dp(N + 1, vvl(2, vl(3, -LINF)));
  dp[0][0][0] = 0;
  rep(i, N) {
    rep(k, 3) {
      chmax(dp[i + 1][1][k], dp[i][0][k] + A[i]);
      chmax(dp[i + 1][0][k], dp[i][1][k]);
      if (k < 2) chmax(dp[i + 1][0][k + 1], dp[i][0][k]);
    }
  }

  if (N & 1) cout << max(dp[N][0][1], dp[N][1][2]) << "\n";
  else cout << max(dp[N][0][0], dp[N][1][1]) << "\n";
}
