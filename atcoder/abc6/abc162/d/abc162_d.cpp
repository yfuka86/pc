...
void solve() {
  ...
  rep(i, N) {
    rep2(j, i + 1, N) {
      if (S[i] == S[j]) continue;
      for (char c:"RGB") { if (c == S[i] || c == S[j]) continue;
        ll idx = j * 2 - i;
        char ban = (idx < N) ? S[j * 2 - i] : 'N';
        if (c == 'R') ans += rf[N] - rf[j + 1] - (ban == 'R');
        if (c == 'G') ans += gf[N] - gf[j + 1] - (ban == 'G');
        if (c == 'B') ans += bf[N] - bf[j + 1] - (ban == 'B');
      }
      // cout << i << " " << j << " " << ans << "\n";
    }
  }
