...
void solve() {
  ...
  vl f(R + 1, 0), cnt(R + 1, 0);
  rep2(i, 2, R + 1) {
    f[i] = floor(R, i) - ceil(L, i) + 1;
    cnt[i] = f[i] * (f[i] - 1) / 2;
  }
  ...
  ll gcd = accumulate(cnt.begin() + 2, cnt.end(), 0LL);
  ll mul = accumulate(f.begin() + L, f.end(), 0LL, [](ll a, ll b) { return max(a + b - 1, 0LL); });
  cout << (gcd - mul) * 2 << "\n";
}
