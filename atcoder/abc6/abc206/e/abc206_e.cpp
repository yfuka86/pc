...
void solve() {
  ll L, R; cin >> L >> R;
  vl pri = primes_below(500000);
  vector<set<ll>> primes(7);
  rep(i, pri.size()) primes[0].insert(pri[i]);
  rep(i, 6) for (ll p: primes[i]) {
    for (ll j: primes[0]) {
      if (p % j == 0) continue;
      if (p * j > 500000) break;
      primes[i + 1].insert(p * j);
    }
  }
  ll ans = 0;
  rep(i, 7) {
    for (ll p: primes[i]) {
      ll num = floor(R, p) - ceil(L, p) + 1;
      if (num > 1) {
        ll t = num * (num - 1);
