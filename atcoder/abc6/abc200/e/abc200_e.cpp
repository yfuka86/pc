...
void solve() {
  ...
  vl dp(N * 3 + 1, 0), sum(N * 3 + 2, 0);
  rep2(i, 3, N * 3 + 1) {
    dp[i] = (i - 1) * (i - 2) / 2;
    if (i > N + 2) dp[i] -= dp[i - N] * 3;
    if (i > (2 * N) + 2) dp[i] -= dp[i - (2 * N)] * 6;
    sum[i + 1] = sum[i] + dp[i];
  }

  ll num = lower_bound(all(sum), K) - sum.begin() - 1;

  // num 使うものの K - sum[num] 個目
  ll offset = K - sum[num];
  // cout << num << "\n";
  // cout << offset << "\n";
  rep2(i, max<ll>(1, num - N * 2), min<ll>(N, num - 2) + 1) {
    ll x = num - i;
    ll c = x - 1;
    if (x > N + 1) c -= (x - N - 1) * 2;
    if (c >= offset) {
      ll j = max<ll>(1, x - N) + offset - 1;
      cout << i << " " << j << " " << num - i - j << "\n";
      return;
    } else { offset -= c; }
  }
