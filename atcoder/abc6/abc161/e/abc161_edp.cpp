...
void solve() {
  ...
  vl dp(n + 1, 0);
  vector<set<ll>> dpfrom(n + 1);

  rep(i, n) {
    if (chmax(dp[i + 1], dp[i])) {
      dpfrom[i + 1] = dpfrom[i];
    } else if (dp[i + 1] == dp[i]) {
      auto it = dpfrom[i].begin();
      while (it != dpfrom[i].end() && dpfrom[i + 1].size() < 2) {
        dpfrom[i + 1].insert(*it);
      }
    }
    if (s[i] == 'o') {
      ll to = min(i + c + 1, n);
      if (chmax(dp[to], dp[i] + 1)) {
        dpfrom[to] = {i};
  ...
  if (dp[n] > k) { cout << "\n"; return; }
    ...
    ll cur = n;
    while (cur > 0) {
      if (dpfrom[cur].size() == 0) break;
      if (dpfrom[cur].size() == 1) { ans.insert(*dpfrom[cur].rbegin()); }
      cur = *dpfrom[cur].rbegin();
    }
  }
  for (auto &d: ans) {
    cout << d + 1 << "\n";
