...
void solve() {
  ...
  fore(_,v,dic) sort(all(v));

  ll cur1 = 0, cur2 = 0;
  ll cost1 = 0, cost2 = 0;
  fore(_, v, dic) {
    ll l = v.front(), r = v.back();
    ll cosl = LINF, cosr = LINF;
    chmin(cosl, cost1 + abs(cur1 - r) + r - l);
    chmin(cosl, cost2 + abs(cur2 - r) + r - l);
    chmin(cosr, cost1 + abs(cur1 - l) + r - l);
    chmin(cosr, cost2 + abs(cur2 - l) + r - l);
    cur1 = l; cur2 = r;
    cost1 = cosl; cost2 = cosr;
  }
  OUT(min(cost1 + abs(cur1), cost2 + abs(cur2)));
}
