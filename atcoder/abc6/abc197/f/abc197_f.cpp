...
void solve() {
  ...
  fore(_, v, mp) {
    rep(i, v.size()) rep(j, i) {
      auto [a1, b1] = v[i];
      auto [a2, b2] = v[j];
      G.add_edge(a1 * n + a2, b1 * n + b2);
      G.add_edge(a1 * n + b2, b1 * n + a2);
      G.add_edge(a2 * n + a1, b2 * n + b1);
      G.add_edge(b2 * n + a1, a2 * n + b1);
    }
  }
  // 0 * n + (n - 1)から
  vl cost = dijkstra(G, n - 1);

  ll ans = LINF;
  rep(i, n) {
    chmin(ans, cost[i * n + i] * 2);
  }
  rep(i, goal.size()) {
    chmin(ans, cost[goal[i]] * 2 + 1);
  }
