...
void solve() {
  ...
  vvl abit(30, vl(n)), arbit(30, vl(n, 1)), bbit(30, vl(n));
  rep(j, 30) {
    rep(i, n) {
      if (a[i] & 1 << j) abit[j][i] = 1;
      if (a[i] & 1 << j) arbit[j][i] = 0;
      if (b[i] & 1 << j) bbit[j][i] = 1;
    }
  }
  vvl tabit = abit, tarbit = arbit;
  rep(i, 30) abit[i].insert(abit[i].end(), all(tabit[i]));
  rep(i, 30) arbit[i].insert(arbit[i].end(), all(tarbit[i]));

  RollingHash rh;
  vector<vector<uint64_t>> arh(30), arrh(30), brh(30);
  rep(i, 30) {
    arh[i] = rh.build(abit[i]);
    arrh[i] = rh.build(arbit[i]);
    brh[i] = rh.build(bbit[i]);
  }
  ...
  rep(i, n) {
    ...
    rep(j, 30) {
      if (x & 1 << j) {
        if (rh.query(arrh[j], i, i + n) != rh.query(brh[j], 0, n)) { valid = false; break; }
      } else {
        if (rh.query(arh[j], i, i + n) != rh.query(brh[j], 0, n)) { valid = false; break; }
      }
    }
    if (valid) cout << i << " " << x << "\n";
  }
