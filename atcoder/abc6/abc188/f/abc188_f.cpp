...
void solve() {
  ...
  ll ans = LINF;
  // 2倍する回数を全探索
  for (ll b = 0; x * POW(2,b) < LINF; b++) {
    ll t = x * POW(2,b);
    ll diff = abs(t - y);
    ll cnt = 0;

    map<LP, ll> memo;
    function<ll(ll, ll)> dfs = [&](ll rem, ll d) {
      if (memo.count({rem, d})) return memo[{rem, d}];
      ll u = POW(2, d);
      if (d == b) return memo[{rem, d}] = rem / u;

      if (rem / u & 1) return memo[{rem, d}] = min(dfs(rem + u, d + 1), dfs(rem - u, d + 1)) + 1;
      else return memo[{rem, d}] = dfs(rem, d + 1);
    };

    chmin(ans, dfs(diff, 0) + b);
  }
  OUT(ans);
}
