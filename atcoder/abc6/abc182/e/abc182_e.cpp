...
void solve() {
  ...
  rep(i, h) {
    rep(j, w) {
      if (j < w - 1 && g1[i][j + 1] != -1) chmax(g1[i][j + 1], g1[i][j]);
    }
    rep_r(j, w) {
      if (j > 0 && g2[i][j - 1] != -1) chmax(g2[i][j - 1], g2[i][j]);
    }
  }

  rep(j, w) {
    rep(i, h) {
      if (i < h - 1 && g[i + 1][j] != -1) chmax(g3[i + 1][j], g3[i][j]);
    }
    rep_r(i, h) {
      if (i > 0 && g[i - 1][j] != -1) chmax(g4[i - 1][j], g4[i][j]);
    }
  }

  ll ans = 0;
  rep(i, h) rep(j, w) {
    if (g1[i][j] > 0 || g2[i][j] > 0 || g3[i][j] > 0 || g4[i][j] > 0) ans++;
  }
  cout << ans << "\n";
}
