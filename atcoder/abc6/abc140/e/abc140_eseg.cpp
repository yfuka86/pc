...
void solve() {
  ...
  rep_r(i, n) {
    ll r = seg.max_right(rev[i], [&](S x) { return x.x <= i; });
    ll r2;
    if (r < n - 1 && seg.prod(r + 1, n).x > i) {
      r2 = seg.max_right(r + 1, [&](S x) { return x.x <= i; });
    } else {
      r2 = n;
    }
    ll l = seg.min_left(rev[i], [&](S x) { return x.x <= i; }) - 1;
    ll l2;
    if (l > 0 && seg.prod(0, l).x > i) {
      l2 = seg.min_left(l, [&](S x) { return x.x <= i; }) - 1;
    } else {
      l2 = -1;
    }

    debug(l2, l, rev[i], r, r2);
    seg.set(rev[i], {rev[i], i});

    ans += (r2 - r) * (rev[i] - l) * (i + 1);
    ans += (l - l2) * (r - rev[i]) * (i + 1);
    debug(ans);

    // if (idx.size()) {
    //   ans += (idx[r] - rev[i] + 1) * (idx[l] - idx[l2] + 1);
    //   ans += (rev[i] - idx[l] + 1) * (idx[r2] - idx[r] + 1);
    // }
