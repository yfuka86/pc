...
void solve() {
  ...
  vv(ll, cost, n, 3, LINF); cost[s][0] = 0;
  mpq<LT> que; que.push({0, s, 0});
  while(!que.empty()) {
    auto [c, v, m3] = que.top(); que.pop(); if (cost[v][m3] < c) continue;
    for(auto &to: G[v]) { ll nc = cost[v][m3] + to.cost; if (chmin(cost[to][(m3 + 1) % 3], nc)) que.push({nc, to, (m3 + 1) % 3}); }
  }

  if (cost[t][0] != LINF) OUT(cost[t][0] / 3); else OUT(-1);
}
