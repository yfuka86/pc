...
void solve() {
  ...
  vector<BIT<mint>> dp(k, BIT<mint>(cnt.size()));
  rep(i, cnt.size()) dp[0].add(i, cnt[i]);
  rep(i, k - 1) {
    rep(j, cnt.size()) {
      dp[i + 1].add(j, dp[i].sum(0, cnt.size() - j) * cnt[j]);
    }
  }
  OUT(dp[k - 1].sum(0, cnt.size()));
}
