...
void solve() {
  ...
  vl leftup(n), leftdown(n), rightup(n), rightdown(n);

  {
    BIT<ll> bt(n);
    rep(i, n) {
      leftup[i] = bt.sum(0, y[ord[i]]);
      rightup[i] = bt.sum(y[ord[i]], n);
      bt.add(y[ord[i]], 1);
    }
  }
  {
    BIT<ll> bt(n);
    rep_r(i, n) {
      leftdown[i] = bt.sum(0, y[ord[i]]);
      rightdown[i] = bt.sum(y[ord[i]], n);
      bt.add(y[ord[i]], 1);
    }
  }

  vmi p2(n + 1); p2[0] = 1; rep(i, n) p2[i + 1] = p2[i] * 2;

  mint ans = p2[n - 1] * n; // その点を含む
  rep(i, n) {
    ans += (p2[leftup[i]] - 1) * (p2[rightdown[i]] - 1) * p2[n - 1 - leftup[i] - rightdown[i]];
    ans += (p2[rightup[i]] - 1) * (p2[leftdown[i]] - 1) * p2[n - 1 - rightup[i] - leftdown[i]];
    ans -= (p2[leftup[i]] - 1) * (p2[rightdown[i]] - 1) * (p2[rightup[i]] - 1) * (p2[leftdown[i]] - 1);
  }
  OUT(ans);
}
