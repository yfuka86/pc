...
void solve() {
  ...
  vlp p; rep(i, n) p.emplace_back(cx[i], cy[i]);
  sort(all(p));
...

  {
    ...
    rep(i, n) {
      auto [x, y] = p[i];
      num[i][0] += bt.sum(0, y); num[i][1] += bt.sum(y + 1, n + 1);
      bt.add(y, 1);
    }
  }
  sort(rall(p));
  {
    ...
    rep(i, n) {
      auto [x, y] = p[i];
      num[n - i - 1][2] += bt.sum(0, y); num[n - i - 1][3] += bt.sum(y + 1, n + 1);
      bt.add(y, 1);
    }
  }
  ...
  rep(i, n) {
    ans += mint(2).pow(n - 1);
    mint a = (mint(2).pow(num[i][0]) - 1) * (mint(2).pow(num[i][3]) - 1);
    mint b = (mint(2).pow(num[i][1]) - 1) * (mint(2).pow(num[i][2]) - 1);
    ll an = n - num[i][0] - num[i][3] - 1;
    ll bn = n - num[i][1] - num[i][2] - 1;
    ans += a * mint(2).pow(an) + b * mint(2).pow(bn) - a * b;
  }
  cout << ans << "\n";
