...
void solve() {
  ...
  rep(i, n) {
    if (!vis[i]) { cycle.pb({}); dfs(i); }
  }
  // debug(cycle);

  ll ans = -LINF;
  for (auto &v: cycle) {
    ll cnt = (k / v.size()), rem = k % v.size();

    vl vs = csum(v);
    rep(start, v.size()) {
      rep2(sz, 1, v.size() + 1) {
        ll t = vs[min<ll>(start + sz, v.size())] - vs[start];
        if (start + sz >= v.size()) t += vs[start + sz - v.size()];

        if (sz <= k) chmax(ans, t);
        if (cnt >= 1) {
          if (sz > rem) chmax(ans, (cnt - 1) * vs.back() + t);
          else chmax(ans, cnt * vs.back() + t);
        }
      }
    }
  }
