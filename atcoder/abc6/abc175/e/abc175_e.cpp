...
void solve() {
  ...
  v3l dp(r, vvl(c, vl(4, 0)));
  if (item[0][0]) dp[0][0][1] = item[0][0];

  rep(i, r) rep(j, c) rep(k, 4) {
    if (i < r - 1) chmax(dp[i + 1][j][0], dp[i][j][k]);
    if (j < c - 1) chmax(dp[i][j + 1][k], dp[i][j][k]);
    if (i < r - 1) chmax(dp[i + 1][j][1], dp[i][j][k] + item[i + 1][j]);
    if (j < c - 1 && k < 3) chmax(dp[i][j + 1][k + 1], dp[i][j][k] + item[i][j + 1]);
  }
  // debug(dp);

  cout << *max_element(all(dp[r - 1][c - 1])) << "\n";
}
