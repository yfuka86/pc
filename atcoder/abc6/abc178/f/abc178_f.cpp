...
void solve() {
  ...
  while(1){
    ...
    map<ll, ll> cnt, cnta, cntb;
    rep(i, n) { cnt[a[i]]++; cnt[b[i]]++; cnta[a[i]]++; cntb[b[i]]++; }
    fore(k, v, cnt) if (v > n) OUTRET("No");

    set<LP> rema;
    multiset<ll> remb;
    fore(k, v, cnta) rema.insert({-v, k});
    rep(i, n) remb.insert(b[i]);

    rep(i, n) {
      auto it = rema.lower_bound({-cnta[a[i]], a[i]});
      if (it != rema.end() && it->se == a[i]) {
        --cnta[a[i]];
        rema.erase(it);
        if (cnta[a[i]]) rema.insert({-cnta[a[i]], a[i]});
      }
      ...
      while (rema.size()) {
        ...
        if (it == rema.end()) {
          auto bt = remb.upper_bound(a[i]);
          if (bt != remb.end()) {
            ans.pb(*bt); remb.erase(bt);
          } else {
            ans.pb(*remb.begin()); remb.erase(remb.begin());
          }
          done = true;
          break;
        }
        auto bt = remb.lower_bound(it->se);
        if (bt != remb.end() && *bt == it->se) {
          ans.pb(*bt); remb.erase(bt);
          done = true;
          break;
        } else {
    ...
    bool valid = true;
    rep(i, n) {
      if (a[i] == ans[i]) valid = false;
    }
    if (valid) break;
    rg.shuffle(aid);
    rep(i, n) a[i] = aid[i].fi;
  }
