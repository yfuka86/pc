...
ll naive(ll n, ll m) {
  ...
  vl people(n); iota(all(people), 1);
  map<LP, ll> card;
  ll cur = 0;
  rep(i, n) {
    rep(j, m) {
      auto [a, b] = ans[j]; a--; b--;
      card[{people[(a + cur) % n], people[(b + cur) % n]}]++;
    }
    cur++;
  }

  for (auto [k, v]: card) {
    if (v > 1) {debug(card); return 1;}
  }
  return 0;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll m = 3;
    ll n = rg.l(m * 2 + 1, 10);
    auto so = solve(n, m); auto na = naive(n, m);
...
void solve() {
  ...
  rep(i, m) {
    ...
    while (prohibit.find(n - (r - l)) != prohibit.end() || n - (r - l) == r - l) {
      cur++;
      r--;
    }
    ans[i] = {l, r};
    prohibit.insert(n - (r - l));
    prohibit.insert(r - l);
  }
