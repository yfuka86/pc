...
void solve() {
  ...
  vl diff(n / 2), rdiff(n / 2);
  rep(i, n / 2) diff[i] = abs(h[i * 2 + 1] - h[i * 2]);
  reverse(all(h));
  rep(i, n / 2) rdiff[i] = abs(h[i * 2 + 1] - h[i * 2]);
  reverse(all(h));

  vl ds = csum(diff), rds = csum(rdiff);

  ll ans = LINF;
  rep(i, n / 2 + 1) {
    ll di = LINF;
    auto it = lower_bound(all(w), h[i * 2]);
    if (it != w.end()) chmin(di, abs(*it - h[i * 2]));
    if (it != w.begin()) chmin(di, abs(*(--it) - h[i * 2]));
    chmin(ans, di + ds[i] + rds[n / 2 - i]);
  }
  OUT(ans);
}
