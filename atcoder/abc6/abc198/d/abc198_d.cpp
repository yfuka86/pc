...
void solve() {
  vs s(3); IN(s);
  map<char, ll> mp;
  rep(i, 3) fore(c, s[i]) mp[c]++;
  if (mp.size() > 10) OUTRET("UNSOLVABLE");

  ll cur = 0;
  fore(c, v, mp) v = cur++;

  vl p10(10); p10[0] = 1; rep(i, 9) p10[i + 1] = p10[i] * 10;
  vl p(10); iota(all(p), 0);
  do {
    vl ns(3);
    bool valid = true;
    rep(i, 3) {
      if (p[mp[s[i][0]]] == 0) { valid = false; break; }
      rep(j, s[i].size()) {
        ns[i] += p[mp[s[i][j]]] * p10[s[i].size() - 1 - j];
      }
    }
    if (valid && ns[0] + ns[1] == ns[2]) {
      OUTARRAY(ns, 0ll, "\n");
      return;
    }
