...
i128 POW(__uint128_t x, int n) { assert(n >= 0); i128 res = 1; for(; n; n >>= 1, x *= x) if(n & 1) res *= x; return res; }
...
void solve() {
  ...
  auto check = [&](ll ba) {
    ...
    rep_r(i, x.size()) {
      ll d = x[i] - '0';
      if (d > m / c) return false;
      else sum += d * c;
      if (i != 0) {
        if (c > m / ba) return false;
        c *= ba;
      }
    }
    return sum <= m;
