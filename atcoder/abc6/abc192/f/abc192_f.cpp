...
void solve() {
  ...
  ll ans = x - *max_element(all(a));

  rep(m, 2, 100) { // mod
    vv(ll, dp, m + 1, m, -LINF); // dp[個選んで][mod m] = 最大
    dp[0][0] = 0;

    rep(i, n) rep_r(j, m) rep(mo, m) {
      if (dp[j][mo] == -LINF) continue;
      chmax(dp[j + 1][(mo + a[i]) % m], dp[j][mo] + a[i]);
    }
    if (dp[m][x % m] >= 0) chmin(ans, (x - dp[m][x % m]) / m);
  }
  OUT(ans);
}
