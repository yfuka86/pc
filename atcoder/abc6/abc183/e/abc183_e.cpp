...
void solve() {
  ...
  vv(mint, dps1, h, w + 1);
  vv(mint, dps2, w, h + 1);
  vv(mint, dps3, h + w, (h + w) / 2 + 1);
  vv(mint, dp, h, w); dp[0][0] = 1;
  dps1[0][1] = 1;
  dps2[0][1] = 1;
  dps3[w - 1][1] = 1;
  ...
  rep(i, h) rep(j, w) {
    ...
    dp[i][j] = dps1[i][j] + dps2[j][i] + dps3[i - j + w - 1][(i + j) / 2];

    dps1[i][j + 1] = dps1[i][j] + dp[i][j];
    dps2[j][i + 1] = dps2[j][i] + dp[i][j];
    dps3[i - j + w - 1][(i + j) / 2 + 1] = dps3[i - j + w - 1][(i + j) / 2] + dp[i][j];
  }
  OUT(dp[h - 1][w - 1]);
}
