...
void solve() {
  ...
  if (n == 2) { cout << abs(p[0] - p[1]) / 2 << "\n"; return; }

  ld ans = LINF;

  rep(ai, n) rep(bi, ai) {
    Point pa = p[ai], pb = p[bi];
    Point cent = {(pa.real() + pb.real()) / 2, (pa.imag() + pb.imag()) / 2};
    ld r = abs(pa - cent);

    bool valid = true;
    rep(i, n) if (abs(cent - p[i]) > r + EPS) { valid = false; break; }
    if (valid) chmin(ans, r);
  }

  rep(ai, n) rep(bi, ai) rep(ci, bi) {
    Circle c = circumcircle(p[ai], p[bi], p[ci]);
    bool valid = true;
    rep(i, n) if (abs(p[i] - c.p) > c.r + EPS) { valid = false; break; }
    if (valid) chmin(ans, c.r);
  }
...
signed main() {
  ...
  cin.tie(nullptr); cout.tie(nullptr); cout << fixed << setprecision(20);
  int t = 1; //cin >> t;
  while (t--) solve();
  // while (t--) compare();
}
