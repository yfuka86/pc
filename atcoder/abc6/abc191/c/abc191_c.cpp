...
void solve() {
  ...
  rep(i, h) rep(j, w) {
    ...
    rep(d, 4) {
      ...
      if (s[di][dj] != '#') e[d].pb({i, j});
    }
  }

  ll ans = 0;
  rep(i, 4) {
    UnionFind uf(e[i].size());
    ...
    rep(j, e[i].size()) {
      ...
      if (i == 0 || i == 2) {
        rep(k, e[i].size()) {
          if (t[k] == mp(x, y + 1) || t[k] == mp(x, y - 1)) uf.unite(j, k);
        }
      } else {
        rep(k, e[i].size()) {
          if (t[k] == mp(x + 1, y) || t[k] == mp(x - 1, y)) uf.unite(j, k);
        }
      }
    }
    set<ll> root;
    rep(j, e[i].size()) root.insert(uf.root(j));
    ans += root.size();
  }
  OUT(ans);
}
