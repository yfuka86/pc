...
void solve() {
  ...
  x = abs(round(x) - x), y = abs(round(y) - y);
  x = min(1.0 - x, x), y = min(1.0 - y, y);

  ll ans = 0;
  rep(i, -100000, 100001) {
    if ((x - i) * (x - i) + y * y > r * r) continue;
    // debug((x - i) * (x - i) + y * y, r * r);

    ll rl = binary_search([&](ll mid) {
      return (x - i) * (x - i) + (y - (ld)mid) * (y - (ld)mid) <= r * r;
    }, 0, -100001);

    ll rr = binary_search([&](ll mid) {
      return (x - i) * (x - i) + (y - (ld)mid) * (y - (ld)mid) <= r * r;
    }, 0, 100001);
    ans += rr - rl + 1;
  }
