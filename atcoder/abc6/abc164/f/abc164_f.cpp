...
void solve() {
  LL(n); VL(s,n); VL(t,n); VEC(ull, u, n); VEC(ull, v, n);

  vv(ull, ans, n, n);
  rep(b, 64) {
    vv(ull, a, n, n, -1);
    // 残り満たすべき条件の集合　行の論理和（1列でも1, 行の論理積（1列でも0, 列の論理和（1行でも1, 列の論理積（1行でも0
    set<ll> s1, s0, t1, t0;
    rep(i, n) {
      if (s[i] == 0) {
        if (u[i] & 1ull << b) rep(j, n) a[i][j] = 1;
        else s0.insert(i);
      } else {
        if (!(u[i] & 1ull << b)) rep(j, n) a[i][j] = 0;
        else s1.insert(i);
      }
    }
    rep(i, n) {
      if (t[i] == 0) {
        if (v[i] & 1ull << b) rep(j, n) a[j][i] = 1;
        else t0.insert(i);
      } else {
        if (!(v[i] & 1ull << b)) rep(j, n) a[j][i] = 0;
        else t1.insert(i);
      }
    }
    // すでに満たした条件削除
    rep(i, n) rep(j, n) if (a[i][j] != -1) {
      if (a[i][j]) { s1.erase(i); t1.erase(j); } else { s0.erase(i); t0.erase(j); }
    }
    // 2つ一気に満たせるか、片側しか条件がないケース
    rep(i, n) rep(j, n) if (a[i][j] == -1) {
      if (s1.count(i) && !t0.count(j)) { a[i][j] = 1; s1.erase(i); t1.erase(j); }
      if (s0.count(i) && !t1.count(j)) { a[i][j] = 0; s0.erase(i); t0.erase(j); }
      if (t1.count(j) && !s0.count(i)) { a[i][j] = 1; t1.erase(j); s0.erase(i); }
      if (t0.count(j) && !s1.count(i)) { a[i][j] = 0; t0.erase(j); s1.erase(i); }
    }
    // 残りをうまくやる （ここ結局市松模様でいいらしい、、、賢すぎる
    vl sdeg(n), tdeg(n);
    rep(i, n) rep(j, n) if (a[i][j] == -1) {
      sdeg[i]++; tdeg[j]++;
    }
    rep(i, n) rep(j, n) if (a[i][j] == -1) {
      ...
      if (s1.count(i) && sdeg[i] <= tdeg[j]) {
        a[i][j] = 1;
        s1.erase(i);
        continue;
      }
      if (t1.count(j) && tdeg[j] <= sdeg[i]) {
        a[i][j] = 1;
        t1.erase(j);
        continue;
      }
    ...
    if (0) {
      vl sbit(n), tbit(n);
      rep(i, n) if (u[i] >> b & 1) sbit[i] = 1;
      rep(i, n) if (v[i] >> b & 1) tbit[i] = 1;
      debug(sbit);
      debug(tbit);
      debug(b, a);
    }
