...
void solve() {
  ...
  rep(i, n) {
    vertices[s[i].substr(0, 3)] = 0;
    vertices[s[i].substr(s[i].size() - 3, 3)] = 0;
  }
  ll m = vertices.size();
  ll cur = 0;
  for(auto [st, _]: vertices) {
    vertices[st] = cur;
    cur++;
  }
  ...
  vl deg(m, 0);
  rep(i, n) {
    ll u = vertices[s[i].substr(0, 3)], v = vertices[s[i].substr(s[i].size() - 3, 3)];
    revG.add_directed_edge(v, u);
    deg[u]++;
  }

  vl win(m, -1);
  queue<ll> que;
  // 行き止まりの頂点を負けとして実装した方がわかりやすい
  rep(i, m) if (deg[i] == 0) { win[i] = 0; que.push(i); }
  while(!que.empty()) {
    auto v = que.front(); que.pop();
    for (auto &from: revG[v]) {
      if(win[from] != -1) continue;
      deg[from]--;
      if (win[v] == 0) {
        win[from] = 1; que.push(from);
      } else if (deg[from] == 0) {
        win[from] = 0; que.push(from);
      }
    }
  }
  ...
  rep(i, m) {
    for (auto &e: revG[i]) {
      ans[e.idx] = win[e.from];
    }
  }
  rep(i, n) {
    if (ans[i] == 0) cout << "Takahashi" << "\n";
    else if (ans[i] == 1) cout << "Aoki" << "\n";
    else cout << "Draw" << "\n";
  }
}
