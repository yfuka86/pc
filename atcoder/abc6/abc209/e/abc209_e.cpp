...
void solve() {
  ...
  Graph<ll> G(mp.size()), rG(mp.size());
  vl deg(mp.size(), 0);
  rep(i, n) {
    ll from = mp[s[i].substr(0, 3)];
    ll to = mp[s[i].substr(s[i].size() - 3, 3)];
    G.add_directed_edge(from, to);
    rG.add_directed_edge(to, from);
    deg[from]++;
  }

  vl dp(mp.size(), -1);
  queue<ll> que;
  rep(i, mp.size()) if (deg[i] == 0) { dp[i] = 0; que.push(i); }
  ...
  rep(i, n) {
    ll res = dp[mp[s[i].substr(s[i].size() - 3, 3)]];
    if (res == -1) {
      cout << "Draw" << "\n";
    } else {
