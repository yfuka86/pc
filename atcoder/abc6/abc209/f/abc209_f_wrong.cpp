...
void solve() {
  ...
  vector<vmi> dp(n, vmi(n + 1, 0));
  function<mint(ll, ll)> sec = [&](ll l, ll r) {
    ...
    mint tmp = 0;
    rep2(i, l, r) {
      if (i == l && h[i] >= h[i + 1]) { tmp += sec(l + 1, r); continue; }
      if (i == r - 1 && h[i] >= h[i - 1]) { tmp += sec(l, r - 1); continue; }
      if (h[i] >= h[i + 1] && h[i] >= h[i - 1]) tmp += sec(l, i) * sec(i + 1, r) * comb(r - l - 1, i - l);
    }
    return dp[l][r] = tmp;
  };
