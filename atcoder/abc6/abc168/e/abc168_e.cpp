...
void solve() {
  ...
  rep(i, n) {
    if (a[i] == 0 && b[i] == 0) { zz++; continue; }
    if (a[i] == 0) { z1++; continue; }
    if (b[i] == 0) { z2++; continue; }

    ll gc = gcd(a[i], b[i]);
    if ((a[i] < 0) ^ (b[i] < 0)) {
      S1[{abs(a[i]) / gc, abs(b[i]) / gc}]++;
    } else {
      S0[{abs(b[i]) / gc, abs(a[i]) / gc}]++;
    }
  }

  mint ans = mint(2).pow(z1) + mint(2).pow(z2) - 1;
  fore(p, cn, S0) {
    auto it = S1.find(p);
    if (it != S1.end()) {
      auto [_, cn2] = *it;
      ans *= mint(2).pow(cn) + mint(2).pow(cn2) - 1;
      S1.erase(it);
    } else {
      ans *= mint(2).pow(cn);
    }
