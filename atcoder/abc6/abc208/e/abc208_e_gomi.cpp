...
void solve() {
  ...
  auto prod = [](LT4 l) {
    auto [i, j, k, m] = l;
    if (i < 0) return 0ll;
    assert(i >= 0 && j >= 0 && k >= 0 && m >= 0);
    return POW(2, i) * POW(3, j) * POW(5, k) * POW(7, m);
  };
  function<LT4(ll, LT4)> nl = [&](ll d, LT4 l) {
    auto [l2, l3, l5, l7] = l;
    LT4 ret;
    if (l2 == -100) return l;
    if (d == 0 && l2 == -1) return l;
    if (d == 0) ret = {-100, -100, -100, -100};
    if (l2 == -1) {
      l2++; l3++; l5++; l7++;
    }
    if (d == 1) ret = {l2, l3, l5, l7};
    if (d == 2) ret = {l2 + 1, l3, l5, l7};
    if (d == 3) ret = {l2, l3 + 1, l5, l7};
    if (d == 4) ret = {l2 + 2, l3, l5, l7};
    if (d == 5) ret = {l2, l3, l5 + 1, l7};
    if (d == 6) ret = {l2 + 1, l3 + 1, l5, l7};
    if (d == 7) ret = {l2, l3, l5, l7 + 1};
    if (d == 8) ret = {l2 + 3, l3, l5, l7};
    if (d == 9) ret = {l2, l3 + 2, l5, l7};
    return ret;
  };

  vector<vector<map<LT4, ll>>> dp(d + 1, vector<map<LT4, ll>>(2));
  dp[0][1][{-1, -1, -1, -1}] = 1;
  rep(i, d) {
    rep(j, 10) {
      for(auto[lt, cnt] :dp[i][0]) {
        LT4 nnl = nl(j, lt);
        dp[i + 1][0][nnl] += cnt;
      }
      if (j <= t[i]) {
        for(auto[lt, cnt] :dp[i][1]) {
          LT4 nnl = nl(j, lt);
          dp[i + 1][j == t[i]][nnl] += cnt;
        }
      }
    }
  }
  ll ans = 0;
  for (auto[l, cnt] :dp[d][0]) {
    if (get<0>(l) != -1 && prod(l) <= k) ans += cnt;
  }

  for (auto[l, cnt] :dp[d][1]) {
    if (prod(l) <= k) ans += cnt;
  }
  cout << ans << "\n";
}
