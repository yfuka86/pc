...
void solve() {
  ...
  vector<vector<map<ll, ll>>> dp(d + 1, vector<map<ll, ll>>(2));
  dp[0][1][11] = 1;
  rep(i, d) {
    rep(j, 10) {
      for(auto[p, cnt] :dp[i][0]) {
        ll np = p == 11 ? (j == 0 ? 11 : j) : p * j;
        dp[i + 1][0][np] += cnt;
      }
      if (j <= t[i]) {
        for(auto[p, cnt] :dp[i][1]) {
          ll np = p == 11 ? (j == 0 ? 11 : j) : p * j;
          dp[i + 1][j == t[i]][np] += cnt;
        }
      }
  ...
  ll ans = 0;
  for (auto[p, cnt] :dp[d][0]) if (p != 11 && p <= k) ans += cnt;
  for (auto[p, cnt] :dp[d][1]) if (p != 11 && p <= k) ans += cnt;
  cout << ans << "\n";
}
