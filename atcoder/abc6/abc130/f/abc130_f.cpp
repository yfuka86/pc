...
void solve() {
  ...
  rep(i, n) {
    ld x, y; string s;
    cin >> x >> y >> s;
    if (s[0] == 'D') { xconst.pb(x); yd.pb(y); }
    if (s[0] == 'U') { xconst.pb(x); yu.pb(y); }
    if (s[0] == 'R') { xr.pb(x); yconst.pb(y); }
    if (s[0] == 'L') { xl.pb(x); yconst.pb(y); }
  }
  sort(all(xconst)); sort(all(xr)); sort(all(xl));
  sort(all(yconst)); sort(all(yu)); sort(all(yd));

  vd cand;

  cand.pb(0);
  if (xconst.size() && xr.size()) cand.pb(abs(xconst.back() - xr.back()));
  if (xconst.size() && xl.size()) cand.pb(abs(xconst.back() - xl.back()));
  if (xl.size() && xr.size()) cand.pb(abs(xr.back() - xl.back()));
  if (xconst.size() && xr.size()) cand.pb(abs(xconst.front() - xr.front()));
  if (xconst.size() && xl.size()) cand.pb(abs(xconst.front() - xl.front()));
  if (xl.size() && xr.size()) cand.pb(abs(xr.front() - xl.front()));
  if (yconst.size() && yu.size()) cand.pb(abs(yconst.back() - yu.back()));
  if (yconst.size() && yd.size()) cand.pb(abs(yconst.back() - yd.back()));
  if (yd.size() && yu.size()) cand.pb(abs(yd.back() - yu.back()));
  if (yconst.size() && yu.size()) cand.pb(abs(yconst.front() - yu.front()));
  if (yconst.size() && yd.size()) cand.pb(abs(yconst.front() - yd.front()));
  if (yu.size() && yd.size()) cand.pb(abs(yu.front() - yd.front()));
  // debug(cand);

  set<ld> s;
  for (auto c1: cand) for (auto c2: cand) {
    s.insert((c1 + c2) / 2.0);
  }
  ...
  for (auto t: s) {
    ld xmax = -LINF;
    ld xmin = LINF;
    if (xconst.size()) chmax(xmax, xconst.back());
    if (xr.size()) chmax(xmax, xr.back() + t);
    if (xl.size()) chmax(xmax, xl.back() - t);
    if (xconst.size()) chmin(xmin, xconst.front());
    if (xr.size()) chmin(xmin, xr.front() + t);
    if (xl.size()) chmin(xmin, xl.front() - t);
    ld ymax = -LINF;
    ld ymin = LINF;
    if (yconst.size()) chmax(ymax, yconst.back());
    if (yu.size()) chmax(ymax, yu.back() + t);
    if (yd.size()) chmax(ymax, yd.back() - t);
    if (yconst.size()) chmin(ymin, yconst.front());
    if (yu.size()) chmin(ymin, yu.front() + t);
    if (yd.size()) chmin(ymin, yd.front() - t);
    // debug(t, (xmax - xmin) * (ymax - ymin));
