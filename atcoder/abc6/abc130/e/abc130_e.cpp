...
void solve() {
  LL(n, m); VL(s, n); VL(t, m);

  vv(mint, dp, n + 1, m + 1); vv(mint, dps, n + 2, m + 2);
  dp[0][0] = 1;
  rep(i, n) rep(j, m) {
    dps[i + 1][j + 1] += dps[i + 1][j] + dps[i][j + 1] + dp[i][j] - dps[i][j];
    if (s[i] == t[j]) dp[i + 1][j + 1] += (dps[i + 1][j + 1] + dps[0][0] - dps[i + 1][0] - dps[0][j + 1]);
  }
  mint ans = 0;
