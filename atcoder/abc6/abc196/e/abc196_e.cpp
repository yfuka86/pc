...
template<class T> void OUTARRAY(vector<T>& v, T offset = T(0), string sep = " ") { rep(i, v.size()) { if (i > 0) cout << sep; if (offset != T(0)) { T t; t = v[i] + offset; cout << t; } else cout << v[i]; } cout << "\n"; }
...
void solve() {
  ...
  rep(i, n) {
    ...
    if (t[i] == 1) {
      ...
      // max
      auto rt = S.lower_bound(aa);
      for (auto it = S.begin(); it->fi < aa; it = S.erase(it)) {
        if (!S.count(aa)) S[aa] = move(it->se);
        else S[aa].insert(S[aa].begin(), (it->se).begin(), (it->se).end());
      }
    } else {
      // min
      for (auto it = S.upper_bound(aa); it != S.end(); it = S.erase(it)) {
        if (!S.count(aa)) S[aa] = move(it->se);
        else S[aa].insert(S[aa].begin(), (it->se).begin(), (it->se).end());
      }
    }
  ...
  // debug(S);
  vl ans(q);
  fore(k, v, S) fore(i, v) ans[i] = k + offset;
  OUTARRAY(ans, 0ll, "\n");
}
