...
void solve() {
  ...
  reverse(all(tv)); reverse(all(rtv));

  // debug(sv, tv);
  // debug(rsv, rtv);

  vl ans0 = convolution_ll(sv, tv);
  vl ans1 = convolution_ll(rsv, rtv);
  ll ans = t.size();
  // debug(ans0);
  // debug(ans1);
  rep2(i, t.size() - 1, ans0.size() - t.size() + 1) {
    chmin(ans, (ll)t.size() - (ans0[i] + ans1[i]));
  }
  cout << ans << "\n";
}
