...
void solve() {
  ...
  ll xma = *max_element(all(x)), yma = *max_element(all(y));
  set<ll> sx, sy;
  rep(i, h) if (x[i] == xma) sx.insert(i);
  rep(i, w) if (y[i] == yma) sy.insert(i);

  if (sx.size() * sy.size() > m) OUT(xma + yma);
  else {
    bool found = false;
    fore(i, sx) fore(j, sy) {
      if (found) break;
      auto it = lower_bound(all(g[i]), j);
      if (it == g[i].end() || *it != j) { found = true; break; }
    }
    if (found) OUT(xma + yma); else OUT(xma + yma - 1);
  }
}
