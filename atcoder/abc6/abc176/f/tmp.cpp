...
void solve() {
  LL(n); VL(a, n*3, 1); reverse(all(a));

  ll off = 0;
  {
    vl er;
    rep(i, 1, n*3 - 2, 3) if (a[i] == a[i + 1] && a[i + 1] == a[i + 2]) er.pb(i);
    reverse(all(er));
    fore(i, er) a.erase(a.begin() + i, a.begin() + i + 3);
    off += er.size();
    n -= er.size();
  }
  // debug(a);
  ...
  vvl d(2001);
  rep(i, n * 3) {
    d[a[i]].pb(min(n, (i + 2) / 3));
  }

  v3l ops(n);
  rep(i, 2001) {
    rep(j, d[i].size() - 2) {
      ops[d[i][j]].pb({d[i][j], d[i][j + 1], d[i][j + 2]});
      // if (d[i][j] == d[i][j + 1]) j++;
    }
  }
  // debug(ops);
  vector<SEG> dp(n + 1, SEG(n + 1));
  dp[0].set(0, 0);
  ...
  rep(i, n) {
    fore(v, ops[i]) {
      ...
      if (v[0] == v[1]) { // 一つだけ持ってくる
        rep(j, n + 1) {
          ll ma = dp[j].prod(0, v[0] + 1);
          op.pb({j, v[2], ma + 1});
        }
      } else { // 二つ持ってくる
        ll ma = -LINF;
        rep(j, i + 1) chmax(ma, dp[j].prod(0, v[0] + 1));
        op.pb({v[1], v[2], ma + 1});
      }
    ...
    //   rep(j, n + 1) {
    //     ll t = dp[i].get(j);
    //     cout << (t < 0 ? "-" : to_string(dp[i].get(j))) << " ";
    //   }cout << "\n";
    // }cout << "\n";
  }

  ll ans = -LINF;
  rep(i, n + 1) chmax(ans, dp[i].all_prod());
  OUT(ans + off);
}
