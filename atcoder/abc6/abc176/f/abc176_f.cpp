...
void solve() {
  ...
  if (n == 1) {
    if (a[0] == a[1] && a[1] == a[2]) OUT(1); else OUT(0);
    return;
  }
  ...
  rep(i, 5) rep(j, i) {
    set<ll> ok;
    rep(k, 5) if (k != i && k != j) ok.insert(a[k]);
    dp[a[i]][a[j]] = (ok.size() == 1 ? 1 : 0);
    dp[a[j]][a[i]] = (ok.size() == 1 ? 1 : 0);
    chmax(xma[a[i]], dp[a[i]][a[j]]);
    chmax(xma[a[j]], dp[a[i]][a[j]]);
    chmax(ma, dp[a[i]][a[j]]);
  }

  rep(i, 5, n*3-3, 3) {
    vl v; v.pb(a[i]); v.pb(a[i + 1]); v.pb(a[i + 2]);
    if (v[0] == v[1] && v[1] == v[2]) { off++; continue; }
    // debug(v);
    ...
    rep(j, 3) {
      vl rem;
      rep(k, 3) if (j != k) rem.pb(v[k]);
      // 1つ使う場合
      op.pb({rem[0], rem[1], dp[v[j]][v[j]] + 1});
      // 2つ使う場合
      if (rem[0] == rem[1]) rep(x, n) {
        op.pb({x, v[j], dp[x][rem[0]] + 1});
      }

      // 1つ入れ替え
      rep(x, n) op.pb({x, v[j], xma[x]});
      // 2つ入れ替え
      op.pb({rem[0], rem[1], ma});
    }
  ...
  dp[a[n*3-1]][a[n*3-1]] += 1;
  chmax(ma, dp[a[n*3-1]][a[n*3-1]]);
  OUT(ma + off);
}
