...
void solve() {
  ...
  rep(i, n) allsum ^= a[i];
  xor_basis<ll> basis;
  rep(i, n) basis.add(a[i]);
  debug(basis.basis);

  ll bs = basis.basis.size();
  if (bs == 0) OUTRET(0);

  if (bs == 60) {
    ll ans = 0;
    rep(b, 60) {
      if (allsum & 1ll << b) ans += 1ll << b;
      else ans += 1ll << b + 1;
    }
    OUTRET(ans);
  }
  ...
  vector<map<ll, ll>> dp(bs + 1); dp[0][0] = 0;
  vl dpma(bs + 1);

  ll ans = 0;
  rep(bi, bs) {
    ll fromb = bi == 0 ? 60 : msb(basis.basis[bi]) + 1;
    ll tob = bi == bs - 1 ? 0 : msb(basis.basis[bi + 1]) + 1;
    ll mi = 1ll << msb(basis.basis[bi]) + 2;
    fore(st, v, dp[bi]) {
      if (v < dpma[bi] - mi) continue;
      vl S = {st, st ^ basis.basis[bi]};

      if (bi == bs - 1) {
        fore(s, S) {
          ll sum = 0;
          rep(i, tob, fromb) {
            if (allsum & 1ll << i) sum += 1ll << i;
            else if (s & 1ll << i) sum += 1ll << i + 1;
          }
          chmax(ans, v + sum);
        }
      } else {
        ll filter = (1ll << msb(basis.basis[bi + 1]) + 1) - 1;
        fore(s, S) {
          ll sum = 0;
          rep(i, tob, fromb) {
            if (allsum & 1ll << i) sum += 1ll << i;
          ...
          chmax(dp[bi + 1][s & filter], v + sum);
          chmax(dpma[bi + 1], dp[bi + 1][s & filter]);
        }
      }
    }
  }
