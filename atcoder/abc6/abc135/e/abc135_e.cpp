...
void solve() {
  ...
  if (x < 0) { x = -x; xrev = true; }
  if (y < 0) { y = -y; yrev = true; }
  ...
  if ((x + y) % k == 0) {
    while (tx + ty != x + y) shot();
  } else {
    while (k * 2 < x + y - tx - ty) {
      shot();
    }
    if (!(k & 1) && (k * 2 - (x - tx + y - ty)) & 1) OUTRET(-1);
    if ((k * 2 & 1) != ((x + y - tx - ty) & 1)) {
      // ここから3回
      shot();
    }
    ans.pb(manhattan_samedist({x, y}, {tx, ty}, k));
    ans.pb({x, y});
  }

  OUT(ans.size());
  fore(x, y, ans) {
    OUT(xrev ? -x : x, yrev ? -y : y);
  }
}
