...
void solve() {
  ...
  string s = _s;
  do {
    s.insert(s.end(), _s.begin(), _s.end());
  ...
  rep(i, s.size() - t.size() + 1) {
    if (rh.query(sh, i, i + t.size()) == rh.query(th, 0, t.size())) {
      G.add_directed_edge(i % n, (i + t.size()) % n);
      rG.add_directed_edge((i + t.size()) % n, i % n);
    }
  }
  vl tp = topo_sort(G);
  if (tp.size() == 0) OUTRET(-1);
  reverse(all(tp));

  vl dp(n);
  fore(v, tp) {
    fore(from, rG[v]) {
      chmax(dp[from], dp[v] + 1);
    }
  }
