...
void solve() {
  STR(r); LL(k);
  ll n = r.size();
  v3(ll, dp, n + 1, k + 1, 2); dp[0][0][1] = 1;

  rep(i, n) rep(j, k + 1) rep(ma, 2) {
    rep(d, 10) {
      bool same = (r[i] - '0') == d;
      if (d == 0) dp[i + 1][j][ma & same] += dp[i][j][ma];
      else if (j < k && !(ma && d > (r[i] - '0'))) dp[i + 1][j + 1][ma & same] += dp[i][j][ma];
    }
  }
  // debug(dp);

  OUT(dp[n][k][0] + dp[n][k][1]);
}
