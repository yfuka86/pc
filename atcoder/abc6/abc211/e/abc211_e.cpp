...
void solve() {
  ...
  rep(i, N) {
    string s; cin >> s;
    rep(j, N) if (s[j] == '#') grid[i][j] = 0;
  }

  map<ull, ll> used_check;
  ll ans = 0, cur = 0;

  function<void(ll, ull, set<ll>)> bfs = [&](ll v, ull used, set<ll> S){
    ll i = v / N, j = v % N;
    used |= (1ULL << v);

    if (used_check.count(used)) return;
    used_check[used]++;

    if (__builtin_popcountll(used) == K) {
      ans++;
      return;
    }
    ...
    rep(_, 4) {
      ll di = i + dx[_], dj = j + dy[_];
      if (di < 0 || di >= N || dj < 0 || dj >= N) continue;
      ll cand = di * N + dj;
      if (cand < cur) continue;
      if (grid[di][dj] && !(used >> cand & 1)) S.insert(cand);
    }
    // cout << S.size() << " ";
    for (ll next: S) {
      bfs(next, used, S);
