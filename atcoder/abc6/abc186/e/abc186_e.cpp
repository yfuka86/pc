...
template<class T, class S> void OUTARRAY(vector<T>& v, S offset = S(0), string sep = " ") { rep(i, v.size()) { if (i > 0) cout << sep; if (offset != T(0)) { T t; t = v[i] + offset; cout << t; } else cout << v[i]; } cout << "\n"; }
template<class T, class S> void OUTMAT(vector<vector<T>>& v, S offset = S(0)) { rep(i, v.size()) { OUTARRAY(v[i], offset); } }
...
ll naive(ll n, ll s, ll k) {
  rep(i, 100000) {
    if ((s + k * i) % n == 0) return i;
  }
  return -1;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    ll s = rg.l(1, n);
    ll k = rg.l(1, 1e9);
    auto so = solve(n, s, k); auto na = naive(n, s, k);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, s, k); debug(so); debug(na);
