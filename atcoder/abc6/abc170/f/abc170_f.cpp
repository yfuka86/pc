...
void solve() {
  LL(h,w,k);
  LL(sx,sy,tx,ty); sx--; sy--; tx--; ty--;
  vs c(h); IN(c);
  vv(ll, leafx, h);
  vv(ll, leafy, w);
  rep(i, h) leafx[i].pb(-1);
  rep(j, w) leafy[j].pb(-1);
  rep(i, h) rep(j, w) if (c[i][j] == '@') { leafx[i].pb(j); leafy[j].pb(i); }
  rep(i, h) leafx[i].pb(w);
  rep(j, w) leafy[j].pb(h);

  v3(ll, cost, h, w, 5, LINF);
  cost[sx][sy][4] = 0;
  queue<LT> que;
  que.push({sx, sy, 4});
  while (!que.empty()) {
    auto [x, y, i] = que.front(); que.pop();
    rep(d, 4) {
      ll mak = k + 1;
      if (d == 0) {
        auto it = lower_bound(all(leafy[y]), x);
        if (it != leafy[y].end()) chmin(mak, *it - x);
      }
      if (d == 1) {
        auto it = lower_bound(all(leafx[x]), y);
        if (it != leafx[x].end()) chmin(mak, *it - y);
      }
      if (d == 2) {
        auto it = lower_bound(all(leafy[y]), x);
        if (it != leafy[y].begin()) chmin(mak, x - *prev(it));
      }
      if (d == 3) {
        auto it = lower_bound(all(leafx[x]), y);
        if (it != leafx[x].begin()) chmin(mak, y - *prev(it));
      }
      rep_r(kk, 1, mak) {
        ll di = x + dx[d] * kk, dj = y + dy[d] * kk;
        if (chmin(cost[di][dj][d], cost[x][y][i] + 1)) que.push({di, dj, d}); else break;
      }
    }
  }
  // debug(cost);
