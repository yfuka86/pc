...
void solve() {
  ...
  segtree<S, op, e> seg(200020);
  rep(i, 200020) {
    if (s[i].size()) seg.set(i, *prev(s[i].end()));
  }
  ...
  rep(i, q) {
    ...
    s[from].erase(s[from].find(a[c]));
    s[to].insert(a[c]);

    if (s[from].size()) seg.set(from, *prev(s[from].end())); else seg.set(from, LINF);
    if (s[to].size()) seg.set(to, *prev(s[to].end())); else seg.set(to, LINF);
