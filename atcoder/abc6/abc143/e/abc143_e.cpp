...
void solve() {
  ...
  rep(start, n) {
    mpq<LP> que;
    cost[start][start] = 0; que.push({0, start});
    while(!que.empty()) {
      auto [c, v] = que.top(); que.pop(); if (cost[start][v] < c) continue;
      for(auto &to: G[v]) {
        if (to.cost > l) continue;
        ll rem = l - cost[start][v] % l;
        ll nc = cost[start][v] + to.cost;
        if (cost[start][v] / l != nc / l && nc % l > 0) nc = nc / l * l + to.cost;
        if (chmin(cost[start][to], nc)) que.push({nc, to});
      }
    }
  }
  ...
  rep(i, q) {
    LL(s, t); --s; --t;
    if (cost[s][t] == LINF) OUT(-1);
    else OUT((cost[s][t] - 1) / l);
  }
}
