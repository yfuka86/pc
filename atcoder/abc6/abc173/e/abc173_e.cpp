...
void solve() {
  ...
  rep(i, n) {
    if (a[i] == 0) continue;
    if (a[i] < 0) neg.push(abs(a[i])); else pos.push(a[i]);
  }

  if (k > pos.size() + neg.size()) OUTRET(0);

  // 正の数を作れる
  bool valid = !(k&1);

  if (k < pos.size() + neg.size() && pos.size()) valid = true;
  if (pos.size() && !(neg.size() & 1)) valid = true;

  if (valid) {
    mint ans = 1;
    if (k & 1) { ans *= pos.top(); pos.pop(); }
    pq<ll> q;
    while (pos.size() > 1) {
      ll p = pos.top(); pos.pop();
      q.push(p * pos.top()); pos.pop();
    }
    while (neg.size() > 1) {
      ll n = neg.top(); neg.pop();
      q.push(n * neg.top()); neg.pop();
    }
    rep(_, k / 2) { ans *= q.top(); q.pop(); }
    OUTRET(ans);
  }

  if (n - pos.size() - neg.size()) OUTRET(0);

  // 負の数
  mpq<ll> pque, nque;
  while (pos.size()) { pque.push(pos.top()); pos.pop(); }
  while (neg.size()) { nque.push(neg.top()); neg.pop(); }

  mint ans = nque.top(); nque.pop();
  k--;
  if (k & 1) { ans *= pque.top(); pque.pop(); }

  mpq<ll> q;
  while (pque.size() > 1) {
    ll p = pque.top(); pque.pop();
    q.push(p * pque.top()); pque.pop();
  }
  while (nque.size() > 1) {
    ll n = nque.top(); nque.pop();
    q.push(n * nque.top()); nque.pop();
  }
  rep(_, k / 2) { ans *= q.top(); q.pop(); }
  OUT(-ans);
}
