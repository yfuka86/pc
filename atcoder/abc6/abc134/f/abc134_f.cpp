...
void solve() {
  ...
  // dp[i個目][溜めた数][奇妙さ]
  v3(mint, dp, n + 1, n + 1, K + 1);
  dp[0][0][0] = 1;

  rep(i, n) rep(j, n + 1) rep(k, K + 1) {
    { // そのまま使うか、i/piのどちらかを溜めたものと組ませ、どちらかを保持
      ll nk = k + j * 2;
      if (nk <= K) dp[i + 1][j][nk] += dp[i][j][k] * (2 * j + 1);
    }
    { // i/piをそれぞれ溜めたものと相殺
      ll nk = k + (j - 1) * 2;
      if (j > 0 && nk <= K) dp[i + 1][j - 1][nk] += dp[i][j][k] * j * j;
    }
    { // 溜める
      ll nk = k + (j + 1) * 2;
      if (j < n - 1 && nk <= K) dp[i + 1][j + 1][nk] += dp[i][j][k];
    }
  }
  OUT(dp[n][0][K]);
}
