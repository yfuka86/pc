...
#define VL(name, size, off) vl name(size); IN(name); rep(i, size) name[i]-=off
...
void solve() {
  ...
  VL(p, n, 1);
  VEC3(ll, a, b, c, n);
  rep(i, n) { chmin(b[i], a[i]); chmin(c[i], a[i]); }
  vl revp(n); rep(i, n) revp[p[i]] = i;
  vl as = csum(a), bs = csum(b), cs = csum(c);

  segtree<S, op, e> seg(n);

  rep(i, n) {
    ll t = bs[i];
    S x = seg.prod(0, revp[i]);
    chmin(t, x.x + as[i]);
    // debug(x.x, t);
    // 累積和をオフセットとしてsegにいれる
    seg.set(revp[i], {i, t - as[i + 1]});
  }
  ...
  rep(i, n) {
    S x = seg.get(revp[i]);
    // オフセットとして入れた累積和を相殺
    chmin(ans, x.x + as[x.p + 1] + cs[n] - cs[i + 1]);
  }
  OUT(ans);
}
