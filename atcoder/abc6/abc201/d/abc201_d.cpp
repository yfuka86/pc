...
void solve() {
  ...
  rep(i, H) {
    string s; cin >> s;
    rep(j, W) {
      if (s[j] == '+') grid[i][j] = 1; else grid[i][j] = -1;
    }
  }
  grid[0][0] = 0;
  // if (H == 1 && W == 1) { cout << "Draw" << "\n"; return; }

  vvl dp(H, vl(W, LINF));
  dp[H - 1][W - 1] = 0;

  function<ll(ll, ll)> dfs = [&](ll x, ll y) {
    if (dp[x][y] != LINF) return dp[x][y];
    // 青木くんの選択
    if (x + y & 1) {
      ll t = LINF;
      if (x < H - 1) chmin(t, dfs(x + 1, y) - grid[x + 1][y]);
      if (y < W - 1) chmin(t, dfs(x, y + 1) - grid[x][y + 1]);
      return dp[x][y] = t;
    } else {
      ll t = -LINF;
      if (x < H - 1) chmax(t, dfs(x + 1, y) + grid[x + 1][y]);
      if (y < W - 1) chmax(t, dfs(x, y + 1) + grid[x][y + 1]);
      return dp[x][y] = t;
    }
  };
  ll ans = dfs(0, 0);
