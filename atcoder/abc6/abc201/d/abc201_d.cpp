...
void solve() {
  ...
  rep_r(i, h) rep_r(j, w) {
    ...
    if (i + j & 1) {
      ll mi = LINF;
      if (i < h - 1 && dp[i + 1][j] != -LINF) chmin(mi, dp[i + 1][j] - a[i + 1][j]);
      if (j < w - 1 && dp[i][j + 1] != -LINF) chmin(mi, dp[i][j + 1] - a[i][j + 1]);
      if (mi != LINF) dp[i][j] = mi;
    // takahashi
    } else {
      ll ma = -LINF;
      if (i < h - 1 && dp[i + 1][j] != -LINF) chmax(ma, dp[i + 1][j] + a[i + 1][j]);
      if (j < w - 1 && dp[i][j + 1] != -LINF) chmax(ma, dp[i][j + 1] + a[i][j + 1]);
      if (ma != -LINF) dp[i][j] = ma;
    }
  }
  // debug(dp);
