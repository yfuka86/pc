...
void solve() {
  ...
  rep(i, n) {
    ll a = i * 2, b = a + 1, na = a + 2, nb = b + 2;
    G.add_directed_edge(a, b, x[1][i]);
    G.add_directed_edge(b, a, x[0][i]);
    if (i < n - 1) {
      G.add_directed_edge(a, na, x[0][i + 1]);
      G.add_directed_edge(b, nb, x[1][i + 1]);
    }
  }

  mpq<LP> que; vl cost(n * 2, LINF);
  cost[0] = x[0][0], cost[1] = x[1][0];
  que.push({0, cost[0]});
  que.push({1, cost[1]});
  while(!que.empty()) {
    auto [c, v] = que.top(); que.pop(); if (cost[v] < c) continue;
