...
void solve() {
  ...
  multiset<ll> neg, pos;
  rep(i, n) {
    if (a[i] < 0) neg.insert(a[i]); else pos.insert(a[i]);
  }

  vl cand;
  rep(_, 3) if (neg.size()) {
    cand.pb(*neg.begin()); neg.erase(neg.begin());
  }
  rep(_, 3) if (pos.size()) {
    cand.pb(*pos.begin()); pos.erase(pos.begin());
  }
  rep(_, 3) if (neg.size()) {
    cand.pb(*prev(neg.end())); neg.erase(prev(neg.end()));
  }
  rep(_, 3) if (pos.size()) {
    cand.pb(*prev(pos.end())); pos.erase(prev(pos.end()));
  }

  ll m = cand.size();
  ld mians = LINF, maans = -LINF;
  rep(i, m) rep(j, i + 1, m) rep(k, j + 1, m) {
    ld sum = (ld)cand[i] + cand[j] + cand[k];
    ld pro = (ld)cand[i] * cand[j] * cand[k];
    ld t = sum / pro;
    chmin(mians, t); chmax(maans, t);
  }
  OUT(mians);
  OUT(maans);
}
