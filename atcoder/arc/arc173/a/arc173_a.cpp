...
string solve(ll k) {
  if (k <= 9) return to_string(k);
  vv(ll, dp, 15, 10);
  rep(i, 1, 10) dp[1][i] = 1;
  rep(i, 1, 10) dp[2][i] = 9;

  rep(i, 3, 15) {
    rep(d, 1, 10) rep(from, 10) {
      if (from != d) {
        dp[i][d] += dp[i - 1][from];
      }
      dp[i][d] += dp[i - 2][from];
  ...
  rep_r(i, 1, 15) {
    ll sum = 0;
    rep(d, 10) {
      if (ans.size() > 0 && ans.back() - '0' == d) continue;
      sum += dp[i][d];
      if (k <= sum) {
        // debug(i, d);
        if (d != 0) {
...
string naive(ll k) {
  ...
  rep(i, 1, 100000000) {
    ...
    bool valid = true;
    rep(j, d.size() - 1) {
      if (d[j] == d[j + 1]) valid = false;
    }
    if (valid) cnt++;
    if (cnt == k) return to_string(i);
  }
}
