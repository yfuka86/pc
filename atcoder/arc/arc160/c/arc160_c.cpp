...
template< int mod > struct ModInt {
  ...
  bool operator<(const ModInt &p) const { return ModInt(*this).x < p.x; }
};
...
mint solve(ll n, vl a) {
  ...
  rep(i, 200050) {
    ...
    rep(op, 100010) {
      ll need = op * 2;
      need -= f[i + 1];
      auto it = lower_bound(all(dp[i]), mp(need, mint(0)));
      if (it == dp[i].end()) break;
      ll l = it - dp[i].begin();
      dp[i + 1].pb({op, cs.back() - cs[l]});
    }
  }
  ...
  while(que.size()) {
    ...
    rep(i, t.size() - 1) {
      if (t[i] == t[i + 1]) {
        vl nt;
        rep(j, t.size()) {
          if (j != i && j != i + 1) nt.pb(t[j]);
        } nt.pb(t[i] + 1);
        sort(all(nt));
        if (s.find(nt) == s.end()) { s.insert(nt); que.push(nt); }
      }
    }
  }
  return s.size();
