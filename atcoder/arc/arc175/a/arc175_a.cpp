...
void solve() {
  ...
  // p[0]からスタートさせるように置き換え
  string s = "";
  rep(i, n) s.pb(_s[(p[0] + i) % n]);
  ll off = n - p[0];
  rep(i, n) p[i] = (p[i] + off) % n;

  // dp[i][最初残ってるか][最後残ってるか] = 通り
  v3(mint, dp, n + 1, 2, 2);

  vl pinv(n);
  rep(i, n) pinv[p[i]] = i;

  if (s[0] == 'R' || s[0] == '?') dp[1][1][0] = 1;
  if (s[0] == 'L' || s[0] == '?') dp[1][0][1] = 1;

  rep(i, 1, n) rep(j, 2) {
    if (s[i] == 'L' || s[i] == '?') {
      dp[i + 1][j][1] += dp[i][j][1];
      if (pinv[i] > pinv[i - 1]) { // 先に取られうる場合
        dp[i + 1][j][0] += dp[i][j][0];
      }
    }
    if (s[i] == 'R' || s[i] == '?') {
      dp[i + 1][j][0] += dp[i][j][0];
      if (pinv[(i + 1) % n] < pinv[i]) { // 先に取られうる場合
        dp[i + 1][j][1] += dp[i][j][1];
      }
    }
  }

  OUT(dp[n][1][0] + dp[n][0][1]);
}
