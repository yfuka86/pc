...
void solve() {
  ...
  auto [tree, root] = CartesianTree(r);

  if (*max_element(all(l)) <= r[root]) {
    vl ans(n, *max_element(all(l)));
    OUTARRAY(ans);
    return;
  }
  ...
  function<void(ll, ll, ll)> dfs = [&](ll v, ll le, ll ri) {
    if (le != -1 && ri != -1) {
      ll left = max(ans[le], seg.prod(le, v));
      ll right = max(ans[ri], seg.prod(v + 1, ri + 1));
      if (left > r[v] && right > r[v]) {
        ans[v] = r[v];
      } else {
        ans[v] = max(l[v], min(left, right));
      }
    } else if (ri != -1) {
      ll right = max(ans[ri], seg.prod(v + 1, ri + 1));
      if (right > r[v]) {
  ...
  // vl c; rep(i, n) { c.pb(l[i]); c.pb(r[i]); }
