...
bool solve() {
  ...
  // RandGen rg;
  // ll n = rg.l(2, 8);
  // ll k = rg.l(2, 8); while (n == k) k = rg.l(1, 8);
  // string s = rg.str(n, "abcde");
  // debug(n, k, s);
  ...
  string c = s.substr(0, gc), rev = c;
  reverse(all(rev));

  if (!is_palindrome(c)) {
    ll np = n / gc, kp = k / gc;
    if ((np & 1) ^ (kp & 1)) return false;
  }

  string t = c;
  ll cur = 0;
  while (t.size() < s.size()) {
    if (cur) t += c; else t += rev;
    cur = 1 - cur;
  }
