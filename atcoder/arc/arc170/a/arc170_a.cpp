...
void solve() {
  ...
  // tにおけるaより前にa to bがあるか bよりあとにb to aがある
  // 以外は可能
  ll firsta = LINF, lastb = -LINF;
  rep(i, n) if (t[i] == 'A') { firsta = i; break; }
  rep_r(i, n) if (t[i] == 'B') { lastb = i; break; }

  set<ll> atob, btoa;
  rep(i, n) {
    if (s[i] == 'A' && t[i] == 'B') atob.insert(i);
    if (s[i] == 'B' && t[i] == 'A') btoa.insert(i);
  }

  if (atob.size() > 0 && *atob.begin() <= firsta) OUTRET(-1);
  if (btoa.size() > 0 && *prev(btoa.end()) >= lastb) OUTRET(-1);

  ll ans = 0;
  for(auto it = btoa.begin(); it != btoa.end();) {
    auto it2 = atob.lower_bound(*it);
    if (it2 != atob.end()) atob.erase(it2);
    ans++;
    it = btoa.erase(it);
  }
  ans += atob.size();
