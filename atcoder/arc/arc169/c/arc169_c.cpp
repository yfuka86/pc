...
void solve() {
  ...
  mint allsum = a[0] != -1 ? 1 : n;
  vmi sums(n + 1, 0), tmpsums(n + 1, 0);
  ...
  rep(i, 1, n) {
    mint tmpsum = 0;
    function<void(ll,ll)> remove = [&](ll idx, ll idx2) {
      mint tmp = dp[idx][idx2];
      tmpsum -= tmp;
      tmpsums[idx] -= tmp;
      dp[idx][idx2] = 0;
    };
