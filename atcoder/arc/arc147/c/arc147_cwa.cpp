...
void solve() {
  ...
  ll cnt = 0, ans = 0, center = -1;
  rep(i, n) {
    auto [l,r,id] = rng[i];
    // debug(l, r, id, cnt);
    if (cnt < n / 2) {
      while (!que.empty() && que.top().fi < l && cnt < n / 2) {
        auto [p, _] = que.top(); que.pop();
        ans += p * cnt - bt.sum(0, cnt);
        bt.add(cnt++, p);
        center = p;
      }
      if (cnt < n / 2) que.push({r, l});
      while (!que.empty() && cnt >= n / 2) {
        que.pop();
        ans += center * cnt - bt.sum(0, cnt);
        bt.add(cnt++, center);
      }
    }
    // debug(cnt);
