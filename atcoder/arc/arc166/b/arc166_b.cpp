...
void solve() {
  ...
  ll ans = LINF;
  { // 1つの場合
    ll t = lcm(lcm(a[0], a[1]), a[2]);
    ll cur = 0;
    rep(i, n) {
      if (b[i] % t == 0) chmin(ans, 0);
      else chmax(cur, b[i] % t);
    }
    chmin(ans, t - cur);
  }
  ...
  if (n > 1) { // 2つの場合
    rep(i, 3) {
      ll t1 = 1, t2 = a[i]; rep(j, 3) if (i != j) t1 = lcm(t1, a[j]);

      vvlp cands(2);
      rep(j, n) {
        cands[0].pb({(t1 - (b[j] % t1))%t1, j});
        cands[1].pb({(t2 - (b[j] % t2))%t2, j});
      }
      rep(j, 2) sort(all(cands[j]));
      // debug(cands);

      rep(j, 2) rep(k, 2) {
        if (cands[0][j].se != cands[1][k].se) chmin(ans, cands[0][j].fi + cands[1][k].fi);
      }
    }
  }
  ...
  if (n > 2) {
    vvlp cands(3);
    rep(i, 3) rep(j, n) {
      cands[i].pb({(a[i] - (b[j] % a[i]))%a[i], j});
    }
    rep(i, 3) sort(all(cands[i]));
    rep(i,3)rep(j,3)rep(k,3) {
      ...
      un.pb(cands[2][k].se);
      uniq(un);
      if (un.size() < 3) continue;
      chmin(ans, cands[0][i].fi + cands[1][j].fi + cands[2][k].fi);
    }
  }
