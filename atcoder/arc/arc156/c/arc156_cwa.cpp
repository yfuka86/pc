...
void solve() {
  ...
  auto [d, pv] = diameter(G);
  ll r1 = pv.fi, r2 = pv.se;

  HeavyLightDecomposition<Graph<ll>> hld(G, r1), hld2(G, r2);

  vl ans(n); iota(all(ans), 0);
  ...
  // vl dep; fore(d, hld.depth) dep.pb(d);
  // vi ord = argsort(dep);
  // // debug(ord);
  // vb vis(n);
  // rep_r(i, n) {
  ...
  //   swap(ans[cur], ans[hld.la(cur, hld.depth[cur] - 1)]);
  //   // vl t;
  //   // while (!vis[cur]) {
  //   //   t.pb(cur);
  //   //   // if (vis[cur]) break;
  //   //   vis[cur] = 1;
  //   //   if (hld.depth[cur] == 0) break;
  //   //   cur = hld.la(cur, hld.depth[cur] - 1);
  //   // }
  //   // vl a;
  //   // fore(v, t) {
  //   //   a.pb(ans[v]);
  ...
  if (0) {
    ...
    do {
      ...
      chmin(mi, ma);
    } while(next_permutation(all(ansp)));
    debug(mi);
  }
