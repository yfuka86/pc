...
void solve() {
  ...
  vvmi powk(401, vmi(401, 0));
  vvmi sumpowk(401, vmi(402, 0));
  rep(k, 401) rep(i, 401) powk[k][i] = mod_pow(i, k);
  rep(k, 401) rep(i, 401) sumpowk[k][i + 1] = sumpowk[k][i] + powk[k][i];

  vvmi dp(N + 1, vmi(C + 1, 0));
  dp[0][0] = 1;

  rep(i, N) {
    rep(j, C + 1) {
      rep(k, j + 1) {
        dp[i + 1][j] += dp[i][j - k] * (sumpowk[k][B[i] + 1] - sumpowk[k][A[i]]);
      }
    }
  }
  // coutmatrix(dp);
