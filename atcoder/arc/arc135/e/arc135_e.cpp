...
void solve() {
...

  {
    ...
    coutarray(expb);
    vl expdiff;
    rep(i, expb.size() - 1) expdiff.pb(expb[i] - expb[i + 1]);
    coutarray(expdiff);
  }
  ...
  while (1) {
    cnt++;
    if (cur % cnt != 0) cur = ((cur / cnt) + 1) * cnt;
    ans += cur;
    if (cur == cnt * cnt || cnt == n) break;
    cur++;
  }

  if (n > cnt) {
    mint a0 = mint(cur) + cnt;
    mint an = mint(cur) + mint(n - cnt) * cnt;
    ans += (a0 + an) * (n - cnt) / 2;
  }
