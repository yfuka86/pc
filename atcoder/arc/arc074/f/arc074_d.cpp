...
void solve() {
  ...
  rep(i, h) {
    ...
    rep(j, w) {
      if (s[j] == 'S') sx = i, sy = j;
      if (s[j] == 'T') tx = i, ty = j;
      if (s[j] == 'o') { grid[i][j] = 1; cnt++; }
    }
  }
  if (sx == tx || sy == ty) { cout << -1 << "\n"; return; }

  mf_graph<ll> mf(h + w + cnt * 2 + 2);
  ll s = h + w + cnt * 2, t = s + 1;
  ll cur = 0;
  rep(i, h) {
    rep(j, w) {
      if (grid[i][j]) {
        ...
        mf.add_edge(j + h, m, 1);
        mf.add_edge(m, m + 1, 1);
        mf.add_edge(m + 1, j + h, 1);
        mf.add_edge(m + 1, i, 1);
        if (i == sx) mf.add_edge(s, m, 1);
        if (j == sy) mf.add_edge(s, m, 1);
        if (i == tx) mf.add_edge(m + 1, t, 1);
        if (j == ty) mf.add_edge(m + 1, t, 1);
      }
    }
  }
  ...
  cout << mf.flow(s,t) << "\n";
  // for (auto e: mf.edges()) {
  //   debug(e.from, e.to, e.cap, e.flow);
  // }
}
