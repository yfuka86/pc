...
void solve() {
  ...
  function<void(ll, ll, ll)> dfs = [&](ll hh, ll ww, ll p) {
    ll po = 1 << p;
    ll x = hh / po, y = ww / po;
    have[p] += x * y;

    ll remx = hh % po, remy = ww % po;
    if (remx > 0 && ww > 0) dfs(remx, ww, p - 1);
    if (remy > 0 && hh - remx > 0) dfs(remy, hh - remx, p - 1);
  };
  dfs(h, w, 25);

  rep_r(i, 26) {
    if (cnt[i] > have[i]) OUTRET("No");

    have[i] -= cnt[i];
    if (i > 0) have[i - 1] += have[i] * 4;
  }
  OUT("Yes");
