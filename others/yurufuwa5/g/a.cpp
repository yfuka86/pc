...
#define Yes(ok) { if (ok) { OUT("Yes"); } else OUT("No"); }
ostream &operator<<(ostream &os, const i128 n) { os << (ll)n; return os; }
...
void solve() {
  ...
  dp[0] = 1;
  rep(i, n) {
    auto r = calc(a[i] + 1, b[i]);
    auto l = calc(max(0, a[i] - m), b[i]);
    // debug(r, l);
    rep(from, 4) {
      rep(j, 3) {
        dpt[from ^ j] += dp[from] * (r[j] - l[j]);
      }
    }
    dp = dpt;
    dpt.assign(4, 0);
