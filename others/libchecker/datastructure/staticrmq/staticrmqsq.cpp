...
void solve() {
  ...
  ll sz = max<ll>(1, sqrt_ceil(n) / 2);
  vl block(n / sz + 1, LINF);

  rep(i, n) {
    chmin(block[i / sz], a[i]);
  }
  ...
  rep(i, q) {
    ...
    ll ans = LINF;
    if (l / sz < (r - 1) / sz - 1) {
      while (l % sz) { chmin(ans, a[l]); l++; }
      while (r - l >= sz) { chmin(ans, block[l / sz]); l += sz; }
    }
    while (l < r) { chmin(ans, a[l]); ++l; }
    cout << ans << "\n";
  }
}
