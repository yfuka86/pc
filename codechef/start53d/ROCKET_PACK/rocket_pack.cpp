...
void solve() {
  ll n, m, k; cin >> n >> m >> k;
  priority_queue<LT, vlt, greater<LT>> que, rem;
  rep(i, k) {
    ll x, y, c, e; cin >> x >> y >> c >> e;
    if (x == 0 && y == 0) que.push({c, 0, e});
    else rem.push({x + y, c, e});
  }

  while(!que.empty()) {
    auto [c, d, e] = que.top(); que.pop();
    if (d + e >= n + m) { cout << c << "\n"; return; }

    while (rem.size() && get<0>(rem.top()) <= d + e) {
      auto [dd, cc, ee] = rem.top(); rem.pop();
      que.push({c + cc, dd, ee});
    }
  }
}
