...
void solve() {
  ...
  if (n == k) {
    ...
    rep(i, k) {
      ...
      fore(j, mo[i]) diffsum = min(diffsum + pma - j, LINF);
    }
    ...
    ll ma = mo[0].size(), mi = mo[k - 1].size();
    ll mic = 0;
    ...
    if (mi == ma) {
      ...
      vmi miv, mav;
      rep(op, m / mi + 1) {
        miv.pb(comb(op + mic - 1, op));
      }
      rep(op, m / ma + 1) {
        mav.pb(comb(op + (k - mic) - 1, op));
      }
      vmi mavs = csum(mav);
      rep(i, miv.size()) {
        ans += miv[i] * mavs[(m - i * mi) / ma + 1];
      }
    }
  }
  // debug(n,m,k);
