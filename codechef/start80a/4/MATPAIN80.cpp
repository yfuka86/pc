...
mint solve(ll n, ll m, ll k, vlt Q) {
  map<ll, mint> r, c;
  rep(i, k) {
    auto [q,x,v] = Q[i];
    if (q == 0) {
  ...
  mint all = mint(1 + n * m) * n * m / 2;
  mint ccsum = 0, efsum = 0;

  fore(cc, ef, c) {
    mint colsum = mint(cc+1 + cc+1+m*(n-1)) * n / 2;
    all += colsum * (ef - 1);

    ccsum += mint(cc + 1) * (ef - 1);
    efsum += ef - 1;
  }

  // debug(all, ccsum, efsum);

  mint r0sum = mint(1 + m) * m / 2;
  // 特定行の変化した列の合計を求める
  fore(rr, ef, r) {
    mint rsum = r0sum + mint(m * m) * rr; // もともとのこの行の割り当て
    // debug(rsum);
    rsum += ccsum + efsum * rr * m; // 列の変化を差し引き
    // debug(rr, rsum);
...
mint naive(ll n, ll m, ll k, vlt q) {
  vv(mint, g, n, m);
  ll cur = 1;
  rep(i, n) rep(j, m) {
    g[i][j] = cur;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    rep(i, k) {
      ll t = rg.l(0, 2);
      q[i] = {t, (t ? rg.l(0, m) : rg.l(0, n)), rg.l(1, 4)};
    }
    auto so = solve(n, m, k, q); auto na = naive(n, m, k, q);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, m, k, q); debug(so); debug(na);
