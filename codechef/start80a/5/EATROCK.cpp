...
struct S {ll idx; mint cnt, cosum, sum;};
S op(S l, S r) {
  mint cosum = 0;
  if (l.idx != -1) cosum += l.cnt * x[l.idx];
  if (r.idx != -1) cosum += r.cnt * x[r.idx];
  return {-1, l.cnt + r.cnt, l.cosum + r.cosum + cosum, l.sum + r.sum};
}
S e() { return {-1, 0, 0, 0}; }

void solve() {
  LL(n);
  x.resize(n); IN(x); VL(w, n);
  vi ord = argsort(w);

  vector<S> v(n);
  rep(i, n) v[i] = {i, 0, 0, 0};
  segtree<S,op,e> seg(v), rseg(v);

  mint tot = 0;
  rep(i, n) {
    ll id = ord[i];
    auto left = rseg.prod(0, id);
    auto right = seg.prod(id + 1, n);
    mint nd = mint(x[id]) * left.cnt - left.cosum + right.cosum - mint(x[id]) * right.cnt;
    S nx = {id, left.cnt + right.cnt + 1, 0, left.sum + right.sum + nd};
    seg.set(id, nx); rseg.set(id, nx);
    tot += left.sum + right.sum + nd;
  }

  OUT(tot / mint(2).pow(n));
}
