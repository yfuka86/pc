...
void solve() {
  ...
  if (m & 1) {
    // 差が1から
    ll l = m / 2, r = l + 1;
    ll k = min(n - r, l - 1); // 操作可能回数
    ll c = n - r; // 最初の1/3の乗数

    ans = (mint(2).pow(2 * k + 3) - mint(3).pow(k + 1) * 2) / mint(3).pow(c + k + 1) / 3;
  } else {
    // 差が2から
    ll l = m / 2 - 1, r = l + 2;
    ll k = min(n - r, l - 1); // 操作可能回数
    ...
    ans = (mint(4).pow(k + 2) - mint(3).pow(k + 1) * 4) / mint(3).pow(c + k + 2) / 3;
  }

  if (m <= n) {
    ans += mint(2).pow(m) / mint(3).pow(n);
  }
  OUT(ans);
}
