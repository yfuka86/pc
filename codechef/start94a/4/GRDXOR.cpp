...
void solve() {
  ...
  rep(b, 30) {
    ...
    // debug(cnt1, xcnt1, ycnt1);
    rep(x, n) rep(y, m) {
      if (a[x][y] >> b & 1) { // 1の場合
        // x,y以外の領域にある1の数
        ll rem1 = cnt1 - (xcnt1[x] + ycnt1[y] - 1);
        ans[x][y] += (rem - rem1) * (1 << b);
      } else {
        // x,y以外の領域にある1の数
        ll rem1 = cnt1 - xcnt1[x] - ycnt1[y];
        ans[x][y] += rem1 * (1 << b);
      }
    }
    // debug(ans);
  ...
  ll aans = 0;
  rep(i, n) rep(j, m) chmax(aans, ans[i][j]);
  OUT(aans);
}
