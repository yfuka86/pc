...
void solve() {
  ...
  rep(i, n) { // 真ん中の0で探索
    mint t = 0;
    if (s[i] == '0') {
      t += mint(cur4) * (four - cur4) * mint(2).pow(free);
      t += mint(curf) * (four - cur4) * mint(2).pow(free - 1);
      t += mint(cur4) * (free - curf) * mint(2).pow(free - 1);
      t += mint(curf) * (free - curf) * mint(2).pow(free - 2);
    } else if (s[i] == '*') {
      t += mint(cur4) * (four - cur4) * mint(2).pow(free - 1);
      t += mint(curf) * (four - cur4) * mint(2).pow(free - 2);
      t += mint(cur4) * (free - curf - 1) * mint(2).pow(free - 2);
      t += mint(curf) * (free - curf - 1) * mint(2).pow(free - 3);
    }
    ans += t;
