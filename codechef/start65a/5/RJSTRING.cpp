...
void solve() {
  ...
  sort(all(p));
  ll off = lower_bound(all(p), s) - p.begin();
  ...
  function<void(ll)> dfs = [&](ll v) {
    rep(i, 26) {
      if (c[i] > 0) {
        c[i]--;
        tmp.pb('a' + i);
        ll d = lower_bound(all(suf), tmp) - suf.begin();
        ans[off + d] = 1;
        if (trie.nxt[v][i] != -1) dfs(trie.nxt[v][i]);
        tmp.pop_back();
        c[i]++;
      }
    }
