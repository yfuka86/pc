...
void solve() {
  ...
  bool alleven = true;
  fore(k, cnt, f) {
    if (cnt & 1) alleven = false;
  }
  if (alleven) OUT("Zenyk"); else OUT("Marichka");


  return;
  map<pair<vl, ll>, ll> dp;
  function<ll(vl, ll)> dfs = [&](vl a, ll last) {
    if (*max_element(all(a)) < last) return dp[{a, last}] = 0ll;
    if (dp.count({a, last})) return dp[{a, last}];
    ...
    return dp[{a, last}] = mex(res);
  };
  enum_check(4, 1, 4, [&](vl &a) {
    debug(a, dfs(a, 1));
    return true;
  });
