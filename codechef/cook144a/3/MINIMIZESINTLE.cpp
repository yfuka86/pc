...
array<int, MA> a0;
struct S {
  ll l = -1, r = -1, off = 0; array<int, MA> a1, a2;
};
using F = ll;
S op(S l, S r) {
  ll o = (r.off - l.off) % 180; if (o < 0) o += 180;
  if (l.l != -1 || r.l != -1) {
    chmax(l.l, r.l); chmax(l.r, r.r);
  } else {
    rep(i, MA) {
      ll j = o ? ((i - o) % 180) : i;
      if (j < 0) j += 180;
      if (l.a1[i] != -1 && r.a1[j] != -1) {
        chmax(l.a2[i], r.a1[j]);
      } else {
        chmax(l.a1[i], r.a1[j]);
        chmax(l.a2[i], r.a2[j]);
...
void solve() {
  ...
  rep(qi, q) {
    ...
    if (t == 1) {
      ...
      auto s = seg.prod(l, r);
      if (s.l != -1) {
        ll a = s.l, b = s.r; if (a > b) swap(a, b);
        OUT(a, b);
      } else {
        vl e, id;
        rep(i, MA) if (s.a1[i] != -1) { e.pb(i); id.pb(s.a1[i]); }

        // debug(e, id);
        ll mi = 91, a = -1, b = -1;
        rep(i, MA) if (s.a2[i] != -1) { mi = 0; a = s.a1[i]; b = s.a2[i]; }

        rep(i, e.size()) {
          ll d = abs(e[i] - e[(i + 1) % e.size()]);
          chmin(d, 180 - d);
          if (chmin(mi, d)) { a = id[i]; b = id[(i + 1) % e.size()]; }
        }
        if (a > b) swap(a, b);
        // debug(mi, a, b);
        OUT(a + 1, b);
