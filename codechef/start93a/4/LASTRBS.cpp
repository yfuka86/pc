...
void solve() {
  ...
  fore(_, len, RLE(s)) {
    rep(j, n + 1) { // 半分にできる
      ...
      if (j + len <= n && chmax(dp[cur + len][j + len], 1)) from[cur + len][j + len] = {cur, j};
      if (j - len >= 0 && chmax(dp[cur + len][j - len], 1)) from[cur + len][j - len] = {cur, j};
    }
    cur += len;
  }
  ...
  if (dp[n][0]) {
    ...
    string ans = "";
    ll curi = n, curj = 0;
    while (from[curi][curj].fi != -1) {
      auto [ni, nj] = from[curi][curj];
      rep(_, curi - ni) {
        if (nj > curj) ans.pb(')');
        else ans.pb('(');
      }
