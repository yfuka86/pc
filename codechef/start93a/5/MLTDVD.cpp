...
using S = vector<pair<mint, mint>>;
S solve(ll n, vl a, ll q, vl k) {
  // gcdは、毎回初回のlcm/gcdがかかっていく
  // lcmは次のgcd
  ll gc = 0;
  map<ll, ll> lc;
  rep(i, n) {
    gc = gcd(gc, a[i]);
    fore(p, cnt, ps.factorize(a[i])) {
      chmax(lc[p],cnt);
    }
  }
  mint mlc = 1;
  ...
  mint ini = gc;
  rep(i, q) {
    ans.pb({ini * mul.pow(k[i]), ini * mul.pow(k[i] + 1)});
  }
  return ans;
}

S naive(ll n, vl _a, ll q, vl k) {
  S ans;
  rep(i, q) {
    vl a = _a;
    rep(times, k[i]+1) {
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    ll q = 1;
    vl k = {2};
    auto so = solve(n, a, q, k); auto na = naive(n, a, q, k);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, a, q, k); debug(so); debug(na);
