...
void solve() {
  ...
  v3(mint, dp, k+1, n, n + 1);
  vv(mint, dpsum, n + 2, n + 2);
  dp[0][0][n] = 1;
  ...
  rep(i, k) {
    if (s[i] == 'L') {
      rep(l, n) {
        rep(r, l + 2, n + 1) {
          dpsum[l][r] = dp[i][l][r] * inv[r - l - 1];
        }
        rep(r, n + 1) {
          dpsum[l][r + 1] += dpsum[l][r];
        }
      }

      rep(l, n) rep(r, l + 1, n + 1) {
        if (r - l == 1) dp[i+1][l][r] += dp[i][l][r];
        dp[i + 1][l][r] += (dpsum[l][n] - dpsum[l][r]);
        // rep(j, r + 1, n + 1) {
        //   dp[i+1][l][r] += dp[i][l][j] / (j - l - 1);
        // }
      }
    } else {
      rep(r, n + 1) {
        rep(l, 0, r - 1) {
          dpsum[r][l] = dp[i][l][r] * inv[r - l - 1];
        }
        rep(l, n) {
          dpsum[r][l + 1] += dpsum[r][l];
        }
      }

      rep(l, n) rep(r, l + 1, n + 1) {
        if (r - l == 1) dp[i+1][l][r] += dp[i][l][r];
        if (l > 0) dp[i + 1][l][r] += dpsum[r][l - 1];
        // rep(j, 0, l) {
        //   dp[i+1][l][r] += dp[i][j][r] / (r - j - 1);
        // }
      }
    }

    rep(i, n + 2) rep(j, n + 2) dpsum[i][j] = 0;
  }
  // debug(dp);
