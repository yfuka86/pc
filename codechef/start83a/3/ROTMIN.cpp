...
string solve(ll n, ll p, ll q, string s) {
  ll r = binary_search([&](ll mid) {
    ...
    rep_r(i, 26) {
      ll can = min(qrem / (26 - i), f[i]);
      sum -= can * i;
      qrem -= (26 - i) * can;
    }
    return sum <= p;
  }, 0, n + 1);
  if (r == n) return string(n, 'a');
  ...
  while(sum > p && que.size()) {
    q -= (26 - que.top()); sum -= que.top();
    que.pop();
  }
  p -= sum;
...
string naive(ll n, ll p, ll q, string s) {
  string ans = s;
  rep(S, 1 << n) {
    string t = s;
    ll pt = p, qt = q;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 5;
    ll p = rg.l(0, n * 26 + 1), q = rg.l(0, n * 26 + 1);
    string s = rg.straz(n);
    auto so = solve(n, p, q, s); auto na = naive(n, p, q, s);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, p, q, s); debug(so); debug(na);
