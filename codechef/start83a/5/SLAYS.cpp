...
vl solve(ll n, ll q, vl s, vlp qu) {
  map<ll, vl> fr;
  rep(i, n) {
    fr[s[i]].pb(i);
  }
  ...
  fore(l, r, qu) {
    S x = seg.prod(l, r);
    if (pro[x.idx] >= r) ans.pb(x.x); else ans.pb(x.x + 1);
  }
  return ans;
}

vl naive(ll n, ll q, vl s, vlp qu) {
  vl ret;
  fore(l, r, qu) {
    ll ma = *max_element(rng_of(s, l, r));
    ll cur = ma;
    bool valid = true;
    rep(i, l, r) {
      if (s[i] > cur) { valid = false; break; }
      if (s[i] == cur) cur--;
    }
    if (valid) ret.pb(ma); else ret.pb(ma + 1);
  }
  return ret;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp qu(5);
    rep(i, q) {
      ll l = rg.l(0, n), r = rg.l(l + 1, n + 1);
      qu[i] = {l, r};
    }
    auto so = solve(n, q, a, qu); auto na = naive(n, q, a, qu);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, a, qu); debug(so); debug(na);
