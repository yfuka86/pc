...
ll solve(vl a) {
  bool abdiff = false, bcdiff = false;
  ll ans = 0;
  rep_r(b, 30) {
    vl t;
    rep(i, 3) t.pb(a[i] >> b & 1);
  ...
  if (abdiff && bcdiff) return ans; else return -1;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    vl a = rg.vecl(3, 1, 1e2);
    auto so = solve(a); auto na = naive(a);
    if (!check || (na >= 0 != so >= 0)) { cout << c << "times tried" << "\n";
      rep(i, 3) OUTBIN(a[i], 10);
      debug(a); debug(so); debug(na);
