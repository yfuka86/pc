...
// ll naive(ll n, vl t, vl c) {
//   rep(S, 1 << n) {
//     ll a = 0, b = 0, score = 0, cur = 0;
//     rep(i, n) {
//       if (S & 1 << i) a++; else b++;
//       // if ()
//     }
...
void solve() {
  ...
  rep(i, n) {
    rep(j, sz) {
      if (0 < j) chmax(dp[i + 1][j - 1], dp[i][j]);
      if (j < sz - 1) chmax(dp[i + 1][j + 1], dp[i][j]);
    }
