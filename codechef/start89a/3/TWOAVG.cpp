...
void solve() {
  ...
  ll as = sum_of(a), bs = sum_of(b);
  if (as * m > bs * n) OUTRET(0);

  ll ans = INF;
  rep(ai, (n + m) * 2 + 10) { // a側に加えるkの数
    ll asumnew = as + k * ai;
    ll asznew = n + ai;
    ll x = binary_search([&](ll mid) {
      if (mid == INF) return true;
      if (mid == -1) return false;
      return (i128)asumnew * (m + mid) > (i128)(bs + mid) * asznew;
    }, INF, -1);

    chmin(ans, ai + x);
  }

  rep(bi, (n + m) * 2 + 10) { // b側に加える1の数
    ll bsumnew = bs + bi;
    ll bsznew = m + bi;
    ll x = binary_search([&](ll mid) {
      if (mid == INF) return true;
      if (mid == -1) return false;
      return (i128)(as + k * mid) * bsznew > (i128)bsumnew * (n + mid);
    }, INF, -1);
