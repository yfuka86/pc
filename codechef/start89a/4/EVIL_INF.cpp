...
void solve() {
  ...
  set<ll> takeout;
  mpq<LP> que;
  rep(i, n) que.push({y[i], i});
  rep(i, n+1) {
    while (que.size() && que.top().fi < x[i]) {
      auto [y, yi] = que.top(); que.pop();
      auto it = takeout.begin();
      while (it != takeout.end()) {
        if (*it != yi) {
          t[yi] = *it;
          break;
        }
  ...
  Graph<ll> G(n);
  rep(i, n) {
    if (t[i] != -1) G.add_directed_edge(i, t[i]);
  }
  vl tp = topo_sort(G);
  vl dp(n, 0);
  rep_r(i, n) {
    fore(to, G[tp[i]]) {
      chmax(dp[tp[i]], dp[to] + 1);
    }
  }
