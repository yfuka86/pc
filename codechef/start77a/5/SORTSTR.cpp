...
void solve() {
  ...
  rep_r(l, n) {
    ll r = n, d = s[l] - 'a';
    rep(j, 26) if (abs(d - j) != 1 && lasti[j] != n) G.add_directed_edge(l, lasti[j]);
    lasti[d] = l;
  }

  vl deg(n), ret; mpq<LP> que;
  rep(i, n) for (Edge e: G[i]) deg[e.to]++;
  rep(i, n) if (deg[i] == 0) que.push({s[i] - 'a', i});
  ...
  while (!que.empty()) {
    ...
    ans.pb('a' + c);
    ret.pb(v);
    for(ll next: G[v]) { deg[next]--; if (deg[next] == 0) que.push({s[next] - 'a', next}); }
    G[v].clear();
  }
  OUT(ans);
}
