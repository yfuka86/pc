...
vl solve(ll n, vl a) {
  ...
  ll pi = -1; bool pi2 = false;
  rep(i, n) rep(j, i) {
    if (a[i] == a[j]) { pi = i; }
  }
  if (pi == -1) rep(i, n) if (a[i] == 2) { pi = i; pi2 = true; }

  vl ans; vvl anss;
  auto change = [&](ll i, ll j) {
    a[i] = a[j];
    ans.pb(i);
    anss.pb({j});
  };
  ...
  if (pi2) {
    ...
    rep(i, n) rep(j, n) {
      if (!done && a[i] == 1 && a[j] == 3) {
        ...
        a[pi] = 2;
        ans.pb(pi); anss.pb({i, j});
      }
    }
  } else {
    rep(i, n) {
      if (sorted[pi] == a[i]) { change(pi, i); break; }
    }
  }

  return a;

  // if (a == sorted) OUTRET(0);

  // OUT(ans.size());
  // rep(i, ans.size()) {
  //   OUT(ans[i] + 1, anss[i].size());
  //   OUTARRAY(anss[i], 1);
  // }
}
