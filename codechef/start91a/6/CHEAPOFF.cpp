...
void solve() {
  ...
  ll ans = LINF;
  function<ll(vl, ll)> dfs = [&](vl ids, ll ms) {
    // debug(ids, ms);
    if (ms == -1) return 0ll;
    vl bv0, bv1;
    fore(i, ids) if (a[i] >> ms & 1) { f1[i] = 1; bv1.pb(i); } else { f0[i] = 1; bv0.pb(i); }

    ll ma0 = 0, ma1 = 0;
    fore(v, bv0) fore(u, es[v]) if (f0[u]) {
      uf0.unite(v, u);
      chmax(ma0, uf0.size(u));
    }
    fore(v, bv1) fore(u, es[v]) if (f1[u]) {
      uf1.unite(v, u);
      chmax(ma1, uf1.size(u));
    }
    uf0.rollback(); uf1.rollback();
    fore(i, ids) if (a[i] >> ms & 1) { f1[i] = 0; } else { f0[i] = 0; }

    ll ret = LINF;
    if (ma0 < k && ma1 < k) {
      chmin(ret, dfs(ids, ms - 1) + (1 << ms));
    } else {
      if (ma0 >= k) chmin(ret, dfs(bv0, ms - 1));
      if (ma1 >= k) chmin(ret, dfs(bv1, ms - 1));
    }
    // debug(ret);
    return ret;
  };
  vl ids(n); iota(all(ids), 0);
  OUT(dfs(ids, 30));
}
