...
void solve() {
  ...
  ll mi = binary_search([&](ll mid) { // 最小の値をいくつにできるか
    ll need = 0;
    rep(i, n) {
      need += max(0, mid - a[i]);
    }
  ...
  mpq<ll> que; rep(i, n) que.push(b[i]);
  while (op < k) {
    ll i = que.top(); que.pop();
    que.push(i + 1);
    op++;
  }

  vl ans; while (que.size()) { ans.pb(que.top()); que.pop(); }

  ll sum = sum_of(ans);
  mint anssum = 0;
  fore(i, ans) anssum += (mint(sum) - i) * i;
  OUT(anssum / 2);
}
