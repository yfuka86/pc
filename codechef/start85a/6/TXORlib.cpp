...
struct ReRooting {
  G& g; ReRooting(G& _g): g(_g) {}

  template <typename M, typename R>
  void run(vector<S> &dp, vector<S> &dpr, const M &merge, const R &remove, S e, bool invertible = true) {
    function<void(ll, ll)> dfs = [&](ll v, ll p) {
      for(auto &&to: g[v]) { if (to == p) continue;
        dfs(to, v);
        merge(dp[v], dp[to], to.idx);
      }
    };
    dfs(0, -1);

    function<void(ll, ll, S&)> dfsr = [&](ll v, ll p, S &par) {
      if (invertible) {
        for(auto &&to: g[v]) { if (to == p) continue;
          merge(dpr[v], dp[to], to.idx);
        }
        for(auto &&to: g[v]) { if (to == p) continue;
          remove(dpr[v], dp[to], to.idx);
          merge(dpr[to], dpr[v], to.idx);
          dfsr(to, v, dpr[v]);
          merge(dpr[v], dp[to], to.idx);
        }
      } else {
        ll n = g[v].size(); vector<S> csum(n + 1, e), crsum(n + 1, e);
        rep(i, n) {
          merge(csum[i + 1], csum[i], 0);
          if (g[v][i] == p) merge(csum[i + 1], par, g[v][i].idx);
          else merge(csum[i + 1], dp[g[v][i]], g[v][i].idx);
        }
        rep_r(i, n) {
          merge(csum[i], csum[i + 1], 0);
          if (g[v][i] == p) merge(csum[i], par, g[v][i].idx);
          else merge(csum[i], dp[g[v][i]], g[v][i].idx);
        }
        dpr[v] = csum[n];

        rep(i, n) { if (g[v][i] == p) continue;
          S nx = e; merge(nx, csum[i], -1); merge(nx, crsum[n - i], -1);
          dfsr(g[v][i], v, nx);
        }
      }
    };
...
using S = array<array<int, 2>, 32>;
vector<S> dp(100010), dpr(100010);
...
void solve() {
  ...
  ReRooting<Graph<ll>, S> reroot(G);
  auto merge = [&](S &dpv, S &dpc, ll ei) {
    rep(b, 32) {
      ll j = b ? b - 1 : 31; ll p = cost[ei] >> b & 1;
      dpv[b][p] += dpc[j][0] + 1;
      dpv[b][p ^ 1] += dpc[j][1];
    }
    return dpv;
  ...
  reroot.run(dp, dpr, merge, remove, S());

  // debug(reroot.dp);

  vl ans(n);
  rep(i, n) rep(b, 32) ans[i] += dpr[i][b][1] * (1ll << b);
  OUTARRAY(ans);

  rep(i, n) rep(b, 32) rep(j, 2) dp[i][b][j] = dpr[i][b][j] = 0;
}
