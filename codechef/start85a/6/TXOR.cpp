...
v3(ll, dp, 100010, 32, 2, 0);
v3(ll, dp2, 100010, 32, 2, 0);

vl btoj(32);

void merge(S &dpv, S &dpc, ll c) {
  rep(b, 32) {
    ll j = btoj[b]; ll p = c >> b & 1;
    dpv[b][p] += dpc[j][0] + 1;
    dpv[b][p ^ 1] += dpc[j][1];
  }
}

void remove(S &dpv, S &dpc, ll c) {
  rep(b, 32) {
    ll j = btoj[b];; ll p = c >> b & 1;
    dpv[b][p] -= dpc[j][0] + 1;
    dpv[b][p ^ 1] -= dpc[j][1];
...
void dfs(ll v, ll p, Graph<ll> &G) {
  fore(to, G[v]) { if (to == p) continue;
    dfs(to, v, G);
    merge(dp[v], dp[to], to.cost);
  }
}

void dfs2(ll v, ll p, Graph<ll> &G) {
  if (v == 0) dp2[v] = dp[v];

  fore(to, G[v]) { if (to == p) continue;
    remove(dp2[v], dp[to], to.cost);
    dp2[to] = dp[to];
    merge(dp2[to], dp2[v], to.cost);
    dfs2(to, v, G);
    merge(dp2[v], dp[to], to.cost);
  }
}
...
void solve() {
  ...
  rep(i, n) {
    rep(b, 32) {
      ans[i] += dp2[i][b][1] * (1ll << b);
    }
  }
  OUTARRAY(ans);

  rep(i, n) rep(j, 32) rep(k, 2) {
    dp[i][j][k] = 0;
    dp2[i][j][k] = 0;
  }
}
