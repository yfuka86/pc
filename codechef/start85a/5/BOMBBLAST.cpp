...
void solve() {
  ...
  RangeAdd<ll> bt2(n + 1), bt1(n + 1), bt0(n + 1);

  rep(i, m) {
    LL(x, y);
    bt2.range_add(max(0, x - y), x, -1);
    bt1.range_add(max(0, x - y), x, 2*x + 1);
    bt0.range_add(max(0, x - y), x, -(x*x + x));

    bt2.range_add(x + 1, min(n, x + y) + 1, 1);
    bt1.range_add(x + 1, min(n, x + y) + 1, -(2*x - 1));
    bt0.range_add(x + 1, min(n, x + y) + 1, x*x - x);
  }

  vl ans(n);
  rep(i, 1, n + 1) {
    ans[i - 1] = bt2.get(i) * i * i + bt1.get(i) * i + bt0.get(i);
  }
  OUTARRAY(ans);
}
