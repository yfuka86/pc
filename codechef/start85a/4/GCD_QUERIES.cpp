...
PrimeSieve ps(1000001);
vector<set<LP>> s(1000001);

void solve() {
  LL(n);
  VL(a, n);
  set<ll> used; used.insert(1);
  rep(i, n) {
    s[1].insert({a[i], i});
    fore(f, _, factorize(a[i])) {
      used.insert(f);
      s[f].insert({a[i], i});
    }
  }
  ...
  rep(i, q) {
    LL(x);
    LP torem = {LINF, -1};
    fore(f, _, factorize(x)) if (s[f].size()) chmin(torem, *s[f].begin());
    if (torem.fi == LINF) torem = *s[1].begin();
    // 実際に削除
    auto [ai, idx] = torem;
    cout << ai << " ";
    s[1].erase({ai, idx});
    fore(f, _, factorize(ai)) s[f].erase({ai, idx});
  }
  cout << "\n";

  // 後処理
  fore(i, used) s[i].clear();
}
