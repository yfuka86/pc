...
void solve() {
  ...
  VV(ll, a, n, m);
  csum2d asum(a);
  if (asum.query(0, 0, n, m) < k + 1) OUTRET(-1);

  ll MA = max(n, m);
  ll ans = LINF;
  rep(i, n) rep(j, m) { if (a[i][j] == 0) continue;
    ll t = binary_search([&](ll mid) {
      if (mid == -1) return false;
      ll x1 = max(0, i - mid), y1 = max(0, j - mid);
      ll x2 = min(n, i + mid + 1), y2 = min(m, j + mid + 1);
      return asum.query(x1, y1, x2, y2) >= k + 1;
    }, MA, -1);
    chmin(ans, t);
  }
  OUT(ans);
