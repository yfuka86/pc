...
ll solve(ll n, ll k, vl a, vl b) {
  vlp c;
  rep(i, n) {
    c.pb({a[i] + b[i], i});
  }
  sort(all(c));
  vl cc; rep(i, n) cc.pb(c[i].fi);
  vl rev(n); rep(i, n) rev[c[i].se] = i;
  vl cs = csum(cc);

  if (cs.back() - *max_element(all(b)) <= k) return n;
  ...
  rep(i, n) {
    if (a[i] > k) continue;
    ll r = upper_bound(all(cs), k - a[i]) - cs.begin();

    if (rev[i] < r) {
      if (cs[r] - b[i] <= k) chmax(ans, r); else chmax(ans, r - 1);
    } else {
      chmax(ans, r);
    }
    // debug(i, r, ans);
...
ll naive(ll n, ll k, vl a, vl b) {
  ll ma = 0;
  rep(i, n) {
    ll sum = a[i], cnt = 1;
    vl c;
    rep(j, n) if (i != j) c.pb(a[j] + b[j]);
    sort(rall(c));
    while (c.size() && sum + c.back() <= k) {
      sum += c.back();
      c.pop_back();
      cnt++;
    }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vl a = rg.vecl(n, 1, 5);
    vl b = rg.vecl(n, 0, 5);
    auto so = solve(n, k, a, b); auto na = naive(n, k, a, b);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, a, b); debug(so); debug(na);
