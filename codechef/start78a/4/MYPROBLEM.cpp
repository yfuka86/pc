...
void solve() {
  ...
  {
    set<ll> s;
    rep(i, ds.size()) s.insert(ds[i]);
    if (s.size() <= k) OUTRET(0);
  }

  if (k == 1) {
    ll b = ds.front();
    ll sum = 0, p10 = 1;
    rep(i, ds.size()) { sum += b * p10; p10 *= 10; }
    if (sum > n) { OUTRET(sum - n); }
    else { OUTRET(sum / b * (b + 1) - n); }
  }
  ...
  rep(to, 1, ds.size() + 1) {
    set<ll> s;
    rep(i, to) s.insert(ds[i]);

    vl rem;
    rep_r(i, to, ds.size()) rem.pb(ds[i]);

    if (s.size() == k - 1) {
      vl add;
      s.insert(rem.back() + 1);
      if (s.size() == k) {
        rep(_, rem.size() - 1) add.pb(*s.begin());
        add.pb(rem.back() + 1);
        chmin(ans, kto10(add, 10) - kto10(rem, 10));
      } else {
        rep(_, rem.size() - 1) add.pb(0);
        add.pb(rem.back() + 1);
        chmin(ans, kto10(add, 10) - kto10(rem, 10));
      }
    }
