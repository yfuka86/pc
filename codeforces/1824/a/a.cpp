...
void solve() {
  ...
  vl nums;
  rep(i, n) {
    if (x[i] == -1) left++; else if (x[i] == -2) right++; else nums.pb(x[i]);
  }
  uniq(nums);
  // debug(nums);
  ...
  ll ans = 0;
  rep(i, nums.size()) {
    ll leftroom = nums[i] - i - 1;
    ll rightroom = m - nums[i] - (nums.size() - i - 1);
    chmax(ans, nums.size() + min(leftroom, left) + min(rightroom, right));
  }
  chmax(ans, min(nums.size() + right, m));
  chmax(ans, min(nums.size() + left, m));
  OUT(ans);
}
