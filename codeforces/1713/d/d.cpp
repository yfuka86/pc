...
void solve() {
  ...
  while(que.size() > 3) {
    vl t; rep(_, 4) {t.pb(que.front()); que.pop();}
    ll ans = 0;
    IQUERY({t[0], t[2]}, ans);
    if (ans == 0) {
      IQUERY({t[1], t[3]}, ans);
      if (ans == 1) que.push(t[1]); else que.push(t[3]);
    } else if (ans == 1) {
      IQUERY({t[0], t[3]}, ans);
      if (ans == 1) que.push(t[0]); else que.push(t[3]);
    } else {
      IQUERY({t[2], t[1]}, ans);
      if (ans == 1) que.push(t[2]); else que.push(t[1]);
    }
  }
  if (que.size() > 1) {
    vl t; rep(_, 2) {t.pb(que.front()); que.pop();}
    ll ans = 0;
    IQUERY({t[0], t[1]}, ans);
    if (ans == 1) que.push(t[0]); else que.push(t[1]);
  }
