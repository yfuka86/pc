...
string solve(ll n, ll k, vl x, vl c) {
  rep(i, k) if (x[i] < c[i]) return "";
  rep(i, k - 1) if (x[i+1] - x[i] < c[i+1] - c[i]) return "";
...
string naive(ll n, ll k, vl x, vl c) {
  return "";
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 10, k = 2;
    vl x = {3, 7}, C = {3, 5};
    auto so = solve(n, k, x, C); auto na = naive(n, k, x, C);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, x, C); debug(so); debug(na);
    if (check || (!check && c > loop)) break; }
  }
}

void solve() {
  LL(n, k); VL(x, k); VL(c, k);
  string ans = solve(n, k, x, c);
  if (ans.size() == 0) {
    OUT("NO");
  } else {
    OUT("YES");
