...
void solve() {
  ...
  rep_r(i, n) {
    ...
    if (!(sz[i] & 1)) { dp[i][0] = 1; dpr[i][0] = {i}; }

    if (p[i] != -1) {
      if (none_of(all(dp[p[i]]), [&](bool b) { return b; })) {
        dp[p[i]] = dp[i];
        dpr[p[i]] = dpr[i];
      } else {
        vb t(32); vvl tr(32);
        rep(j, 32) rep(k, 32) {
          if (dp[i][j] && dp[p[i]][k]) {
            vl op = dpr[p[i]][k];
            op.insert(op.end(), all(dpr[i][j]));

            if (t[j ^ k]) {
              if (tr[j ^ k].size() > op.size()) tr[j ^ k] = op;
            } else {
              t[j ^ k] = true;
              tr[j ^ k] = op;
            }
