...
bool build_aux_tree(HeavyLightDecomposition<Graph<ll>> &hld, vl& vs) {
  vlp v;
  ...
  map<ll, ll> G; stack<ll> st;
  st.push(v[0].second);
  rep2(i, 1, v.size()) {
    auto [_, next] = v[i];
    while (hld.up[st.top()] < hld.up[next]) st.pop();
    G[st.top()]++; G[next]++;
    if (G[st.top()] > 2 || G[next] > 2) return false;
    st.push(next);
  }
  return true;
}
...
void solve() {
  ...
  rep(i, q) {
    ll k; cin >> k;
    vlin(p, k, 1);
    if (build_aux_tree(hld, p)) cout << "YES" << "\n"; else cout << "NO" << "\n";
  }
}
