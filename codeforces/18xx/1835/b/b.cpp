...
LP solve(ll n, ll m, ll k, vl &a) {
  function<ll(ll)> calcl = [&](ll c) {
    ll left = lower_bound(all(a), c + 1) - a.begin();
    // leftが下側にある数
    if (left >= k) {
      return c - (a[left - k] + c) / 2;
    } else {
      return c + 1;
    }
  };

  function<ll(ll)> calcr = [&](ll c) {
    ll r = lower_bound(all(a), c) - a.begin();
    // ll right = n - r + 1;
    // rightが上側にある数
    if (r + k - 1 < n) {
      return (a[r + k - 1] + c - 1) / 2 - c;
    } else {
      return m - c;
    }
  };
  ...
  ll ans1 = 0, ans2 = 0;
  {
    ll t = calcl(0) + calcr(0);
    if (chmax(ans1, t)) ans2 = 0;
    else if (ans1 == t) chmin(ans2, 0);
  }
  rep(i, n) {
    rep(d, -2, 3) {
      ll cand = a[i] + d;
      if (cand < 0 || m < cand) continue;
      ll t = calcl(cand) + calcr(cand);
      if (chmax(ans1, t)) ans2 = cand;
      else if (ans1 == t) chmin(ans2, cand);
    }
  }
  return {ans1, ans2};
}

LP naive(ll n, ll m, ll k, vl &a) {
  ll ans1 = 0, ans2 = 0;
  rep(i, m + 1) {
    ll valid = 0;
    rep(target, m + 1) {
      ll diff = abs(target - i), cnt = 0;
      rep(i, n) {
        if (diff >= abs(a[i] - target)) cnt++;
      }
      if (cnt < k) valid++;
    }
    if (chmax(ans1, valid)) ans2 = i; else if (ans1 == valid) chmin(ans2, i);
  }
  return {ans1, ans2};
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 5, m = 5, k = 10;
    vl a = rg.vecl(n, 0, m + 1); sort(all(a));
    auto so = solve(n, m, k, a); auto na = naive(n, m, k, a);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, m, k, a); debug(so); debug(na);
