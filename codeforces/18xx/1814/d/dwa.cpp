...
void solve() {
  ...
  vlt p(n);
  rep(i, n) { p[i] = {f[i] * d[i], f[i], 0}; }
  sort(all(p));

  ll l = get<0>(p.front()), r = get<0>(p.back());
  ...
  // 減らしていく側
  {
    ...
    ll cnt = 0;
    while (get<0>(*prev(s.end())) >= l) {
      debug(s);
      ll left = get<0>(*s.begin());
      auto it = prev(s.end());
      auto [cur, f, st] = *it; s.erase(it);
      if (cur - left <= k) { chmin(ans, cnt); break; }
  ...
  // 増やしていく側
  {
    ...
    rep(i, n) s.insert(p[i]);
    ll cnt = 0;
    while (get<0>(*s.begin()) <= r) {
      debug(s);
      ll right = get<0>(*prev(s.end()));
      auto it = s.begin();
      auto [cur, f, st] = *it; s.erase(it);
      if (right - cur <= k) { chmin(ans, cnt); break; }

      ll d = right - cur;
      cur += d / f * f;
      if (right - cur > k) cur += f;
      if (!st) cnt++;
      s.insert({cur, f, 1});
    }
  }
