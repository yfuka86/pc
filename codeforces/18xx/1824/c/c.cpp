...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    if (v != 0 && G[v].size() == 1) { dp[v].insert(a[v]); dpc[v] = 0; return; }
    // 最大のセットを持ってくる
    ll most = -1;
    {
      ll ma = 0;
      fore(to, G[v]) { if (to == p) continue;
        dfs(to, v);
        dpc[v] += dpc[to];
        if (chmax(ma, dp[to].size())) most = to;
      }
    }
    swap(dp[v], dp[most]);
    ...
    ll dupma = 0;
    map<ll, ll> dup;
    fore(to, G[v]) { if (to == p || to == most) continue;
      dp[to].foreach([&](S x) {
        auto [it, b] = dp[v].insert(x);
        if (!b) { dup[x]++; chmax(dupma, dup[x]); }
      });
    }
    if (dupma) {
      dp[v].clear();
      fore(i, c, dup) if (dupma == c) dp[v].insert(i);
    }

    // 必要な操作回数
    dpc[v] += G[v].size() - (v != 0) - dupma - 1;
    // 全てにa[v]をxorする操作
    dp[v].apply(a[v]);
  };
  dfs(0, -1);
  // debug(dp[0].data);
  // debug(dpc);
  OUT(dpc[0] + (dp[0].find(0) == dp[0].end()));
}
