...
struct HeavyLightDecomposition {
 ...
 public:
  G& g; int id; vector<ll> size, depth, down, drev, up, nxt, par;
...
mint solve(ll n, ll k, Graph<ll> G) {
  if (k & 1) return 1;

  HeavyLightDecomposition<Graph<ll>> hld(G);

  mint ans = 0, all = comb(n, k);

  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    mint cur = 0;
    fore(to, G[v]) { if (to == p) continue;
      dfs(to, v);
      cur += comb(n - hld.size[to], k / 2) * comb(hld.size[to], k / 2);
    }
...
mint naive(ll n, ll k, Graph<ll> G) {
  if (k == 1) return 1;
  ...
  if (k == 2) {
    ...
    rep(i, n) rep(j, n) if (i != j) {
      ll mi = LINF, cnt = 0;
      rep(t, n) {
        ll tmp = hld.dist(t, i) + hld.dist(t, j);
        if (mi == tmp) cnt++; else if (chmin(mi, tmp)) cnt = 1;
      }
      ans += cnt;
    }
    return ans /= n * (n - 1);
    ...
    rep(i, n) rep(j, i) rep(k, j) {
      ll mi = LINF, cnt = 0;
      rep(t, n) {
        ll tmp = hld.dist(t, i) + hld.dist(t, j) + hld.dist(t, k);
        if (mi == tmp) cnt++; else if (chmin(mi, tmp)) cnt = 1;
      }
      ans += cnt;
    }
    return ans /= n * (n - 1) * (n - 2) / 6;
  }
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp tr = rand_tree(n);
    Graph<ll> G(n);
    fore(u, v, tr) G.add_edge(u, v);
    auto so = solve(n, k, G); auto na = naive(n, k, G);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, tr); debug(so); debug(na);
