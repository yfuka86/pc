...
ll solve(ll n, vl a) {
  ...
  rep(l, n) {
    BIT<ll> bt(n); bt.add(a[l], 1);
    ll cur = 0, t = 0;
    stack<LP> st;
    rep(r, l + 1, n) {
      ll diff = r - l - bt.sum(0, a[r]);
      if (diff > 0) {
        if (st.size() == 0 || st.top().se < r - diff) {
          cur += diff;
          st.push({r - diff, r});
        } else {
          ll left = LINF;
          while (st.size() && st.top().se >= r - diff) {
            chmin(left, st.top().fi);
            cur -= st.top().se - st.top().fi;
            st.pop();
          }
          st.push({min(r - diff, left), r});
          cur += r - min(r - diff, left);
        }
      }
      t += cur;
      // debug(l, r, rgs.ranges, cur);
      bt.add(a[r], 1);
    }
    ans += t;
  }
...
ll naive(ll n, vl a) {
  ll ans = 0;
  rep(l, n) rep(r, l + 2, n + 1) {
    Rngset rgs;
    vl t; rep(i, l, r) t.pb(a[i]);
    vl sorted = t; sort(all(sorted));
    ll mi = LINF;
    ...
    rep(S, 1 << n - 1) {
      ...
      // debug(rngs);
      rep(i, rngs.size() / 2) {
        // debug(rngs[i * 2], rngs[i * 2 + 1]);
        sort(tt.begin() + rngs[i * 2], tt.begin() + rngs[i * 2 + 1] + 1);
      }
      // debug(tt);
      if (tt == sorted) chmin(mi, __builtin_popcount(S));
    }
    if (mi == LINF) debug(l, r);
    ans += mi;
    // debug(l, r, rgs.ranges);
  }
  return ans;
}
