...
void solve() {
  ...
  RandGen rg;
  vmi depth(n); rep(i, n) depth[i] = mint(rg.l(0, mod));
  vmi th(n);

  function<void(ll, ll, ll)> dfsh = [&](ll v, ll p, ll d) {
    th[v] = 1;
    fore(to, G[v]) {
      if (to == p) continue;
      dfsh(to, v, d + 1);
      mint h = th[to] + depth[d];
      th[v] *= h;
    }
  };
  dfsh(0, -1, 0);

  function<bool(ll, ll)> dfs = [&](ll v, ll p) {
    map<ll, vl> mp;
    fore(to, G[v]) {
      if (to == p) continue;
      mp[th[to].x].pb(to);
    ...
    ll sym = 0, nx = -1;
    fore(_, v, mp) {
      if (v.size() & 1) { sym++; nx = v[0]; }
    }

    if (sym > 1) return false;
    else {
      if (sym == 0) return true; else return dfs(nx, v);
    }
  };
