...
ll naive(ll n, ll k, vl l, vl r) {
  ...
  rep(S, 1 << n) {
    ...
    if (sum >= k) {
      chmin(ans, r[last] + __builtin_popcount(S) * 2 - min(sum - k, r[last] - l[last] + 1));
      // debug(ans, S);
    }
  }
  if (ans == LINF) return -1; else return ans;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    auto so = solve(n, k, l, r); auto na = naive(n, k, l, r);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, l, r); debug(so); debug(na);
