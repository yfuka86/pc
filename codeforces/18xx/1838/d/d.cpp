...
void solve() {
  ...
  STR(s);
  vl a(n);
  rep(i, n) if (s[i] == '(') a[i] = 1; else a[i] = -1;
  vl ra = a;
  reverse(all(ra)); rep(i, n) ra[i] = -ra[i];
  vl aa = csum(a);
  vl raa = csum(ra);
  aa.erase(aa.begin()); raa.erase(raa.begin());

  // 括弧列の合計が偶数である <=> nが偶数
  // 前から見て0<になる前に'(('(連続の1)が存在する
  // 後から見て0<になる前に'))'(連続の1)が存在する

  set<ll> plus2, rplus2; plus2.insert(LINF); rplus2.insert(LINF);
  rep(i, n - 1) {
    if (a[i] + a[i + 1] == 2) plus2.insert(i);
    if (ra[i] + ra[i + 1] == 2) rplus2.insert(i);
  }
  lazy_segtree<S, op, e, F, mapping, composition, id> seg(aa), rseg(raa);

  rep(i, q) {
    LL(x); --x;
    if (n & 1) { OUT("NO"); continue; }

    {
      a[x] = -a[x];
      if (x > 0)  {
        if (a[x - 1] + a[x] == 2) plus2.insert(x - 1);
        else plus2.erase(x - 1);
      }
      if (x < n - 1) {
        if (a[x] + a[x + 1] == 2) plus2.insert(x);
        else plus2.erase(x);
      }
    ...
    }
    {
      ...
      ra[y] = -ra[y];
      if (y > 0)  {
        if (ra[y - 1] + ra[y] == 2) rplus2.insert(y - 1);
        else rplus2.erase(y - 1);
      }
      if (y < n - 1) {
        if (ra[y] + ra[y + 1] == 2) rplus2.insert(y);
        else rplus2.erase(y);
      }
    ...
    ll right = seg.max_right(0, [&](ll mi) {
      return mi >= 0;
    });
    ll right2 = rseg.max_right(0, [&](ll mi) {
      return mi >= 0;
    });
    // debug(right, right2);
    if (right < n) {
      if (*plus2.begin() > right - 2) valid = false;
    }
    if (right2 < n) {
      if (*rplus2.begin() > right2 - 2) valid = false;
    }
    YES(valid);
  }
}
