...
void solve() {
  ...
  rep(i, k) {
    LL(x1, y1, x2, y2);
    x1--; y1--; x2--; y2--;
    uf.unite(x1 * m + y1, x2 * m + y2);

    ll mix = min(x1, x2), miy = min(y1, y2);
    ll par = (mix + miy) & 1;
    if (y1 > y2) {
      if (d[x1][y2] == 1) valid = false;
      d[x1][y2] = 0 ^ par;
      byx[x1].pb(y2);
    } else {
      if (d[x1][y1] == 0) valid = false;
      d[x1][y1] = 1 ^ par;
      byx[x1].pb(y1);
    }
  }
  ...
  rep(i, n - 1) rep(j, m - 1) {
    vl t;
    t.pb(uf.root(i * m + j));
    t.pb(uf.root((i + 1) * m + j));
    t.pb(uf.root(i * m + j + 1));
    t.pb(uf.root((i + 1) * m + j + 1));
    uniq(t);
    if (t.size() <= 2) OUTRET("NO");
  }


  rep(i, n - 1) sort(all(byx[i]));

  map<LP, vl> ys;
  rep(i, n - 1) {
    rep(y1, byx[i].size()) rep(y2, y1 + 1, byx[i].size()) {
      ys[{byx[i][y1], byx[i][y2]}].pb(i);
    }
  }
  ...
  fore(yp, xv, ys) {
    auto [y1, y2] = yp;
    rep(i, xv.size() - 1) {
      ll x1 = xv[i], x2 = xv[i+1];
      ll sum = d[x1][y1] + d[x1][y2] + d[x2][y1] + d[x2][y2];
      debug(d[x1][y1], d[x1][y2], d[x2][y1], d[x2][y2]);
      debug(x1, x2, y1, y2, sum);
      if (sum & 1) OUTRET("NO");
    }
  }
  OUT("YES");
