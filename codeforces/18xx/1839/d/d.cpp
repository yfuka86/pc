...
void solve() {
  ...
  vv(ll, dp, n + 1, n + 1, -LINF);
  // dp[最後の位置][スキップ回数] = 最長増加部分列長
  dp[0][0] = 0;

  rep(i, n) {
    rep(j, n + 1) {
      if (i == 0 || a[i - 1] < a[i]) chmax(dp[i + 1][j], dp[i][j] + 1);
      rep(from, i + 1) {
        if ((from == 0 || a[from - 1] < a[i]) && j < n) chmax(dp[i + 1][j + 1], dp[from][j] + 1);
      }
    }
  }
  // debug(dp);
  ...
  rep(k, 1, n + 1) {
    ...
    ans.pb(min(ans.size() ? ans.back() : LINF, t));
  }
  OUTARRAY(ans);
}
