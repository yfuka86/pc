...
void solve() {
  ...
  segtree<S, op, e> seg(n+1), segr(n+1);
  rep(i, n+1) {
    if (hld.down[i] == -1) continue;
    seg.set(hld.down[i], S(a[i]));
    segr.set(hld.down[i], S(-a[i]));
  }

  fore(u, v, k, query) {
    S ma(0), mi(0);
    hld.path_noncommutative_query(u, v, true, [&](ll a, ll b){
      if (a > b) {
        swap(a, b);
        ma = op(ma, rev(seg.prod(a, b)));
      } else {
        ma = op(ma, seg.prod(a, b));
      }
    });
    hld.path_noncommutative_query(u, v, true, [&](ll a, ll b){
      if (a > b) {
        swap(a, b);
        mi = op(mi, rev(segr.prod(a, b)));
      } else {
        mi = op(mi, segr.prod(a, b));
      }
    });
    ll l = -mi.max, r = ma.max;
    // debug(l, r);
    YES(incl(k, l, r + 1));
  }
}
