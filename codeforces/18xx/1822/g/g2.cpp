...
void solve() {
  ...
  fore(num, c, mp) {
    ...
    rep(j, 2, 1001) { // 公比が1000以下の場合
      if (num < j * j) break;
      if (num % (j * j)) continue;
      if (mp.count(num / j) && mp.count(num / j / j)) {
        ans += c * mp[num / j] * mp[num / j / j];
      }
    }

    if (num <= THRES) { // 公比が1001以上
      auto it = mp.upper_bound(num * 1000);
      while (it != mp.end()) {
        if (it->fi % num == 0) {
          ll j = it->fi / num;
          if (it->fi * j > 1e9) break; // これがないとTLEする
          if (mp.count(it->fi * j)) {
            ans += c * it->se * mp[it->fi * j];
          }
        }
        it++;
      }
