...
void solve() {
  LL(n, m);
  if (m == 0) {
    if (n == 1) { OUT("FINITE"); OUT(1); OUT(1); } else OUT("INFINITE");
    return;
  }

  VEC2(ll, a, b, m);
  vvl g(n + 1);
  rep(i, m) g[b[i]].pb(a[i]);

  vl dp(n + 1, LINF); dp[1] = 1;
  queue<ll> que; que.push(1);
  while (!que.empty()) {
    ll v = que.front(); que.pop();
    fore(to, g[v]) {
      if (chmin(dp[to], dp[v] + 1)) que.push(to);
    }
  }
  rep(i, 1, n + 1) if (dp[i] == LINF) OUTRET("INFINITE");

  ll cur = 0;
  map<ll, ll> rf;
  vvl mp(n + 1); mp[1].pb(cur); rf[0] = 1; cur++;
  rep(i, 2, n + 1) {
    rep(_, dp[i]) {
      mp[i].pb(cur);
      rf[cur] = i;
  ...
  rep(i, 1, n + 1) {
    fore(to, g[i]) {
      rep(j, mp[to].size() - 1) {
        G.add_directed_edge(mp[to][j], mp[i][j]);
        G.add_directed_edge(mp[i][j], mp[to][j + 1]);
      }
    }
  }
