...
ll solve(ll n, vl a, ll l, ll r) {
  ll g = 0;
  fore(i, a) {
    if (i >= l + r || i < l) continue;
    g ^= (i / l);
  }
  return g;
}

ll naive(ll n, vl a, ll l, ll r) {
  vl dp(201, -1); dp[0] = 0;
  function<ll(ll)> dfs = [&](ll len) {
    if (dp[len] != -1) return dp[len];
    if (len < l) return dp[len] = 0;
    vl t;
    rep(tocut, l, r + 1) {
      rep(i, len - tocut + 1) {
        ll left = i, right = len - tocut - i;
        t.pb(dfs(left) ^ dfs(right));
      }
    }
    return dp[len] = mex(t);
  };
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    vl a(20); iota(all(a), 1);
    ll n = sum_of(a), l = rg.l(1, 10), r = rg.l(l + 1, 20);
    auto so = solve(n, a, l, r); auto na = naive(n, a, l, r);
    if (!check || !na != !so) { cout << c << "times tried" << "\n";
      debug(n, a, l, r); debug(so); debug(na);
...
void solve() {
  ...
  map<ll, ll> sz;
  rep(i, n) sz[uf.root(i)] = uf.size(i);
  vl a;
  fore(_, c, sz) a.pb(c);
  if (solve(n, a, l, r)) OUT("Alice"); else OUT("Bob");
}
