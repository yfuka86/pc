...
void solve() {
  ...
  v3(ll, dp, n + 1, k + 1, k + 1);
  rep(i, k + 1) dp[0][0][i] = 1;

  rep(i, n) {
    rep(able, k + 1) {
      // needが0になるパターン
      rep(j, able, k + 1) {
        dp[i + 1][able][0] += dp[i][j][able];
        dp[i + 1][able][0] %= p;
      }
      // needが1以上 => 最後のものと今回のもの（able）の差がneedになる
      rep(need, 1, able + 1) {
        dp[i + 1][able][need] += dp[i][able - need][able];
        dp[i + 1][able][need] %= p;
      }
    }

    rep(j, k + 1) rep(kk, k) {
      dp[i + 1][j][kk + 1] += dp[i + 1][j][kk];
      dp[i + 1][j][kk + 1] %= p;
    }
  }
  ll ans = 0;
