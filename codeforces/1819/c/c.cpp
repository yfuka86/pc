...
pair<bool, vl> solve(ll n, Graph<ll> &G) {
  ll deg = 0, st = -1;
  rep(i, n) if (chmax(deg, G[i].size())) st = i;

  vl path; path.pb(st);
  vb used(n); used[st] = 1;
  HeavyLightDecomposition<Graph<ll>> hld(G, st);

  auto con = [&](ll v) {
    assert(used[v] == 0);
    ll lastd = hld.depth[path.back()], vd = hld.depth[v];
    if (abs(lastd - vd) == 2 || (lastd == vd && hld.par[path.back()] == hld.par[v])) {
      path.pb(v); used[v] = 1;
    }
  };
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    ...
    if (!used[v]) { path.pb(v); used[v] = 1; }
  };

  dfs(st, -1);
  bool valid = true;
  rep(i, n) {
    if (hld.dist(path[i], path[(i+1) % n]) > 2) valid = false;
  }
  if (valid) return mp(true, path);

  // path.clear(); path.pb(st); path.pb(G[p.fi][0]);
  // used.assign(n, 0); used[p.fi] = 1; used[G[p.fi][0]] = 1;
  // dfs(p.fi, -1);
  // debug(path);
...
pair<bool, vl> naive(ll n, Graph<ll> &G) {
  HeavyLightDecomposition<Graph<ll>> hld(G);
  vl path(n); iota(all(path), 0);
  do {
    bool valid = true;
    rep(i, n) {
      if (hld.dist(path[i], path[(i+1) % n]) > 2) valid = false;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    vlp e = rand_tree(n);
    Graph<ll> G(n);
    rep(i, n - 1) G.add_edge(e[i].fi, e[i].se);
    auto so = solve(n, G); auto na = naive(n, G);
    if (!check || na.fi != so.fi) { cout << c << "times tried" << "\n";
      debug(n, e); debug(so); debug(na);
