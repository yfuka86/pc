...
vv(LP, xs, MA);
vv(LP, ys, MA);
vb xused(MA), yused(MA), used(MA);

vlp solve(ll n, vl x, vl y) {
  ll xma = 0, yma = 0;
  rep(i, n) { chmax(xma, x[i]); chmax(yma, y[i]); }
  ll S = 0;
  rep(i, n) S += x[i] * y[i];
  ...
  set<ll> xl, yl;
  rep(i, n) { xl.insert(x[i]); yl.insert(y[i]); }
  ...
  auto check = [&](ll a, ll b) {
    ...
    fore(xu, xuu) xused[xu] = 0;
    fore(yu, yuu) yused[yu] = 0;
    fore(u, uu) used[u] = 0;
    return valid;
  };
  ...
  fore(d, xl) {
    if (S % d) continue;
    ll a = d, b = S / d;
    if (a < xma || b < yma) continue;
    if (a > 1000000 || b > 1000000) continue;
    if (xs[d].size() == 0 && ys[S / d].size() == 0) continue;

    if (check(a, b)) ans.insert({a, b});
  }
  fore(d, yl) {
    if (S % d) continue;
    ll a = S / d, b = d;
    if (a < xma || b < yma) continue;
    if (a > 1000000 || b > 1000000) continue;
    if (xs[S / d].size() == 0 && ys[d].size() == 0) continue;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    auto so = solve(n, xs, ys);

    bool valid = true;
    if (so.size() == 0 || so.size() > 2) valid = false;
    else {
      bool any = false;
      fore(xx, yy, so) {
        if (xx == _x && yy == _y) any = true;
      }
      if (any == false) valid = false;
    }

    if (!check || !valid) { cout << c << "times tried" << "\n";
      debug(n, xs, ys); debug(so);
