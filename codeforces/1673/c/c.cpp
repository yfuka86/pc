...
signed main() {
  ...
  rep2(i, 1, 10) pal.pb(i);
  rep2(i, 1, 10) pal.pb(i * 11);
  rep2(i, 1, 10) rep(j, 10) pal.pb(i * 101 + j * 10);
  rep2(i, 1, 10) rep(j, 10) pal.pb(i * 1001 + j * 110);
  rep2(i, 1, 10) rep(j, 10) rep(k, 10) if (i * 10001 + j * 1010 + k * 100 <= 40000) pal.pb(i * 10001 + j * 1010 + k * 100);

  dp[0] = 1;
  for (auto &t: pal) {
    rep(i, 40000) {
      if (i + t <= 40000) dp[i + t] += dp[i];
    }
  }
