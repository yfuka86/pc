...
void solve() {
  ...
  rep(j, m) {
    rep(i, n) {
      ...
      if (i < n - 1 && g[i + 1][j] == '#') continue;
      if (i < n - 1 && dp[j][i + 1] != LINF) {
        if (chmin(dp[j + 1][i], dp[j][i + 1] + (g[i][j] != '#'))) from[j + 1][i] = i + 1;
      }
      if (i > 0 && dp[j][i - 1] != LINF) {
        if (chmin(dp[j + 1][i], dp[j][i - 1] + (g[i][j] != '#'))) from[j + 1][i] = i - 1;
      }
    }
  }
  ...
  ll i = min_element(all(dp[m])) - dp[m].begin();
  if (dp[m][i] != LINF) {
    OUT("YES");
    ll cur = i;
    rep_r(j, m) {
      g[cur][j] = '#';
      if (cur >= 0) cur = from[j + 1][cur];
    }
    rep(i, n) OUT(g[i]);
    return;
  }
