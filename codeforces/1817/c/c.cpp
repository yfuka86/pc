...
void solve() {
  ...
  VEC(mint, a, d + 1);
  VEC(mint, b, d + 1);
  // vmi xs(d + 1); iota(all(xs), 0);
  // debug(lagrange_polynomial(xs, a));
  // debug(lagrange_polynomial(xs, b));

  mint xda, xdb, xda2, xdb2;
  rep(i, d + 1) {
    mint deno = fac(i) * fac(d - i);
    if ((d - i) & 1) deno = -deno;
    xda += a[i] / deno;
    xdb += b[i] / deno;
  }
  ...
  // debug(xda, xdb, xda2, xdb2);
  // 問題の設定より最高次の係数は同じにならねばならない　すなわち xda = xdb
  // ax^d + bx^(d-1) == a(x+s)^d + c(x+s)^(d-1)　としてこれを展開すると、求めたいsはx^(d-1)の係数が等しくなるべきことより(b-c)/ad = sとわかる
  mint s = (xda2 - xdb2) / xda / d;
  OUT(-s);
}
