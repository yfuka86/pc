...
void solve() {
  ...
  mint ans = mint(2).pow(n) - n - 1;
  debug(ans);
  rep(l, 1, n) rep(r, l + 1, n - 1) {
    ll mid = x[r] - x[l];
    ll left = lower_bound(all(x), x[l] - mid) - x.begin(); // [leftまでに1つ以上選ばれていてあとは自由
    ll right = lower_bound(all(x), x[r] + mid) - x.begin(); // right)までに1つ以上選ばれていてあとは自由
    // if (l - left < 0 || right - r - 1 < 0) continue;
    mint lf = mint(2).pow(left) * (mint(2).pow(l - left) - 1);
    mint rf = mint(2).pow(n - right) * (mint(2).pow(right - r - 1) - 1);
    // debug(x[l], x[r], left, l, r, right);
    // debug(lf, rf);
    ans += lf * rf;
    // debug(ans);
  }
