...
void solve() {
  ...
  vl cost(n + 1, LINF); cost[n] = 0;
  vl from(n + 1, -1);

  set<ll> rem; rep(i, n) rem.insert(i);
  queue<ll> que;
  que.push(n);
  while(!que.empty()) {
    auto v = que.front(); que.pop();
    if (v == 0) break;
    auto lt = rem.lower_bound(v + b[v] - a[v + b[v]]);
    auto rt = rem.upper_bound(v + b[v]);
    while (lt != rt) {
      ll t = *lt;
      if (chmin(cost[t], cost[v] + 1)) {
        from[t] = v;
        que.push(t);
      }
      lt = rem.erase(lt);
  ...
  if (cost[0] == LINF) cout << -1 << "\n";
  else {
    cout << cost[0] << "\n";
    ll cur = 0; vl t; t.pb(0);
    while (from[cur] >= 0) {
      t.pb(from[cur]);
      cur = from[cur];
    }
    reverse(all(t)); t.erase(t.begin());
    coutarray(t);
  }
}
