...
void solve() {
  ll n, k; cin >> n >> k;
  vl x(n, 0); rep(i, k) { ll t; cin >> t; t--; x[t] = 1; }
  Graph<ll> G(n);
  rep(i, n - 1) {
    ll u, v; cin >> u >> v; u--; v--;
    G.add_edge(u, v);
  ...
  vl depth(n, -1);
  function<void(ll, ll, ll)> dep = [&](ll v, ll p, ll d) {
    depth[v] = d;
    for (auto to: G[v]) { if (to != p) dep(to, v, d + 1); }
  };
  dep(0, -1, 0);

  vl dp(n, LINF), fr(n, LINF);
  ...
  function<void(ll,ll)> dfs = [&](ll v, ll p) {
    ...
    if (sum > 0 && sum < LINF) dp[v] = sum;
    if (fr[v] <= depth[v]) dp[v] = 1;
    // cout << v << " " <<  dp[v] << " " <<  fr[v] << "\n";
  };
  dfs(0, -1);
  // coutarray(dp);
  // coutarray(fr);
  if (dp[0] > LINF / 2) cout << -1 << "\n"; else cout << dp[0] << "\n";
}
