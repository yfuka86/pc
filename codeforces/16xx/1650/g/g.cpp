...
void solve() {
  ...
  Graph<ll> G(n);
  rep(i, m) {
    LL(u, v); --u; --v; G.add_edge(u, v);
  }
  auto [co, from] = dijkstra_restore(G, t);
  ll dist = co[s];
  ll cur = s; vl path;
  while (cur != -1) {
    path.pb(cur);
  ...
  queue<LT> que;
  dp[s][0] = 1; que.push({s, 0, 0});
  while(!que.empty()) {
    auto [v, k, c] = que.front(); que.pop();
    if (done[v][k]) continue; else done[v][k] = 1;
    for(auto &to: G[v]) {
      if (c + 1 + co[to] == dist) {
        dp[to][0] += dp[v][k];
        que.push({to, 0, c + 1});
      } else if (c + 1 + co[to] == dist + 1) {
        dp[to][1] += dp[v][k];
        que.push({to, 1, c + 1});
      }
    }
