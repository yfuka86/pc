...
void solve() {
  ...
  ll l = x[0] - 1, m = x[0], r = x[0] + 1;
  bool lv = true, mv = true, rv = true;
  rep2(i, 1, n) {
    if (abs(x[i] - (l + i)) > 1) { lv = false; break; }
  }
  rep2(i, 1, n) {
    if (abs(x[i] - (m + i)) > 1) { mv = false; break; }
  }
  rep2(i, 1, n) {
    if (abs(x[i] - (r + i)) > 1) { rv = false; break; }
  }

  if (lv || mv || rv) cout << "YES" << "\n"; else cout << "NO" << "\n";
}
