...
void solve() {
  ...
  ll ansx = 0, ansy = 0;
  rep(i, com.size()) {
    rep(j, 4) if (com[i] == "DLUR"[j]) { sx += dx[j], sy += dy[j]; }
    chmin(xmi, sx), chmax(xma, sx), chmin(ymi, sy), chmax(yma, sy);
    // cout << xmi << " " << xma << " " << ymi << " " <<  yma << "\n";
    if (abs(xmi) + abs(xma) >= n || abs(ymi) + abs(yma) >= m) break;
    ansx = abs(xmi), ansy = abs(ymi);
  }
