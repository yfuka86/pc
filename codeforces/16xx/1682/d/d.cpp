...
void solve() {
  ...
  ll zero = 0;
  rep(i, n) {
    if (s[i] == '0') { mi += 2; zero++; } else mi++;
  }
  if (mi & 1 || zero > n - 2) { cout << "NO" << "\n"; return; }

  vlp ans;
  vlp connect;

  bool good = false;
  if (zero == n - 2) good = true;
  if (good && s[0] == '1' && s[n - 1] == '1') ans.pb({n - 2, n - 1});
  ...
  sort(all(connect));
  rep(i, connect.size()) {
    auto [e, flag] = connect[i];
    auto [e2, flag2] = connect[(i + 1) % connect.size()];
