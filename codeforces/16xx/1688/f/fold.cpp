...
struct S { ll a, b; }; using F = ll;
S op(S l, S r) { return S{l.a + r.a, l.b + r.b}; }
S e() { return S{0, 0}; }
S mapping(F f, S x) {
  if (f == -1) return x;
  else {
    if (f == 1) return { x.a + x.b, 0 }; else return { 0, x.a + x.b };
  }
}
...
void solve() {
  ...
  lazy_segtree<S, op, e, F, mapping, composition, id> aseg(as);
  lazy_segtree<S, op, e, F, mapping, composition, id> bseg(bs);
  ...
  while (rng.size()) {
    ...
    for (auto itr = rng.end(); itr != rng.begin();) {
      itr--;
      auto [r, l] = *itr; l = -l;
      if (aseg.prod(l, r).a + bseg.prod(l, r).a == csum[r] - csum[l]) {
        // cout << i << "applied" << "\n";
    ...
    for(auto t: remove) rng.erase(t);
    if (sz == rng.size()) break;
  }

  bool valid = true;
  rep(i, n + 1) {
    // cout << aseg.prod(0, i).a + bseg.prod(0, i).a << " " <<  csum[i] << "\n";
    if (aseg.prod(0, i).a + bseg.prod(0, i).a != csum[i]) { valid = false; break; }
  }
  if (valid) cout << "YES" << "\n"; else cout << "NO" << "\n";
}
