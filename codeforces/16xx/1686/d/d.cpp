...
void solve() {
  ...
  string s; cin >> s;
  ll n = s.size();
  ll acnt = a + c + d, bcnt = b + c + d;

  map<char, ll> freq;
  rep(i, n) freq[s[i]]++;

  if (freq['A'] != acnt || freq['B'] != bcnt) { cout << "NO" << "\n"; return; }

  map<string, ll> unit;
  ll cur = 0;
  rep(i, n - 1) {
    if (s[i] == s[i + 1]) {
      if (i - cur > 0) unit[s.substr(cur, i + 1 - cur)]++;
      cur = i + 1;
    } else if (i == n - 2) {
      unit[s.substr(cur)]++;
    }
  ...
  for (auto [str, cnt]: unit) {
    if (str.size() & 1) {
      ...
      if (str[0] == 'A') {
        rep(i, cnt) {
          if (c >= sz) c -= sz; else any += sz - 1;
        }
      } else {
        rep(i, cnt) {
          if (d >= sz) d -= sz; else any += sz - 1;
        }
      }
    }
  }
