...
void solve() {
  ...
  bool o_odd = false, o_even = false, e_odd = false, e_even = false;
  rep(i, n) {
    if (i & 1) {
      if (a[i] & 1) o_odd = true; else o_even = true;
    } else {
      if (a[i] & 1) e_odd = true; else e_even = true;
    }
  }
  if (o_odd && o_even) { cout << "NO" << "\n"; return; }
  if (e_odd && e_even) { cout << "NO" << "\n"; return; }
  cout << "YES" << "\n";
}
