...
void solve() {
  ...
  fore(_, v, xmp) sort(all(v));
  fore(_, v, ymp) sort(all(v));

  UnionFind uf(n);
  fore(_, v, xmp) rep(i, v.size() - 1) {
    if (v[i + 1].fi - v[i].fi <= k) uf.unite(v[i + 1].se, v[i].se);
  }
  fore(_, v, ymp) rep(i, v.size() - 1) {
    if (v[i + 1].fi - v[i].fi <= k) uf.unite(v[i + 1].se, v[i].se);
  }
  ...
  rep(i, n) {
    ll r = uf.root(i);
    if (!roots.count(r)) roots[r] = LINF;
    chmin(roots[r], tim[i]);
  }

  multiset<ll> comp; fore(_, t, roots) comp.insert(t);
  ll cur = 0;
  while (comp.size()) {
    while (comp.size() && *comp.begin() <= cur) comp.erase(comp.begin());
    if (comp.size()) comp.erase(prev(comp.end()));
