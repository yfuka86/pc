...
struct BinaryTrie{
  ...
  T kth_element(ll k){//下からk番目、0-indexed
    Node* pos = root;
    bit = rev;
    T val = 0;
    for(ll i=size-1;i>=0;i--){
...
int main(){
  ...
  BinaryTrie tri;
  for(auto &z:mp){
    vector<ll>add(1,z.se[0]);
    for(auto zz:z.se){
      tri.insert(zz);
      tri.xor_all(zz);
      ll mx=tri.max_element();
      //cout<<mx spa z.fi spa zz spa (mx^zz)<<endl;
      if((mx^zz)>=k)add={zz,mx};
      tri.xor_all(zz);
    }
    for(auto &zz:add)ret.PB(idx[zz]+1);
    for(auto zz:z.se){
      tri.erase(zz);
    }
  }
  if(ret.size()<2)cout<<-1<<endl;
  else{
    cout<<ret.size()<<endl;
    debug(ret);
  }
