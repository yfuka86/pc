...
bool check(ll n, ll i) {
  if (i <= 1) return false;
  if (n < i * (i + 1) / 2) return false;
  if (i & 1) { if (n % i == 0) return true; }
  else { if (n % i == i / 2) return true; }
  return false;
}
...
ll solve(ll n) {
  ...
  rep(i, 100) if (check(n, i)) return i;
  rep(p, 31) { ll i = POW(2, p); if (check(n, i)) return i; }

  auto f = factorize(n);
  // coutarray(f);
  for (auto i: f) {
    if (n < (__uint128_t)i * (i + 1) / 2) break;
