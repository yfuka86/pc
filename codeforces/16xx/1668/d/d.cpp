...
void solve() {
  ...
  segtree<S, op, e> seg1(cur), seg2(cur);
  seg1.set(rev[0], {0, 0});
  seg2.set(rev[0], {0, 0});

  map<ll, ll> lastv; lastv[0] = 0;

  rep(i, 1, n + 1) {
    ll mid = rev[i];
    ll ma = -LINF;
    S x = seg1.prod(0, mid);
    S y = seg2.prod(mid + 1, cur);

    if (lastv.count(as[i])) chmax(ma, lastv[as[i]]);
    ...
    chmax(ma, y.x - i);
    // debug(ma);
    if (lastv.count(as[i])) chmax(lastv[as[i]], ma); else lastv[as[i]] = ma;
    if (seg1.get(mid).x < ma - i) seg1.set(mid, {i, ma - i});
    if (seg2.get(mid).x < ma + i) seg2.set(mid, {i, ma + i});
  }
