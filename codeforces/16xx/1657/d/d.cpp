...
void solve() {
  ll n, C; cin >> n >> C;
  vl c(n), d(n), h(n); rep(i, n) cin >> c[i] >> d[i] >> h[i];
  ll m; cin >> m;
  vl D(m), H(m); rep(i, m) cin >> D[i] >> H[i];
  vl mx(C + 1, 0), mxa(C + 1, 0);

  rep(i, n) {
    if (c[i] <= C) {
      chmax(mx[c[i]], d[i] * h[i]);
    }
  }
  rep2(i, 1, C + 1) {
    for(ll j = i; j <= C; j += i) {
      chmax(mxa[j], mx[i] * j / i);
    }
  }
  // coutarray(mxa);
  segtree<S, op, e> seg(mxa);
  rep(i, m) {
    ll target = D[i] * H[i];
