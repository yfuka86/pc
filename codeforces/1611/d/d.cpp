...
void solve() {
  ll n; cin >> n;
  vl b(n), p(n); rep(i, n) { cin >> b[i]; b[i]--; } rep(i, n) { cin >> p[i]; p[i]--; }
  ll root;
  Graph<ll> G(n);
  rep(i, n) {
    if (i != b[i]) G.add_edge(b[i], i); else root = i;
  }
  ...
  rep(i, n) {
    if (p[i] == root) { ans[p[i]] = 0; continue; }

    if (ans[b[p[i]]] == -1) { cout << -1 << "\n"; return; }
    else {
      ans[p[i]] = max(cur + 1, ans[b[p[i]]] + 1);
      cur = ans[p[i]];
    }
  }

  rep(i, n) {
    cout << ans[i] - ans[b[i]] << (i == n - 1 ? "\n" : " ");
  }
}
