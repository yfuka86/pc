...
void solve() {
  ...
  vl used(n + 1); rep(i, n / 2) used[b[i]] = 1;
  set<ll> s; rep(i, 1, n + 1) if (!used[i]) s.insert(i);
  if (s.size() > n / 2) OUTRET(-1);

  vl a(n + 1);
  rep(i, 1, n + 1) {
    if (used[i]) a[i] = -1; else a[i] = 1;
  }
  vl as = csum(a); rep(_, 2) as.erase(as.begin());
  debug(as);
  lazy_segtree<S, op, e, F, mapping, composition, id> seg(as);
  if (seg.prod(0, n) < 0) OUTRET(-1);

  vl ans;
  rep(i, n / 2) {
    ll id = seg.min_left(b[i] - 1, [&](S x) {
      return x >= 1;
    });
    // debug(id);
    ans.pb(id + 1); ans.pb(b[i]);
    seg.apply(id, b[i] - 1, 1);
    // rep(i, n) {
    //   cout << seg.prod(i, i + 1) << " ";
    // } cout << "\n";
  }
