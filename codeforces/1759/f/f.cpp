...
void solve() {
  ...
  rep(i, 1, n + 1) {
    carried[i] += 1;
    if (carried[i] < p) break; else carried[i] = 0;
  }
  ...
  ll ans = LINF;
  {
    ...
    ll cur = a[0] - 1; if (cur < 0) cur += p;
    while (S.find(cur) != S.end() && cur != a[0]) {
      cur--;
      if (cur < 0) cur += p;
    }
    chmin(ans, (cur + p - a[0]) % p);
  }

  {
    set<ll> S; rep(i, n) S.insert(a[i]); rep(i, n + 1) S.insert(carried[i]);
    // debug(S);
    ll cur = a[0] - 1; // 1 ~ a[0] - 1まで必要
    while (S.find(cur) != S.end() && cur > 0) cur--;
    chmin(ans, tocarry + cur);
  }
