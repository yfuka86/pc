...
//   a = min(man(query[0], query[1], xx1, yy1), man(query[0], query[1], xx2, yy2));
// }
...
void solve() {
  ...
  // RandGen rg;
  // xx1 = rg.l(1, n + 1), yy1 = rg.l(1, m + 1), xx2 = rg.l(1, n + 1), yy2 = rg.l(1, m + 1);

  vl res(4);
  IQUERY({1ll, 1ll}, res[0]);
  IQUERY({n, 1ll}, res[1]);
  IQUERY({1ll, m}, res[2]);

  LP cand1, cand2;
  {
    ll y = res[0] + res[1] - n + 1;
    if (y & 1) cand1 = {1, 1};
    y = y / 2;
    cand1 = {res[0] - y + 1, y + 1};
  }
  ...
  if (incl(cand1.fi, 1, n + 1) && incl(cand1.se, 1, m + 1)) {
    IQUERY({cand1.fi, cand1.se}, res[3]);
    if (res[3] == 0) {
      IANSWER({cand1.fi, cand1.se});
    } else {
      IANSWER({cand2.fi, cand2.se});
    }
  } else IANSWER({cand2.fi, cand2.se});
