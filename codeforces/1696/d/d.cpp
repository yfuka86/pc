...
void solve() {
  ...
  segtree<S, miop, mie> segmi(a);
  segtree<S, maop, mae> segma(a);

  ll cur = 0, ans = 0;
  while (cur < n - 1) {
    ll mari = segma.max_right<function<bool(S)>>(cur, [&](S x) { return x <= a[cur]; });
    ll miidx = cur == mari ? cur : inv[segmi.prod(cur, mari)];
    ll miri = segmi.max_right<function<bool(S)>>(cur, [&](S x) { return x >= a[cur]; });
    ll maidx = cur == miri ? cur : inv[segma.prod(cur, miri)];
    // cout << mari << " " << miidx << " " << miri << " " << maidx << "\n";
    ans++;
    cur = max(cur + 1, max(miidx, maidx));
  }
  cout << ans << "\n";
}
