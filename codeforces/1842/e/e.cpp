...
struct S {ll idx, x;}; using F = ll;
S op(S l, S r) {
  if (l.idx * A + l.x <= r.idx * A + r.x) return l; else return r;
}
S e() { return {0, LINF}; }
S mapping(F f, S x) { x.x += f; return x; }
F composition(F f, F g) { return f + g; }
F id() {return 0;}

using S2 = ll; using F2 = ll;
S2 op2(S2 l, S2 r) {
  return min(l, r);
}
S2 e2() { return LINF; }
S2 mapping2(F2 f, S2 x) { return x + f; }
...
void solve() {
  ...
  lazy_segtree<S2, op2, e2, F2, mapping2, composition, id> seg2(v2);

  rep(x, k) {
    rep(i, ps[x].size()) {
      auto [y, cc] = ps[x][i];
      S p1 = seg.prod(y + 1, k - x);
      ll p2 = seg2.prod(k - x, k + 1);

      ll mi1 = p1.x + (p1.idx - y) * A;
      ll mi2 = p2 + (k - x - y) * A;
      ll nx = min(mi1, mi2);
      ...
      // if (x == 2 && y == 0) {
      //   debug(p1.x, mi1, p2, mi2, nx);
      // }

      seg.apply(y + 1, k + 1, cc);
      seg2.apply(y + 1, k + 1, cc);
    }
