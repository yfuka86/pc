...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    ...
    sort(all(t));
    if (t.size() > 1) chmin(mi, t[1]);
    dp[v] = {mi, (t.size() ? t[0] + 1: 1)};
  };
  dfs(0, -1);
  // debug(dp);

  vlp dp2(n, {LINF, 0});
  function<void(ll, ll, LP)> dfs2 = [&](ll v, ll p, LP re) {
    vlp mi, t;
    if (re.fi != -1) {
      mi.pb({re.fi, -1});
      t.pb({re.se, -1});
    ...
    sort(all(t));
    ll agg = mi[0].fi;
    if (t.size() > 1) chmin(agg, t[1].fi);
    dp2[v] = {agg, (t.size() ? t[0].fi + 1: 1)};
    ...
    fore(to, G[v]) {
      ...
      ll mmm = LINF;
      if (mm.size()) chmin(mmm, mm[0].fi);
      if (tt.size() > 1) chmin(mmm, tt[1].fi);
      LP nx = {mmm, (tt.size() ? tt[0].fi + 1: 1)};
      dfs2(to, v, nx);
    }
  };
  dfs2(0, -1, {-1, -1});
  ...
  ll ans = 0;
  rep(i, n) {
    chmax(ans, min(dp2[i].fi, dp2[i].se));
  }
  OUT(ans);
}
