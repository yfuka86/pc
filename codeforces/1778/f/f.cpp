...
void solve() {
  ...
  if (k == 0) OUTRET(a[0]);
  rep(i, n) a[i] = gcd(a[0], a[i]);

  auto pf = factorize(a[0]);
  vl ps; fore(p, _, pf) ps.pb(p);

  vl div = divisor(a[0] * a[0]);
  ll m = div.size();
  rep(i, m) divid[div[i]] = i;
  ...
  function<void(ll, ll)> dfs = [&](ll v, ll p) {
    ...
    rep_r(i, m) {
      if (gcd(a[v], div[i]) < div[i]) continue;
      // a[v]の約数そのままか、a[v] * div[i]を使うか(+1)
      bool valid = true;
      ll sum = 0, gc = gcd(a[v], div[i]);
      fore(to, G[v]) {
        if (to == p) continue;
        if (dp[to][i] >= LINF) { valid = false; break; }
        sum += dp[to][i];
      }
      if (valid) {
        chmin(dp[v][i], sum);

        // 遡るために部分木の全てという条件が必要だが、最後はa[0]だけでいいのでv==0で分岐
        ll d = divid[gcd(a[0] * a[0], (v == 0 ? a[v] : gc) * div[i])];
        chmin(dp[v][d], sum + 1);
      }
    }

    rep_r(i, m) fore(d, trans[i]) chmin(dp[v][d], dp[v][i]);
  };
  dfs(0, -1);
  rep(i, m) divid[div[i]] = -1;

  rep_r(i, m) {
    if (dp[0][i] <= k) OUTRET(div[i]);
  }
}
