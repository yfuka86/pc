...
set<ll> S; vi mi(300005, 0); set<ll> diff;
...
void solve() {
  ...
  vl ans(q);
  rep(i, q) {
    LL(l, r); --l; mo.insert(l, r);
  }

  auto down = [&](ll x) {
    if (--mi[x] == 0) { diff.erase(x); }
  };
  auto up = [&](ll x) {
    if (++mi[x] == 1) { diff.insert(x); }
  };
  auto add = [&](int i) {
    auto [it, _] = S.insert(a[i]);

    if (S.size() > 1) {
      auto lt = S.begin(), rt = prev(S.end());
      set<ll>::iterator nx, pr;
      if (it != lt) {
        pr = prev(it);
        up(a[i] - *pr);
      }
