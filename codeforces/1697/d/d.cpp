...
ll cnt = 0;
template<typename A> void query(vl q, A &a, string str = "? ") {
  string ans = "aabbccddeeffgghhiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz";
  vl v(q);
  if (v[0] == 1) {
    cnt++; cout << cnt << "\n";
    coutarray(v);
...
void solve() {
  ...
  auto f = [&](ll l, ll r) {
    ...
    if (ans[r - 1] != ' ') {
      map<char, ll> mp;
      for (int i = l; i < r; i++) mp[ans[i]]++;
      return dp[l][r] = mp.size();
    }

    iquery({2ll, l + 1, r}, dp[l][r]);
    return dp[l][r];
  };
  ...
  rep2(i, 1, n) {
    ll id = binary_search([&](ll mid) {
      auto [id, _] = li[mid];
      if (id == -1) return true;
      return f(id, i) == f(id, i + 1);
    }, 0, 26);

    if (li[id].fi == -1) {
      iquery({1ll, i + 1}, ans[i]);
      rep(c, 26) if (li[c].se == ans[i]) li[c].fi = i;
    } else {
      ans[i] = li[id].se;
      li[id].fi = i;
    }
