...
void solve() {
  ...
  vector<P> q(m);
  rep(i, m) {
    int t = read(), id = read(); id--;
    q[i] = {t, id};
  }

  vi freq(m, 0);
  rep(i, n) if (x[i] + y[i] < m) freq[x[i] + y[i]]++;

  int thres = sqrt_ceil(m) / 2;
  int thres2 = 1000000;
  vvi dp(thres + 1, vi(thres + 1, 0));
  vector<RangeAdd<ll>> mdp(m);
  rep(i, thres + 1) mdp[i] = RangeAdd<ll>(i + 1);
  rep(k, m) if (freq[k] > thres2) mdp[k] = RangeAdd<ll>(k + 1);
  ...
  rep(i, m) {
    ...
    auto f = [&](ll from, ll to, ll x) {
      auto &a = mdp[c];
      if (from < to) a.range_add(from, to, x);
      else {
        a.range_add(from, c + 1, x);
        a.range_add(0, to, x);
      }
    };
