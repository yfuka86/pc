...
string solve(ll n, string s) {
  ll first1; rep(i, n) if (s[i] == '1') { first1 = i; break; }
  s = s.substr(first1);
  n = s.size();

  vl zid; rep(i, n) if (s[i] == '0') zid.pb(i);
  if (zid.size() == 0) return s;
  if (zid.size() == n) return "0";

  ll ma = 0; vl cand;
  rep(i, zid.front()) { if (s[i] == '0') continue;
    ll off = zid.front() - i;
    ll cur = 0;
    fore(id, zid) {
      if (s[id - off] == '1') cur++; else break;
    }
    if (chmax(ma, cur)) {
      cand.clear();
      cand.pb(i);
...
string naive(ll n, string s) {
  ll first1 = -1; rep(i, n) if (s[i] == '1') { first1 = i; break; }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 10;
    string s = rg.strnum(n, 0, 2);
    auto so = solve(n, s); auto na = naive(n, s);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, s); debug(so); debug(na);
