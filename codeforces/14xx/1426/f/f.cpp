...
void solve() {
  ...
  vl freqa(n + 1, 0), freqc(n + 1, 0), freqany(n + 1, 0);
  rep(i, n) {
    freqa[i + 1] = freqa[i] + (s[i] == 'a' ? 1 : 0);
    freqc[i + 1] = freqc[i] + (s[i] == 'c' ? 1 : 0);
    freqany[i + 1] = freqany[i] + (s[i] == '?' ? 1 : 0);
  }

  mint ans = 0;
  rep(i, n) {
    if (s[i] == 'b' || s[i] == '?') {
      ans += mint(1) * freqa[i] * (freqc[n] - freqc[i + 1]) * mod_pow(3, freqany[n] - (s[i] == '?' ? 1 : 0));
      ans += mint(1) * freqa[i] * (freqany[n] - freqany[i + 1]) * mod_pow(3, freqany[n] - 1 - (s[i] == '?' ? 1 : 0));
      ans += mint(1) * freqany[i] * (freqc[n] - freqc[i + 1]) * mod_pow(3, freqany[n] - 1 - (s[i] == '?' ? 1 : 0));
      ans += mint(1) * freqany[i] * (freqany[n] - freqany[i + 1]) * mod_pow(3, freqany[n] - 2 - (s[i] == '?' ? 1 : 0));
    }
  }
  cout << ans << "\n";
}
