...
void solve() {
  ll n; cin >> n;
  vl ans(2001, -1);
  segtree<S, op, e> seg(ans);
  function<ll(ll)> q = [&](ll i) {
    if (seg.get(i) != -1) return seg.get(i);
    ll t = iquery(i); seg.set(i, t);
    return t;
  };
  ...
  ll ok = 4000000, ng = wmin - 1, wmax = 2000;
  rep2(i, 1, 2001) if (seg.get(i) > 0) chmin(ok, i * seg.get(i));
  ...
  while (ok > ng + 1) {
    ...
    rep2(i, wmin, wmax + 1) {
      ll h = mid / i;
      if (i > mid || seg.prod(i, 2001) > h) continue;
      else {
        h = q(i);
        if (mid >= h * i) { ok = h * i; valid = true; break; }
        if (ok > h * i) { ok = h * i; valid = true; break; }
      }
    }
    if (!valid) ng = mid;
  }
