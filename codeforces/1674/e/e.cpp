...
void solve() {
  ll n; cin >> n;
  vl a(n + 2, LINF); rep(i, n) cin >> a[i];

  vl sorted = a; sort(all(sorted));
  ll ok = ceil(sorted[0], 2) + ceil(sorted[1], 2), ng = 0;

  while (ok > ng + 1) {
    ll mid = (ok + ng) / 2;
    bool able = false;
    rep(i, n) {
      if (max(ceil(max(a[i], a[i + 1]), 2), ceil(a[i] + a[i + 1], 3)) <= mid) { able = true; break; }

      ll x = a[i], y = a[i + 2]; if (x < y) swap(x, y);
      if (ceil(x - y, 2) + y <= mid) { able = true; break; }
    }
    if (able) ok = mid; else ng = mid;
  }
