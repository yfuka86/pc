...
void solve() {
  ...
  rep(i, n) {
    diff[i] = a[i] - s[i];
    if (i < n - 1 && a[i] + 1 > a[i + 1]) negdiff[i + 1] = a[i] + 1 - a[i + 1];
  }
  debug(s, diff, negdiff);

  vector<S> data(n);
  rep(i,n) data[i] = {i, diff[i]};
  segtree<S, op, e> seg(data);
  segtree<S2, op2, e2> seg2(negdiff);
  ...
  rep(i, q) {
    LL(p, x); --p;
    ll off = 0;
    if (a[p] == x) { OUT(ans); continue; }

    if (a[p] < x) {
      ll d = min(p == 0 ? 1 : s[p - 1] + 1, x) - a[p];
      ll cur = p + 1;
      off += d;
      while (d > 0) {
        // debug(d, cur);
        ll id = seg.max_right(cur, [&](S x) {
          return x.x >= d;
        });
        // debug(id, cur, d);
        off += (id - cur) * d;
