...
void solve() {
  ...
  vl rem(n - 3); iota(rall(rem), 4);

  if (sum < ma * 2) coutret("NO");

  if (sum == ma * 2) {
    vl v;
    rep(i, n) {
      if (i == 0) v.pb(p.fi);
      else if (i == ma) v.pb(p.se);
      else if (id == 0 && i == a[2]) v.pb(3);
      else if (id == 1 && i == a[0]) v.pb(1);
      else if (id == 2 && i == a[1]) v.pb(2);
      else { if (rem.size() == 0) coutret("NO"); v.pb(rem.back()); rem.pop_back(); }
    }
    cout << "YES" << "\n";
    rep(i, n - 1) {
      cout << v[i] << " " << v[i + 1] << "\n";
    ...
    if ((sum - ma & 1) ^ (ma & 1)) coutret("NO");
    ll len = (sum - ma * 2) / 2;
    if (n < ma + len) coutret("NO");

    vl v; v.pb(p.fi);
    rep(i, ma - 1) { if (rem.size() == 0) coutret("NO"); v.pb(rem.back()); rem.pop_back(); }
    v.pb(p.se);

    ll from;
    if (id == 0) from = a[2] - len;
    if (id == 1) from = a[0] - len;
    if (id == 2) from = a[1] - len;
    // debug(from);
    vl v2; v2.pb(v[from]);
    rep(i, len - 1) { if (rem.size() == 0) coutret("NO"); v2.pb(rem.back()); rem.pop_back(); }
    if (id == 0) v2.pb(3);
    if (id == 1) v2.pb(1);
    if (id == 2) v2.pb(2);
    while (rem.size()) { v2.pb(rem.back()); rem.pop_back(); }

    cout << "YES" << "\n";
    rep(i, v.size() - 1) cout << v[i] << " " << v[i + 1] << "\n";
    rep(i, v2.size() - 1) cout << v2[i] << " " << v2[i + 1] << "\n";
  }
}
