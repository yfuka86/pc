...
vlp solve(ll n, vl _a) {
  auto maidx = [&](vl &a) {
    ll abma = 0, ret = -1;
    rep(i, n) {
      if (chmax(abma, abs(a[i]))) ret = i;
    }
    return ret;
  };

  ll mai = maidx(_a);
  if (n == 1 || mai == -1) return {}; //全部0 か n = 1


  rep(i, n - 1) rep(j, n - 1) {
    vlp ans;
    vl a = _a;
    if (a[n - 1] + a[i] + a[j] <= 0) continue;
    ans.pb({n - 1, i});
    a[n - 1] += a[i];
    ans.pb({n - 1, j});
    a[n - 1] += a[j];
    ...
    vl t; t.pb(a.back());
    rep(i, 30) t.pb(t.back() * 2);

    vvl op(30);
    rep(i, n - 2) {
      if (a[i] > a[i + 1]) {
        ll need = a[i] - a[i + 1];
        ll idx = lower_bound(all(t), need) - t.begin();
        a[i + 1] += t[idx];
        op[idx].pb(i + 1);
      }
    }
    ...
    while (ans.size() && ans.back() == mp((ll)n - 1, (ll)n - 1) && a[n - 1] / 2 >= a[n - 2]) {
      ans.pop_back();
      a[n - 1] /= 2;
    }
    if (ans.size() <= 31) {
  ...
  rep(i, 1, n) rep(j, 1, n) {
    ...
    while (ans.size() && ans.back() == mp(0ll, 0ll) && a[0] / 2 <= a[1]) {
      ans.pop_back();
      a[0] /= 2;
    }
    if (ans.size() <= 31) {
...
signed main() {
  ...
  while (t--) {
    ...
    if (ans.size() > 31 || !valid) debug(_a, a, ans);
