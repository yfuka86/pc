...
v4(ll, dp, 41, 40, 40, 100);
v4(LT, dpr, 41, 40, 40, 100, {-1, -1, -1});
...
void solve() {
  ...
  rep(i, n + 1) rep(j, a) rep(k, b) rep(l, m) {
    dp[i][j][k][l] = 0;
    dpr[i][j][k][l] = {-1, -1, -1};
  }
  ...
  rep(i, n) {
    ll d = s[i] - '0';
    rep(am, a) rep(bm, b) rep(diff, m) if (dp[i][am][bm][diff]) {
      ll na = (am * 10 + d) % a;
      ll nb = (bm * 10 + d) % b;
      dp[i + 1][na][bm][diff + 1] = 1;
      dpr[i + 1][na][bm][diff + 1] = {am, bm, diff}; // 赤

      dp[i + 1][am][nb][diff - 1] = 1;
      dpr[i + 1][am][nb][diff - 1] = {am, bm, diff}; // 黒
    }
  }

  rep(diff, n) {
    ll cur = n, acur = 0, bcur = 0, d = -1;
    if (dp[n][0][0][off - diff]) d = off - diff;
    if (dp[n][0][0][off + diff]) d = off + diff;
    if (d != -1) {
      string ans = "";
      while (get<0>(dpr[cur][acur][bcur][d]) != -1) {
        auto [anxt, bnxt, dnxt] = dpr[cur][acur][bcur][d];
        if (dnxt < d) ans.pb('R'); else ans.pb('B');
        acur = anxt; bcur = bnxt; d = dnxt;
        cur--;
      }
      reverse(all(ans));
      OUTRET(ans);
