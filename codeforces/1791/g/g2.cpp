...
void solve() {
  ...
  mpq<LT> que;
  rep(i, n) {
    que.push({i + 1 + a[i], i, 0});
  }
  rep(i, n) {
    que.push({n - i + a[i], i, 1});
  }

  vl used(n, -1);
  ll ans = 0;
  vlp costs;
  while (que.size() && get<0>(que.top()) <= c) {
    auto [co, i, t] = que.top(); que.pop();
  ...
  bool valid = false;
  rep(i, n) if (used[i] == 0) valid = true;
  if (valid) OUTRET(ans);
  ...
  rep(i, n) {
    if (used[i] == -1) {
      ll nc = i + 1 + a[i];
      chmax(ans2, ans - bt.lower_bound(nc - c));
    } else {
      ll nc = i + 1 - (n - i);
      if (nc <= c) OUTRET(ans);
      ll ii = costsidx[i];
      bt.add(ii, -costs[ii].fi);
      // debug(ii, costs[ii].fi);
      // どこかから捻出する
      ll need = bt.lower_bound(nc - c);
      if (need > ii) need--;
      chmax(ans2, ans - 1 - need);
      bt.add(ii, costs[ii].fi);
    }
    // debug(i, ans2);
  }
