...
void solve() {
  ...
  ll cnt1 = 0; rep(i, n) if (a[i]) cnt1++;
  if (n - cnt1 < cnt1) rep(i, n) a[i] = 1 - a[i];
  ...
  rep(i, n) {
    ll rem = n - i; ll jl = max(m - rem, 0);
    rep_r(j, jl, m) {
      ll cost = abs(balls[j] - i);
      rep_r(l, k - cost + 1) {
        if (l + cost <= k) dp[j + 1][l + cost] += dp[j][l];
      }
    }
  }
