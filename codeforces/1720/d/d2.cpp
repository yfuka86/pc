...
void solve() {
  ...
  rep(i, min(n * 30, 6000000)) rep(j, 2) trie[i][j] = -1;
  ll sz = 1;
  ...
  ll m = sz;
  vv(ll, dp, m, 2, 0); // dp[trienode][bit of a_i] = max length
  rep(i, n) {
    ll cur = 0;
    ll x = a[i] ^ i;
    ll ma = 0;
    rep_r(b, 30) {
      if (x & 1 << b) {
        if (trie[cur][0] != -1) chmax(ma, dp[trie[cur][0]][!(a[i] >> b & 1)]);
        cur = trie[cur][1];
      } else {
        if (trie[cur][1] != -1) chmax(ma, dp[trie[cur][1]][a[i] >> b & 1]);
        cur = trie[cur][0];
      }
    }
    ...
    cur = 0;
    rep_r(b, 30) {
      if (x & 1 << b) cur = trie[cur][1]; else cur = trie[cur][0];
      chmax(dp[cur][a[i] >> b & 1], ma + 1);
    }
  ...
  ll ans = 0;
  rep(i, m) rep(j, 2) chmax(ans, dp[i][j]);
  OUT(ans);
}
