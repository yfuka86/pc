...
struct S {ll x, ma, rma, lma;};
S op(S l, S r) {
  ll nx = l.x + r.x;
  ll nma = maxe(l.ma, r.ma, l.rma + r.lma);
  ll nrma = max(r.rma, r.x + l.rma);
  ll nlma = max(l.lma, l.x + r.lma);
  return {nx, nma, nrma, nlma};
}
S e() { return {0, 0, 0, 0}; }

ll solve(ll n, ll k, ll x, vl a) {
  segtree<S, op, e> seg(n);
  auto set = [&](ll i) {
    ll ma = max(a[i], 0);
    seg.set(i, {a[i], ma, ma, ma});
  };
...
ll naive(ll n, ll k, ll x, vl a) {
  ll ret = 0;
  enum_check(k, 0, n, [&](vl& ids) {
    if (ids[0] == ids[1]) return true;
    // debug(a, ids);
    vl at(a);
    rep(i, n) {
      at[i] -= x;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 4, k = 2, x = rg.l(-10, 10);
    vl a = rg.vecl(n, -10, 10);
    auto so = solve(n, k, x, a); auto na = naive(n, k, x, a);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, x, a); debug(so); debug(na);
