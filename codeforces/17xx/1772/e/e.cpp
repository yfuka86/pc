...
void solve() {
  ...
  ll need1 = 0, need2 = 0, needboth = 0;
  rep(i, n) {
    if (p[i] != i && p[i] != n - i - 1) needboth++;
    else {
      if (p[i] != i) need1++;
      if (p[i] != n - i - 1) need2++;
    }
  }
  // debug(need1, need2, needboth);

  if (need1 + needboth <= need2) OUTRET("First");
  if (need2 + needboth < need1) OUTRET("Second");
  OUT("Tie");
}
