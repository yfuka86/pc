...
struct S { ll v, id; };
S miop(S l, S r) { return l.v < r.v ? l : r; }
S mie() { return {LINF, -1}; }

S maop(S l, S r) { return l.v < r.v ? r : l; }
S mae() { return {-LINF, -1}; }
...
void solve() {
  ...
  segtree<S, miop, mie> miseg(v);
  segtree<S, maop, mae> maseg(v);

  ll ans = 0;

  function<void(ll, ll, bool)> dfs = [&](ll l, ll r, bool subt = false) {
    if (l >= r) return;
    S mi = miseg.prod(l, r);
    ll mival = mi.v;
    S ma = maseg.prod(l, r);
    ...
    if (mival == maval) {
      ll cnt = (r - l) * (r - l + 1) / 2;
      if (subt) ans -= cnt; else ans += cnt;
      return;
    }

    ll mr = max(mi.id, ma.id);
    ll ml = min(mi.id, ma.id);
    // debug(ml, mr);
    // debug(mival, maval);
    mr++;
    if (maval % mival == 0) {
      ll cnt = (r - mr + 1) * (ml - l + 1);
      if (subt) ans -= cnt; else ans += cnt;
    }

    dfs(l, mr - 1, subt); dfs(ml + 1, r, subt); dfs(ml + 1, mr - 1, !subt);
  };
  dfs(0, n, false);
  OUT(ans);
}
