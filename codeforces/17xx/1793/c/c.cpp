...
LP solve(ll n, vl p) {
  ...
  while (x < y && (pos[x] == l || pos[x] == r - 1 || pos[y] == l || pos[y] == r - 1)) {
    if (pos[x] == l) { x++; l++; continue; }
    if (pos[x] == r - 1) { x++; r--; continue; }
    if (pos[y] == l) { y--; l++; continue; }
    if (pos[y] == r - 1) { y--; r--; continue; }
  }

  if (l < r) {
    ll mi = *min_element(p.begin() + l, p.begin() + r);
    ll ma = *max_element(p.begin() + l, p.begin() + r);
    if (mi == p[l] || ma == p[l] || mi == p[r - 1] || ma == p[r - 1]) {
      return {-1, -1};
    } else {
      return {l + 1, r};
    }
