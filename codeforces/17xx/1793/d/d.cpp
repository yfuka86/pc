...
void solve() {
  LL(n); VL(p, n, 1); VL(q, n, 1);

  vl posp(n), posq(n);
  rep(i, n) posp[p[i]] = i;
  rep(i, n) posq[q[i]] = i;

  auto rng = [&](ll x) { return x * (x + 1) / 2; };

  ll ans = 0;

  ll x = posp[0], y = posq[0];
  if (x > y) swap(x, y);
  ans += rng(x - 0);
  ans += rng(y - x - 1);
  ans += rng(n - y - 1);
  ...
  rep(i, 1, n) {
    ll nl = -1, nr = n;
    bool valid = true;
    if (posp[i] < l) chmax(nl, posp[i]);
    else if (posp[i] > r) chmin(nr, posp[i]);
    else valid = false;

    if (posq[i] < l) chmax(nl, posq[i]);
    else if (posq[i] > r) chmin(nr, posq[i]);
    else valid = false;
