...
void solve() {
  ...
  vl dep(5000001);
  dep[1] = 0;
  rep(i, 2, 5000001) dep[i] = dep[tot[i]] + 1;

  BIT<ll> bt(n);
  rep(i, n) bt.add(i, dep[a[i]]);
  segtree<S, op, e> seg(a);

  set<ll> s;
  rep(i, n) if (a[i] > 1) s.insert(i);

  rep(i, m) {
    LL(t, l, r); --l;
    if (t == 1) {
      auto lt = s.lower_bound(l), rt = s.lower_bound(r);
      while (lt != rt) {
        a[*lt] = tot[a[*lt]];
        seg.set(*lt, a[*lt]);
        bt.add(*lt, -1);
        if (a[*lt] == 1) lt = s.erase(lt); else lt++;
      }
    } else {
      OUT(bt.sum(l, r) - dep[seg.prod(l, r)] * (r - l));
    }
  }
}
