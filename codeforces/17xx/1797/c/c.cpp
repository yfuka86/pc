...
// template<typename Q, typename A> void IQUERY(initializer_list<Q> _q, A &a, string str = "? ") {
//   vl q(_q);
//   a = max(abs(x - q[0]), abs(y - q[1]));
// }

void solve() {
  LL(n, m);
  // RandGen rg;
  // ll n = rg.l(10, 100), m = rg.l(10, 100);
  // ll x = rg.l(1, n + 1), y = rg.l(1, m + 1);

  ll a;
  IQUERY({1ll,1ll}, a);

  ll x = -1, y = -1;
  if (a + 1 <= m) IQUERY({1ll, a + 1}, x);
  if (a + 1 <= n) IQUERY({a + 1, 1ll}, y);
