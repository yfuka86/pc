...
void solve() {
  ...
  rep(i, n - 1) {
    LL(a, b); --a; --b;
    if (s[a] == s[b]) { dp[a][b] = 2; dp[b][a] = 2; } else { dp[a][b] = 1; dp[b][a] = 1; }
    G.add_edge(a, b);
  }
  ...
  function<ll(ll, ll)> dfs = [&](ll a, ll b) {
    ...
    ll na, nb;
    if (lc == a) {
      na = hld.la(b, hld.depth[a] + 1);
      nb = hld.la(b, hld.depth[b] - 1);
    } else if (lc == b) {
      nb = hld.la(a, hld.depth[b] + 1);
      na = hld.la(a, hld.depth[a] - 1);
    } else {
      nb = hld.la(b, hld.depth[b] - 1);
      na = hld.la(a, hld.depth[a] - 1);
    }
    ll ma = 0;
    chmax(ma, (dfs(na, nb) + (s[a] == s[b] ? 2 : 0)));
    chmax(ma, dfs(na, b));
    chmax(ma, dfs(a, nb));
    return dp[a][b] = dp[b][a] = ma;
  };
