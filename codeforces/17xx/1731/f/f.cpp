...
void solve() {
  ...
  rep(x, 2, n + 2) { // k（種類数)
    rep(i, n) rep(j, 1, x + 1) { // a_iがjである時
      ll l = i, r = n - i - 1;
      vmi a(l + 1);
      rep(mi, l + 1) { // 小さい側の場合の数前計算
        a[mi] = mint(j - 1).pow(mi) * mint(x - j + 1).pow(l - mi) * comb(l, mi); // 小さいもの
      }
      vmi as = csum(a);
      rep(ma, r + 1) { // 大きい側 mi < ma となる前者の累積和を掛け算
        mint t = mint(x - j).pow(ma) * mint(j).pow(r - ma) * comb(r, ma) * as[min(ma, as.size() - 1)];
        f[x] += t * j;
      }
    }
  }
