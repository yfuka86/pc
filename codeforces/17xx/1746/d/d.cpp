...
void solve() {
  ...
  function<LP(ll, ll, ll)> dfs = [&](ll v, ll p, ll cnt) {
    if (v != 0 && G[v].size() == 1) {
      return mp(cnt * s[v], (cnt + 1) * s[v]);
    }
    ll sum = s[v] * cnt;
    ll sz = G[v].size() - 1; if (v == 0) sz++;
    ll ncnt = cnt / sz, rem = cnt % sz;

    vl t;
    fore(to, G[v]) {
      if (to == p) continue;
      auto [a, b] = dfs(to, v, ncnt);
      sum += a;
      t.pb(b - a);
    }
    sort(rall(t));
