...
void solve() {
  ...
  ll mi = LINF;
  rep(i, min(TH, n - 1)) {
    fore(to, cs) chmin(mi, hld.dist(c[i], to));
    ans[i] = mi;
    cs.pb(c[i]);
  }
  ...
  if (TH < n - 1) {
    vl cost(n, LINF);
    queue<LT> que; que.push({c0, 0, -1});
    rep(i, TH) que.push({c[i], 0, -1});
    while (que.size()) {
      auto [v, co, p] = que.front(); que.pop();
      fore(to, G[v]) {
        if (to == p) continue;
        if (chmin(cost[to], co + 1)) {
          que.push({to, co + 1, v});
        }
      }
    }
    chmin(mi, cost[c0]);
    rep(i, TH) chmin(mi, cost[c[i]]);

    function<void(ll, ll)> dfs = [&](ll v, ll d) {
      if (d > mi) return;
      fore(to, G[v]) {
        if (chmin(cost[to], d + 1)) dfs(to, d + 1);
      }
    };

    rep(i, TH, n - 1) {
      chmin(mi, cost[c[i]]); ans[i] = mi;
      if (mi == 1) {
        rep(j, i, n - 1) ans[j] = mi;
        break;
      }
      cost[c[i]] = 0;
      dfs(c[i], 0);
