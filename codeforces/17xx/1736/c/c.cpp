...
void solve() {
  ...
  vl ad(n); rep(i, n) ad[i] = a[i] - i - 1;
  segtree<S, op, e> seg(ad);

  // dpはそこまででどれだけ無効な区間があるか、rdpはそこからどれだけ無効な区間があるか
  vl dp(n + 1, 0), rdp(n + 1, 0);
  ll cur = 0;
  rep(i, n) {
    chmin(cur, ad[i]);
    dp[i + 1] = dp[i] - cur;
  ...
  rep_r(i, n) {
    ll l = seg.max_right(i, [&](S x) {
      return ad[i] <= x;
    });
    rdp[i] = rdp[l] + (0 - min(0, ad[i])) * (l - i);
  }
  debug(ad);
  debug(dp);
  debug(rdp);
  ...
  rep(i, q) {
    LL(p, x); --p;
    ll thres = min(seg.prod(0, p), ad[p] + x - a[p]);
    ll r = seg.max_right(p + 1, [&](S x) { return thres <= x; });
    ll sum = dp[p] - min(0, thres)*(r - p) + rdp[r];
    OUT(all - sum);
  }
}
