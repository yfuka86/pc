...
void solve() {
  ...
  rep(i, n) {
    while (1) {
      if (a[i] % 10 == 0 || a[i] % 10 == 2) break;
      a[i] += a[i] % 10;
    }
  }
  // debug(a);

  set<ll> s; rep(i, n) s.insert(a[i] % 10);
  if (s.size() == 2) coutret("No");

  if (*s.begin() == 0) {
    rep(i, n - 1) if (a[i] != a[i + 1]) coutret("No");
    coutret("Yes");
  } else {
    vb eo(2);
    rep(i, n) eo[a[i] / 10 & 1] = 1;
    if (eo[0] && eo[1]) coutret("No");
    coutret("Yes");
  }
}
