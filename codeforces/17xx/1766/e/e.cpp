  ...
  if (0) {
    ...
    rep(l, n) rep(r, l + 1, n + 1) {
      ...
      rep(i, l, r) {
        bool done = false;
        fore(_v, v) {
          if (_v.back() & a[i]) { done = true; _v.pb(a[i]); break; }
        }
        if (!done) v.pb({a[i]});
      }
      aa += v.size();
      vvl t; vl suf; rep(i, v.size()) if (v[i][0] != 0) { t.pb(v[i]); suf.pb(v[i].back()); }
      sta.insert(suf);
      // debug(l, r, t);
    }
    debug(sta.size());
  ...
  rep(i, n) {
    if (a[i] == 0) {
      ...
      fore(_v, cnt, dp) {
        ...
        bool done = false;
        rep(j, v.size()) {
          if (v[j] & a[i]) { done = true; v[j] = a[i]; break; }
        }
        if (!done) { v.pb(a[i]); ans += cnt * (n - i); }
        dpt[v] += cnt;
      }
      ans += (n - i) * (1 + cur);
      dpt[{a[i]}] += 1 + cur;
