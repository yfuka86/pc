...
void solve() {
  ...
  rep(i, m) {
    ll x, y; cin >> x >> y;
    ll gc = gcd(x, y);
    if (n % gc) { ans[i] = -1; continue; }
    auto [p, q] = extGCD(x, y);
    ll c = n / gc; p *= c; q *= c;
    ll punit = y / gc, qunit = x / gc;
    ...
    // debug(p, punit, q, qunit);
    // pスタートmod punitのものを走査
    if (0 <= p * x && 0 <= q) query.pb({p * x, punit * x, i}); else ans[i] = -1;
  }

  map<LP, ll> mp;
  for (auto [p, mop, id]: query) {
    if (mp[{p, mop}]) ans[id] = mp[{p, mop}];
    else {
      ll t = 0;
      // debug(p, mop, id);
      for (ll j = p; j <= n; j += mop) {
        chmax(t, sc[j]);
      }
      ans[id] = mp[{p, mop}] = t;
    }
