...
void solve() {
  ...
  v3l dp(s.size(), vvl(s.size() + 1, vl(2, -1)));

  function<ll(ll, ll, ll)> dfs = [&](ll l, ll r, ll right) {
    if (dp[l][r][right] != -1) return dp[l][r][right];
    if (r - l == 2) {
      if (s[l] == s[l + 1]) return dp[l][r][right] = 0;
      else return dp[l][r][right] = 1;
    }

    if (r - l & 1) {
      ll mi = 1;
      if (right) {
        if (s[r] == s[r - 1]) chmin(mi, dfs(l, r - 1, 1));
        if (s[r] == s[l]) chmin(mi, dfs(l + 1, r, 0));
        return dp[l][r][right] = mi;
      } else {
        if (s[l - 1] == s[r - 1]) chmin(mi, dfs(l, r - 1, 1));
        if (s[l - 1] == s[l]) chmin(mi, dfs(l + 1, r, 0));
        return dp[l][r][right] = mi;
      }
    } else {
      return dp[l][r][right] = max(dfs(l + 1, r, 0), dfs(l, r - 1, 1));
    }
  };
  ll ans = dfs(0, s.size(), 0);
  if (ans) cout << "Alice" << "\n"; else cout << "Draw" << "\n";
}
