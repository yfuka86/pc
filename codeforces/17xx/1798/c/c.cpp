...
using S = ll;
S gop(S l, S r) { return gcd(l, r); }
S ge() { return 0; }

S lop(S l, S r) {
  if (l == LINF || r == LINF) return LINF;
  return lcm(l, r) > (ll)1e13 ? LINF : lcm(l, r);
}
S le() { return 1; }
...
void solve() {
  ...
  segtree<S, gop, ge> segg(c);
  segtree<S, lop, le> segl(b);

  lazy_segtree<S, op, e, F, mapping, composition, id> dp(n + 1); dp.set(0, 0);

  rep(i, n) {
    ll r = binary_search([&](ll mid) {
      if (mid > n) return false;
      ll gc = segg.prod(i, mid);
      ll lc = segl.prod(i, mid);
      return gc % lc == 0;
    }, i + 1, n + 1);
    // debug(i, r);
    dp.apply(i + 1, i + 2, dp.get(i) + 1);
    dp.apply(i + 1, r + 1, dp.get(i) + 1);
  }
