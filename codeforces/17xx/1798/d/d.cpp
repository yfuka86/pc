...
vl solve(ll n, vl a) {
  ll thres = *max_element(all(a)) - *min_element(all(a));

  vl ans;
  multiset<ll> pos, neg;
  rep(i, n) if (a[i] > 0) pos.insert(a[i]); else if (a[i] < 0) neg.insert(a[i]); else ans.pb(a[i]);

  ll cur = 0, mi = 0, ma = 0;
  auto upd = [&](ll x) {
    chmin(mi, x); chmax(ma, x);
  };
  while (pos.size()) {
    ll nx = *prev(pos.end());
    while (cur + nx - mi >= thres) {
      cur += *neg.begin();
      ans.pb(*neg.begin());
      neg.erase(neg.begin());
      upd(cur);
...
vl naive(ll n, vl a) {
  ...
  do {
    ...
    rep(i, n) {
      cur += a[i];
      chmin(mi, cur); chmax(ma, cur);
    }
    if (thres > ma - mi) return a;
