...
void solve() {
  ...
  v3(ll, dp, 105, 105, 2, -1);

  function<ll(ll, ll, ll)> dfs = [&](ll o, ll e, ll p) {
    if (o + e == 0) { return abs(1 - p); }
    if (dp[o][e][p] != -1) return dp[o][e][p];
    if ((n - o - e) & 1) {
      // bob
      bool win = true;
      if (o > 0) if (!dfs(o - 1, e, p)) win = false;
      if (e > 0) if (!dfs(o, e - 1, p)) win = false;
      return dp[o][e][p] = win;
    } else {
      // alice
      bool win = false;
      if (o > 0) win |= dfs(o - 1, e, p ^ 1);
      if (e > 0) win |= dfs(o, e - 1, p);
      return dp[o][e][p] = win;
    }
  };
  // debug(odd, even, 0);
  bool ans = dfs(odd, even, 0);
  OUT(ans ? "Alice" : "Bob");
}
