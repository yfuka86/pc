...
struct S {ll x, p;};
S op(S l, S r) {
  if (l.p >= r.p) return l; else return r;
}
S lop(S l, S r) {
  if (l.p + (r.x - l.x) >= r.p) return l;
  else return r;
}
S rop(S l, S r) {
  if (r.p + (r.x - l.x) >= l.p) return r;
  else return l;
}
S e() { return {0ll, -LINF}; }
...
void solve() {
  ...
  map<ll, ll> point;
  {
    ...
    ll cur = -LINF, a = 0, cum = 0;
    for (auto [from, to, til] :ev) {
      while (!que.empty() && que.top().fi <= from) {
        auto [to2, til] = que.top(); que.pop();
        cum += (to2 - cur) * a;
        point[to2] = cum;
        cur = to2;
        a -= til;
  ...
  segtree<S, lop, e> lseg(d);
  segtree<S, rop, e> rseg(d);

  string ans = "";

  rep(i, n) {
    ll id = lbs(data, {x[i], -LINF});
    ll li = lbs(data, {from[i], -LINF});
    ll ri = ubs(data, {to[i], LINF});

    ll ma = 0;
    S l = lseg.prod(li, id + 1);
    chmax(ma, l.p - (p[i] - (x[i] - l.x)));
    S r = rseg.prod(id, ri);
    chmax(ma, r.p - (p[i] - (r.x - x[i])));

    if (li != 0) chmax(ma, seg.prod(0, li).p);
    if (ri != data.size()) chmax(ma, seg.prod(ri, data.size()).p);

    // if (i == 1) {
    //   debug(from[i], x[i], to[i]);
    //   debug(li, id, ri);
    //   debug(l.x, l.p);
    //   debug(r.x, r.p);
    //   debug(ma);
    // }
    if (ma <= m) ans.pb('1'); else ans.pb('0');
  }
  cout << ans << "\n";
}
