...
void solve() {
  ...
  vlt p; // 行きがけ順 / 頂点 / 1, 2のどちらか
  rep(i, m1) p.pb({hld.down[a1[i]], a1[i], 0});
  rep(i, m2) p.pb({hld.down[a2[i]], a2[i], 1});
  sort(all(p));

  function<int(ll, ll)> f = [&](ll target, ll from) {
    ll need = hld.dist(target, from) - d;
    // debug(need);
    if (need <= 0) return (int)from;
    ll lc = hld.lca(target, from);
    if (lc == target) {
      return hld.la(from, hld.depth[lc] + need);
    } else if (lc == from) {
      return hld.la(target, hld.depth[lc] + need);
    } else {
      ll up = hld.depth[from] - hld.depth[lc];
      if (up > need) {
        return hld.la(from, hld.depth[from] - need);
      } else {
        need -= up;
        return hld.la(target, hld.depth[lc] + need);
      }
  ...
  fore(_, v, t, p) {
    if (t == 0) {
      ...
      cur1 = v;
      ll to = f(cur1, cur2);
      ans += hld.dist(cur2, to);
      cur2 = to;
    } else {
      ans += hld.dist(cur2, v);
      cur2 = v;
      ll to = f(cur2, cur1);
      ans += hld.dist(cur1, to);
      cur1 = to;
    }
    // debug(v, t, cur1, cur2, ans);
  }
  ans += hld.dist(cur1, 0);
  ans += hld.dist(cur2, 0);
  OUT(ans);
