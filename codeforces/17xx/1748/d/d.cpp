...
void solve() {
  ...
  if (0) {
    d = 6;
    rep(i, 1, 30) rep(j, i + 1, 30) rep(k, 1, 100) {
      if ((i | k) % d == 0 && (j | k) % d == 0) {
        debug(i, j, k);
        break;
      }
  ...
  ll unit = 1 << 30;
  auto [p, q] = extGCD(unit, d);

  ll rem = d - c % d;
  if (rem % gcd(unit, d)) OUTRET(-1);

  ll need = rem / gcd(unit, d);
  // p * need * unit はオーバーフローする
  ll coef = p * need;
  if (coef < 0) coef += ceil(abs(coef), d) * d;
  coef -= coef / d * d;
