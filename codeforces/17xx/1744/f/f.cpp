...
void solve() {
  ...
  vb mexa(n + 1); mexa[0] = 1;
  ll me = 1;
  ll l = rev[0], r = l + 1;

  ll ans = 1; // 0のみ
  { // mexが1の区間をカウント
    if (rev[0] > 0 && p[rev[0] - 1] != 1) ans++;
    if (rev[0] < n - 1 && p[rev[0] + 1] != 1) ans++;
  }
  // debug(me, ans);
  while(me < n) {
    ll id = rev[me];
    ...
    ll sz = r - l;
    ll roomr = min(n - r, me * 2 - sz);
    if (me < n && rev[me] >= r) chmin(roomr, rev[me] - r);
    ll rooml = min(l, me * 2 - sz);
    if (me < n && rev[me] < l) chmin(rooml, l - rev[me] - 1);

    if (rooml > roomr) swap(rooml, roomr);
    ll t = 0;
    rep(i, rooml + 1) {
      t += min(me * 2 - sz - i, roomr) + 1;
    }
    ans += t;
    // debug(me, ans);
  }
  ...
  if (0) {
    ...
    rep(l, n) rep(r, l + 1, n + 1) {
      vl _t(p.begin() + l, p.begin() + r);
      vl t = _t;
      sort(all(t));
      ll md = t[(t.size() + 1) / 2 - 1];
      ll mx = mex(t);
      if (mx > md) {
        cnt++;
        debug(_t);
