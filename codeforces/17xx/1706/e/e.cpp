...
void solve() {
  ...
  while(1) {
    // {mid, index}
    mpq<LP> pq;
    rep(i, n - 1) if (ans[i].fi > ans[i].se + 1) pq.push({(ans[i].fi + ans[i].se) / 2, i});
    if (pq.size() == 0) break;
    ...
    rep(i, m) {
      ...
      while (!pq.empty() && pq.top().fi <= i + 1) {
        auto [mid, id] = pq.top(); pq.pop();
        if (uf.same(id, id + 1)) ans[id].fi = mid; else ans[id].se = mid;
      }
    }
  }

  segtree<S, op, e> seg(n - 1);
  rep(i, n - 1) seg.set(i, ans[i].fi);

  vl aans(q);
  rep(i, q) {
    LL(l, r); --l; --r;
    if (l == r) aans[i] = 0;
    else aans[i] = seg.prod(l, r);
  }
  OUTARRAY(aans);
}
