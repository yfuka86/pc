...
struct S {ll s, sz;}; using F = ll;
S op(S l, S r) { return {l.s + r.s, l.sz + r.sz}; }
S e() { return {0, 1}; }
S mapping(F f, S x) { return {x.s + f * x.sz, x.sz}; }
F composition(F f, F g) { return f + g; }
F id() { return 0; }

void solve() {
  LL(n); VEC3(ll, l, r, c, n);
  rep(i, n) { ++r[i]; --c[i]; }

  vl cp; cp.pb(-LINF); cp.pb(-LINF + 1); cp.pb(LINF); cp.pb(LINF + 1);
  rep(i, n) { cp.pb(l[i]); cp.pb(r[i]); }
  auto [cmp, rcmp] = compmap(cp);

  lazy_segtree<S, op, e, F, mapping, composition, id> seg(n * 2 + 10);
  vvlt rng(n);
  rep(i, n) {
    rng[c[i]].pb({cmp[l[i]], cmp[r[i]], i});
    seg.apply(cmp[l[i]], cmp[r[i]], 1);
  }
  seg.apply(cmp[-LINF], cmp[-LINF + 1], 1);
  seg.apply(cmp[LINF], cmp[LINF + 1], 1);

  vl ans(n, LINF);
  rep(i, n) {
    fore(l, r, _, rng[i]) seg.apply(l, r, -1);
    fore(l, r, id, rng[i]) {
      if (seg.prod(l, r).s != 0) { ans[id] = 0; continue; }
      ll rid = seg.max_right(r, [&](S x) {
        return x.s == 0;
      });
      ll lid = seg.min_left(l, [&](S x) {
        return x.s == 0;
      });

      chmin(ans[id], rcmp[rid] - rcmp[r] + 1);
      chmin(ans[id], rcmp[l] - rcmp[lid] + 1);
    }
