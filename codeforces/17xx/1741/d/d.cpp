...
void solve() {
  ...
  if (n == 1) OUTRET(0);
  bool valid = true; ll cnt = 0;

  function<void(ll, ll, ll)> dfs = [&](ll l, ll r, ll off) {
    if (!valid) return;
    if (r - l <= 1) return;
    ll m = (r - l) / 2;
    // debug(l, r, off, m);
    if (p[l] <= off + m && off + m < p[l + m] && p[l + m] <= off + r - l) {
      dfs(l, l + m, off);
      dfs(l + m, r, off + m);
    } else if (p[l + m] <= off + m && off + m < p[l] && p[l] <= off + r - l) {
      cnt++;
      dfs(l + m, r, off);
      dfs(l, l + m, off + m);
    } else {
