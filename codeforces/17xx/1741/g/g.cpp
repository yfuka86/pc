...
void solve() {
  ...
  unordered_map<ll, vl> cost;
  cost[0] = dijkstra(G, 0);
  rep(i, k) cost[h[p[i]]] = dijkstra(G, h[p[i]]);
  ...
  rep(i, f) {
    ...
    rep(S, 1 << k) {
      vl v; v.pb(h[i]);
      rep(j, k) if (S & 1 << j) v.pb(h[p[j]]);
      uniq(v);

      bool valid = true;
      ll cur = 0, sum = 0;
      while (v.size()) {
        if (cur == h[i]) { valid = false; break; }
        vl t;
        fore(vv, v) t.pb(cost[cur][vv]);
        auto mi = min_element(all(t));
        ll cnt = 0; fore(tt, t) if (tt == *mi) cnt++;
        if (cnt > 1) { valid = false; break; }
        cur = v[mi - t.begin()];
        sum += *mi;
        v.erase(v.begin() + (mi - t.begin()));
      }
      if (valid && sum == cost[0][h[i]]) dp[i][S] = 1;
    }
  }
  ...
  ll ans = k;
  rep(S, 1 << k) {
    if (dp2[S]) chmin(ans, k - __builtin_popcount(S));
  }
  OUT(ans);
}
