...
void solve() {
  ...
  vl depth(n);
  vmi dp(n), dpsum(n);
  rep(i, k) dpsum[a[i]] = dp[a[i]] = mint(1);
  ...
  rep(i, n - 1) {
    ...
    mint su, sv;
    if (depth[u] > depth[v]) {
      su = dpsum[u]; sv = mint(k) - su;
    } else {
      sv = dpsum[v]; su = mint(k) - sv;
    }

    // u -> v
    mint p1 = dp[u] * (mint(1) - dp[v]) / 2;
    ans += p1 * (su - 1) * (sv + 1);
    // v -> u
    mint p2 = dp[v] * (mint(1) - dp[u]) / 2;
    ans += p2 * (sv - 1) * (su + 1);
    // 個数変化なし
    ans += (mint(1) - p1 - p2) * su * sv;

    dp[u] = dp[v] = (dp[u] + dp[v]) / 2;
    // debug(sum_of(dpsum));
  }
  OUT(ans / (k * (k - 1) / 2));
}
