...
public:
  ...
  int add(vector<ll> &s, T val = T()) { int n = s.size(), v = 0; for (int i = 0; i < n; ++i) v = add_node(v, s[i], i == n - 1, val); return v; }
  ...
  int search(vector<ll> &s) { int n = s.size(), v = 0; for (int i = 0; i < n; ++i) { v = walk(v, s[i]); if (v == -1) return v; } return v; }
...
void solve() {
  LL(n, m);
  vvl ps(n);
  rep(i, n) { VL(p, m, 1); ps[i] = p; }

  Trie tr;
  rep(i, n) {
    vl rev(m);
    rep(j, m) rev[ps[i][j]] = j;
    tr.add(rev);
  }
  ...
  rep(i, n) {
    ...
    rep(ok, m) {
      t.pb(ps[i][ok]);
      if (tr.search(t) != -1) chmax(ans, ok + 1);
    }
    aa.pb(ans);
  }
  OUTARRAY(aa);
