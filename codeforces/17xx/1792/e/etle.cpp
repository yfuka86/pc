...
LP solve(ll n, ll m1, ll m2) {
  map<ll, LP> mp;
  set<ll> s;
  {
    vl d1 = divisors(m1), d2 = divisors(m2);
    fore(i, d1) fore(j, d2) {
      s.insert(i * j);
      mp[i * j] = {i, j};
    }
  ...
  const ll ma = min(1000000, n);
  vl last(ma + 1, 0);
  rep(i, 1, ma + 1) {
    ll l = last[i] * n, r = i * n;

    auto lt = s.upper_bound(l);
    auto rt = s.upper_bound(r);
    while (lt != rt) {
      if (*lt % i == 0) {
        ans++;
        xorsum ^= i;
  ...
  auto it = s.upper_bound(n * n);
  while (it != s.end()) it = s.erase(it);
  debug(s.size());

  fore(e, s) {
    auto [i, j] = mp[e];
    vl ds1 = divisors(i), ds2 = divisors(j);
    ll mi = LINF;
    fore(d1, ds1) fore(d2, ds2) {
      if (d1 * d2 > n) break;
      if (e / (d1 * d2) <= n) {
        // debug(e, d);
        chmin(mi, d1 * d2);
...
LP naive(ll n, ll m1, ll m2) {
  set<ll> s;
  fore(div, divisors(m1 * m2)) s.insert(div);
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 1000;
    ll m1 = rg.l(1, 1e9 + 1), m2 = rg.l(1, 1e9 + 1);
    auto so = solve(n, m1, m2); auto na = naive(n, m1, m2);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, m1, m2); debug(so); debug(na);
