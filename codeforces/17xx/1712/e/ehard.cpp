...
void solve(vlt query) {
  PrimeSieve ps(400001);
  BIT<ll> bt(200001);
  ...
  rep2(k, 3, 200001) {
    {
      ...
      // div = k未満のkの約数
      rep_r(i, div.size() - 1) {
        bt.add(div[i], div.size() - 1 - i);
      }

      vi div2 = ps.divisors(k * 2);
      set<LP> s;
      for (auto j: div2) {
        if (k <= j || lcm(j, k) != k * 2) continue;
        for (auto i: div2) {
          if (i != j && i < k && i + j > k) {
    ...
    vi divn = ps.divisors(k);
    sort(all(divn));
    divn.pop_back();
    while (cur < query.size() && get<0>(query[cur]) == k) {
      auto [r, l, i] = query[cur];
      ll n = (r - l + 1);
      ans[i] = n * (n - 1) * (n - 2) / 6;
      ans[i] -= bt.sum(l, 200001);

      // debug(l, r, bt.sum(l, 200001));
      cur++;
    }
  }
  coutarray(ans, 0, "\n");
