...
void solve() {
  ...
  vector<xor_basis<ll>> dp(n); rep(i, n) dp[i].add(a[i]);
  ...
  vector<xor_basis<ll>> dp2(n);
  function<void(ll, ll, xor_basis<ll>)> dfs2 = [&](ll v, ll p, xor_basis<ll> bas) {
    dp2[v] = bas;
    vector<xor_basis<ll>> t;
    rep(i, G[v].size()) {
      ll to = G[v][i];
      if (to == p) t.pb(bas);
      else t.pb(dp[to]);
    }
    vector<xor_basis<ll>> tsum = t, tsumr = t;
    rep(i, t.size() - 1) tsum[i + 1].merge(tsum[i]);
    rep_r(i, t.size() - 1) tsumr[i].merge(tsumr[i + 1]);

    rep(i, G[v].size()) {
      ll to = G[v][i];
      if (to == p) continue;
      xor_basis<ll> nx; nx.add(a[v]);
      if (i > 0) nx.merge(tsum[i - 1]);
      if (i < t.size() - 1) nx.merge(tsumr[i + 1]);
      dfs2(to, v, nx);
    }
  };
  dfs2(0, -1, xor_basis<ll>());
  ...
  rep(i, q) {
    ...
    if (r == v) {
      OUT(dp[0].get_max());
    } else if (v == hld.lca(r, v)) {
      ll v2 = hld.la(r, hld.depth[v] + 1);
      OUT(dp2[v2].get_max());
    } else {
      OUT(dp[v].get_max());
    }
