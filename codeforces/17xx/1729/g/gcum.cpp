...
void solve() {
  ...
  vl id;
  rep(i, n - m + 1) {
    if (s.substr(i, m) == t) id.pb(i);
  }
  id.pb(1000);

  // debug(id);

  vector<incr_csum<mint>> dp(n / m + 1, incr_csum<mint>(n + m + 1));

  ll ansc = 0; mint ans = 1;
  bool succ = false;
  rep(i, id.size() - 1) {
    if (!succ) {
      dp[0].add(id[i], 1);
      ...
      // この区間と被る区間
      auto it = upper_bound(all(id), id[i] - m);
      // 被らない区間のうち最後のものに被覆するケース
      ll l = it == id.begin() ? 0 : *prev(it) + 1;
      rep(j, n / m) {
        dp[j + 1].add(id[i] + m, dp[j].sum(l, id[i] + 1));
      }
    }

    // 連結が切れるたびに集計（このために最後の値入れている）
    if (id[i + 1] >= id[i] + m) {
      rep(j, n / m + 1) {
        mint sum = dp[j].sum(id[i] + 1, id[i] + m + 1);
        if (sum != 0) {
          ansc += j;
          ans *= sum;
          break;
