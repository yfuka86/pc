...
vl solve(ll n, vs g) {
  ...
  vl deg(n);
  rep(i, n) rep(j, i) {
    if (g[i][j] == '1') { uf.unite(i, j); deg[i]++; deg[j]++; }
  }
  // 最初から連結
  if (uf.size(0) == n) return {};

  map<ll, vl> con;
  rep(i, n) con[uf.root(i)].pb(i);

  // クリークでないものがあるケース
  rep(i, n) {
    if (uf.size(i) == 1) return {i};
    if (deg[i] < uf.size(i) - 1) {
      ll r = uf.root(i);
      ll mi = LINF, ans = -1;
      fore(v, con[r]) {
        if (chmin(mi, deg[v])) ans = v;
      }
      return {ans};
    }
  }
  ...
  // 全てクリーク
  if (con.size() > 2) {
    // 3成分以上なら根を二つ選んで操作することで連結になる
    vl ans;
    fore(r, _, con) if (ans.size() < 2) ans.pb(r);
    return ans;
  } else {
    // 2成分なら小さい成分をすべて潰す
    ll mi = LINF, root = -1;
    fore(r, v, con) if (chmin(mi, v.size())) root = r;
    return con[root];
  }
}

vl naive(ll n, vs g) {
  vl op(n); iota(all(op), 0);

  ll mi = LINF; vl ans;
  do {
    vs t = g;
    rep(i, n) {
      rep(j, n) { if (op[i] == j) continue;
        if (t[op[i]][j] == '1') t[op[i]][j] = '0'; else t[op[i]][j] = '1';
        if (t[j][op[i]] == '1') t[j][op[i]] = '0'; else t[j][op[i]] = '1';
      }
      UnionFind uf(n);
      rep(i, n) rep(j, n) {
        if (t[i][j] == '1') uf.unite(i, j);
      }
      if (uf.size(0) == n) {
        if (chmin(mi, i + 1)) { ans = op; ans.erase(ans.begin() + mi, ans.end()); }
        break;
      }
    }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ...
    while (1) {
      ...
      UnionFind uf(n);
      rep(i, n) rep(j, n) {
        if (g[i][j] == '1') uf.unite(i, j);
      }
      if (uf.size(0) < n) break;
    }
    auto so = solve(n, g); auto na = naive(n, g);
    if (!check || na.size() != so.size()) { cout << c << "times tried" << "\n";
      debug(n, g); debug(so); debug(na);
