...
void solve() {
  LL(n, k);
  init_f();
  if (k == 0) OUTRET(mint(3).pow(n));

  mint ans = 0;
  rep(i, 1, k + 1) { // i分割する
    // nの中の自由度（i:(0,0)の分, k - i:各区間の分)
    ll m = n - (k - i) - i;
    ans += comb(m, i) * comb(k - 1, i - 1) * mint(3).pow(n - i * 2); // (1,1)を選ぶ
    ans += comb(m, i - 1) * comb(k - 1, i - 1) * mint(3).pow(n - i * 2 + 1); // i - 1ヶ所の(1,1)を選び、n桁目で繰り上がるケース
  }
  OUT(ans);
}
