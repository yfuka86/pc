...
void solve() {
  ...
  rep(i, n) rep(j, m) {
    if (i < n - 1 && g[i][j] == '#' && g[i + 1][j] == '#') uf.unite(id(i, j), id(i + 1, j));
    if (j < m - 1 && g[i][j] == '#' && g[i][j + 1] == '#') uf.unite(id(i, j), id(i, j + 1));
  }

  map<ll, vl> mp;
  rep(i, n) rep(j, m) {
    if (g[i][j] == '#') mp[uf.root(id(i, j))].pb(id(i, j));
  }
  vvl comp; fore(_, v, mp) comp.pb(v);

  vlt e;
  fore(i, comp[0]) fore(j, comp[1]) {
    auto [x, y] = rid(i);
    auto [x2, y2] = rid(j);

    e.pb({abs(x2 - x) + abs(y2 - y), i, j});
  }
