...
string solve(string s) {
  ...
  rep(_, s.size() / 2) {
    ...
    vl t;
    rep(i, 26) if (f[i]) t.pb(i);

    if (f[t[0]] > 1) {
      t1.pb('a' + t[0]);
      t2.pb('a' + t[0]);
      f[t[0]]-=2;
    } else if (f[t[1]] == 1 || t.size() > 2) { // f[t[0]] == 1
      t1.pb('a' + t[1]);
      t2.pb('a' + t[0]);
      f[t[0]]--;
      f[t[1]]--;
      break;
    } else { // f[t[0]] == 1, f[t[1]] >= 2
      t1.pb('a' + t[1]);
      t2.pb('a' + t[1]);
      f[t[1]]-=2;
    }
  ...
  string mid = "";
  rep(i, 26) {
    rep(_, f[i]) mid.pb('a' + i);
  }

  reverse(all(t2));
  t1.insert(t1.end(), mid.begin(), mid.end());
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 6;
    string s = rg.str(n, "abcdef");
    auto so = solve(s); auto na = naive(s);
