...
ll solve(ll n, ll k, vl a, vl cold, vl hot) {
  vl diff(k);

  vv(ll, dp, k + 1, k + 1, -LINF); dp[k][k] = 0;
  vl ma(k + 1, -LINF); ma[k] = 0;

  rep(i, k) diff[i] = cold[i] - hot[i];
  ...
  rep(i, n) {
    ...
    rep(j, k + 1) {
      ...
      bool cond = (i > 0 && (a[i - 1] == a[i] || j == a[i - 1]));
      if (dp[a[i]][j] != -LINF && cond) chmax(nx[j], dp[a[i]][j] + diff[a[i]]);
      if (dp[j][a[i]] != -LINF && cond) chmax(nx[j], dp[j][a[i]] + diff[a[i]]);
    }
  ...
  ll sum = 0;
  rep(i, n) sum += cold[a[i]];

  ll allma = 0;
  rep(i, k + 1) rep(j, k + 1) chmax(allma, dp[i][j]);
  return sum - allma;
}

ll naive(ll n, ll k, vl a, vl cold, vl hot) {
  ll ans = LINF;
  rep(ord, 1 << n) {
    vl cpu(2, -1);
    ll sum = 0;
    rep(i, n) {
      if (ord & 1 << i) {
        if (cpu[0] == a[i]) sum +=hot[a[i]];
        else {
          sum += cold[a[i]];
          cpu[0] = a[i];
        }
      } else {
        if (cpu[1] == a[i]) sum +=hot[a[i]];
        else {
          sum += cold[a[i]];
          cpu[1] = a[i];
        }
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 4, k = 3;
    vl a = rg.vecl(n, 0, k);
    vl cold = rg.vecl(k, 1, 1e2);
    vl hot(k); rep(i, k) hot[i] = rg.l(1, cold[i] + 1);
    auto so = solve(n, k, a, cold, hot); auto na = naive(n, k, a, cold, hot);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, k, a, cold, hot); debug(so); debug(na);
