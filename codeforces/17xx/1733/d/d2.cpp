...
void solve() {
  ...
  vl idx;
  rep(i, n) if (a[i] != b[i]) idx.pb(i);

  if (idx.size() & 1) OUTRET(-1);
  ...
  if (x >= y) {
    if (idx.size() == 2) {
      if (idx[0] + 1 == idx[1]) {
        if (x > y * 2) {
          rep(i, n) {
            if (abs(i - idx[0]) > 1 && abs(i - idx[1]) > 1) OUTRET(y * 2);
          }
        }
        OUTRET(x);
      } else OUTRET(y);
    } else OUTRET(y * idx.size() / 2);
  } else {
    ll m = idx.size();
    vv(ll, dp, m + 1, 2, LINF); dp[0][0] = 0;

    rep(i, m) {
      rep(j, 2) {
        if (i < m - 1) {
          chmin(dp[i + 2][j], dp[i][j] + (idx[i + 1] - idx[i]) * x);
        }
        chmin(dp[i + 1][(j + 1) % 2], dp[i][j] + (j ? y : 0));
      }
    }
