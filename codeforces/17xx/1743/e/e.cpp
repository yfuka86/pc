...
void solve() {
  ...
  // if (t1 == t2) OUTRET(ceil(h, p1 + p2 - s) * t1);
  if (t1 > t2) { swap(t1, t2); swap(p1, p2); }

  vv(ll, dp, h + 1, h + 5, LINF);
  dp[0][0] = 0;
  ...
  rep(i, h + 1) rep(j, h + 1) {
    ...
    // 1を打つ
    if (t1 * (j + 1) / t2 > t1 * j / t2) {
      if (i + p2 >= h) chmin(ans, dp[i][j] + (t1 * j / t2 + 1) * t2);
      chmin(dp[min(h, i + p1 + p2)][j + 1], dp[i][j] + t1);
      chmin(dp[min(h, i + both)][0], dp[i][j] + t1);
    } else {
      chmin(dp[min(h, i + p1)][j + 1], dp[i][j] + t1);
      chmin(dp[min(h, i + (t2 / t1 - 1) * p1 + both)][0], dp[i][j] + t2);
    }
  }

  rep(i, h + 1) chmin(ans, dp[h][i]);
  OUT(ans);
}
