...
void solve() {
  ...
  ll low = count(all(rating), '0');
  vl lower(low); vl higher(N - low);
  rep(i, low) lower[i] = i + 1;
  rep(i, N - low) higher[i] = i + low + 1;
  // coutarray(lower);
  // coutarray(higher);

  rep(i, N) {
    if (rating[i] == '0') {
      auto itr = lower_bound(all(lower), P[i]);
      if (itr == lower.end()) itr--;
      ans[i] = *itr;
      lower.erase(itr);
    } else {
      auto itr = lower_bound(all(higher), P[i]);
      if (itr == lower.end()) itr--;
      ans[i] = *itr;
      higher.erase(itr);
    }
