...
ll amin(vl a, vl b) {
  ...
  rep(i, 3) {
    mf.add_edge(i + 3, t, b[i]);
  }
  rep(i, 3) {
    mf.add_edge(i, i + 3, LINF);
    mf.add_edge(i, (i + 2) % 3 + 3, LINF);
  }
...
void solve() {
  ll n; cin >> n;
  vl a(3), b(3); rep(i, 3) cin >> a[i]; rep(i, 3) cin >> b[i];
  cout << amin(a, b) << " " << amax(a, b) << "\n";
}
