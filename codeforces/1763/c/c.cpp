...
void solve() {
  LL(n); VL(a, n);
  if (n >= 4) OUTRET(*max_element(all(a)) * n);
  if (n == 2) OUTRET(max(sum_of(a), abs(a[0] - a[1]) * 2));

  ll ma = -LINF;
  rep(i, n) chmax(ma, a[i]);
  if (a[0] == ma || a[2] == ma) OUTRET(ma * 3);

  ll ans = 0;
  chmax(ans, sum_of(a));
  chmax(ans, max(a[0] * 3, a[2] * 3));
  chmax(ans, abs(a[1] - a[2]) * 3);
  chmax(ans, abs(a[0] - a[1]) * 3);
  OUT(ans);
}
