...
void solve() {
  ...
  // debug(big, mid, small, rcap, midcap, lcap);
  if (y == n && j == n) OUTRET(0);
  if (y == n) OUTRET(comb(mid, midcap) * comb(small, lcap));
  mint ans = 0;
  // 頂点の位置で場合分け
  rep(ma, i + 1, j) {
    ll bigr = j - ma - 1;
    ll rem = midcap - big - 1;
    if (mid - rem + small - lcap > rcap) continue;
    ans += comb(big, bigr) * comb(mid, rem) * comb(small, lcap);
    // debug(ans);
  }
  rep(ma, j + 1, n) {
    ll bigl = ma - j - 1;
    ans += comb(big, bigl) * comb(mid, midcap) * comb(small, lcap);
    // debug(ans);
  }
  OUT(ans);
}
