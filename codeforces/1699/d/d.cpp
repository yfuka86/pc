...
void solve() {
  ...
  rep(k, n) {
    ...
    // rep(i, cnt[k] + 2) {
    //   rep2(j, i + 1, cnt[k] + 2) {
    //     ll from = dict[k][i], to = dict[k][j];
    //     if (to - 1 == from || ok[to - 1][from + 1]) chmax(dp[j], dp[i] + 1);
    //   }
    // }
    // chmax(ans, dp[cnt[k] + 1] - 1);
    ...
    for (ll from: dict[k]) {
      for (ll to: dict[k]) {
        if (to <= from) continue;
        if (to - 1 == from || ok[to - 1][from + 1]) chmax(dp[to + 1], dp[from + 1] + 1);
      }
    }
    chmax(ans, dp[n + 1] - 1);
  }
