...
vl solve(ll n, vl a) {
  ...
  rep(i, n) {
    if (sorted[i] == n || sorted[i] == 0) continue;
    ll idx = sorted[i];
    if (sorted[idx] == sorted[idx - 1]) return {};
  }
  if (sorted[0] == n && sorted.back() == 0) return {};
  ...
  ll cur = n + 1;
  rep(i, r) {
    if (chmin(cur, sorted[i])) {
      while (vis[abs(upper)]) upper--;
      ans[i] = upper; vis[abs(upper)] = 1;
      rep_r(j, sorted[i], r) {
        if (j == sorted[i] || (j != r - 1 && ans[j] < ans[j + 1])) {
          while (vis[abs(lower)]) lower++;
          ans[j] = lower; vis[abs(lower)] = 1;
          lower++;
        } else {
          while (vis[abs(lower)]) lower++;
          ans[j] = lower; vis[abs(lower)] = 1;
  ...
  bool valid = true;
  rep(i, n) {
    if (sorted[i] == n) { if (ans.back() + ans[i] <= 0) valid = false; continue; }
    if (sorted[i] == 0) { if (ans.front() + ans[i] > 0) valid = false; continue; }
    ll idx = sorted[i];
    if (ans[idx] + ans[i] > 0) valid = false;
    if (ans[idx - 1] + ans[i] <= 0) valid = false;
  }
  if (!valid) return {};

  map<ll, ll> ansm;
  rep(i, n) ansm[sorted[i]] = ans[i];
...
void solve() {
  ...
  if (0) {
    ...
    rep(i, 1000) {
      vl a = rg.vecl(n, 0, n + 1);

      vl anss = solve(n, a);

      vl ans;
      enum_check(n, -n, n + 1, [&](vl &b) {
        bool valid = true;
        rep(i, n) {
          ll t = 0;
          rep(j, n) {
      ...
      if (anss.size() != ans.size()) debug(a, anss, ans);
    }
  }
}
