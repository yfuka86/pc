...
void solve() {
  ...
  vl dp(n, LINF), dpt(n, LINF);
  dp[0] = a[0];
  rep(i, n - 1) {
    rep(j, n) {
      if (j < n - 1) chmin(dpt[j + 1], dp[j] + max(0, a[i + 1] + k - a[i]));
      chmin(dpt[j], dp[j] + max(0, a[i + 1] - a[i]));
      if (j > 0) chmin(dpt[j - 1], dp[j] + max(0, a[i + 1] - k - a[i]));
    }
    dp = dpt;
    dpt.assign(n, LINF);
    debug(dp);
