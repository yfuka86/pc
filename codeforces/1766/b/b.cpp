...
void solve() {
  if (0) {
    ...
    while (1) {
      ...
      rep(c1, 26) {
        if (s.back() == 'z' && s[s.size() - 2] >= 'a' + c1) continue;
        string t = s.substr(s.size() - 1);
        t.pb('a' + c1);
        if (cand.find(t) == cand.end()) {
          if (s.back() == 'a' + c1) s.pb('a' + c1);
          s.pb('a' + c1);
          cand.insert(t);
          added = true;
          break;
  ...
  if (s.size() <= 3) OUTRET("NO");
  if (s.size() >= 704) OUTRET("YES");

  set<string> S;
  rep(i, n) {
    if (i < n - 1) {
      if (S.find(s.substr(i, 2)) != S.end()) OUTRET("YES");
    }
    if (i > 0) {
