...
void solve() {
  ...
  set<ll> odd, even; rep(i, n) if (a[i] & 1) odd.insert(a[i]); else even.insert(a[i]);

  rep(i, n) {
    if (a[i] & 1) {
      able[1][i] = 1;
      auto it = odd.lower_bound(a[i]);
      if (it != odd.begin()) able[0][i] = 1;
    } else {
      able[0][i] = 1;
      auto it = odd.lower_bound(a[i]);
      if (it != odd.begin()) able[1][i] = 1;
    }
  }

  YES(sum_of(able[0]) == n || sum_of(able[1]) == n);
}
