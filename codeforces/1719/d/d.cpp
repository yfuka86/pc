...
void solve() {
  ...
  // vector<map<LP, ll>> dp(n);
  vvl dp(8192, vl(2, LINF)), dpt(8192, vl(2, LINF));

  // dp[0][{a[0], 0}] = a[0] ? 1 : 0;
  dp[a[0]][0] = a[0] ? 1 : 0;

  auto f = [&](ll id, LP key, ll val) {
    // if (dp[id].find(key) != dp[id].end()) {
    //   chmin(dp[id][key], val);
    // } else {
    //   dp[id][key] = val;
    // }
    chmin(dpt[key.fi][key.se], val);
  };

  rep(i, n - 1) {
    ll na = a[i + 1];
    // for (auto &[p, c]: dp[i]) {
    rep(used, 8192) rep(nc, 2) {
      ll c = dp[used][nc];
      // auto [used, nc] = p;
      // debug(i, na, p, c);
      f(i + 1, {na, 0}, c + (na ? 1 : 0));
      if (na ^ used) {
        f(i + 1, {na ^ used, 0}, c + nc + 1);
        f(i + 1, {used, !nc}, c + nc + 1);
      } else {
        f(i + 1, {used, !nc}, c + nc);
      }
    }
    dp = dpt;
    dpt.assign(8192, vl(2, LINF));
  }
  ll ans = LINF;
  rep(i, 8192) rep(j, 2) {
    chmin(ans, dp[i][j]);
