...
void solve() {
  string s; cin >> s;
  vector<pair<char, ll>> p(s.size());
  rep(i, s.size()) {
    p[i] = {s[i], (s.back() < s.front() ? -i : i)};
  }
  sort(all(p));

  ll from = -1, to = -1;
  rep(i, p.size()) {
    if (abs(p[i].se) == 0) from = i;
    if (abs(p[i].se) == s.size() - 1) to = i;
  }

  vl ans;
  rep2(i, min(from, to), max(from, to) + 1) {
    ans.pb(abs(p[i].se));
  }
  if (from > to) reverse(all(ans));


  cout << abs(s.back() - s.front()) << " " << ans.size() << "\n";
  coutarray(ans, 1, " ");
}
