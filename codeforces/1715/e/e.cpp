...
void solve() {
  ...
  vvl cost(n, vl(k + 1, LINF));
  priority_queue<LT, vector<LT>, greater<LT>> que;

  function<ll(ll, ll)> fc = [&](ll dist, ll times) {
    if (times >= dist) return dist;
    ll div = dist / times, rem = dist % times;
    return (div + 1) * (div + 1) * rem + div * div * (times - rem);
  };
  ...
  rep(i, k + 1) cost[0][i] = 0;
  que.push({0, 0, 0});
  while (!que.empty()) {
    auto [c, v, kk] = que.top(); que.pop();
    if (cost[v][kk] < c) continue;

    vl ori = cost[v];
    rep(i, k - kk + 1) {
      // iフライトを使える時
      if (i) for (int d = 0; fc(d, i) < c; d++) {
        ll cd = fc(d, i);
        rep(j, k - i + 1) {
          if (v - d >= 0) chmin(cost[v][j + i], cost[v - d][j] + cd);
          if (v + d < n) chmin(cost[v][j + i], cost[v + d][j] + cd);
        }
      }
    }
    // debug(v, kk, cost[v]);

    rep(i, k + 1) if (i == kk || ori[i] > cost[v][i]) {
      for (auto &to: G[v]) {
        ll nc = cost[v][i] + to.cost;
        if (chmin(cost[to][i], nc)) que.push({nc, to, i});
      }
    }
  }
