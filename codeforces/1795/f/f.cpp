...
void solve() {
  ...
  ll ans = binary_search([&](ll mid) {
    vlp dp(n, {0, 0}); // dp = {生きている最長パス, 上に伸ばすべきパス}
    bool valid = true;

    function<void(ll, ll)> dfs = [&](ll v, ll p) {
      if (!valid) return;
      if (ord[v] != -1) {
        ll need = mid / k + (mid % k > ord[v]);
        ll ma = 0;
        fore(to, G[v]) {
          if (to == p) continue;
          dfs(to, v);
          if (dp[to].se > 0) valid = false;
          chmax(ma, dp[to].fi);
        }
        if (ma >= need) dp[v] = {0, 0};
        else dp[v] = {0, need};
      } else {
        ll pcnt = 0, need = 0, remma = 0;
        fore(to, G[v]) {
          if (to == p) continue;
          dfs(to, v);
          if (dp[to].se > 0) { pcnt++; chmax(need, dp[to].se); }
          if (dp[to].fi > 0) { chmax(remma, dp[to].fi); }
        }
        if (pcnt > 1) valid = false;
        else if (pcnt == 1) {
          if (need <= remma + 1) dp[v] = {0, 0};
          else dp[v] = {0, need - 1};
        } else dp[v] = {remma + 1, 0};
      }
    };
    dfs(0, -1);
    if (dp[0].se > 0) valid = false;
