...
void solve() {
  ...
  if (0) {
    vv(ll, board, n, n);
    auto place = [&](ll i, ll j) {
      ll di = i - min(i, j);
      ll dj = j - min(i, j);
      rep(off, n) {
        if (di + off >= n || dj + off >= n) break;
        board[di + off][dj + off] = 1;
      }
      rep(nj, n) board[i][nj] = 1;
      rep(ni, n) board[ni][j] = 1;
    };
  }

  // if (1) {
  //   enum_check(4, 1, n * n, [&](vl v) {
  //     rep(i, n) rep(j, n) board[i][j] = 0;

  //     place(0, 0);
  //     fore(e, v) place(e / n, e % n);
  //     ll c = 0;
  //     rep(i, n) rep(j, n) if (board[i][j]) c++;
  //     if (c == n * n) {
  //       // debug(v);
  //       fore(e, v) {
  //         LP p = {e / n, e % n};
