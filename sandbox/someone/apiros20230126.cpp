...
vl solve(string s, vlp q) {
  vl ret;
  ll n = s.size();
  vl a(n + 1);
  rep(i, n) a[i + 1] = a[i] + (s[i] - '0');
  ...
  fore(l, r, q) {
    ...
    ll rng = a[l + len] - a[l];
    if (rng == c1 / 2) { ret.pb(l); }
    else if (rng > c1 / 2) {
      ll i = binary_search([&](ll mid) {
        return a[mid + len] - a[mid] > c1 / 2;
      }, l, r - len);
      ret.pb(i + 1);
    } else {
      ll i = binary_search([&](ll mid) {
  ...
  rep(i, q.size()) {
    ...
    ll c1 = a[r] - a[l];
    ll len = (r - l) / 2;
    if (a[ret[i] + len] - a[ret[i]] != c1 / 2) assert(false);
  }

  return ret;
}

vl naive(string s, vlp q) {
  vl ret;
  // ll n = s.size();
  // vl a(n + 1);
  // rep(i, n) a[i + 1] = a[i] + (s[i] - '0');
  ...
  //   ll len = (r - l) / 2;
  //   rep(i, l, r - len + 1) {
  //     if (a[i + len] - a[i] == c1 / 2) { ret.pb(i); break; }
  //   }
  // }
  return ret;
}
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 100000;
    string s = rg.str(n, "01");
    vl a(n + 1);
    rep(i, n) a[i + 1] = a[i] + (s[i] - '0');
    vlp q;
    rep(i, 1000) {
      ll l = rg.l(0, n - 2), r = rg.l(l + 2, n + 1);
      if ((r - l) & 1) continue;
      if ((a[r] - a[l]) & 1) continue;
      q.pb({l, r});
    }
    auto so = solve(s, q); auto na = naive(s, q);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(s, q); debug(so); debug(na);
