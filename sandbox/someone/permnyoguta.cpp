...
ll solve(ll N, vl a) {
  ...
  ll ans = 0;
  rep(i, 99) {
    ll mi = min(invdict[i], invdict[i + 1]);
    ans += mi;
    invdict[i] -= mi;
    invdict[i + 1] -= mi;
  }
...
ll naive(ll N, vl a) {
  ...
  do {
    if (p > a) break;
    dict[p] = { cur, inv_num(p) };
    cur++;
  } while (next_permutation(all(p)));
  debug(dict);

  mf_graph<ll> mf(dict.size() + 2);
  ll s = dict.size(), t = s + 1;

  for (auto [p, v]: dict) {
    auto [id, inv] = v;
    vl pt = p;
    if (inv & 1) mf.add_edge(id, t, 1); else mf.add_edge(s, id, 1);
    rep(i, N - 1) {
      swap(pt[i], pt[i + 1]);
      if (dict.find(pt) != dict.end()) {
        auto [id2, _] = dict[pt];
        if (inv & 1) mf.add_edge(id2, id, 1); else mf.add_edge(id, id2, 1);
      }
      swap(pt[i], pt[i + 1]);
    }
  }
...
signed main() {
  ...
  int t = 1; //cin >> t;
  RandGen rg;
  // while (t--) cout << solve(10, rg.vecperm(10)) << "\n";
  while (t--) compare();
}
