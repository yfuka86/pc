...
mint solve(ll n, ll m, vl l, vl r) {
  m -= sum_of(l);

  ll coef = 0;
  rep(i, n) coef += r[i] - l[i];

  // [x^(m - ΣLi - coef)]
  ll d = m - sum_of(l) - coef;

  init_f();
  return comb(d + n - 1, n - 1);
}

mint naive(ll n, ll m, vl l, vl r) {
  vv(mint, dp, n + 1, m + 1); dp[0][0] = 1;

  rep(i, n) {
    rep(j, l[i], r[i] + 1) {
      rep(k, m) {
        if (k + j <= m) dp[i + 1][k + j] += dp[i][k];
      }
    }
  }
  return dp[n][m];
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 3, m = 20;
    vl l = rg.vecl(n, 1, 10);
    vl r(n); rep(i, n) r[i] = rg.l(l[i], 10);
    auto so = solve(n, m, l, r); auto na = naive(n, m, l, r);
    if (!check || na != so) { cout << c << "times tried" << "\n";
      debug(n, m, l, r); debug(so); debug(na);
