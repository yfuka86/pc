...
bool solve(ll n, string s) {
  map<char, set<ll>> dict;
  rep(i, n) dict[s[i]].insert(i);
  ...
  rep(i, n) {
    ...
    chmax(cur, i);
    auto it = dict[s[i]].upper_bound(cur);
    if (it == dict[s[i]].end()) return false;
    debug(s[i], i, *it);
    used[i] = true;
    chmax(cur, *it);
    used[*it] = true;
...
bool naive(ll n, string s) {
  rep(S, 1 << n) {
    if (__builtin_popcount(S) != n / 2) continue;
    string a = "", b = "";
    rep(i, n) {
      if (S & 1 << i) a.pb(s[i]); else b.pb(s[i]);
    }
    if (a == b) return true;
  }
  return false;
...
void compare(bool check = true) { RandGen rg; ll c = 0, loop = 10;
  while (++c) { if (c % loop == 0) cout << "reached " << c / loop << "loop" <<  "\n", cout.flush();
    ll n = 10;
    string s = rg.straz(n/2, 0, 2);
    string t = s; s.insert(s.end(), t.begin(), t.end());
    shuffle(all(s), rg.mt);
